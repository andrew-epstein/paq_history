// fpaq3b - Stationary order 3 (state of 28 bits) file compressor.
// (C) 2004, Matt Mahoney under GPL, http://www.gnu.org/licenses/gpl.txt
// 02.12.2006 modified by Nania Francesco Antonio (Italia - Merì (ME))
...
unsigned long filesize,pos,control,rc[2];// record state bit of y and not y
unsigned char buf[1<<28];  // 0 and 1 state count in context

  // Assume a stationary order 3 stream of 9-bit symbols
  int p()  {return 4096*(buf[rc[1]]+1)/(buf[rc[1]]+buf[rc[0]]+2);}
