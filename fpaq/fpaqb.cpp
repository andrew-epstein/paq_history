/* fpaqb ver. 2 - Stationary order 0 file compressor.
...
To compress:   fpaqb c input output
To decompress: fpaqb d input output

To compile:

  g++ -O3 -march=pentiumpro -fomit-frame-pointer -s fpaqb.cpp -o fpaqb

To use arithmetic coder (instead of Duda's asymmetric coder), compile
with -DARITH

Ver. 2 improves compression speed over ver. 1 (Dec. 17, 2007) while
maintaining archive compatibility.
...
then the bytes are popped and written.
...
The decoder expects to read x as a 3 byte big-endian integer.  If bit
7 of the first byte is 0, then n = B, otherwise B is read as a 3 byte
big-endian integer.  This is followed by the compressed bytes.
...
// Initialize
Encoder::Encoder(Mode m, FILE* f): mode(m), archive(f), x(1<<N), n(0), 
    ins(0), outs(0) {
  if (mode==COMPRESS) {
    alloc(ins, B);
    alloc(outs, BO);
...
int main(int argc, char** argv) {
  ...
  // Check arguments: fpaqb c/d input output
  if (argc!=4 || (argv[1][0]!='c' && argv[1][0]!='d')) {
    printf("fpaqb ver. 2 (C) 2007, Matt Mahoney\n"
