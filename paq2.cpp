/* PAQ2 - File archiver and compressor.
(C) 2002, Matt Mahoney, mmahoney@cs.fit.edu

Modified by Serge Osnach, ench@netcity.ru
...
To compress:      PAQ2 archive file file...  (1 or more file names), or
  or (MSDOS):     dir/b | PAQ2 archive       (read file names from input)
  or (UNIX):      ls    | PAQ2 archive
To decompress:    PAQ2 archive
...
line, then PAQ2 prompts for them, reading until the first blank line or
end of file.
...
which must exist.  PAQ2 does not create directories.  If the file to be
...
PAQ2 combines the outputs of 5 models:
...
PAQ2 uses a compromise between these two positions:
...
The following results were obtained by PAQ2 and various popular
...
PAQ2 was compiled using DJGPP (g++) 2.95.2 with optimization (gxx -O).
...
CHANGED 01/05/2003 by Serge Osnach. Added full SSE on probability
of current bit, previous bits and submodels prediction. Some fields
changed to be public.

File        Size   RK -mx3  Ratio    PAQ1   Ratio     PAQ2   Ratio
------    ------   -------  -----   ------  -----  --------  -----
PAPER2     82199     22111   26.8    22323  27.16     21952  26.71
PAPER1     53161     13085   24.6    12783  24.05     12581  23.67
BOOK1     768771    201950   26.2   206539  26.87    203343  26.45
BOOK2     610856    139591   22.8   132151  21.63    129638  21.22
OBJ1       21504      9397   43.6    10027  46.63      9851  45.81
GEO       102400     47960   46.8    52244  51.02     50599  49.41
OBJ2      246814     63754   25.8    66416  26.91     65768  26.65
PIC       513216     30595    5.9    46975   9.15     45714   8.91
BIB       111261     24146   21.7    23011  20.68     22338  20.08
PROGL      71646     13692   19.1    12661  17.67     12290  17.15
PROGC      39611     11614   29.3    10571  26.69     10396  26.25
PROGP      49379     10207   20.6     9048  18.32      8780  17.78
TRANS      93695     14273   15.2    13465  14.37     12939  13.81
NEWS      377109    104435   27.6    97917  25.97     95938  25.44
------    ------   -------  -----   ------  -----   -------  -----
14       3141622    706810   22.4   716386  22.80    702382  22.36
...
/* SSE context. Methods:
   ...
   SSEContext.init(int InitVal) - Initializes context with given probability.
     Probability is a fixed-point value with precision 1/2^16.
   SSEContext.getMean() - gives current probability and automatically decrements
     probability. Incrementation of probability in this implementation are
     always explicit.
   SSEContext.update() - Checks, where adaptation speed (1<<Shift) must be 
    derceased.
*/
class SSEContext {
public:
  int Summ;
  unsigned char Shift, MaxShift;
  unsigned short Count;
  void init(int InitVal) {
    Summ = InitVal; Shift=4; Count = 8;
    MaxShift = 7;
    //Summ = InitVal; Shift=4; Count=12;
  }
  unsigned getMean() {
    unsigned int RetVal = Summ >> (Shift - 4);
    //unsigned int RetVal = (Summ >> (Shift - 5))+1>>1;
    //if(Shift==4) RetVal = Summ;
    Summ -= (Summ>>Shift);
    //Summ -= RetVal>>4;
    if (RetVal > 0xffff) {
      RetVal = 0xffff;
...
Encoder::Encoder(Mode m, FILE* f): predictor(), mode(m), archive(f), x1(0),
  ...
  // Prepare map of probabilities
  for( i=0;i<4;i++)sseMap[i]=i;
  for( i=4;i<8;i++)sseMap[i]=2+(i>>1);
  for( i=8;i<32;i++)sseMap[i]=4+(i>>2);
  for( i=32;i<128;i++)sseMap[i]=16+(i>>3);
  for( i=128;i<256;i++)sseMap[i]=63-sseMap[255-i];
}
...
*/
int Encoder::encode(int y, int pi) {
  ++encodes;
  ...
  int ww=1,ww1=1; predictor.m2.predict(ww,ww1);
  c0 = c0*4+((ww*3>ww1)+(ww>ww1)+(ww>3*ww1));

  ww=1,ww1=1; predictor.m3.predict(ww,ww1);
  predictor.m4.predict(ww,ww1);
  c0 = c0*4+((ww*3>ww1)+(ww>ww1)+(ww>3*ww1));
  
  int sse_p = sse[c0][q2].getMean();
  // Update neighbour SSE contexts
  if( q2 ) sse[c0][q2-1].Summ -= (sse[c0][q2-1].Summ>>sse[c0][q2-1].Shift+1);
  if( q2>1 ) sse[c0][q2-2].Summ -= (sse[c0][q2-2].Summ>>sse[c0][q2-2].Shift+1);
  if( q2<63 ) sse[c0][q2+1].Summ -= (sse[c0][q2+1].Summ>>sse[c0][q2+1].Shift+1);
  if( q2<62 ) sse[c0][q2+2].Summ -= (sse[c0][q2+2].Summ>>sse[c0][q2+2].Shift+1);
  ...
  */
  ...
  if( !y ){
    ...
    if( q2 ) sse[c0][q2-1].Summ += 0x800;
    if( q2>1 ) sse[c0][q2-2].Summ += 0x800;
    if( q2<63 ) sse[c0][q2+1].Summ += 0x800;
    if( q2<62 ) sse[c0][q2+2].Summ += 0x800;
  }
  // Descrease speed of SSE adaptation, if necessary.
  sse[c0][q2].update();	
...
int main(int argc, char** argv) {
  ...
  if (argc<2) {
    printf(
      "PAQ2 file compressor/archiver, (C) 2002-03, Matt Mahoney, mmahoney@cs.fit.edu\n"
      "and Serge Osnach, ench@netcity.ru\n" 
      ...
      "\n"
      "To compress:         PAQ2 archive filenames...  (archive will be created)\n"
      "  or (MSDOS):        dir/b | PAQ2 archive  (reads file names from input)\n"
      "To extract/compare:  PAQ2 archive  (does not clobber existing files)\n"
  ...
  if (archive) {
    ...
    // Read "PAQ2\r\n" at start of archive
    if (getline(archive) != "PAQ2") {
      printf("Archive file %s not in PAQ2 format\n", argv[1]);
      return 1;
    }
