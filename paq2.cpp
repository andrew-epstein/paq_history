...
void Encoder::print() {
  int now = clock();
  if( encodes > 0 )
    printf( "%ld/%ld = %6.4f bpc (%4.2f%%) in %1.2f sec\n", xchars, encodes / 8, xchars * 64.0 / encodes,
            xchars * 800.0 / encodes, double( now - start_time ) / CLOCKS_PER_SEC );
  else
    printf( "0 bytes\n" );
...
int main( int argc, char **argv ) {
  ...
  // Check arguments
  if( argc < 2 ) {
    printf( "PAQ2 file compressor/archiver, (C) 2002-03, Matt Mahoney, mmahoney@cs.fit.edu\n"
