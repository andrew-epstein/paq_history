...
function task() {
	...
	echo "$FILENAME"
	FILENAME="${FILENAME%.*}"
	$CXX -Wall -Wextra $WARN -flto -fomit-frame-pointer -fno-stack-protector -lz -lpthread -fno-rtti -std=c++17 -O3 -m64 -march=native -funroll-loops -ftree-vectorize -DUNIX  $1 asm/paq7asm.s -o bin/$FILENAME
	#$CXX -Wno-overflow -lpthread -lz -std=c++14 -O0 -m64 -DUNIX "$1" asm/paq7asm.s -o "bin/$FILENAME"
	if [ $? -ne 0 ]; then
		echo "$1" >> bad.log
	fi
}
