  ...
  if (isdigit(argv[1][0])) {
	...
	fb_stop=fb_len;	if (fb_stop>&file_buf[FB_SIZE]) fb_stop=&file_buf[FB_SIZE];
      ...
      if (k==0) TextFlag=1;
        ...
        if (file_buf[0]=='M' && file_buf[1]=='Z')	{ ExeFlag=1; E8E9_encode(fb_pos, fb_len-8-fb_pos); }
      }

    // Encode header: version 6, memory option, file size
    out=fopen(argv[3], "wb");
    if (!out) perror(argv[3]), exit(1);
    fprintf(out, "pQ%c%c%c%c%c%c", 6, argv[1][0],
...
#ifndef WIKI
	if (TextFlag)
	if ( (pos==6*1024*1024 && DP_SHIFT==16) || (pos==1024*1024 && DP_SHIFT==15) || DP_SHIFT==14 )
#else
  ...
  else
  {
	...
	{ c=e.decompress(); *p++=c; if(c>0x7f) ++k; }
	  ...
	  for (s=0, TextFlag=1; s<ss; ++s)
		if (file_buf[s]==0x1f||file_buf[s]==0x20) file_buf[s]^=0x3f;
	  }
      else
      if (file_buf[0]=='M' && file_buf[1]=='Z') ExeFlag=1;
      fb_pos=p;
      }
