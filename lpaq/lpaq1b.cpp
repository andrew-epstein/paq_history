/* lpaq1b.cpp file compressor, July 24, 2007.
...
To compile (g++ 3.4.5, upx 3.00w):
  g++ -Wall lpaq1b.cpp -O2 -Os -march=pentiumpro -fomit-frame-pointer 
      -s -o lpaq1b.exe
  upx -qqq lpaq1b.exe
...
bool use_order1,use_order2,use_order3,use_order4,use_order6,use_word,use_match,use_apm;
...
int main(int argc, char **argv) {
  ...
  // Check arguments
  if (argc!=5 || !isdigit(argv[1][0]) && argv[1][0]!='d') {
    printf(
      "lpaq1b EXPERIMENTAL file compressor (C) 2007, Matt Mahoney\n"
      "Licensed under GPL, http://www.gnu.org/copyleft/gpl.html\n"
      "\n"
      "To compress:   lpaq1b N models input output  (N=0..9, uses 3+3*2^N MB)\n"
      "To decompress: lpaq1b d models input output  (needs same memory)\n"
	  "where models = [12346wma] (order1,order2,order3,order4,order6,word,match,apm)\n"
	  "example: lpaq1b 3 12346w input output (selects all models except match+apm)\n"
	  "attention: you should specify the same models at decompression\n");
    return 1;
  }

  use_order1=use_order2=use_order3=use_order4=use_order6=use_word=use_match=use_apm=false;
  ...
  int mod_len=strlen(argv[2]);
  for (int i=0; i<mod_len; i++)
  {
	 switch(argv[2][i])
	 {
		 case '1': use_order1=true; break;
		 case '2': use_order2=true; break;
		 case '3': use_order3=true; break;
		 case '4': use_order4=true; break;
		 case '6': use_order6=true; break;
		 case 'w': use_word=true; break;
		 case 'm': use_match=true; break;
		 case 'a': use_apm=true; break;
		 default: printf("unknown model %c ignored\n",argv[2][i]);
	 }
  }
