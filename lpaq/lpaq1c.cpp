...
void Predictor::update(int y) {
  ...
  if (c0>=256) {
	...
	if (use_order4)
	    h[3]=c4*5;  // order 4
	if (use_order5)
	    h[4]=h[4]*(11<<5)+c0*13&(1<<25)-1;  // order 5
	if (use_word)
	{
...
int main(int argc, char **argv) {
  ...
  if (argc!=5 || !isdigit(argv[1][0]) && argv[1][0]!='d') {
    printf(
      "lpaq1c EXPERIMENTAL file compressor (C) 2007, Matt Mahoney\n"
      "Licensed under GPL, http://www.gnu.org/copyleft/gpl.html\n"
      "\n"
      "To compress:   lpaq1c N models input output  (N=0..9, uses 3+3*2^N MB)\n"
      "To decompress: lpaq1c d models input output  (needs same memory)\n"
	  "where models = [12345wma] (order1,order2,order3,order4,order5,word,match,apm)\n"
	  "example: lpaq1c 3 12345w input output (selects all models except match+apm)\n"
	  "attention: you should specify the same models at decompression\n");
    return 1;
  }

  use_order1=use_order2=use_order3=use_order4=use_order5=use_word=use_match=use_apm=false;
