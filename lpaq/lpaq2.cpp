...
#define MI 7
int *mxr_tx; // MI inputs

#if 0 // ATTENTION !  CHANGE this to 1 if you start to use                                                             \
      //		<mixer max inputs>!=7 in your versions.
...
template <int B>
HashTable<B>::HashTable( int n ) : t( 0 ), N( n ), NB( n - B ) {
  assert( B >= 2 && ( B & B - 1 ) == 0 );
...
public:
  ...
  int p( int y );      // update bit y (0..1), predict next bit to m
};
...
void Predictor::update( int y ) {
  ...
  *cp[5] = nex( *cp[5], y );
  ...
  if( len == 0 )
    order = static_cast<int>( *cp[1] != 0 ) + static_cast<int>( *cp[2] != 0 ) + static_cast<int>( *cp[3] != 0 ) + static_cast<int>( *cp[4] != 0 );
  else
