/* paq8jc file compressor/archiver.
(C) 2006, Matt Mahoney under GPL, http://www.gnu.org/licenses/gpl.txt
Release by Serge Osnach (enc@intelserv.com).

To install and use in Windows:

- To install, put paq8jc.exe or a shortcut to it on your desktop.
- To compress a file or folder, drop it on the paq8jc icon.
- To decompress, drop a .paq8jc file on the icon.

A .paq8jc extension is added for compression, removed for decompression.
The output will go in the same folder as the input.

While paq8jc is working, a command window will appear and report
...
- To install, put paq8jc.exe somewhere in your PATH.
- To compress:      paq8jc [-N] file1 [file2...]
- To decompress:    paq8jc [-d] file1.paq8jc [dir2]
- To view contents: more < file1.paq8jc

The compressed output file is named by adding ".paq8jc" extension to
the first named file (file1.paq8jc).  Each file that exists will be
...
If file1.paq8jc exists then it is overwritten.

If the first named file ends in ".paq8jc" then it is assumed to be
...
The -d option forces extraction even if there is not a ".paq8jc"
...
During extraction, directories are created as needed.  For example:
  ...
  paq8jc -4 c:\tmp\foo bar

compresses foo and bar (if they exist) to c:\tmp\foo.paq8jc at level 4.

  paq8jc -d c:\tmp\foo.paq8jc .
...
There are 2 files: paq8jc.cpp (C++) and paq7asm.asm (NASM/YASM).
paq7asm.asm is the same as in paq7 and paq8x.  paq8jc.cpp recognizes the
...
Recommended compiler commands and optimizations:
  ...
  MINGW g++:
    ...
    g++ paq8jc.cpp -DWINDOWS -O2 -Os -s -march=pentiumpro -fomit-frame-pointer -o paq8jc.exe paq7asm.obj

  Borland:
    nasm paq7asm.asm -f obj --prefix _
    bcc32 -DWINDOWS -O -w-8027 paq8jc.cpp paq7asm.obj

  Mars:
    nasm paq7asm.asm -f obj --prefix _
    dmc -DWINDOWS -Ae -O paq8jc.cpp paq7asm.obj

  UNIX/Linux (PC):
    nasm -f elf paq7asm.asm
    g++ paq8jc.cpp -DUNIX -O2 -Os -s -march=pentiumpro -fomit-frame-pointer -o paq8jc paq7asm.o

  Non PC (e.g. PowerPC under MacOS X)
    g++ paq8jc.cpp -O2 -DUNIX -DNOASM -s -o paq8jc
...
Then
  ...
  paq8jc archive \dir1\file1.txt \dir2

will create archive.paq8jc with the header:
...
Decompression will fail if the first 6 bytes are not "paq8jc -".  Sizes
...
paq8jc uses a neural network to combine a large number of models.  The
...
paq8jc uses preprocessing transforms on certain data types to improve
...
// To compress to file1.paq8jc: paq8jc [-n] file1 [file2...]
// To decompress: paq8jc file1.paq8jc [output_dir]
