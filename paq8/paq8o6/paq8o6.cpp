/* paq8o6 file compressor/archiver.  Release by KZ, Sept. 28, 2007
...
- To install, put paq8o6.exe or a shortcut to it on your desktop.
- To compress a file or folder, drop it on the paq8o6 icon.
- To decompress, drop a .paq8o6 file on the icon.

A .paq8o6 extension is added for compression, removed for decompression.
The output will go in the same folder as the input.

While paq8o6 is working, a command window will appear and report
...
- To install, put paq8o6.exe somewhere in your PATH.
- To compress:      paq8o6 [-N] file1 [file2...]
- To decompress:    paq8o6 [-d] file1.paq8o6 [dir2]
- To view contents: more < file1.paq8o6

The compressed output file is named by adding ".paq8o6" extension to
the first named file (file1.paq8o6).  Each file that exists will be
...
If file1.paq8o6 exists then it is overwritten.

If the first named file ends in ".paq8o6" then it is assumed to be
...
The -d option forces extraction even if there is not a ".paq8o6"
...
During extraction, directories are created as needed.  For example:
  ...
  paq8o6 -4 c:\tmp\foo bar

compresses foo and bar (if they exist) to c:\tmp\foo.paq8o6 at level 4.

  paq8o6 -d c:\tmp\foo.paq8o6 .
...
There are 2 files: paq8o6.cpp (C++) and paq7asm.asm (NASM/YASM).
paq7asm.asm is the same as in paq7 and paq8x.  paq8o6.cpp recognizes the
...
Recommended compiler commands and optimizations:
  ...
  MINGW g++:
    ...
    g++ paq8o6.cpp -DWINDOWS -O2 -Os -s -march=pentiumpro -fomit-frame-pointer -o paq8o6.exe paq7asm.obj

  Borland:
    nasm paq7asm.asm -f obj --prefix _
    bcc32 -DWINDOWS -O -w-8027 paq8o6.cpp paq7asm.obj

  Mars:
    nasm paq7asm.asm -f obj --prefix _
    dmc -DWINDOWS -Ae -O paq8o6.cpp paq7asm.obj

  UNIX/Linux (PC):
    nasm -f elf paq7asm.asm
    g++ paq8o6.cpp -DUNIX -O2 -Os -s -march=pentiumpro -fomit-frame-pointer -o paq8o6 paq7asm.o

  Non PC (e.g. PowerPC under MacOS X)
    g++ paq8o6.cpp -O2 -DUNIX -DNOASM -s -o paq8o6
...
Then
  ...
  paq8o6 archive \dir1\file1.txt \dir2

will create archive.paq8o6 with the header:
...
Decompression will fail if the first 7 bytes are not "paq8o6 -".  Sizes
...
paq8o6 uses a neural network to combine a large number of models.  The
...
paq8o6 uses preprocessing transforms on certain data types to improve
...
#define PROGNAME "paq8o6" // Please change this if you change the program.
...
int pgmModel( Mixer &m ) {
  ...
  //															windows bmp     compression
  if( bpos == 0 ) {
    if( buf( 54 ) == 'B' && buf( 53 ) == 'M' && i4( 44 ) == 1078 && i4( 40 ) == 40 && i4( 24 ) == 0 ) {
...
int jpegModel( Mixer &m ) {
  ...
  if( hbcount == 0 ) {
    ...
    cxt[3] = hash( ++n, hc, rs1, comp0, adv_pred[2] );
    cxt[4] = hash( ++n, hc, rs1, comp0, adv_pred[0] );
    cxt[5] = hash( ++n, hc, rs1, comp0, adv_pred[1] );
    cxt[6] = hash( ++n, hc, adv_pred[2], adv_pred[0] );
    cxt[7] = hash( ++n, hc, comp0, cbuf[cpos - width * mcusize], adv_pred[3] );
    cxt[8] = hash( ++n, hc, comp0, cbuf[cpos - ls[mcupos >> 6]], adv_pred[3] );
    cxt[9] = hash( ++n, hc, lcp[0], lcp[1], adv_pred[1] );
    cxt[10] = hash( ++n, hc, lcp[0], lcp[1], coef );
    cxt[11] = hash( ++n, hc, zu + 8 * comp0, lcp[0], lcp[2] / 2 );
    cxt[12] = hash( ++n, hc, zv + 8 * comp0, lcp[1], lcp[3] / 2 );
    ...
    cxt[18] = hash( ++n, hc, mcupos >> 3, ssum3 >> 3, adv_pred[1] );
    cxt[19] = hash( ++n, hc, lcp[0] >> 2, lcp[1] >> 2, adv_pred[5] );
    cxt[20] = hash( ++n, hc, adv_pred[1] );
    cxt[21] = hash( ++n, hc, rs1, comp0, adv_pred[5] );
    cxt[22] = hash( ++n, hc, comp0, adv_pred[2] );
    cxt[23] = hash( n, hc, comp0, adv_pred[0] );
    cxt[24] = hash( n, hc, comp0, adv_pred[1] );
    cxt[25] = hash( ++n, hc, coef, adv_pred[4] );
    cxt[26] = hash( ++n, hc, coef, adv_pred[6] );
  ...
  int pr = m1.p();
  m.add( stretch( pr ) );
  pr = a1.p( pr, (hc & 255) | ( adv_pred[1] & 63 ) << 8, 1023 );
  pr = a2.p( pr, (hc & 255) | coef << 8, 512 );
  m.add( stretch( pr ) );
  return 2 + ( hc & 255 );
}
...
int contextModel2() {
  ...
  int ispgm = pgmModel( m );             // Image width (bytes) if PGM (P5,PGM_MAXVAL = 255) detected, or 0
...
// To compress to file1.paq8o6: paq8o6 [-n] file1 [file2...]
// To decompress: paq8o6 file1.paq8o6 [output_dir]
