/* paq8n file compressor/archiver.  Release by Andreas Morphis, Aug. 22, 2007.
...
- To install, put paq8n.exe or a shortcut to it on your desktop.
- To compress a file or folder, drop it on the paq8n icon.
- To decompress, drop a .paq8n file on the icon.

A .paq8n extension is added for compression, removed for decompression.
The output will go in the same folder as the input.

While paq8n is working, a command window will appear and report
...
- To install, put paq8n.exe somewhere in your PATH.
- To compress:      paq8n [-N] file1 [file2...]
- To decompress:    paq8n [-d] file1.paq8n [dir2]
- To view contents: more < file1.paq8n

The compressed output file is named by adding ".paq8n" extension to
the first named file (file1.paq8n).  Each file that exists will be
...
If file1.paq8n exists then it is overwritten.

If the first named file ends in ".paq8n" then it is assumed to be
...
The -d option forces extraction even if there is not a ".paq8n"
...
During extraction, directories are created as needed.  For example:
  ...
  paq8n -4 c:\tmp\foo bar

compresses foo and bar (if they exist) to c:\tmp\foo.paq8n at level 4.

  paq8n -d c:\tmp\foo.paq8n .
...
There are 2 files: paq8n.cpp (C++) and paq7asm.asm (NASM/YASM).
paq7asm.asm is the same as in paq7 and paq8x.  paq8n.cpp recognizes the
...
Recommended compiler commands and optimizations:
  ...
  MINGW g++:
    ...
    g++ paq8n.cpp -DWINDOWS -O2 -Os -s -march=pentiumpro -fomit-frame-pointer -o paq8n.exe paq7asm.obj

  Borland:
    nasm paq7asm.asm -f obj --prefix _
    bcc32 -DWINDOWS -O -w-8027 paq8n.cpp paq7asm.obj

  Mars:
    nasm paq7asm.asm -f obj --prefix _
    dmc -DWINDOWS -Ae -O paq8n.cpp paq7asm.obj

  UNIX/Linux (PC):
    nasm -f elf paq7asm.asm
    g++ paq8n.cpp -DUNIX -O2 -Os -s -march=pentiumpro -fomit-frame-pointer -o paq8n paq7asm.o

  Non PC (e.g. PowerPC under MacOS X)
    g++ paq8n.cpp -O2 -DUNIX -DNOASM -s -o paq8n
...
Then
  ...
  paq8n archive \dir1\file1.txt \dir2

will create archive.paq8n with the header:
...
Decompression will fail if the first 7 bytes are not "paq8n -".  Sizes
...
paq8n uses a neural network to combine a large number of models.  The
...
paq8n uses preprocessing transforms on certain data types to improve
...
// To compress to file1.paq8n: paq8n [-n] file1 [file2...]
