...
void sparseModel(Mixer& m) {
  ...
  static SmallStationaryContextMap scm1(0x20000), scm2(0x20000), scm3(0x20000), scm4(0x20000), scm5(0x10000), scm6(0x20000), scm7(0x2000), scm8(0x8000), scm9(0x1000);
...
/////  }
  ...
  if (bpos==0) {
    ...
    if(f2=='.'||f2=='?'||f2=='!'||f2==')') { if (b1!=f2 && buf(3)!=f2 ) i=13, x4=x4*256+f2; }
...
void Predictor::update() {
  ...
  if (c0>=256) {
	...
	if(c0=='.' || c0=='?' || c0=='!' || c0==')') {	w5=(w5<<8)|0x3ff, x5=(x5<<8)+c0, f4=(f4&0xfffffff0)+2;
					if(c0!='!' && c0!='?') w4|=12;
					if(c0!='!') b2='.';
                                }
	f4=f4*16+(c0>>4);
...
int main(int argc, char** argv) {
  ...
  header=ftell(f);
	...
	{
	FILE *dictfile=fopen("./temp_HKCC_dict1.dic","rb"),
		*tmpfi=fopen("./tmp_paq_hp9_tmp.dic","wb");
	if (dictfile==0) perror("temp_HKCC_dict1.dic"), exit(1);
	if (tmpfi   ==0) perror("tmp_paq_hp9_tmp.dic"), exit(1);
	filetype=0;
	Encoder en(COMPRESS, tmpfi);
	en.compress(0);
	for (int i=0; i<420139; ++i)  en.compress(getc(dictfile));
	en.flush();
	}
