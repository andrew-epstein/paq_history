...
// }
	...
	spaces=spaces*2;
	words=words*2;
	if (c==32 || c==10) ++spaces, ++spacecount;
	else
	if ((c>='a' && c<='z') || c>127) ++words, ++wordcount;
...
/////    if (fsize==513216) picModel(m);
		...
		c=c2/64;	if(bpos>1)  c=c0>>(bpos-2);
		c=c1*8+c;
		m.set(c, 2048);

		c=c0*4+(c1/64); /*if(bpos>=6)*/ c=(c&1022) + (c2/128);
		m.set(c, 1024);
...
void Predictor::update() {
  ...
  int pr1=a1.p(pr,  c0, 8)*5+11*pr+8>>4, ps=buf(1), pt=buf(2), pu=buf(3), pv;
		pr=pt; if (pt==10 || pt==32) pr=16;
		pv=pu; if (pu==10 || pu==32) pv=16;
