...
int ContextMap::mix1( Mixer &m, int cc, int bp, int c1, int y1 ) {
  ...
  for( int i = 0; i < cn; ++i ) {
    ...
    if( bpos > 1 && runp[i][0] == 0 )
      ...
      cp0[i] = cpi = t[cxt[i] + cc & t.size() - 1].get( cxt[i] >> 16 );
...
int matchModel( Mixer &m ) {
  ...
  static Array<int> t( MEM * 2 ); // hash table of pointers to contexts
...
static U32 spaces = 0, spacecount = 0, words = 0, wordcount = 0;
...
void wordModel( Mixer &m ) {
  ...
  if( bpos == 0 ) {
    ...
    if( c == 32 || c == 10 )
      ...
      ++words, ++wordcount;
    cm.set( spaces & 0x1fff );
    cm.set( spacecount / 2 );
    cm.set( words & 0xffff );
    cm.set( wordcount / 2 );
    cm.set( wordcount * 64 + spacecount );
  }
  cm.mix( m );
}
...
void recordModel( Mixer &m ) {
  ...
  if( !bpos ) {
    ...
    assert(rlen>0);
#endif
    cm.set( buf( 1 ) << 8 | ( min( 255, pos - cpos1[buf( 1 )] ) / 4 ) );
    cm.set( buf( 1 ) << 17 | buf( 2 ) << 9 | llog( pos - wpos1[w] ) >> 2 );
    int col = pos & 1;
    cm.set( buf( 1 ) << 8 | buf( 2 ) );
...
void sparseModel( Mixer &m ) {
  ...
  if( bpos == 0 ) {
    ...
    cm.set( buf( 1 ) + buf( 5 ) * 256 + ( 1 << 17 ) );
    cm.set( buf( 1 ) + buf( 6 ) * 256 + ( 2 << 17 ) );
    cm.set( buf( 3 ) + buf( 6 ) * 256 + ( 3 << 17 ) );
    cm.set( buf( 4 ) + buf( 8 ) * 256 + ( 4 << 17 ) );
...
int contextModel2() {
  ...
  c = c1 >> 4;
  m.set( order + 8 * c + 128 * ( min( 7, spacecount / 4 ) ), 1024 ); /////+128*(filetype==EXE), 256);
  ...
  c = c0 * 4 + ( c1 / 64 ); /*if(bpos>=6)*/
  c = ( c & 1022 ) + ( c2 / 128 );
  m.set( c, 1024 );

  c = c1 / 32 + 8 * ( c2 / 32 ) + ( c3 & 128 ) + ( c4 >> 31 ) * 64;
  m.set( c, 256 );
  ...
  c = ( min( bpos, 6 ) ) * 256 + ( ( c0 << ( 8 - bpos ) ) & 255 );
  m.set( c + ( ( c1 >> bpos ) & 255 ), 1792 );

  c = bpos * 256 + ( ( c0 << ( 8 - bpos ) ) & 255 );
  c1 = ( words << bpos ) & 255;
  m.set( c + ( ( c1 >> bpos ) & 255 ), 2048 );
...
void Predictor::update() {
  ...
  pr = contextModel2();
  int rate = 6 + ( pos > 1024 * 1024 ) + ( pos > 2048 * 1024 );
  ...
  pv = a5.p( pr1, ( c0 * 16 ) ^ hash( pv >> 4, ps & 0xfc, pr & 0xf8 ) & 0xffff, rate );
  pu = a4.p( pr1, c0 ^ hash( ps, pt, pu ) & 0xffff, rate );
  pt = a3.p( pr1, ( c0 * 4 ) ^ hash( ps, pt ) & 0xffff, rate );
  ps = a2.p( pr1, c0 + 256 * ps, rate );
  pr = ps * 3 + pt * 4 + pu * 6 + pv * 3 + 8 >> 4;
}
...
static char arrdict[398210] = "arrdict";
