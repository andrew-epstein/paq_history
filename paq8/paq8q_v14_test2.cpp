...
int contextModel2() {
  ...
  else if (filetype==IMAGE8) return im8bitModel(m, info), m.p();
  else if (filetype==IMAGE24) return im24bitModel(m, info), m.p();
  else if (filetype==AUDIO) return wavModel(m, info), m.p();
  else if (filetype==JPEG) if (jpegModel(m)) return m.p();
  ...
  if (cmode==6 && filetype!=IMAGE1) {
    ...
    switch (filetype) {
			case TEXT:
				...
				switch (cmode) {
				  case 5: distanceModel(m),indirectModel(m),dmcModel(m),nestModel(m),wordModel(m);break;
					case 4: distanceModel(m),indirectModel(m),dmcModel(m),wordModel(m);break;
					case 3: wordModel(m);break;
					case 2: dmcModel(m);break;
				}
        break;
      case EXE:
				switch (cmode) {
					case 5: sparseModel(m,ismatch,order),ALL_DEFAUL_MODELS(m),exeModel(m);break;
					case 4:
					case 3:
					case 2: exeModel(m);break;
				}
        break;
      default:
				if (useSparseModel) sparseModel(m,ismatch,order);
				for (int i=0;i<dmodelcount;++i) defaultmodels[i](m);
        break;
    }
  }
...
Filetype detect(FILE* in, int n, Filetype type, int &info) {
  ...
  for (int i=0; i<n; ++i) {
    ...
    if (buf1==0x00ffffff && buf0==0xffffffff && !cdi) cdi=i,cda=-1,cdm=0,cddata=new U8[2352],cdd2=new U8[2352];
		if (cdi && i>cdi) {
    ...
    if (txtStart==0 && cmode!=1 && !soi && !pgm && !rgbi && !bmp && !wavi && !cdi && !tga &&
			((c<128 && c>32) || c==10 || c==13 || c==0x12 || c==9 )) txtStart=1,txtOff=i,txtNoDefault=txtUtfi=txtLen=txtEnd=0;
    if (txtStart) {
			if ((c<128 && c>=32) || c==10 || c==13 || c==0x12 || c==9) txtLen++,txtNoDefault=0;
			else if (txtUtf) {
				// valid continue code
				txtUtfi++;
				if ((buf0&0xC0)==0x80) {
					txtLen++;
					// end of UTF symbol
					if (txtUtfi==txtUtf) txtUtfi=txtUtf=txtNoDefault=0;
				} else {
					txtLen-=(txtUtfi);
					unsigned char s;
			...
			else if (((c&0xe0)==0xc0) && !txtUtf) txtLen++,txtUtf=1;
			//if possible UTF8 3 byte
			else if (((c&0xf0)==0xe0) && !txtUtf) txtLen++,txtUtf=2;
			//if possible UTF8 4 byte
			else if (((c&0xf8)==0xf0) && !txtUtf) txtLen++,txtUtf=3;
			//if possible UTF8 5 byte
			else if (((c&0xFC)==0xf8) && !txtUtf) txtLen++,txtUtf=4;
			//if possible UTF8 6 byte
			else if (((c&0xFE)==0xfc) && !txtUtf) txtLen++,txtUtf=5;
			else if (c<32) txtEnd=1;
			else txtNoDefault++,txtLen++;
			if (txtEnd || txtNoDefault>3) {
				txtStart=txtEnd=0;
				if (txtLen>=txtMinLen) {
					if (type==TEXT) return fseek(in, start+txtLen-txtNoDefault, SEEK_SET),DEFAULT;
	...
	if (txtStart) {
		if (txtLen<txtMinLen) txtStart=txtLen=txtOff=0;
...
int decode_txt(Encoder& en, int size, FILE *out, FMode mode, int &diffFound, long s1=0, long s2=0) {
	...
	for (int i=0; i<size; ++i) {
		if (nextOffset==0) {
			en.decompress();
			putc(0x0A, out);
			if (index==lCount) nextOffset=size;
			else {
				nextOffset=offsets[index++]<<8;
				nextOffset|=offsets[index++];
			}
		} else nextOffset--,putc(en.decompress(), out);
	}
...
int encode_cd(FILE* in, FILE* out, int len, int info) {
  ...
  for (int offset=0; offset<len; offset+=BLOCK) {
    if (offset+BLOCK > len) {
      ...
      if (offset+BLOCK*2 > len && blk[15]!=1) fwrite(&blk[16], 1, 4, out), outsize+=4;
    }
  }
  return outsize;
}
...
int decode_cd(FILE *in, int size, FILE *out, FMode mode, int &diffFound) {
  ...
  while (i<size) {
    ...
    if (mode==FDECOMPRESS) fwrite(blk, BLOCK, 1, out);
		else for (int j=0; j<BLOCK; ++j) {
			if (blk[j]!=getc(out)) {
				if (!diffFound) diffFound=i2+j+1;
			}
		}
    i2+=BLOCK;
  }
...
int decode_exe(Encoder& en, int size, FILE *out, FMode mode, int &diffFound, long s1=0, long s2=0) {
  ...
  begin|=en.decompress();
  size-=4;
  for (int i=4; i>=0; i--) c[i]=en.decompress();
...
void compressRecursive(FILE *in, int filesize, Encoder &en, int it=0, int s1=0, int s2=0) {
  static const char* typenames[]={"default", "jpeg", "hdr",
    "1-bit-image", "8-bit-image", "24-bit-image", "audio", "exe", "cd", "text"};
  static const char* audiotypes[]={"8-bit mono", "8-bit stereo", "16-bit mono",
    "16-bit stereo"};
  ...
  FILE* tmp;
	...
	if (it==0) printf("Block segmentation:\n");
	else {
		for (int i=0; i<it; ++i) printf("  ");
  ...
  while (n>0) {
    ...
    if (len>0) {
      ...
      if (type==EXE || type==CD /*|| type==TEXT*/) {
        ...
        if (type==EXE) tmpsize=encode_exe(in, tmp, len, begin);
        else if (type==CD) tmpsize=encode_cd(in, tmp, len, info);
				else if (type==TEXT) tmpsize=encode_txt(in, tmp, len, txtOffsets, txtFailed);
        ...
        long j;
				diffFound=0;
        if (type==EXE) j=decode_exe(en, tmpsize, in, FCOMPARE, diffFound);
        else if (type==CD) j=decode_cd(tmp, tmpsize, in, FCOMPARE, diffFound);
...
void decompressRecursive(FILE *out, long blocksize, Encoder& en, FMode mode,
                         int &diffFound, int it=0, int s1=0, int s2=0) {
  Filetype type;
  long len, i=0;
  FILE *tmp;
  s2+=blocksize;
  while (i<blocksize) {
    ...
    if (type==IMAGE1 || type==IMAGE8 || type==IMAGE24 || type==AUDIO) {
      for (int i=0; i<4; ++i) en.decompress();
    }
...
void decompress(const char* filename, long filesize, Encoder& en) {
  ...
  // Decompress/Compare
  if (!f) mode=FDISCARD,printf("File couldn't be created. Skip...\n");
	else if (mode==FCOMPARE) printf("File found. Comparing...\n");
	else  printf("Extracting %s %ld -> ", filename, filesize);
	setDefaultModels(filename);

	decompressRecursive(f, filesize, en, mode, diffFound);
	if (mode==FCOMPARE && !diffFound) printf("Files are equal. No difference found.\n");
        else if (mode==FCOMPARE) printf("Block difference found at file offset: %u\n", diffFound-1);
	else printf("done        \n");
	if (f) fclose(f);
	delete []defaultmodels;
}
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    if (argc>1 && argv[1][0]=='-' && argv[1][1] && !argv[1][2]) {
        ...
        level=argv[1][1]-'0';
			  if (level<=3) cmode=1;
			} else if (argv[1][1]=='d') doExtract=true;
      else quit("Valid options are -0 through -8 or -d\n");
      --argc;
      ++argv;
      pause=false;
    }
...
//        printf("%s already exists\n", archiveName.c_str()), quit();
      ...
      fprintf(archive, PROGNAME " %d%d\r\n%s\x1A",
        level, cmode, header_string.c_str());
    ...
    // Decompress: open archive for reading and store file names and sizes
      ...
      if (level<0||level>9) level=DEFAULT_OPTION;
			if (cmode<1||cmode>6) cmode=DEFAULT_CMODE;
