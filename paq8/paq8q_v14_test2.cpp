        ...
        if( huffsize + ( rs & 15 ) == huffbits ) { // done decoding
          ...
          // UPDATE_ADV_PRED !!!!
          {
            ...
            if( zz == 0 ) {
              ...
              for( int i = 0; i < 64; ++i ) {
                sumu[zzu[i]] += ( zzv[i] & 1 ? -1 : 1 ) * ( zzv[i] ? 16 * ( 16 + zzv[i] ) : 181 ) * ( qtab[q + i] + 1 )
                                * cbuf2[cpos_dc_mcusize_width + i];
                sumv[zzv[i]] += ( zzu[i] & 1 ? -1 : 1 ) * ( zzu[i] ? 16 * ( 16 + zzu[i] ) : 181 ) * ( qtab[q + i] + 1 )
...
Filetype detect( FILE *in, int n, Filetype type, int &info ) {
  ...
  for( int i = 0; i < n; ++i ) {
    ...
    if( txtStart == 0 && cmode != 1 && !soi && !pgm && !rgbi && !bmp && !wavi && !cdi && !tga
    ...
    if( txtStart ) {
      ...
      else if( ( ( c & 0xe0 ) == 0xc0 ) && !txtUtf )
        txtLen++, txtUtf = 1;
      //if possible UTF8 3 byte
      else if( ( ( c & 0xf0 ) == 0xe0 ) && !txtUtf )
        txtLen++, txtUtf = 2;
      //if possible UTF8 4 byte
      else if( ( ( c & 0xf8 ) == 0xf0 ) && !txtUtf )
        txtLen++, txtUtf = 3;
      //if possible UTF8 5 byte
      else if( ( ( c & 0xFC ) == 0xf8 ) && !txtUtf )
        txtLen++, txtUtf = 4;
      //if possible UTF8 6 byte
      else if( ( ( c & 0xFE ) == 0xfc ) && !txtUtf )
        txtLen++, txtUtf = 5;
      else if( c < 32 )
        txtEnd = 1;
      else
        txtNoDefault++, txtLen++;
      if( txtEnd || txtNoDefault > 3 ) {
        txtStart = txtEnd = 0;
        if( txtLen >= txtMinLen ) {
          if( type == TEXT )
...
int encode_cd( FILE *in, FILE *out, int len, int info ) {
  ...
  for( int offset = 0; offset < len; offset += BLOCK ) {
    if( offset + BLOCK > len ) {
      ...
      outsize += 2048 + 276 * ( info == 3 ) + ( offset == 0 ) * ( 4 + 4 * ( blk[15] != 1 ) );
...
int decode_cd( FILE *in, int size, FILE *out, FMode mode, int &diffFound ) {
  ...
  while( i < size ) {
    ...
    const int tr = 2048 + 276 * ( cdfo == 2 );
    fread( blk + 16 + ( blk[15] != 1 ) * 8, tr, 1, in );
    i += tr;
    if( cdfo == 2 )
      blk[15] = 3;
...
int main( int argc, char **argv ) {
  ...
  try {
    ...
    Mode mode = COMPRESS;
    if( argc > 1 && argv[1][0] == '-' && argv[1][1] == 'm' && argv[1][2] >= '1' && argv[1][2] <= '6' && !argv[1][3] ) {
      cmode = argv[1][2] - '0';
      argc--;
      argv++;
    }
    ...
    // Decompress: open archive for reading and store file names and sizes
      ...
      if( strncmp( header, PROGNAME " ", strlen( PROGNAME ) + 1 ) )
