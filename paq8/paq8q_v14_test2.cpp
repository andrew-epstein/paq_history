...
int contextModel2() {
  ...
  m.set( c1 + 8, 264 );
  m.set( c0, 256 );
  m.set( order + 8 * ( c4 >> 5 & 7 ) + 64 * static_cast<int>( c1 == c2 ) + 128 * static_cast<int>( filetype == EXE ), 256 );
  m.set( c2, 256 );
  m.set( c3, 256 );
  m.set( ismatch, 256 );
...
Filetype detect( FILE *in, int n, Filetype type, int &info ) {
  ...
  for( int i = 0; i < n; ++i ) {
    ...
    if( ( buf0 == 0x4d2e4b2e || buf0 == 0x3643484e || buf0 == 0x3843484e // M.K. 6CHN 8CHN
      ...
      long savedpos = ftell( in );
      const int chn = ( ( buf0 >> 24 ) == 0x36 ? 6 : ( ( ( buf0 >> 24 ) == 0x38 || (((buf0 & static_cast<unsigned int>(static_cast<unsigned int>(0xff == 0x38) != 0U))) != 0U) ) ? 8 : 4 ) );
    ...
    if( txtStart == 0 && cmode != 1 && (soi == 0) && (pgm == 0) && (rgbi == 0) && (bmp == 0) && (wavi == 0) && (cdi == 0) && (tga == 0)
...
int encode_cd( FILE *in, FILE *out, int len, int info ) {
  ...
  for( int offset = 0; offset < len; offset += BLOCK ) {
    if( offset + BLOCK > len ) {
      ...
      outsize += 2048 + 276 * static_cast<int>( info == 3 ) + static_cast<int>( offset == 0 ) * ( 4 + 4 * static_cast<int>( blk[15] != 1 ) );
...
int main( int argc, char **argv ) {
  ...
  try {
    ...
    Mode mode = COMPRESS;
    if( argc > 1 && argv[1][0] == '-' && argv[1][1] == 'm' && argv[1][2] >= '1' && argv[1][2] <= '6' && (argv[1][3] == 0) ) {
      cmode = argv[1][2] - '0';
      argc--;
      argv++;
    }
