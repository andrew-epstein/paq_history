/*  paq8q file compressor/archiver.  Release by Simon Berger May. 31, 2009
...
- To install, put paq8q.exe or a shortcut to it on your desktop.
- To compress a file or folder, drop it on the paq8q icon.
- To decompress, drop a .paq8q file on the icon.

A .paq8q extension is added for compression, removed for decompression.
The output will go in the same folder as the input.

While paq8q is working, a command window will appear and report
...
- To install, put paq8q.exe somewhere in your PATH.
- To compress:      paq8q [-N] [-mM] file1 [file2...]
- To decompress:    paq8q [-d] file1.paq8q [dir2]
- To view contents: more < file1.paq8q

The compressed output file is named by adding ".paq8q" extension to
the first named file (file1.paq8q).  Each file that exists will be
added to the archive and its name will be stored without a path.
The option -N specifies a compression level ranging from -0 to -8.
The default is -5.
-mM specifies the a compression mode ranging from -m1 (fastest) to 
-m6 (slowest). Default is -m5 while -m5 is an "ultra maximum mode"
disableing TEXT detection and using all models.
If there is no option and only one file, then 
the program will pause when finished until you press the ENTER key 
(to support drag and drop).
If file1.paq8q exists then it is overwritten.

If the first named file ends in ".paq8q" then it is assumed to be
...
The -d option forces extraction even if there is not a ".paq8q"
...
During extraction, directories are created as needed.  For example:
  ...
  paq8q -4 c:\tmp\foo bar

compresses foo and bar (if they exist) to c:\tmp\foo.paq8q at level 4
and mode -m5.

  paq8q -d c:\tmp\foo.paq8q .
...
There are 2 files: paq8q.cpp (C++) and paq7asm.asm (NASM/YASM).
paq7asm.asm is the same as in paq7 and paq8x.  paq8q.cpp recognizes the
...
-DEFAULT_CMODE=M to change the default compression mode
...
Recommended compiler commands and optimizations:
  ...
  MINGW g++:
    ...
    g++ paq8q.cpp -DWINDOWS -O2 -Os -s -march=pentiumpro -fomit-frame-pointer -o paq8q.exe paq7asm.obj

  Borland:
    nasm paq7asm.asm -f obj --prefix _
    bcc32 -DWINDOWS -O -w-8027 paq8q.cpp paq7asm.obj

  Mars:
    nasm paq7asm.asm -f obj --prefix _
    dmc -DWINDOWS -Ae -O paq8q.cpp paq7asm.obj

  UNIX/Linux (PC):
    nasm -f elf paq7asm.asm
    g++ paq8q.cpp -DUNIX -O2 -Os -s -march=pentiumpro -fomit-frame-pointer -o paq8q paq7asm.o

  Non PC (e.g. PowerPC under MacOS X)
    g++ paq8q.cpp -O2 -DUNIX -DNOASM -s -o paq8q
...
M is the compression mode (1 to 6)
...
Then
  ...
  paq8q archive \dir1\file1.txt \dir2

will create archive.paq8q with the header:
...
Decompression will fail if the first 6 bytes are not "paq8q ".  Sizes
...
paq8q uses a neural network to combine a large number of models.  The
...
- Image.  For uncompressed 24-bit color BMP, TIFF, PPM, TGA images.
...
- Image.  For 1-bit uncompressed BMP, PBM images.  Contexts are the surrounding
  pixels already seen.

- Image.  For uncompressed 8-bit color BMP, RGB, PGM, TGA images.  Contexts
  are the high order bits of the surrounding pixels and linear
  combinations of those pixels, including other color planes.  The
  image width is detected from the file header.  When an image is
  detected, other models are turned off to improve speed.

- Analog. For uncompressed WAVE files.
...
paq8q uses preprocessing transforms on certain data types to improve
...
- CD/BIN: detects and transforms sectors of cd images
...
Improved TIFF image detection (1-, 8- and 24-bit uncompressed)
Added text detection (from paq8p3)
Splited sparseMotel to sparseTextModel and sparseModel (from paq8p3)
Model long matches directly after normal model (from paq8p3)
Added -mM (mode/speed) parameter
Added chance to disable models by file suffix

DIFFERENCES FROM PAQ8Q_V5
Replaced text detection with an improved variant
Added all improvements from paq8px up to v42
*/

#define PROGNAME "paq8q" // Please change this if you change the program.
...
#endif
#ifndef DEFAULT_CMODE
#  define DEFAULT_CMODE 6
#endif
...
void wordModel( Mixer &m ) {
  ...
  if( bpos == 0 ) {
    ...
    cm.set( col << 8 | buf( 1 ) );
    cm.set( col * static_cast<int>( c == 32 ) );
    cm.set( col );
  }
  cm.mix( m );
}
...
void sparseTextModel( Mixer &m ) {
  static ContextMap cm( MEM, 4 );
  static int mask = 0;
  static SmallStationaryContextMap scm1( 0x8000 );
  static SmallStationaryContextMap scm2( 0x10000 );
...
void sparseModelComplete( Mixer &m, int seenbefore, int howmany ) {
  static ContextMap cm( MEM * 2, 40 );
  if( bpos == 0 ) {
    cm.set( seenbefore );
    cm.set( howmany );
...
void im24bitModel( Mixer &m, int w ) {
  ...
  static SmallStationaryContextMap scm10( SC );
...
void im1bitModel( Mixer &m, int w ) {
  ...
  cxt[7] =
      0x4000 + ( r0 >> 4 & 0x2AC | r1 & 0xA4 | r2 & 0x349 | static_cast<int>( static_cast<int>( r3 ) == 0U ) & 0x14D );
            ...
            for( int i = 0; i < 3; ++i )
              for( int st = 0; st < 8; ++st ) {
                ...
                if( st == 0 ) {
                  ...
                  adv_pred[i] += st * 2 + static_cast<int>( p > 0 ) << 6;
                  if( abs( p / 4 ) > abs( adv_pred[i + 4] ) + 1 )
                    adv_pred[i + 4] += st * 2 + static_cast<int>( p > 0 ) << 6;
                  break;
                }
              }
            x = 2 * sumu[zzu[zz]] + 2 * sumv[zzv[zz]];
            for( int i = 0; i < 8; ++i )
              x -= static_cast<int>( zzu[zz] < i ) * sumu[i] + static_cast<int>( zzv[zz] < i ) * sumv[i];
            x /= ( qtab[q + zz] + 1 ) * 181;
            if( zz == 0 )
  ...
  if( ( bpos == 0 ) && blpos >= w ) {
    ...
    if( msb == 0 ) {
      ...
      for( l = 0; l <= min( S, counter[chn] - 1 ); l++ )
        F[0][l][chn] = F[0][l][chn] * a + X1( l + 1 ) * k;
      for( l = 1; l <= min( D, counter[chn] ); l++ )
        F[0][l + S][chn] = F[0][l + S][chn] * a + X2( l + 1 ) * k;
      if( channels == 2 ) {
        k = X2( 2 );
        for( l = 1; l <= min( D, counter[chn] ); l++ )
          F[S + 1][l + S][chn] = F[S + 1][l + S][chn] * a + X2( l + 1 ) * k;
        for( l = 1; l <= min( S, counter[chn] - 1 ); l++ )
          F[l][S + 1][chn] = F[l][S + 1][chn] * a + X1( l + 1 ) * k;
      }
      if( ++n[chn] == ( 256 >> level ) ) {
        if( channels == 1 )
          for( k = 1; k <= S + D; k++ )
...
void exeModel( Mixer &m ) {
  ...
  if( bpos == 0 ) {
    for( int i = 0; i < N; ++i )
      cm.set( execxt( i + 1, buf( 1 ) * static_cast<int>( i > 6 ) ) );
  }
  cm.mix( m );
}
...
typedef enum { DEFAULT, JPEG, HDR, IMAGE1, IMAGE8, IMAGE24, AUDIO, EXE, CD, TEXT } Filetype;
#define ALL_DEFAUL_MODELS( m ) distanceModel( m ), recordModel( m ), indirectModel( m ), dmcModel( m )
...
int contextModel2() {
  ...
  if( filetype == IMAGE1 )
    im1bitModel( m, info );
  else if( filetype == IMAGE8 )
    return im8bitModel( m, info ), m.p();
  else if( filetype == IMAGE24 )
    return im24bitModel( m, info ), m.p();
  else if( filetype == AUDIO )
    return wavModel( m, info ), m.p();
  ...
  // Model long matches directly, len 1024
  if( ismatch > 160 && cmode != 6 ) {
    m.set( 0, 8 );
    return m.p();
  }

  if( cmode == 6 && filetype != IMAGE1 ) {
    sparseModelComplete( m, ismatch, order );
    distanceModel( m );
    recordModel( m );
    wordModel( m );
    indirectModel( m );
    ...
    switch( filetype ) {
      case TEXT:
        // dublicate calls for maximum speed
        switch( cmode ) {
          case 5:
            distanceModel( m ), indirectModel( m ), dmcModel( m ), nestModel( m ), wordModel( m );
            break;
          case 4:
            distanceModel( m ), indirectModel( m ), dmcModel( m ), wordModel( m );
            break;
          case 3:
            wordModel( m );
            break;
      ...
      case EXE:
        switch( cmode ) {
          case 5:
            sparseModel( m, ismatch, order ), ALL_DEFAUL_MODELS( m ), exeModel( m );
            break;
          case 4:
          case 3:
          case 2:
  ...
  m.set( c1 + 8, 264 );
  m.set( c0, 256 );
  m.set( order + 8 * ( c4 >> 5 & 7 ) + 64 * static_cast<int>( c1 == c2 ) + 128 * static_cast<int>( filetype == EXE ),
         256 );
  m.set( c2, 256 );
  m.set( c3, 256 );
  m.set( ismatch, 256 );
...
int expand_cd_sector( U8 *data, U8 *d2, int a, int test ) {
  eccedc_init();
  d2[0] = d2[11] = 0;
  for( int i = 1; i < 11; ++i )
    d2[i] = 255;
  ...
  if( mode == 2 )
    for( int i = 16; i < 24; ++i )
      d2[i] = data[i - 4 * static_cast<int>( i >= 20 )];
  if( form == 1 ) {
    if( mode == 2 ) {
...
Filetype detect( FILE *in, int n, Filetype type, int &info ) {
  ...
  int wavch;
  int wavbps;
  int wavm; // For WAVE detection
  int aiff = 0;
  int aiffm;
  int aiffs; // For AIFF detection
  int s3mi = 0;
  int s3mno;
  int s3mni; // For S3M detection
  int bmp = 0;
  int bsize;
  int imgbpp;
  int bmpx;
  int bmpy;
  int bmpof; // For BMP detection
  int rgbi = 0;
  int rgbx;
  int rgby; // For RGB detection
  int tga = 0;
  int tgax;
  int tgay;
  int tgaz;
  int tgat; // For TGA detection
  int pgm = 0;
  int pgmcomment = 0;
  int pgmw = 0;
  int pgmh = 0;
  ...
  for( int i = 0; i < n; ++i ) {
    ...
    if( buf1 == 0x00ffffff && buf0 == 0xffffffff && ( cdi == 0 ) )
      cdi = i, cda = -1, cdm = 0, cddata = new U8[2352], cdd2 = new U8[2352];
    if( ( cdi != 0 ) && i > cdi ) {
    ...
    if( ( buf0 == 0x4d2e4b2e || buf0 == 0x3643484e || buf0 == 0x3843484e // M.K. 6CHN 8CHN
      ...
      const int chn =
          ( ( buf0 >> 24 ) == 0x36
                ? 6
                : ( ( ( buf0 >> 24 ) == 0x38
                      || ( ( ( buf0 & static_cast<unsigned int>( static_cast<unsigned int>( 0xff == 0x38 ) != 0U ) ) )
                           != 0U ) )
                        ? 8
                        : 4 ) );
    ...
    if( ( buf0 & 0xffff ) == 16973 )
      imgbpp = bsize = bmpx = bmpy = bmpof = 0, bmp = i; //possible 'BM'
    if( bmp != 0 ) {
      const int p = i - bmp;
      if( p == 4 )
        bsize = bswap( buf0 ); //image size
      else if( p == 12 )
        bmpof = bswap( buf0 );
      else if( p == 16 && buf0 != 0x28000000 )
        bmp = 0; //windows bmp?
      else if( p == 20 )
        bmpx = bswap( buf0 ), bmp = ( ( bmpx == 0 || bmpx > 0x30000 ) ? 0 : bmp ); //width
      else if( p == 24 )
        bmpy = abs( ( int ) bswap( buf0 ) ), bmp = ( ( bmpy == 0 || bmpy > 0x10000 ) ? 0 : bmp ); //height
      else if( p == 27 )
        imgbpp = c, bmp = ( ( imgbpp != 1 && imgbpp != 8 && imgbpp != 24 ) ? 0 : bmp );
      else if( p == 31 ) {
        if( imgbpp != 0 && buf0 == 0 ) {
          if( imgbpp == 1 )
    ...
    if( txtStart == 0 && cmode != 1 && ( soi == 0 ) && ( pgm == 0 ) && ( rgbi == 0 ) && ( bmp == 0 ) && ( wavi == 0 )
        && ( cdi == 0 ) && ( tga == 0 ) && ( ( c < 128 && c > 32 ) || c == 10 || c == 13 || c == 0x12 || c == 9 ) )
      txtStart = 1, txtOff = i, txtNoDefault = txtUtfi = txtLen = txtEnd = 0;
    if( txtStart != 0 ) {
      if( ( c < 128 && c >= 32 ) || c == 10 || c == 13 || c == 0x12 || c == 9 )
        txtLen++, txtNoDefault = 0;
      else if( txtUtf != 0 ) {
        // valid continue code
        txtUtfi++;
        if( ( buf0 & 0xC0 ) == 0x80 ) {
          ...
          // end of UTF symbol
          if( txtUtfi == txtUtf )
            txtUtfi = txtUtf = txtNoDefault = 0;
        } else {
          txtLen -= ( txtUtfi );
          unsigned char s;
          // text if valid normal ascii
          for( int j = 0; j < txtUtfi + 1; ++j ) {
            if( j < 4 )
              s = ( buf0 >> ( j * 8 ) );
            else
              s = ( buf1 >> ( j - 4 ) );

            if( !( ( s < 128 && s >= 32 ) || s == 10 || s == 13 || s == 0x12 || s == 9 ) ) {
              if( s < 32 )
                txtEnd = 1;
              else
                txtNoDefault++, txtLen++;
      ...
      //if possible UTF8 2 byte
      else if( ( ( c & 0xe0 ) == 0xc0 ) && ( txtUtf == 0 ) )
        txtLen++, txtUtf = 1;
      //if possible UTF8 3 byte
      else if( ( ( c & 0xf0 ) == 0xe0 ) && ( txtUtf == 0 ) )
        txtLen++, txtUtf = 2;
      //if possible UTF8 4 byte
      else if( ( ( c & 0xf8 ) == 0xf0 ) && ( txtUtf == 0 ) )
        txtLen++, txtUtf = 3;
      //if possible UTF8 5 byte
      else if( ( ( c & 0xFC ) == 0xf8 ) && ( txtUtf == 0 ) )
        txtLen++, txtUtf = 4;
      //if possible UTF8 6 byte
      else if( ( ( c & 0xFE ) == 0xfc ) && ( txtUtf == 0 ) )
        txtLen++, txtUtf = 5;
      else if( c < 32 )
        txtEnd = 1;
      else
        txtNoDefault++, txtLen++;
      if( ( txtEnd != 0 ) || txtNoDefault > 3 ) {
        txtStart = txtEnd = 0;
        if( txtLen >= txtMinLen ) {
          if( type == TEXT )
            return fseek( in, start + txtLen - txtNoDefault, SEEK_SET ), DEFAULT;
...
int encode_txt( FILE *in, FILE *out, int len, Array<U8> &offsets, int &failed ) {
  int c;
  int lastOffset = 0;
  int gapOffset;
  ...
  for( int i = 0; i < len; ++i ) {
    c = getc( in );
    if( c == 0x0A ) {
      gapOffset = i - lastOffset;
      if( gapOffset > ( 1 << 16 ) - 1 ) {
        failed = 1;
        return 0;
      }
      offsets.push_back( gapOffset >> 8 );
      offsets.push_back( gapOffset );
      lastOffset = i;
      putc( 0x20, out );
    } else
      putc( c, out );
...
int decode_txt( Encoder &en, int size, FILE *out, FMode mode, int &diffFound, long s1 = 0, long s2 = 0 ) {
  Array<U8> offsets;
  int lCount;
  int index = 2;
  int nextOffset;
  lCount = en.decompress();
  lCount |= en.decompress() << 8;
  lCount |= en.decompress() << 16;
  lCount |= en.decompress() << 24;

  for( int i = 0; i < lCount; ++i ) {
    offsets.push_back( en.decompress() );
  }

  nextOffset = offsets[0] << 8 | offsets[1];
  ...
  for( int i = 0; i < size; ++i ) {
    if( nextOffset == 0 ) {
      ...
      if( index == lCount )
        ...
        nextOffset = offsets[index++] << 8;
        nextOffset |= offsets[index++];
      }
    } else
      nextOffset--, putc( en.decompress(), out );
  }

  return size;
}

int encode_cd( FILE *in, FILE *out, int len, int info ) {
  ...
  for( int offset = 0; offset < len; offset += BLOCK ) {
    if( offset + BLOCK > len ) {
      ...
      if( info == 3 )
        blk[15] = 3;
      if( offset == 0 )
        fwrite( &blk[12], 1, 4 + 4 * static_cast<int>( blk[15] != 1 ), out );
      fwrite( &blk[16 + 8 * static_cast<int>( blk[15] != 1 )], 1, 2048 + 276 * static_cast<int>( info == 3 ), out );
      outsize += 2048 + 276 * static_cast<int>( info == 3 )
                 + static_cast<int>( offset == 0 ) * ( 4 + 4 * static_cast<int>( blk[15] != 1 ) );
      if( offset + BLOCK * 2 > len && blk[15] != 1 )
        fwrite( &blk[16], 1, 4, out ), outsize += 4;
    }
  }
  return outsize;
}
...
int decode_cd( FILE *in, int size, FILE *out, FMode mode, int &diffFound ) {
  ...
  while( i < size ) {
    if( size - i == q ) {
      ...
      if( blk[15] != 1 )
        fread( blk + 16, 4, 1, in );
      cdfo = 1 + static_cast<int>( blk[15] == 3 );
      a = -1;
      i += 4 * static_cast<int>( blk[15] != 1 ) + 4;
    } else {
      a = ( blk[12] << 16 ) + ( blk[13] << 8 ) + blk[14];
    }
    const int tr = 2048 + 276 * static_cast<int>( cdfo == 2 );
    fread( blk + 16 + static_cast<int>( blk[15] != 1 ) * 8, tr, 1, in );
    i += tr;
    if( cdfo == 2 )
      blk[15] = 3;
...
int encode_exe( FILE *in, FILE *out, int len, int begin ) {
...
void direct_encode_block( Filetype type, FILE *in, int len, int info, Encoder &en, int s1, int s2 ) {
  ...
  if( info != -1 ) {
    en.compress( info >> 24 ), en.compress( info >> 16 ), en.compress( info >> 8 ), en.compress( info );
  }
  printf( "Compressing... " );
...
void setDefaultModels( const char *filename ) {
  //distanceModel(m),recordModel(m),indirectModel(m),dmcModel(m)
  ...
  // Implement more file formats here
  if( len > 5 && strcmp( filename + len - 4, ".mpr" ) == 0 ) {
    switch( cmode ) {
      case 5:
        useSparseModel = 1;
        dmodelcount = 4;
        defaultmodels = new modelpointer[dmodelcount];
        defaultmodels[0] = distanceModel;
        defaultmodels[1] = recordModel;
        defaultmodels[2] = indirectModel;
        defaultmodels[3] = dmcModel;
        break;
      case 4:
      case 3:
      case 2:
        ...
        dmodelcount = 2;
        defaultmodels = new modelpointer[dmodelcount];
        defaultmodels[0] = recordModel;
        defaultmodels[1] = dmcModel;
        break;
    }
  } else {
    switch( cmode ) {
      ...
      case 2:
        ...
        defaultmodels[0] = distanceModel;
        defaultmodels[1] = recordModel;
        defaultmodels[2] = dmcModel;
        break;
    }
  }
  assert( defaultmodels != 0 );
}

#define MAX_ITER 5

void compressRecursive( FILE *in, int filesize, Encoder &en, int it = 0, int s1 = 0, int s2 = 0 ) {
  static const char *typenames[] = {"default",      "jpeg",  "hdr", "1-bit-image", "8-bit-image",
                                    "24-bit-image", "audio", "exe", "cd",          "text"};
  static const char *audiotypes[] = {"8-bit mono", "8-bit stereo", "16-bit mono", "16-bit stereo"};
  ...
  if( it == 0 )
    ...
    for( int i = 0; i < it; ++i )
      printf( "  " );
    printf( "On segmentation level %d\n", it );
  }
  ...
  while( n > 0 ) {
    ...
    if( len > 0 ) {
      ...
      for( int i = 0; i < it; ++i )
        printf( "  " );
      printf( "%3d | %12s | %d bytes [%d - %d]", blnum++, typenames[type], len, begin, end - 1 );
      if( type == AUDIO )
        printf( " (%s)", audiotypes[info % 4] );
      else if( type == IMAGE1 || type == IMAGE8 || type == IMAGE24 )
        printf( " (width: %d)", info );
      else if( type == CD )
        printf( " (m%d/f%d)", info == 1 ? 1 : 2, info != 3 ? 1 : 2 );
      printf( "\n" );
      if( type == EXE || type == CD /*|| type==TEXT*/ ) {
        Array<U8> txtOffsets;
        int txtFailed = 0;
        long tmpsize;
        ...
        if( tmp == nullptr )
          perror( "tmpfile" ), quit();
        if( type == EXE )
          tmpsize = encode_exe( in, tmp, len, begin );
        else if( type == CD )
          tmpsize = encode_cd( in, tmp, len, info );
        else if( type == TEXT )
          tmpsize = encode_txt( in, tmp, len, txtOffsets, txtFailed );
        ...
        diffFound = 0;
        if( type == EXE )
          j = decode_exe( en, tmpsize, in, FCOMPARE, diffFound );
        else if( type == CD )
          j = decode_cd( tmp, tmpsize, in, FCOMPARE, diffFound );

        // Test fails, compress without transform
        if( ( diffFound != 0 ) || ( txtFailed != 0 ) ) {
          ...
          if( type == CD ) {
            ...
            compressRecursive( tmp, tmpsize, en, it + 1, s1, s2 );
            for( int i = 0; i < it; ++i )
              printf( "  " );
            printf( "On segmentation level %d\n", it );
          } else if( type == TEXT ) {
            en.compress( type );
            en.compress( tmpsize >> 24 );
            en.compress( tmpsize >> 16 );
            en.compress( tmpsize >> 8 );
            en.compress( tmpsize );
            en.compress( txtOffsets.size() ), en.compress( txtOffsets.size() >> 8 );
            en.compress( txtOffsets.size() >> 16 ), en.compress( txtOffsets.size() >> 24 );
            for( int i = 0; i < txtOffsets.size(); ++i )
              en.compress( txtOffsets[i] );
...
void compress( const char *filename, long filesize, Encoder &en ) {
  ...
  long start = en.size();
  setDefaultModels( filename );
  compressRecursive( in, filesize, en );
  delete[] defaultmodels;
  if( in != nullptr )
    fclose( in );
...
void decompressRecursive( FILE *out, long blocksize, Encoder &en, FMode mode, int &diffFound, int it = 0, int s1 = 0,
                          int s2 = 0 ) {
  Filetype type;
  long len;
  long i = 0;
  ...
  while( i < blocksize ) {
    ...
    //if (type==TEXT) len=decode_txt(en, len, out, mode, diffFound, s1, s2);
    if( type == EXE )
      len = decode_exe( en, len, out, mode, diffFound, s1, s2 );
    else if( type == CD ) {
      tmp = tmpfile();
      decompressRecursive( tmp, len, en, FDECOMPRESS, diffFound, it + 1, s1 + i, s2 - len );
      if( mode != FDISCARD ) {
        rewind( tmp );
...
void decompress( const char *filename, long filesize, Encoder &en ) {
  ...
  // Decompress/Compare
  if( f == nullptr )
    mode = FDISCARD, printf( "File couldn't be created. Skip...\n" );
  else if( mode == FCOMPARE )
    printf( "File found. Comparing...\n" );
  else
    printf( "Extracting %s %ld -> ", filename, filesize );
  setDefaultModels( filename );

  decompressRecursive( f, filesize, en, mode, diffFound );
  if( mode == FCOMPARE && ( diffFound == 0 ) )
    printf( "Files are equal. No difference found.\n" );
  else if( mode == FCOMPARE )
    printf( "Block difference found at file offset: %u\n", diffFound - 1 );
  else
    printf( "done        \n" );
  if( f != nullptr )
    fclose( f );
...
int main( int argc, char **argv ) {
  ...
  try {
...
#endif
                       "To compress:\n"
                       "  " PROGNAME " -level [-mode] file               (compresses to file." PROGNAME ")\n"
                       "  " PROGNAME " -level [-mode] archive files...   (creates archive." PROGNAME ")\n"
                       "  " PROGNAME " file                      (level -%d, pause when done)\n"
                       "level: -0 = store, -1 -2 -3 -4 -5 -6 -7 -8 uses 35, 48, 59, 133, 233, 435, 837, 1643 MB\n"
                       "mode: -m1 to -m6 (-m6 = maximum compression; -m1 = fastest; -m6 = Default)\n"
    ...
    Mode mode = COMPRESS;
    if( argc > 1 && argv[1][0] == '-' && argv[1][1] == 'm' && argv[1][2] >= '1' && argv[1][2] <= '6'
        && ( argv[1][3] == 0 ) ) {
      cmode = argv[1][2] - '0';
      argc--;
      argv++;
    ...
    if( mode == COMPRESS ) {
      ...
      fprintf( archive, PROGNAME " %d%d\r\n%s\x1A", level, cmode, header_string.c_str() );
    ...
    // Decompress: open archive for reading and store file names and sizes
      ...
      while( getline( archive ) != nullptr )
        ++files; // count files
      printf( "Extracting %d file(s) from %s -%d -m%d\n", files, archiveName.c_str(), level, cmode );
      ...
      for( int i = 0; i < files; ++i ) {
        ...
        while( ( *p != 0 ) && *p != '\r' )
          ++p;
        if( *p == 0 )
          printf( "%s: header corrupted at %d\n", archiveName.c_str(), p - &filenames[0] ), quit();
