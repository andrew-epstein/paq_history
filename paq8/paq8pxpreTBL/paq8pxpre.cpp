...
#else
for (i=0; i<N; ++i) m.add(sttab[sm[i].p(t[cxt[i]])]);
#endif
}
...
#else
  case 0: for (int i=0; i<N; ++i) cp[i]=t[cxt[i]]+1, m1.add(sttab[sm[i].p(*cp[i])]); break;
  case 1: { int hc=1+(huffcode&1)*3; for (int i=0; i<N; ++i) cp[i]+=hc, m1.add(sttab[sm[i].p(*cp[i])]); } break;
  default: { int hc=1+(huffcode&1); for (int i=0; i<N; ++i) cp[i]+=hc, m1.add(sttab[sm[i].p(*cp[i])]); } break;
#endif
  }
...
void parsePrecompSwitch(char *s) {
  ...
  switch (toupper(s[1])) {
    case 'I':
      {
        ...
        switches.ignore_list = (unsigned int*)realloc(switches.ignore_list, (switches.ignore_list_len + 1) * sizeof(int));
    ...
    case 'B':
      {
        if ((toupper(s[2]) == 'R') && (toupper(s[3]) == 'U') && (toupper(s[4]) == 'T') && (toupper(s[5]) == 'E')) {
    ...
    case 'P':
      {
        if ((toupper(s[2]) == 'D') && (toupper(s[3]) == 'F') && (toupper(s[4]) == 'B') && (toupper(s[5]) == 'M') && (toupper(s[6]) == 'P')) {
          switch (s[7]) {
            case '+':
...
void compress(const char* filename, long filesize, Encoder& en) {
  ...
  // Precompress file
  {
    ...
    int i=0;
    while (i<255 && filename[i]!=0) name[i]=filename[i], i++; name[i]=0;
  ...
  if (!in) perror("~temp.pcf"), quit();

  fseek(in, 0, SEEK_END);
  long tsize=ftell(in);
  rewind(in);
  long start=en.size();
  for (int i=0; i<4; i++) en.compress(0);
  en.compress(4);
  for (int i=0; i<4; i++) en.compress((tsize>>(8*i))&255);
...
void decompress(const char* filename, long filesize, Encoder& en) {
  ...
  if (f) mode=FDISCARD,printf("Skipping");
  else {
    // Create file
    f=fopen("~temp.pcf", "wb");
  ...
  long tsize=0;
  for (int i=0; i<5; i++) en.decompress();
  for (int i=0; i<4; i++) tsize+=(en.decompress())<<(8*i);
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    // Load precomp.dll
    HINSTANCE hDll;
    if((hDll = LoadLibrary("precomp.dll")) == NULL) quit("Error loading precomp.dll\n");
