/* paq8pxpre file compressor/archiver.  Release by Jan Ondrus, Jan. 9, 2010
    ...
    (C) 2007/08 Christian Schneider, schnaader@gmx.de, http://schnaader.info

    Copyright (C) 2010 Matt Mahoney, Serge Osnach, Alexander Ratushnyak,
...
- To install, put paq8pxpre.exe or a shortcut to it on your desktop.
- To compress a file or folder, drop it on the paq8pxpre icon.
- To decompress, drop a .paq8pxpre file on the icon.

A .paq8pxpre extension is added for compression, removed for decompression.
The output will go in the same folder as the input.

While paq8pxpre is working, a command window will appear and report
...
- To install, put paq8pxpre.exe somewhere in your PATH.
- To compress:      paq8pxpre [-N] file1 [file2...]
- To decompress:    paq8pxpre [-d] file1.paq8pxpre [dir2]
- To view contents: more < file1.paq8pxpre

The compressed output file is named by adding ".paq8pxpre" extension to
the first named file (file1.paq8pxpre).  Each file that exists will be
...
If file1.paq8pxpre exists then it is overwritten.

If the first named file ends in ".paq8pxpre" then it is assumed to be
...
The -d option forces extraction even if there is not a ".paq8pxpre"
...
During extraction, directories are created as needed.  For example:
  ...
  paq8pxpre -4 c:\tmp\foo bar

compresses foo and bar (if they exist) to c:\tmp\foo.paq8pxpre at level 4.

  paq8pxpre -d c:\tmp\foo.paq8pxpre .
...
There are 2 files: paq8pxpre.cpp (C++) and paq7asm.asm (NASM/YASM).
paq7asm.asm is the same as in paq7 and paq8x.  paq8pxpre.cpp recognizes the
...
Recommended compiler commands and optimizations:
  ...
  MINGW g++:
    ...
    g++ paq8pxpre.cpp -DWINDOWS -O2 -Os -s -march=pentiumpro -fomit-frame-pointer -o paq8pxpre.exe paq7asm.obj

  Borland:
    nasm paq7asm.asm -f obj --prefix _
    bcc32 -DWINDOWS -O -w-8027 paq8pxpre.cpp paq7asm.obj

  Mars:
    nasm paq7asm.asm -f obj --prefix _
    dmc -DWINDOWS -Ae -O paq8pxpre.cpp paq7asm.obj

  UNIX/Linux (PC):
    nasm -f elf paq7asm.asm
    g++ paq8pxpre.cpp -DUNIX -O2 -Os -s -march=pentiumpro -fomit-frame-pointer -o paq8pxpre paq7asm.o

  Non PC (e.g. PowerPC under MacOS X)
    g++ paq8pxpre.cpp -O2 -DUNIX -DNOASM -s -o paq8pxpre
...
Then
  ...
  paq8pxpre archive \dir1\file1.txt \dir2

will create archive.paq8pxpre with the header:
...
The command:
  ...
  paq8pxpre archive.paq8pxpre C:\dir3
...
Decompression will fail if the first 7 bytes are not "paq8pxpre -".  Sizes
...
paq8pxpre uses a neural network to combine a large number of models.  The
...
paq8pxpre uses preprocessing transforms on certain data types to improve
...
Added pregenerated tables, compile with -GENTABS to remove, by [IO]
*/

#define PROGNAME "paq8pxpre" // Please change this if you change the program.
...
#ifndef GENTABS
#  include "OVL/squashtab.h" // Why gen when you can lookup ?
#  include "OVL/stretch.h"
#  include "OVL/ilog.h"
#endif
...
typedef bool ( *PRECOMP_CALL )( char *, char *, char *, Switches );
PRECOMP_CALL precompress_file, recompress_file;
Switches switches;
...
int jpegModel( Mixer &m ) {
  ...
  switch( hbcount ) {
    ...
    case 0:
      for( int i = 0; i < N; ++i )
        cp[i] = t[cxt[i]] + 1, m1.add( sttab[sm[i].p( *cp[i] )] );
      break;
    case 1: {
      int hc = 1 + ( huffcode & 1 ) * 3;
      for( int i = 0; i < N; ++i )
        cp[i] += hc, m1.add( sttab[sm[i].p( *cp[i] )] );
    } break;
    default: {
      int hc = 1 + ( huffcode & 1 );
      for( int i = 0; i < N; ++i )
...
//Precomp error routine

#define ERR_IGNORE_POS_TOO_BIG 1
#define ERR_IDENTICAL_BYTE_SIZE_TOO_BIG 2
#define ERR_ONLY_SET_MIN_SIZE_ONCE 3

void error( int error_nr ) {
  printf( "\nERROR: " );
  switch( error_nr ) {
    case ERR_IGNORE_POS_TOO_BIG:
      printf( "Ignore position bigger than 10 GB" );
      break;
    case ERR_IDENTICAL_BYTE_SIZE_TOO_BIG:
      printf( "Identical bytes size bigger than 10 GB" );
      break;
...
//Precomp switches parsing routine

void parsePrecompSwitch( char *s ) {
  static bool min_ident_size_set = false;
  int j;
  if( s[0] != '-' )
    return;
  switch( toupper( s[1] ) ) {
    case 'I': {
      ...
      for( j = ( strlen( s ) - 3 ); j >= 0; j-- ) {
        ignore_pos += ( ( unsigned int ) ( s[j + 2] ) - 48 ) * multiplicator;
        if( ( multiplicator * 10 ) < multiplicator ) {
          error( ERR_IGNORE_POS_TOO_BIG );
        }
        multiplicator *= 10;
      }
      switches.ignore_list =
          ( unsigned int * ) realloc( switches.ignore_list, ( switches.ignore_list_len + 1 ) * sizeof( int ) );
      switches.ignore_list[switches.ignore_list_len] = ignore_pos;
      switches.ignore_list_len++;
      break;
    }
    case 'S': {
      if( toupper( s[2] ) == 'L' ) { //slow mode
        if( ( toupper( s[3] ) == 'O' ) && ( toupper( s[4] ) == 'W' ) ) {
          switches.slow_mode = true;
        } else {
        ...
        unsigned int multiplicator = 1;
        for( j = ( strlen( s ) - 3 ); j >= 0; j-- ) {
          ident_size += ( ( unsigned int ) ( s[j + 2] ) - 48 ) * multiplicator;
          if( ( multiplicator * 10 ) < multiplicator ) {
            error( ERR_IDENTICAL_BYTE_SIZE_TOO_BIG );
          }
          multiplicator *= 10;
        }
        switches.min_ident_size = ident_size;
        min_ident_size_set = true;
      }
      break;
    }
    case 'B': {
      if( ( toupper( s[2] ) == 'R' ) && ( toupper( s[3] ) == 'U' ) && ( toupper( s[4] ) == 'T' )
          && ( toupper( s[5] ) == 'E' ) ) {
        switches.brute_mode = true;
      } else {
        printf( "ERROR: Unknown switch \"%s\"\n", s );
        quit();
      }
    ...
    case 'P': {
      if( ( toupper( s[2] ) == 'D' ) && ( toupper( s[3] ) == 'F' ) && ( toupper( s[4] ) == 'B' )
          && ( toupper( s[5] ) == 'M' ) && ( toupper( s[6] ) == 'P' ) ) {
        switch( s[7] ) {
          case '+':
            switches.pdf_bmp_mode = true;
            break;
          case '-':
    ...
    case 'T': {
      ...
      switch( s[2] ) {
        case '+':
          switches.use_pdf = false;
          switches.use_zip = false;
          switches.use_gzip = false;
          switches.use_png = false;
          switches.use_gif = false;
          switches.use_jpg = false;
          set_to = true;
          break;
        case '-':
      ...
      for( j = 3; j < ( int ) strlen( s ); j++ ) {
        switch( toupper( s[j] ) ) {
          case 'P': //PDF
            switches.use_pdf = set_to;
            break;
          case 'Z': //ZIP
            switches.use_zip = set_to;
            break;
          case 'G': //GZip
            switches.use_gzip = set_to;
            break;
          case 'N': //PNG
            switches.use_png = set_to;
            break;
          case 'F': //GIF
            switches.use_gif = set_to;
            break;
          case 'J': //JPG
            switches.use_jpg = set_to;
            break;
          default:
    ...
    case 'C': {
      for( j = 0; j < 9; j++ ) {
        switches.use_comp_level[j] = false;
      }
      switches.level_switch = true;
      for( j = 0; j < ( int ) ( strlen( s ) - 2 ); j++ ) {
        int comp_level_to_use = ( char( s[j + 2] ) - '1' );
        if( ( comp_level_to_use >= 0 ) && ( comp_level_to_use <= 8 ) ) {
          switches.use_comp_level[comp_level_to_use] = true;
        } else {
          printf( "ERROR: Invalid compression level %c\n", s[j + 2] );
          quit();
        }
      }
      break;
    ...
    case 'M': {
      for( j = 0; j < 9; j++ ) {
        switches.use_mem_level[j] = false;
      }
      switches.level_switch = true;
      for( j = 0; j < ( int ) ( strlen( s ) - 2 ); j++ ) {
        int mem_level_to_use = ( char( s[j + 2] ) - '1' );
        if( ( mem_level_to_use >= 0 ) && ( mem_level_to_use <= 8 ) ) {
          switches.use_mem_level[mem_level_to_use] = true;
        } else {
          printf( "ERROR: Invalid memory level %c\n", s[j + 2] );
          quit();
        }
      }
      break;
...
void compressRecursive( FILE *in, long n, Encoder &en, char *blstr, int it = 0, int s1 = 0, int s2 = 0 ) {
  static const char *typenames[9] = {"default",   "jpeg",  "hdr", "1b-image", "8b-image",
                                     "24b-image", "audio", "exe", "cd"};
  ...
  while( n > 0 ) {
    ...
    if( len > 0 ) {
      ...
      if( type == EXE || type == CD || type == IMAGE24 ) {
        ...
        int diffFound = 0;
        if( type == IMAGE24 )
          decode_bmp( en, tmpsize, info, in, FCOMPARE, diffFound );
        else if( type == EXE )
          decode_exe( en, tmpsize, in, FCOMPARE, diffFound );
        else if( type == CD )
...
void compress( const char *filename, long filesize, Encoder &en ) {
  ...
  // Precompress file
  {
    char msg[256], name[256], tempname[16] = "~temp.pcf";
    int i = 0;
    while( i < 255 && filename[i] != 0 )
      name[i] = filename[i], i++;
    name[i] = 0;
    if( !precompress_file( name, tempname, msg, switches ) ) {
      printf( "%s\n", msg );
      quit();
    }
  }

  FILE *in = fopen( "~temp.pcf", "rb" );
  if( !in )
    perror( "~temp.pcf" ), quit();
  ...
  en.compress( 4 );
  for( int i = 0; i < 4; i++ )
    en.compress( ( tsize >> ( 8 * i ) ) & 255 );

  printf( "Block segmentation:\n" );
  char blstr[32] = "";
  compressRecursive( in, tsize, en, blstr );
  if( in ) {
    fclose( in );
    remove( "~temp.pcf" );
  }
  printf( "Transformed from %ld to %ld and compressed to %ld bytes.\n", filesize, tsize, en.size() - start );
}
...
int decompressRecursive( FILE *out, long size, Encoder &en, FMode mode, int it = 0, int s1 = 0, int s2 = 0 ) {
  ...
  while( i < size ) {
    ...
    if( type == IMAGE24 )
      len = decode_bmp( en, len, info, out, mode, diffFound );
    else if( type == EXE )
...
void decompress( const char *filename, long filesize, Encoder &en ) {
  ...
  // read size of transformed file
  long tsize = 0;
  for( int i = 0; i < 5; i++ )
    en.decompress();
  for( int i = 0; i < 4; i++ )
    tsize += ( en.decompress() ) << ( 8 * i );

  // Decompress/Compare
  int r = decompressRecursive( f, tsize, en, mode );
  ...
  if( mode == FDECOMPRESS ) {
    ...
    while( i < 255 && filename[i] != 0 )
      name[i] = filename[i], i++;
    name[i] = 0;
    if( !recompress_file( tempname, name, msg, switches ) ) {
      printf( "%s\n", msg );
      quit();
    }
    remove( "~temp.pcf" );
...
// To compress to file1.paq8pxpre: paq8pxpre [-n] file1 [file2...]
// To decompress: paq8pxpre file1.paq8pxpre [output_dir]
int main( int argc, char **argv ) {
  bool pause = argc <= 2; // Pause when done?
  try {
    // Load precomp.dll
    HINSTANCE hDll;
    if( ( hDll = LoadLibrary( "precomp.dll" ) ) == NULL )
      quit( "Error loading precomp.dll\n" );
    precompress_file = ( PRECOMP_CALL ) GetProcAddress( hDll, "_Z16precompress_filePcS_S_8Switches" );
    recompress_file = ( PRECOMP_CALL ) GetProcAddress( hDll, "_Z15recompress_filePcS_S_8Switches" );
    ...
    while( argc > 1 && argv[1][0] == '-' && argv[1][1] ) {
      if( !argv[1][2] && argv[1][1] >= '0' && argv[1][1] <= '9' )
        level = argv[1][1] - '0';
      else if( !argv[1][2] && argv[1][1] == 'd' )
        doExtract = true;
      else
        parsePrecompSwitch( argv[1] );
      --argc;
      ++argv;
      pause = false;
    }
    ...
    if( argc < 2 ) {
      printf( PROGNAME " archiver (C) 2008, Matt Mahoney et al.\n"
                       ...
                       "\n",
              DEFAULT_OPTION );
      printf( "Precomp switches (and their <default values>):\n" );
      printf( "  f           Fast mode - use first found compression lvl for all streams <off>\n" );
      printf( "  c[1..9]     Compression levels to try for compression <123456789>\n" );
      printf( "  m[1..9]     Memory levels to try for compression <123456789>\n" );
      printf( "  i[pos]      Ignore stream at input file position [pos] <none>\n" );
      printf( "  s[size]     Set minimal identical byte size to [size] <4>\n" );
      printf( "  slow        Detect raw zLib headers, too. Slower and more sensitive <off>\n" );
      printf( "  brute       Brute force zLib detection. VERY Slow and most sensitive <off>\n" );
      printf( "  pdfbmp[+-]  Wrap a BMP header around PDF images <on>\n" );
      printf( "  v           Verbose (debug) mode <off>\n\n" );
      printf( "  t[+-][pzgnfj] Compression type switch <jpeg disabled>\n" );
      printf( "              t+ enables certain compression types and disables the other ones\n" );
      printf( "              t- disables certain compression types and enables the other ones\n" );
