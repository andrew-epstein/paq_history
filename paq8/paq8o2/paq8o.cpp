/* paq8o file compressor/archiver.  Release by Andreas Morphis(andreasmorphis@yahoo.com), Aug. 24, 2007.
...
- To install, put paq8o.exe or a shortcut to it on your desktop.
- To compress a file or folder, drop it on the paq8o icon.
- To decompress, drop a .paq8o file on the icon.

A .paq8o extension is added for compression, removed for decompression.
The output will go in the same folder as the input.

While paq8o is working, a command window will appear and report
...
- To install, put paq8o.exe somewhere in your PATH.
- To compress:      paq8o [-N] file1 [file2...]
- To decompress:    paq8o [-d] file1.paq8o [dir2]
- To view contents: more < file1.paq8o

The compressed output file is named by adding ".paq8o" extension to
the first named file (file1.paq8o).  Each file that exists will be
...
If file1.paq8o exists then it is overwritten.

If the first named file ends in ".paq8o" then it is assumed to be
...
The -d option forces extraction even if there is not a ".paq8o"
...
During extraction, directories are created as needed.  For example:
  ...
  paq8o -4 c:\tmp\foo bar

compresses foo and bar (if they exist) to c:\tmp\foo.paq8o at level 4.

  paq8o -d c:\tmp\foo.paq8o .
...
There are 2 files: paq8o.cpp (C++) and paq7asm.asm (NASM/YASM).
paq7asm.asm is the same as in paq7 and paq8x.  paq8o.cpp recognizes the
...
Recommended compiler commands and optimizations:
  ...
  MINGW g++:
    ...
    g++ paq8o.cpp -DWINDOWS -O2 -Os -s -march=pentiumpro -fomit-frame-pointer -o paq8o.exe paq7asm.obj

  Borland:
    nasm paq7asm.asm -f obj --prefix _
    bcc32 -DWINDOWS -O -w-8027 paq8o.cpp paq7asm.obj

  Mars:
    nasm paq7asm.asm -f obj --prefix _
    dmc -DWINDOWS -Ae -O paq8o.cpp paq7asm.obj

  UNIX/Linux (PC):
    nasm -f elf paq7asm.asm
    g++ paq8o.cpp -DUNIX -O2 -Os -s -march=pentiumpro -fomit-frame-pointer -o paq8o paq7asm.o

  Non PC (e.g. PowerPC under MacOS X)
    g++ paq8o.cpp -O2 -DUNIX -DNOASM -s -o paq8o
...
Then
  ...
  paq8o archive \dir1\file1.txt \dir2

will create archive.paq8o with the header:
...
Decompression will fail if the first 7 bytes are not "paq8o -".  Sizes
...
paq8o uses a neural network to combine a large number of models.  The
...
paq8o uses preprocessing transforms on certain data types to improve
...
#define PROGNAME "paq8o" // Please change this if you change the program.
...
int jpegModel( Mixer &m ) {
  ...
  static Array<int> adv_pred( 24 );
  static Array<int> sum( 16 );
        ...
        if( huffsize + ( rs & 15 ) == huffbits ) { // done decoding
          ...
          // UPDATE_ADV_PRED !!!!
          {
            ...
            for( i = 1; i < ( mcusize >> 6 ); i++ )
              if( color[( acomp + i ) % ( mcusize >> 6 )] == color[acomp] )
                left_s = ( ( mcusize >> 6 ) - i ) << 6;
            ...
            if( zz == 0 ) {
              ...
              for( i = 0; i < 64; i++ ) {
                ...
                sum[8 + zzv[i]] += ( zzu[i] != 0u ? 256 : 181 ) * ( (zzu[i] & 1) != 0 ? -1 : +1 ) * ( qtab[q + i] + 1 )
                                   * cbuf2[cpos_dc + i - left_s];
              }
            } else {
            ...
            for( int st = 0; st < 8; st++ ) {
              ...
              for( i = 3 * st; i < 3 * st + 3; i++ ) {
                ...
                if( zz2 == 0 )
                  adv_pred[i] -= cbuf2[cpos_dc - left_s];
                adv_pred[i] = ( adv_pred[i] < 0 ? -1 : 1 ) * ilog( 8 * abs( adv_pred[i] ) + 1 ) / 8;
              }
            }
            for( i = 0; i < 3; i++ ) {
              int tmp = 0, j = 1;
              while( j < 8 && abs( tmp = adv_pred[i + 3 * j] ) < abs( adv_pred[i] ) + 2 )
                j++;
              if( j != 8 )
                adv_pred[i] += 128 * j + 64 * static_cast<int>( tmp > 0 );
            }
          } // !!!!
        }
      }
