/* paq8ja file compressor/archiver.
(C) 2006, Matt Mahoney under GPL, http://www.gnu.org/licenses/gpl.txt
Release by Serge Osnach.

To install and use in Windows:

- To install, put paq8ja.exe or a shortcut to it on your desktop.
- To compress a file or folder, drop it on the paq8ja icon.
- To decompress, drop a .paq8ja file on the icon.

A .paq8ja extension is added for compression, removed for decompression.
The output will go in the same folder as the input.

While paq8ja is working, a command window will appear and report
...
- To install, put paq8ja.exe somewhere in your PATH.
- To compress:      paq8ja [-N] file1 [file2...]
- To decompress:    paq8ja [-d] file1.paq8ja [dir2]
- To view contents: more < file1.paq8ja

The compressed output file is named by adding ".paq8ja" extension to
the first named file (file1.paq8ja).  Each file that exists will be
...
If file1.paq8ja exists then it is overwritten.

If the first named file ends in ".paq8ja" then it is assumed to be
...
The -d option forces extraction even if there is not a ".paq8ja"
...
During extraction, directories are created as needed.  For example:
  ...
  paq8ja -4 c:\tmp\foo bar

compresses foo and bar (if they exist) to c:\tmp\foo.paq8ja at level 4.

  paq8ja -d c:\tmp\foo.paq8ja .
...
There are 2 files: paq8ja.cpp (C++) and paq7asm.asm (NASM/YASM).
paq7asm.asm is the same as in paq7 and paq8x.  paq8ja.cpp recognizes the
...
Recommended compiler commands and optimizations:
  ...
  MINGW g++:
    ...
    g++ paq8ja.cpp -DWINDOWS -O2 -Os -s -march=pentiumpro -fomit-frame-pointer -o paq8ja.exe paq7asm.obj

  Borland:
    nasm paq7asm.asm -f obj --prefix _
    bcc32 -DWINDOWS -O -w-8027 paq8ja.cpp paq7asm.obj

  Mars:
    nasm paq7asm.asm -f obj --prefix _
    dmc -DWINDOWS -Ae -O paq8ja.cpp paq7asm.obj

  UNIX/Linux (PC):
    nasm -f elf paq7asm.asm
    g++ paq8ja.cpp -DUNIX -O2 -Os -s -march=pentiumpro -fomit-frame-pointer -o paq8ja paq7asm.o

  Non PC (e.g. PowerPC under MacOS X)
    g++ paq8ja.cpp -O2 -DUNIX -DNOASM -s -o paq8ja
...
Then
  ...
  paq8ja archive \dir1\file1.txt \dir2

will create archive.paq8ja with the header:
...
Decompression will fail if the first 6 bytes are not "paq8ja -".  Sizes
...
paq8ja uses a neural network to combine a large number of models.  The
...
paq8ja uses preprocessing transforms on certain data types to improve
...
#define PROGNAME "paq8ja" // Please change this if you change the program.
...
// To compress to file1.paq8ja: paq8ja [-n] file1 [file2...]
// To decompress: paq8ja file1.paq8ja [output_dir]
