...
void recordModel( Mixer &m ) {
  ...
  if( bpos == 0 ) {
    ...
    if( rcount1 > 15 && rlen != rlen1 )
      rlen = rlen1, rcount1 = rcount2 = rcount3 = 0 /*, printf("R1: %d  \n",rlen)*/;
    if( rcount2 > 20 && rlen != rlen2 )
      rlen = rlen2, rcount1 = rcount2 = rcount3 = 0 /*, printf("R2: %d  \n",rlen)*/;
    if( rcount3 > 25 && rlen != rlen3 )
      rlen = rlen3, rcount1 = rcount2 = rcount3 = 0 /*, printf("R3: %d  \n",rlen)*/;
...
void recordModel1( Mixer &m ) {
  ...
  static ContextMap co( 32768 * 2, 4 );
...
#define B64_DET( type, start_pos, header_len, base64len )                                                              \
  return dett = ( type ), deth = ( header_len ), detd = base64len, fseek( in, start + ( start_pos ), SEEK_SET ), HDR
...
Filetype detect( FILE *in, int n, Filetype type, int &info ) {
  ...
  for( int i = 0; i < n; ++i ) {
    ...
    if( b64s == 0 && buf0 == 0x73653634 && ( buf1 & 0xffffff ) == 0x206261 )
      b64s = 1, b64p = i - 6, b64h = 0, b64slen = 0; //' base64'
      ...
      if( i - b64nl + 1 < b64line ) { // if smaller and not padding
        ...
        if( base64end - base64start > 512 )
          B64_DET( BASE64, b64p, b64slen, base64end - base64start );
        b64s = 0;
      }
    ...
    if( txtStart1 == 0 && (soi == 0) && (pgm == 0) && (rgbi == 0) && (bmp == 0) && (wavi == 0) && (tga == 0)
        && ( ( c < 126 && c >= 32 ) || c == 10 || c == 13 || c == 0x12 || c == 9 ) )
      txtStart1 = 1, txtOff1 = i;
    if( txtStart1 != 0 ) {
      if( ( ( c < 126 && c >= 32 ) || c == 10 || c == 13 || c == 0x12 || c == 9 || c >= 192 ) && txtTXT0 == 0 ) {
        ...
        txtLen1 = ( txtLen1 >> 1 ) << 1;
        if( type == TXT0 )
          return fseek( in, start + txtLen1, SEEK_SET ), DEFAULT;
...
int decode_base64( FILE *in, int size, FILE *out, FMode mode, int &diffFound ) {
  ...
  while( feof( in ) == 0 ) {
    ...
    if( blocksout >= ( linesize / 4 ) ) {
      if( (blocksout != 0) && (feof( in ) == 0) ) { //no nl if eof
        fprintf( dtmp1, "\r\n" );
        fle++;
        fle++;
      }
...
void compressRecursive( FILE *in, long n, Encoder &en, char *blstr, int it = 0, int s1 = 0, int s2 = 0 ) {
  static const char *typenames[13] = {"default", "jpeg", "hdr",  "1b-image", "8b-image", "24b-image", "audio",
                                      "exe",     "cd",   "text", "utf-8",    "txt 0",    "base64"};
  ...
  while( n > 0 ) {
    ...
    if( len > 0 ) {
      ...
      if( type == EXE || type == CD || type == IMAGE24 || type == TEXT || type == TXTUTF8 || type == TXT0
        ...
        if( type == TXT0 && ( tmpsize == ( len >> 1 ) ) )
          printf( " (size %d)", tmpsize );
        rewind( tmp );
        en.setFile( tmp );
        fseek( in, begin, SEEK_SET );
        int diffFound = 0;
        if( type == IMAGE24 )
          ...
          decode_txt0( tmp, tmpsize, in, FCOMPARE, diffFound );
        else if( type == BASE64 )
          decode_base64( tmp, tmpsize, in, FCOMPARE, diffFound );
        else if( type == CD )
