/* paq8kx file compressor/archiver.  Release by Jan Ondrus, Feb. 7, 2010
...
- To install, put paq8kx.exe or a shortcut to it on your desktop.
- To compress a file or folder, drop it on the paq8kx icon.
- To decompress, drop a .paq8kx file on the icon.

A .paq8kx extension is added for compression, removed for decompression.
The output will go in the same folder as the input.

While paq8kx is working, a command window will appear and report
...
- To install, put paq8kx.exe somewhere in your PATH.
- To compress:      paq8kx [-N] file1 [file2...]
- To decompress:    paq8kx [-d] file1.paq8kx [dir2]
- To view contents: more < file1.paq8kx

The compressed output file is named by adding ".paq8kx" extension to
the first named file (file1.paq8kx).  Each file that exists will be
...
If file1.paq8kx exists then it is overwritten.

If the first named file ends in ".paq8kx" then it is assumed to be
...
The -d option forces extraction even if there is not a ".paq8kx"
...
During extraction, directories are created as needed.  For example:
  ...
  paq8kx -4 c:\tmp\foo bar

compresses foo and bar (if they exist) to c:\tmp\foo.paq8kx at level 4.

  paq8kx -d c:\tmp\foo.paq8kx .
...
There are 2 files: paq8kx.cpp (C++) and paq7asm.asm (NASM/YASM).
paq7asm.asm is the same as in paq7 and paq8x.  paq8kx.cpp recognizes the
...
Recommended compiler commands and optimizations:
  ...
  MINGW g++:
    ...
    g++ paq8kx.cpp -DWINDOWS -O2 -Os -s -march=pentiumpro -fomit-frame-pointer -o paq8kx.exe paq7asm.obj

  Borland:
    nasm paq7asm.asm -f obj --prefix _
    bcc32 -DWINDOWS -O -w-8027 paq8kx.cpp paq7asm.obj

  Mars:
    nasm paq7asm.asm -f obj --prefix _
    dmc -DWINDOWS -Ae -O paq8kx.cpp paq7asm.obj

  UNIX/Linux (PC):
    nasm -f elf paq7asm.asm
    g++ paq8kx.cpp -DUNIX -O2 -Os -s -march=pentiumpro -fomit-frame-pointer -o paq8kx paq7asm.o

  Non PC (e.g. PowerPC under MacOS X)
    g++ paq8kx.cpp -O2 -DUNIX -DNOASM -s -o paq8kx
...
Then
  ...
  paq8kx archive \dir1\file1.txt \dir2

will create archive.paq8kx with the header:
...
Decompression will fail if the first 7 bytes are not "paq8kx -".  Sizes
...
paq8kx uses a neural network to combine a large number of models.  The
...
paq8kx uses preprocessing transforms on certain data types to improve
...
Added Statemap, APM, indirectModel2, chartModel from paq8k
Combined contextModel2 from paq8k and paq8kx
Changed memory usage of some models
*/

#define PROGNAME "paq8kx" // Please change this if you change the program.
...
class Mix {
  ...
  Mix *mp;             // points to a Mix to combine results
public:
  Mix( int n = 4000, int m = 8000, int s = 12, int w = 0 );

  // Adjust weights to minimize coding cost of last prediction
  void update() {
    for( int i = 0; i < ncxt; ++i ) {
      int err = ( ( y << 12 ) - pr[i] ) * ( 3 + ( rnd() & 3 ) );
...
Mix::Mix( int n, int m, int s, int w ) :
    N( ( n + 7 ) & -8 ),
    M( m ),
    S( s ),
    tx( N ),
...
public:
  Mixer( int n, int m, int s, int w );
  void set( int cxt, int range, int det = 3 ) {
    if( det & 1 )
      m2[0].set( cxt, range );
    if( det & 2 )
      m2[1].set( cxt, range );
...
Mixer::Mixer( int n, int m, int s, int w = 0 ) {
  m2 = new Mix[2];
}
...
void indirect2Model( Mixer &m ) {
  static ContextMap cm( MEM * 4, 24 );
  static Array<U32> t1( 256 );
  static Array<U16> t2( 0x10000 );

  int p = pos & 7, buf1 = buf( 1 );
  if( !bpos ) {
    for( int i = 8, c1 = buf( 8 ), c2 = buf( 9 ), c3 = buf( 10 ); i > 0; c3 = c2, c2 = c1, c1 = buf( --i ) ) {
      U32 &r1 = t1[c2];
      r1 = r1 << 8 | c1;
      U16 &r2 = t2[c3 << 8 | c2];
      r2 = r2 << 8 | c1;
      U32 t = c1 | t1[c1] << 8;
      cm.set( t & 0xffff );
      cm.set( hash( buf1, c1, p ) );
      t = c2 << 8 | c1 | t2[c2 << 8 | c1] << 16;
      cm.set( t & 0xffffff );
    }
  }
  cm.mix( m );
...
// Originally in paq8 k, by Bill Pettis (ilovemyking@gmail.com).
...
int chartModel( Mixer &m, int ismatch ) {
  static U32 chart[20];
  static U8 indirect[1088];
  static U8 indirect2[256];
  static U8 indirect3[256 * 256];
  static ContextMap cm( MEM * 16, 64 ), cn( MEM * 4, 23 );
  if( !bpos ) {
...
int contextModel2() {
  static ContextMap cm( MEM * 16, 9 );
  static RunContextMap rcm7( MEM ), rcm9( MEM ), rcm10( MEM );
  static Mixer m( 4000, 8000, 12 );
  ...
  static const int bb[8] = { 0, 0, 0, 1, 1, 1, 2, 2 };
  static const int cc[8] = { 0, 1, 2, 0, 1, 2, 0, 1 };
  static int mask;
  ...
  const int d[3] = { c4 >> 15 & 448 | c4 >> 10 & 56 | c4 >> 5 & 7, c4 >> 12 & 448 | c4 >> 7 & 56 | c4 >> 2 & 7,
                     c4 >> 10 & 48 | c4 >> 4 & 12 | c4 & 3 };
  ...
  //m.set(tt,8);
  //m.set(tt+8*c0, 2048);
  m.set( order + 8 * c0, 2048, 1 );
  m.set( order2 + 8 * c0, 2048, 2 );
  m.set( d[bb[bpos]] + cc[bpos], 514 );
  m.set( c1 + 8, 264 );
  m.set( c0, 256 );
  m.set( order + 8 * ( c4 >> 6 & 3 ) + 32 * ( bpos == 0 ) + 64 * ( c1 == c2 ) + 128 * ( filetype == EXE ), 256, 1 );
  m.set( order2 + 8 * ( c4 >> 6 & 3 ) + 32 * ( bpos == 0 ) + 64 * ( c1 == c2 ) + 128 * ( filetype == EXE ), 256, 2 );
  m.set( c2, 256 );
  m.set( c3, 256 );
  m.set( ismatch, 256 );
  m.set( mask, 32 );
...
void Predictor::update() {
  static APM a( 256 ), a1( 16384 ), a2( 16384 ), a3( 8192 ), a4( 16384 ), a5( 16384 ), a6( 8192 );
  ...
  if( paq_isspecial )
    ...
    int pr1 = a1.p( pr0, c0 + 256 * ( 63 & c4 >> 2 ), 8 );
    int pr2 = a2.p( pr0, c0 + 256 * ( 28 & c4 >> 3 | 3 & c4 ), 8 );
    int pr3 = a3.p( pr0, c0 + 256 * ( 31 & c4 ), 8 );
    pr0 = pr1 + pr2 + pr3 + pr0 + 2 >> 2;

    int pr4 = a4.p( pr, c0 + 256 * ( 63 & c4 >> 2 ), 4 );
    int pr5 = a5.p( pr, c0 + 256 * ( 28 & c4 >> 3 | 3 & c4 ), 4 );
    int pr6 = a6.p( pr, c0 + 256 * ( 31 & c4 ), 4 );
    pr = pr4 + pr5 + pr6 + pr + 2 >> 2;

    pr = pr * ( 96 - charts ) + pr0 * ( 32 + charts ) + 64 >> 7;
  }
}
...
// To compress to file1.paq8kx: paq8kx [-n] file1 [file2...]
// To decompress: paq8kx file1.paq8kx [output_dir]
