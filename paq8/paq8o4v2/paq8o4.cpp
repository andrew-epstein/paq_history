/* paq8o4 v2 file compressor/archiver.  Release by Matt Mahoney, Sept. 17, 2007
...
- To install, put paq8o4.exe or a shortcut to it on your desktop.
- To compress a file or folder, drop it on the paq8o4 icon.
- To decompress, drop a .paq8o4 file on the icon.

A .paq8o4 extension is added for compression, removed for decompression.
The output will go in the same folder as the input.

While paq8o4 is working, a command window will appear and report
...
- To install, put paq8o4.exe somewhere in your PATH.
- To compress:      paq8o4 [-N] file1 [file2...]
- To decompress:    paq8o4 [-d] file1.paq8o4 [dir2]
- To view contents: more < file1.paq8o4

The compressed output file is named by adding ".paq8o4" extension to
the first named file (file1.paq8o4).  Each file that exists will be
...
If file1.paq8o4 exists then it is overwritten.

If the first named file ends in ".paq8o4" then it is assumed to be
...
The -d option forces extraction even if there is not a ".paq8o4"
...
During extraction, directories are created as needed.  For example:
  ...
  paq8o4 -4 c:\tmp\foo bar

compresses foo and bar (if they exist) to c:\tmp\foo.paq8o4 at level 4.

  paq8o4 -d c:\tmp\foo.paq8o4 .
...
There are 2 files: paq8o4.cpp (C++) and paq7asm.asm (NASM/YASM).
paq7asm.asm is the same as in paq7 and paq8x.  paq8o4.cpp recognizes the
...
Recommended compiler commands and optimizations:
  ...
  MINGW g++:
    ...
    g++ paq8o4.cpp -DWINDOWS -O2 -Os -s -march=pentiumpro -fomit-frame-pointer -o paq8o4.exe paq7asm.obj

  Borland:
    nasm paq7asm.asm -f obj --prefix _
    bcc32 -DWINDOWS -O -w-8027 paq8o4.cpp paq7asm.obj

  Mars:
    nasm paq7asm.asm -f obj --prefix _
    dmc -DWINDOWS -Ae -O paq8o4.cpp paq7asm.obj

  UNIX/Linux (PC):
    nasm -f elf paq7asm.asm
    g++ paq8o4.cpp -DUNIX -O2 -Os -s -march=pentiumpro -fomit-frame-pointer -o paq8o4 paq7asm.o

  Non PC (e.g. PowerPC under MacOS X)
    g++ paq8o4.cpp -O2 -DUNIX -DNOASM -s -o paq8o4
...
Then
  ...
  paq8o4 archive \dir1\file1.txt \dir2

will create archive.paq8o4 with the header:
...
Decompression will fail if the first 7 bytes are not "paq8o4 -".  Sizes
...
paq8o4 uses a neural network to combine a large number of models.  The
...
paq8o4 uses preprocessing transforms on certain data types to improve
...
Modified JPEG model by Jan Ondrus (paq8fthis3).
...
#define PROGNAME "paq8o4" // Please change this if you change the program.
...
int jpegModel( Mixer &m ) {
  ...
  static Array<int> adv_pred( 3 ), sumu( 8 ), sumv( 8 );
      ...
      if( rs >= 0 ) {
        if( huffsize + ( rs & 15 ) == huffbits ) { // done decoding
          ...
          // UPDATE_ADV_PRED !!!!
          {
            ...
            if( column == 0 )
              adv_pred[1] = adv_pred[2], adv_pred[0] = 0;
            if( row == 0 )
              adv_pred[1] = adv_pred[0], adv_pred[2] = 0;
          } // !!!!
        }
      }
    }
  ...
  if( hbcount == 0 ) {
    ...
    cxt[1] = hash( ++n, hc, rs1, comp, adv_pred[2] );
    cxt[2] = hash( ++n, hc, rs1, comp, adv_pred[0] );
    cxt[3] = hash( ++n, hc, rs1, comp, adv_pred[1] );
    cxt[4] = hash( ++n, hc, adv_pred[2], adv_pred[0] );
    cxt[5] = hash( ++n, hc, coef, column >> 1 );
    cxt[6] = hash( ++n, hc, dc );
    cxt[7] = hash( ++n, hc, ssum >> 4, mcupos );
    cxt[8] = hash( ++n, hc, lcp[0], lcp[1], adv_pred[1] );
    cxt[9] = hash( ++n, hc, rs1, coef );
    cxt[10] = hash( ++n, hc, lcp[0], lcp[1], coef );
    cxt[11] = hash( ++n, hc, mcupos >> 3, ssum3 >> 3 );
    cxt[12] = hash( ++n, hc, mpos >> 4, cbuf[cpos - width * mcusize] );
    cxt[13] = hash( ++n, hc, coef, adv_pred[2] );
    cxt[14] = hash( ++n, hc, coef, adv_pred[0] );
    cxt[15] = hash( ++n, hc, coef, adv_pred[1] );
    cxt[16] = hash( ++n, hc, zu + 8 * comp, lcp[0], lcp[2] / 2 );
    cxt[17] = hash( ++n, hc, zv + 8 * comp, lcp[1], lcp[3] / 2 );
    cxt[18] = hash( ++n, hc, mpos >> 4, cbuf[cpos - ls[mcupos >> 6]] );
  }
  ...
  int pr = m1.p();
  m.add( stretch( pr ) );
  pr = a1.p( pr, hc & 1023, 8 );
  pr = a2.p( pr, (hc & 255) | coef << 8, 8 );
  m.add( stretch( pr ) );
  return 2 + ( hc & 255 );
}
...
// To compress to file1.paq8o4: paq8o4 [-n] file1 [file2...]
// To decompress: paq8o4 file1.paq8o4 [output_dir]
int main( int argc, char **argv ) {
  ...
  try {
    ...
    // Print help message
    if( argc < 2 ) {
      printf( PROGNAME " v2 archiver (C) 2007, Matt Mahoney et al.\n"
