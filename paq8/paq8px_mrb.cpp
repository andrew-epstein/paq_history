...
buffer=new char[N+F+(N+1+N+N+256)*sizeof(int)];
   // buffer=malloc(N+F+(N+1+N+N+256)*sizeof(int)); // 28.5 k !
    ...
    {
	node=(int *)(buffer+N+F);
	for(int i=0;i<256;i++) root[i]=NIL;
	for(int i=NIL;i<N;i++) dad[i]=NIL;
	size=mask=1;
	buf[0]=0;
	i=N-F-F;
...
LZ77::LZ77(): N(4096),F(16),THRESHOLD(3),NIL(-1),dad(node+1),lson(node+1+N),rson(node+1+N+N),root(node+1+N+N+N) {
...
int contextModel2() {
  ...
  if (filetype==IMAGE8 || filetype==MRBRLE) return im8bitModel(m, info), m.p();
...
#define MRBRLE_DET(start_pos,header_len,data_len,width,height) return dett=(MRBRLE),\
...
Filetype detect(FILE* in, int n, Filetype type, int &info, int &info2) {
  ...
  char pgm_buf[32];
	int mrb=0,mrb0=0,mrbsize=0,mrbcsize=0,mrbPictureType=0,mrbPackingMethod=0,mrbTell=0,mrbw=0,mrbh=0; // For MRB detection
  ...
  for (int i=0; i<n; ++i) {
	...
	if (mrb && (i-mrb)==1 && !c==1)	mrb=0; //if not 1 image per/file
...
U16 mrbGetCWord(FILE* in, FILE* out)
{
    ...
    if(b&1) return ((mrbgetbyte(in,out)<<8)|b)>>1;
    return b>>1;
}
...
int decode(Encoder& en) {
  ...
  switch (type) {
    case EXE:  return decode_exe(en);
		case MRBRLE:	 return decode_mrb(en, len);
...
void compress(const char* filename, long filesize, Encoder& en) {
  static const char* typenames[11]={"default", "jpeg", "hdr",
    "1-bit-image", "8-bit-image", "24-bit-image", "audio", "exe", "mrb-rle", "mrb-lz77", "mrb-both"};
    ...
    if (len>0) {
      ...
      if (type==AUDIO) printf(" (%s)", audiotypes[info%4]);
        ...
        fprintf(tmp, "%c%", type);
				...
				else encode_mrb(in, tmp, len, info, info2);
