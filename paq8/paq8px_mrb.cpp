...
public:
  ...
  void compress( FILE *f, FILE *out ) {
    ...
    int mask;
    char buf[17];
    buffer = new char[N + F + ( N + 1 + N + N + 256 ) * sizeof( int )];
...
LZ77::LZ77() :
    ...
    dad( node + 1 ), lson( node + 1 + N ), rson( node + 1 + N + N ), root( node + 1 + N + N + N ) {}
...
typedef enum { DEFAULT, JPEG, HDR, IMAGE1, IMAGE8, IMAGE24, AUDIO, EXE, MRBRLE, MRBLZ77, MRBBOTH } Filetype;
...
int contextModel2() {
  ...
  if( bpos == 0 ) {
    ...
    if( size == 0 )
      ...
      ft2 = ( Filetype ) buf( 1 );
    } else if( size == -5 && ft2 != IMAGE1 && ft2 != IMAGE8 && ft2 != IMAGE24 && ft2 != AUDIO && ft2 != MRBRLE ) {
      size = buf( 4 ) << 24 | buf( 3 ) << 16 | buf( 2 ) << 8 | buf( 1 );
      if( ft2 == EXE )
        size += 8;
      blpos = 0;
    } else if( size == -9 && ft2 != MRBRLE ) {
      ...
      int height;
      int pad;
      size = buf( 12 ) << 24 | buf( 11 ) << 16 | buf( 10 ) << 8 | buf( 9 );
      info = buf( 8 ) << 24 | buf( 7 ) << 16 | buf( 6 ) << 8 | buf( 5 );
      height = buf( 4 ) << 24 | buf( 3 ) << 16 | buf( 2 ) << 8 | buf( 1 );
      pad = ( ( info + 3 ) / 4 ) * 4 - info;
      size = ( info + pad ) * height;
      blpos = 0;
    }
    if( blpos == 0 )
      filetype = ft2;
  ...
  if( filetype == IMAGE1 )
    im1bitModel( m, info );
  if( filetype == IMAGE8 || filetype == MRBRLE )
...
#define MRBRLE_DET( start_pos, header_len, data_len, width, height )                                                   \
  return dett = ( MRBRLE ), deth = ( header_len ), detd = ( data_len ), info = ( width ), info2 = ( height ),          \
         fseek( in, start + ( start_pos ), SEEK_SET ), HDR

// Detect EXE or JPEG data
Filetype detect( FILE *in, int n, Filetype type, int &info, int &info2 ) {
  U32 buf1 = 0;
  ...
  for( int i = 0; i < n; ++i ) {
    ...
    if( ( mrb != 0 ) && ( i - mrb ) == 1 && ( static_cast<int>( static_cast<int>( ( ( c ) ) == 0 ) == 1 ) != 0 ) )
      mrb = 0; //if not 1 image per/file
    //                       5=DDB   6=DIB   8=metafile
    if( ( mrb != 0 ) && ( i - mrb ) == 7 && ( c == 5 || c == 6 || c == 8 ) )
      mrbPictureType = c;
    //                       0=uncomp 1=RunLen 2=LZ77 3=both
    if( ( mrb != 0 ) && ( i - mrb ) == 8 && ( c == 0 || c == 1 || c == 2 || c == 3 ) )
      mrbPackingMethod = c;
    if( ( mrb != 0 ) && ( i - mrb ) == 10 ) {
    ...
    if( type == MRBRLE && ( mrb != 0 ) && ( mrbPictureType == 6 || mrbPictureType == 8 ) && ( mrbsize != 0 ) ) {
      //pgmsize=pgmw *pgmh +mrb+i-1;
      return fseek( in, start + mrbsize, SEEK_SET ), DEFAULT;
    }
    if( ( mrb != 0 ) && ( mrbPictureType == 6 ) && ( mrbsize != 0 ) ) {
      MRBRLE_DET( mrb - 1, mrbsize - mrbcsize, mrbcsize, mrbw, mrbh );
      //return 	fseek(in, start+mrb-1, SEEK_SET),MRB;
    }
...
// MRB
U8 mrbgetbyte( FILE *in, FILE *out ) {
  U8 mrbbyte = getc( in );
  putc( mrbbyte, out );
  return mrbbyte;
}
U32 mrbGetCDWord( FILE *in, FILE *out ) {
  U16 w = mrbgetbyte( in, out );
  w = w | ( mrbgetbyte( in, out ) << 8 );
  if( ( w & 1 ) != 0 ) {
    U16 w1 = mrbgetbyte( in, out );
    w1 = w1 | ( mrbgetbyte( in, out ) << 8 );
    return ( ( w1 << 16 ) | w ) >> 1;
  }
  return w >> 1;
}
U16 mrbGetCWord( FILE *in, FILE *out ) {
  U8 b = mrbgetbyte( in, out );
  if( ( b & 1 ) != 0 )
    return ( ( mrbgetbyte( in, out ) << 8 ) | b ) >> 1;
  return b >> 1;
}

void encode_mrb( FILE *in, FILE *out, int len, int width, int height ) {
  fprintf( out, "%c%c%c%c", len >> 24, len >> 16, len >> 8, len );             // size, MSB first
  fprintf( out, "%c%c%c%c", width >> 24, width >> 16, width >> 8, width );     // size, MSB first
  fprintf( out, "%c%c%c%c", height >> 24, height >> 16, height >> 8, height ); // size, MSB first

  int pad = ( ( width + 3 ) / 4 ) * 4 - width;
  int totalSize = ( width + pad ) * height;

  /*  if(mrbPackingMethod==2)
	{
		printf(" - LZ77\n"); 
		LZ77 nLZ77;
		nLZ77.expand(in,out);
  ...
  /*}
	else if(mrbPackingMethod==0)
	{
		printf(" - NONE\n"); 
		for(int length=0;length<totalsize;length++) putc(getc(in),out);
	}

	return start;*/
}
U32 mrbDGetCDWord( Encoder &en, U8 *mrbData, int &i ) {
  mrbData[i++] = en.decompress();
  U32 w = mrbData[i - 1];
  mrbData[i++] = en.decompress();
  w = ( w & 0xff ) | ( mrbData[i - 1] << 8 );
  if( ( w & 1 ) != 0U ) {
    mrbData[i++] = en.decompress();
    U32 w1 = mrbData[i - 1];
    mrbData[i++] = en.decompress();
    w1 = ( w1 & 0xff ) | ( mrbData[i - 1] << 8 );
    return ( ( w1 << 16 ) | w ) >> 1;
  }
  return w >> 1;
}
U16 mrbDGetCWord( Encoder &en, U8 *mrbData, int &i ) {
  U16 b;
  mrbData[i++] = en.decompress();
  b = mrbData[i - 1];
  if( ( b & 1 ) != 0 ) {
    mrbData[i++] = en.decompress();
    return ( ( mrbData[i - 1] << 8 ) | b ) >> 1;
  }
  return b >> 1;
}

void encodeRLE( U8 *dst, const U8 *src_start, const U8 *src_end ) {
  const U8 *ptr;
  int i = 0;
...
// Decodes all file and then starts returning decoded data.
U8 decode_mrb( Encoder &en, int size ) {
  static int osize = 0;
  ...
  if( osize == 0 ) {
    ...
    width = en.decompress() << 24;
    width |= en.decompress() << 16;
    width |= en.decompress() << 8;
    width |= en.decompress();

    height = en.decompress() << 24;
    height |= en.decompress() << 16;
    height |= en.decompress() << 8;
    height |= en.decompress();

    pad = ( ( width + 3 ) / 4 ) * 4 - width;
    totalsize = ( width + pad ) * height;

    remainsize = 0;
    int i = 0;
    if( mrbData == nullptr ) {
      mrbData = new U8[osize];
    } else {
      delete[] mrbData;
      mrbData = new U8[osize];
    }

    U8 *mrbRLEData = new U8[totalsize];

    int j;
    for( j = 0; j < totalsize; j++ ) {
      mrbRLEData[j] = en.decompress();
    }
    encodeRLE( mrbData + i, mrbRLEData, mrbRLEData + j );

    delete[] mrbRLEData;
  }
  return mrbData[remainsize++];
}
...
void compress( const char *filename, long filesize, Encoder &en ) {
  static const char *typenames[11] = { "default", "jpeg", "hdr",     "1-bit-image", "8-bit-image", "24-bit-image",
                                       "audio",   "exe",  "mrb-rle", "mrb-lz77",    "mrb-both" };
  ...
  while( n > 0 ) {
    Filetype nextType = detect( in, n, type, info, info2 );
    long end = ftell( in );
    ...
    if( len > 0 ) {
      printf( "%3d | %12s | %d bytes [from %ld to %ld]", blnum++, typenames[type], len, begin, end - 1 );
      if( type == AUDIO )
        printf( " (%s)", audiotypes[info % 4] );
      else if( type == IMAGE1 || type == IMAGE8 || type == IMAGE24 )
        printf( " (width: %d)", info );
      else if( type == EXE || type == MRBRLE ) {
        int bmpstart = 0;
        ...
        if( type == EXE )
          ...
          encode_mrb( in, tmp, len, info, info2 );
