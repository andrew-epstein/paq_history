...
int contextModel2() {
  ...
  if( bpos == 0 ) {
    ...
    if( size == -5 && ft2 != IMAGE1 && ft2 != IMAGE8 && ft2 != IMAGE24 && ft2 != AUDIO && ft2 != EXE ) {
      ...
      if( ft2 == CD )
        size = 3 + ( size / 2352 ) * 2048 + ( size % 2352 );
      blpos = 0;
    }
    if( size == -9 ) {
...
Filetype detect( FILE *in, int n, Filetype type, int &info ) {
  ...
  char pgm_buf[32];
  int cdi = 0;
  int cda; // For CD sectors detection
  U8 data[2352];
  ...
  for( int i = 0; i < n; ++i ) {
    ...
    if( ( cdi != 0 ) && i > cdi ) {
      ...
      if( p == 8 && ( buf1 != 0xffffff00 || ( buf0 & 0xff ) != 0x01 ) )
        cdi = 0;
      else if( p == 2068 && ( buf1 != 0 || buf0 != 0 ) )
        cdi = 0;
      else if( p == 0 ) {
        ...
        if( expand_cd_sector( data, cda, 1 ) == 0 ) {
          if( type != CD )
            return fseek( in, start + cdi - 7, SEEK_SET ), CD;
          cda = ( data[12] << 16 ) + ( data[13] << 8 ) + data[14];
        } else
          cdi = 0;
      }
      if( ( cdi == 0 ) && type == CD )
        return fseek( in, start + ( p == 0 ? i - 2352 : i - p ) - 7, SEEK_SET ), DEFAULT;
    }
    if( type == CD )
      continue;
...
int decode( Encoder &en ) {
  ...
  switch( type ) {
    case EXE:
      return decode_exe( en, size );
    case CD:
      return decode_cd( en, size );
    default:
...
void direct_encode_block( Filetype type, int begin, int len, FILE *in, Encoder &en, int info, int t ) {
  printf( "\nCompressing... " );
  en.compress( type );
  en.compress( len >> 24 );
  en.compress( len >> 16 );
...
void compress( const char *filename, long filesize, Encoder &en ) {
  ...
  while( n > 0 ) {
    ...
    if( end > filesize ) { // if some detection reports longer then actual size file is
      end = begin + 1;
      type = DEFAULT;
    }
    ...
    if( len > 0 ) {
      ...
      if( type == EXE || type == CD ) {
        ...
        int c2 = 0;
        for( j = 0; j < len; ++j )
          if( ( c1 = decode( en ) ) != ( c2 = getc( in ) ) )
            break;

        // Test fails, compress without transform
        if( j != len || getc( tmp ) != EOF ) {
          printf( "\nTransform fails at %ld, input=%d decoded=%d, skipping...", j, c2, c1 );
          fseek( in, begin, SEEK_SET );
          direct_encode_block( DEFAULT, begin, len, in, en, -1, filesize );
        } else {
          printf( "\nCompressing... " );
          rewind( tmp );
          int c;
          ...
          while( ( c = getc( tmp ) ) != EOF ) {
            if( ( ( ++j ) & 0xfff ) == 0 )
              printStatus( min( j, begin + len ), filesize );
            en.compress( c );
          }
        }
        fclose( tmp ); // deletes
        ...
        direct_encode_block( type, begin, len, in, en, i1, filesize );
      }
