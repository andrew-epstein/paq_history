/* PAQ8A file compressor/archiver.
(C) 2006, Matt Mahoney under GPL, http://www.gnu.org/licenses/gpl.txt

PAQ8A is a file compressor and archiver.  To use:
...
whether they differ.  Example:
  ...
  paq8a xyz foo bar      Compresses foo and bar to archive xyz.
  paq8a xyz baz          Extracts baz as a copy of foo, compares bar.
...
PAQ8A cannot update an archive.  You must create a new one instead.
...
TO COMPILE
  ...
  MINGW g++:
    ...
    g++ paq8a.cpp -O2 -Os -s -march=pentiumpro -fomit-frame-pointer -o paq8a.exe paq7asm.obj

  DJGPP g++:
    nasm paq7asm.asm -f coff --prefix _
    gxx paq8a.cpp -O2 -Os -s -march=pentiumpro -fomit-frame-pointer -o paq8a.exe paq7asm.obj

  Borland:
    nasm paq7asm.asm -f obj --prefix _
    bcc32 -O -w-8027 paq8a.cpp paq7asm.obj

  Mars:
    nasm paq7asm.asm -f obj --prefix _
    dmc paq8a.cpp paq7asm.obj

  UNIX/Linux (PC):
    nasm -f elf paq7asm.asm
    g++ paq8a.cpp -O2 -Os -s -march=pentiumpro -fomit-frame-pointer -o paq8a paq7asm.o

  Non PC (UNIX):
    g++ paq8a.cpp -O2 -DNOASM -s -o paq8a
...
#define PROGNAME "paq8a" // Please change this if you change the program.
...
int main( int argc, char **argv ) {
  ...
  while( ( filename = getline( f ) ) != 0 ) {
    ...
    if( mode == DECOMPRESS && ++filenum < argc && (strcmp( argv[filenum], filename ) != 0) ) {
