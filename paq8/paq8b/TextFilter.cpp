...
int WRT_detectFileType(FILE* file, int part_length, int parts, int& recordLen)
{
	...
	if ((parts==1 && langSum+shortLangSum>=18) || 
		langSum+shortLangSum>part_length*parts/250) 
	{
		max=0.0;
		for (i=0; i<langCount; i++)
		{
	...
	if (IF_OPTION(OPTION_EOL_CODING) && EOLCount>256 && EOLCount<5120 && EOLCountBad/EOLCount<6 && spaceAfterLF>1000)
	...
	PRINT_DICT(("EOLCount=%d+%d(%d) binCount=%d/%d(%d) punctCount=%d/%d(%d) langSum=%d/%d(%d) ngram=%d\n",EOLCount,EOLCountBad,IF_OPTION(OPTION_EOL_CODING),binCount,part_length*parts/50,!IF_OPTION(OPTION_NORMAL_TEXT_FILTER),punctCount,punctCountBad,IF_OPTION(OPTION_WORD_SURROROUNDING_MODELING),shortLangSum+langSum,part_length*parts/100,IF_OPTION(OPTION_USE_DICTIONARY),IF_OPTION(OPTION_USE_NGRAMS)));
