...
int decode_cd(FILE *in, int size, FILE *out, FMode mode) {
  ...
  while (i<size) {
    ...
    else for (int j=0; j<BLOCK; ++j) if (blk[j]!=getc(out)) return i2+j;
    i2+=BLOCK;
  }
  return i2;
}
  ...
  // Fill queue
	while (offset<size+6) {
		...
		if ((c[0]==0x00 || c[0]==0xFF) && (c[4]==0xE8 || c[4]==0xE9 || (c[5]==0x0F && (c[4]&0xF0)==0x80))
  		...
  		else if (mode==FCOMPARE && c[5]!=getc(out)) return offset-6;
		offset++;
	}
  ...
  while (n>0) {
    ...
    if (len>0) {
      ...
      if (type==EXE || type==CD) {
        ...
        fseek(in, begin, SEEK_SET);
        long j;
        if (type==EXE) j=decode_exe(en, tmpsize, in, FCOMPARE);
        else if (type==CD) j=decode_cd(tmp, tmpsize, in, FCOMPARE);
...
long decompressRecursive(FILE *out, long blocksize, int it, Encoder& en, FMode mode, long prevsize, long totalsize) {
  ...
  while (i<blocksize) {
    ...
    if (type==EXE) len=decode_exe(en, len, out, mode);
    else if (type==CD) {
      tmp=tmpfile();
      ...
      if (mode==FDECOMPRESS) {
        for (int j=0; j<len; ++j) {
          if (!(j&0xfff)) printStatus(j+i+prevsize, totalsize);
...
void decompress(const char* filename, long filesize, Encoder& en) {
  ...
  if (!f) mode=FDISCARD,printf("File couldn't be created. Skip...\n");
	...
	if (saveMode==mode==FCOMPARE) printf("Files are equal. No difference found.\n");
	else printf("done        \n");
	if (mode!=FDISCARD) fclose(f);
