...
Filetype detect( FILE *in, int n, Filetype type, int &info ) {
  ...
  for( int i = 0; i < n; ++i ) {
    ...
    if( txtStart == 0 && cmode != 6 && cmode != 1 && ( soi == 0 ) && ( pgm == 0 ) && ( rgbi == 0 ) && ( bmp == 0 )
        && ( wavi == 0 ) && ( ( c < 128 && c > 32 ) || c == 10 || c == 13 || c == 0x12 || c == 9 ) )
...
int decode_cd( FILE *in, int size, FILE *out, FMode mode ) {
  const int BLOCK = 2352;
  U8 blk[BLOCK];
  U8 d2[BLOCK];
  int a;
...
int decode_exe( Encoder &en, int size, FILE *out, FMode mode ) {
  ...
  while( offset < size + 6 ) {
    ...
    if( mode == FDECOMPRESS )
      putc( c[5], out );
    else if( mode == FCOMPARE && c[5] != getc( out ) )
      return offset - 6;
    offset++;
  }
...
void compressRecursive( FILE *in, int filesize, Encoder &en, int it = 0, int s1 = 0, int s2 = 0 ) {
  static const char *typenames[9] = { "default",      "jpeg",  "hdr", "1-bit-image", "8-bit-image",
                                      "24-bit-image", "audio", "exe", "cd" };
  static const char *audiotypes[4] = { "8-bit mono", "8-bit stereo", "16-bit mono", "16-bit stereo" };
  ...
  while( n > 0 ) {
    ...
    if( len > 0 ) {
      ...
      if( type == EXE || type == CD ) {
        ...
        long j;
        if( type == EXE )
          j = decode_exe( en, tmpsize, in, FCOMPARE );
        else if( type == CD )
          j = decode_cd( tmp, tmpsize, in, FCOMPARE );
...
long decompressRecursive( FILE *out, long blocksize, int it, Encoder &en, FMode mode, long prevsize, long totalsize ) {
  Filetype type;
  long len;
  long i = 0;
  FILE *tmp;
  while( i < blocksize ) {
    ...
    if( type == EXE )
      len = decode_exe( en, len, out, mode );
    else if( type == CD ) {
      tmp = tmpfile();
      decompressRecursive( tmp, len, it + 1, en, FDECOMPRESS, prevsize + i, totalsize );
      rewind( tmp );
      len = decode_cd( tmp, len, out, mode );
      fclose( tmp );
    } else {
      if( mode == FDECOMPRESS ) {
        for( int j = 0; j < len; ++j ) {
          if( ( j & 0xfff ) == 0 )
            printStatus( j + i + prevsize, totalsize );
        ...
        for( int j = 0; j < len; ++j ) {
          if( ( j & 0xfff ) == 0 )
            printStatus( j + i + prevsize, totalsize );
          if( mode == FCOMPARE ) {
            if( en.decompress() != fgetc( out ) ) {
              mode = FDISCARD;
              printf( "First difference found at offset: %u\n", ftell( out ) );
            }
          } else
            en.decompress();
        }
...
void decompress( const char *filename, long filesize, Encoder &en ) {
  ...
  const FMode saveMode = mode;
  decompressRecursive( f, filesize, 0, en, mode, 0, filesize );
  if( static_cast<int>( saveMode == mode ) == FCOMPARE )
