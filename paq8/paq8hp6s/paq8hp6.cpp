...
U32 b1=0, b2=0, b3=0, b4=0, b5=0, b6=0, b7=0, b8=0, c4=0, x4=0, w4=0; // Last 4 whole bytes, packed.  Last byte is bits 0-7.
...
class RunContextMap {
  ...
  U8* cp;
public:
  RunContextMap(int m): t(m/4) {cp=t[0]+2;}
...
/////    if (fsize==513216) picModel(m);
		...
		if (c1==10 || c1==32) c1=16;
		if (c2==10 || c2==32) c2=16;

		c=(w4&15);
		m.set(order + 8*(min(7,spacecount/4)) + 64*c, 1024); /////+128*(filetype==EXE), 256);

		c=(w4>>2)&3;	if(bpos>1)  c=c0>>(bpos-2);
		c=c1*8+c;
		m.set(c, 2048);
...
void Predictor::update() {
  ...
  if (c0>=256) {
	...
	if(c0=='.' || c0=='?') b2='.', w4|=12, x4=(x4&0xffffff00)+b2;
	x4=(x4<<8)+c0;
    c4=(c4<<8)+c0;
    c0=1;
  }
