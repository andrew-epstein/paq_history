/* PAQ8G file compressor/archiver.
(C) 2006, Matt Mahoney under GPL, http://www.gnu.org/licenses/gpl.txt

PAQ8G is a file compressor and archiver.  To use:
...
PAQ8G never clobbers files.  If the file to be extracted already
exists, then PAQ8G compares it with the archived copy and reports
whether they differ.  Example:

  paq8g xyz foo bar      Compresses foo and bar to archive xyz.
  paq8g xyz baz          Extracts baz as a copy of foo, compares bar.

If PAQ8G is run under UNIX or Linix, or compiled with g++ for Windows,
...
PAQ8G cannot update an archive.  You must create a new one instead.
...
There are 3 files: paq8g.cpp (C++), textfilter.hpp, and paq8asm.asm (NASM/YASM).
paq8g.cpp recognizes the following compiler options:
...
Recommended compiler commands and optimizations:
  ...
  MINGW g++:
    ...
    g++ paq8g.cpp -O2 -Os -s -march=pentiumpro -fomit-frame-pointer -o paq8g.exe paq8asm.obj

  Borland:
    nasm paq8asm.asm -f obj --prefix _
    bcc32 -O -w-8027 paq8g.cpp paq8asm.obj

  Mars:
    nasm paq8asm.asm -f obj --prefix _
    dmc -Ae -O paq8g.cpp paq8asm.obj

  UNIX/Linux (PC):
    nasm -f elf paq8asm.asm
    g++ paq8g.cpp -O2 -Os -s -march=pentiumpro -fomit-frame-pointer -o paq8g paq8asm.o

  Non PC (e.g. PowerPC under MacOS X)
    g++ paq8g.cpp -O2 -DNOASM -s -o paq8g
...
Decompression will fail if the first 7 bytes are not "paq8g -".  Sizes
...
paq8g uses a neural network to combine a large number of models.  The
...
PAQ8G uses preprocessing transforms on certain file types to improve
...
#define PROGNAME "paq8g" // Please change this if you change the program.
...
U32 b1 = 0, b2 = 0, b3 = 0, b4 = 0, b5 = 0, b6 = 0, b7 = 0, b8 = 0, c4 = 0, x4 = 0,
    w4 = 0;              // Last 4 whole bytes, packed.  Last byte is bits 0-7.
int bpos = 0, cxtfl = 3; // bits in c0 (0 to 7)
...
static U32 spaces = 0, spacecount = 0, words = 0, wordcount = 0, fails = 0, failcount = 0;
...
void wordModel( Mixer &m ) {
  ...
  static ContextMap cm( MEM * 64, 43 );
  ...
  if( bpos == 0 ) {
    ...
    h <<= 6, c += b2 * 256;
    cm.set( c + ( h & 0xffff0000 ) );
    cm.set( c + ( h & 0x00ff0000 ) );
    h <<= 6, c += b3 << 16;
    cm.set( c + ( h & 0xff000000 ) );
    ...
    cm.set( x4 & 0xff00ff00 );
    cm.set( c + b5 * 256 + ( 1 << 17 ) );
    cm.set( c + b6 * 256 + ( 2 << 17 ) );
    cm.set( x4 & 0xff00ffff );
...
void recordModel( Mixer &m ) {
  ...
  static ContextMap cm( 32768 / 2, 2 ), cn( 32768 / 2, 4 ), co( 32768 * 2, 3 ), cp( 32768, 3 );

  // Find record length
  if( !bpos ) {
    int c = b1, w = ( b2 << 8 ) + c, d = w & 0xf0ff;
#if 0
    int r=pos-cpos1[c];
...
void sparseModel( Mixer &m ) {
  ...
  static SmallStationaryContextMap scm1( 0x20000 ), scm2( 0x20000 ), scm3( 0x20000 ), scm4( 0x20000 );

  if( bpos == 0 ) {
    cn.set( ( x4 & 0xf0f0f0f0 ) + 1 );
    cn.set( ( x4 & 0x00f8f8f8 ) + 2 );
    cn.set( ( x4 & 0xf8f8f8f8 ) + 3 );
    cn.set( ( x4 & 0x00e0e0e0 ) + 4 );
...
static char arrdict[411681] = "arrdict";
...
int contextModel2() {
  ...
  static Mixer m( 412, 8 + 1024 + 2048 + 256 * ( 8 + 6 + 7 + 8 ), 6, 192 );
  ...
  c = ( w4 & 15 );
  m.set( order + 8 * ( min( 7, spacecount / 4 ) ) + 64 * c, 1024 ); /////+128*(filetype==EXE), 256);
  ...
  c = ( w4 & 255 ) + 256 * min( 7, wordcount / 4 );
  m.set( c, 2048 );
...
void Predictor::update() {
  ...
  if( c0 >= 256 ) {
    buf[pos++] = c0;
    c0 -= 256;
    w4 = w4 * 4 + WRT_mpw[c0 >> 4];
    b8 = b7, b7 = b6, b6 = b5, b5 = b4, b4 = b3;
    b3 = b2;
    b2 = b1;
    b1 = c0;
    if( c0 == '.' || c0 == '?' )
      b2 = '.', w4 |= 12, x4 = ( x4 & 0xffffff00 ) + b2;
    x4 = ( x4 << 8 ) + c0;
    c4 = ( c4 << 8 ) + c0;
    c0 = 1;
  }
  ...
  pr = contextModel2();
  int rate = 6 + ( pos > 4096 * 1024 ) + ( pos > 8192 * 1024 );
  int pr1 = a1.p( pr, c0, 8 ) * 3 + 5 * pr + 4 >> 3, ps = b1, pt = b2, pu = b3, pv, pw, pz;
  pw = pt & 0xfc;
  if( pt == 10 || pt == 32 )
    pw = 16;
  pv = pu >> 4;
  ...
  pu = a4.p( pr1, c0 ^ hash( ps, pt, pu ) & 0xffff, rate + 1 );
  pz = a2.p( pr, c0 + 256 * ps, rate );

  pv = a5.p( pz, ( c0 * 16 ) ^ hash( pv, ps & 0xfc, pw ) & 0xffff, rate );
  pt = a3.p( pr, ( c0 * 4 ) ^ hash( ps, pt ) & 0xffff, rate );

  pw = a6.p( pu, c0 + 256 * ( fails & 255 ), rate + 1 );
  pr = pt * 4 + pu + pv * 3 + pw * 8 + 8 >> 4;
}
...
void ExeFilter::encode( FILE *f, int n ) {
  ...
  for( int offset = 0; offset < n; offset += BLOCK ) {
    ...
    for( int i = bytesRead - 1; i >= 4; --i ) {
      if( ( buf[i - 4] == 0xe8 || buf[i - 4] == 0xe9 ) && ( buf[i] == 0 || buf[i] == 0xff ) && offset + i + 1 < end ) {
...
Filter *Filter::make( const char *filename, Encoder *e ) {
    ...
    if( file ) {
      if( fgetc( file ) == 'M' && fgetc( file ) == 'Z' )
        filetype = EXE;
               ...
               || ( !equals( ext, ".dbf" ) && !equals( ext, ".mdb" ) && !equals( ext, ".tar" ) && !equals( ext, ".c" )
                    && !equals( ext, ".cpp" ) && !equals( ext, ".h" ) && !equals( ext, ".hpp" ) && !equals( ext, ".ps" )
                    && !equals( ext, ".hlp" ) && !equals( ext, ".ini" ) && !equals( ext, ".inf" ) ) ) {
  ...
  if( e ) {
    ...
    else if( filetype == TEXT || filetype == BINTEXT )
...
// Compress/decompress files.  Usage: paq8g archive files...
...
int main( int argc, char **argv ) {
  ...
  if( f ) {
    ...
    if( compressed_size > 0 && total_size > 0 && start_time > 0 ) {
      printf( " (%1.4f bpc) in %1.2f sec (%1.3f KB/sec)", 8.0 * compressed_size / total_size,
