/* paq8pxd16 file compressor/archiver.  Release by Kaido Orav, Sept. 17, 2014
...
inline U64 CMlimit( U64 size ) {
  if( size > 0x7FFFFFFF )
    return 0x7FFFFFFF; //limit to 2GB power of 2
  return ( size );
}
  ...
  for( uint64_t i = 0; i < n; ++i ) {
    ...
    if( wavi ) {
      ...
      if( p == 4 )
        ...
        int wavd = bswap( buf0 );
        int wsr = -1;
        /*8000   9600  11025  12000  15000  16000  22050  24000  32000  44100
        ...
        if( wavsr == 8000 )
          wsr = 0;
        //if (wsr==-1) printf("Detect wav sr %d",wavsr),quit();
        if( ( wavch == 1 || wavch == 2 ) && ( wavbps == 8 || wavbps == 16 ) && wavt == 1 && wavd > 0
            && wavsize >= wavd + 36 && wavd % ( ( wavbps / 8 ) * wavch ) == 0 && wsr >= 0 )
          AUD_DET( AUDIO, wavi - 3, 44 + wavm, wavd, wavch + wavbps / 4 - 3 + ( wsr << 5 ) );
        //32bit IEEE
...
//called only when encode_txt output was smaller then input
int decode_audio( Encoder &en, int size, FILE *out, int info, int info2, FMode mode, uint64_t &diffFound ) {
  FILE *dtmp;
  FILE *dtmp1;
  char c;
  int b = 0;
  ...
  fseeko( dtmp, 0, SEEK_SET );
  decompress( dtmp, dtmp1, info2, info, info2 );
  ...
  while( n > 0 ) {
    ...
    if( len > 0 ) {
      ...
      if( type == EXE || type == CD || type == IMAGE24 || ( ( type == TEXT || type == TXTUTF8 ) && len > 0xA00000 )
          || type == BASE64 || type == SZDD || type == AUDIO ) {
        ...
        if( type == IMAGE24 )
          ...
          decode_audio( en, int( tmpsize ), in, info, int( len ), FCOMPARE, diffFound );
        else if( type == EXE )
...
uint64_t decompressStreamRecursive( FILE *out, uint64_t size, Encoder &en, FMode mode, int it = 0, uint64_t s1 = 0,
  ...
  while( i < size ) {
    ...
    if( type == IMAGE24 )
      ...
      len = decode_audio( en, int( len ), out, info, info2, mode, diffFound );
    else if( type == EXE )
        ...
        if( datasegmentsize > 0 ) {              //if segment contains data
          ...
          switch( i ) {
            ...
            case 10: {
              ...
              printf( "Compressing %stext wrt stream.\n", i == 10 ? "big" : "" );
              if( modeQuick )
                predictord = new PredictorFast();
              else
                predictord = new PredictorTXTWRT();
