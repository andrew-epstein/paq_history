...
void XWRT_Common::initializeLetterSet() {
  ...
  for( c = 0; c < 256; c++ ) //                                                - _ . , :
    if( c > 127 || letterSet[c] == LOWERCHAR || letterSet[c] == UPPERCHAR
        || ( letterSet[c] == NUMBERCHAR && wrtnum == 1 ) || c == ' ' /*|| c=='\''*/ ) // || c=='&')
      wordSet[c] = 1;
    else
      wordSet[c] = 0;
}
void XWRT_Common::initializeCodeWords( int word_count, bool initMem ) {
  ...
  PRINT_DICT( ( " %d %d %d %d(%d) charsUsed=%d sizeDict=%d\n", dict1size, dict2size, dict3size, dict4size, dictionary,
                charsUsed, sizeDict ) );
}
  ...
  while( 1 ) {
    ...
    if( outputSet[WRTd_c] ) {
      ...
      if( utfupp ) {
        ...
        if( ( U8 ) WRTd_s[0] == 0xD0 && ( U8 ) WRTd_s[1] >= 0xb0 && ( U8 ) WRTd_s[1] <= 0xbf && s_size == 2 )
          WRTd_s[1] = ( U8 ) WRTd_s[1] - 0x20;
        else if( ( U8 ) WRTd_s[0] == 0xD1 && ( U8 ) WRTd_s[1] >= 0x80 && ( U8 ) WRTd_s[1] <= 0x8f && s_size == 2 )
          WRTd_s[1] = ( U8 ) WRTd_s[1] - 0xe0, WRTd_s[0] = 0xD0;
        //greek
        else if( ( U8 ) WRTd_s[0] == 0xce && ( U8 ) WRTd_s[1] >= 0xb1 && ( U8 ) WRTd_s[1] <= 0xbf && s_size == 2 )
          WRTd_s[1] = ( U8 ) WRTd_s[1] - 0x20;
        else if( ( U8 ) WRTd_s[0] == 0xcf && ( U8 ) WRTd_s[1] >= 0x80 && ( U8 ) WRTd_s[1] <= 0x89 && s_size == 2 )
          WRTd_s[1] = ( U8 ) WRTd_s[1] - 0xe0, WRTd_s[0] = 0xce;
        //latin
        else if( ( U8 ) WRTd_s[0] == 0xc3
                 && ( ( ( U8 ) WRTd_s[1] >= 0xa0 && ( U8 ) WRTd_s[1] <= 0xb6 )
                      || ( ( U8 ) WRTd_s[1] >= 0xb8 && ( U8 ) WRTd_s[1] <= 0xbe ) )
                 && s_size == 2 )
          WRTd_s[1] = ( U8 ) WRTd_s[1] - 0x20;
        utfupp = 0;
      }
...
void XWRT_Decoder::read_dict() {
  ...
  count = bufferData[0];
  bufferData++;
  count += 256 * bufferData[0];
  bufferData++;
  count += 65536 * bufferData[0];
  bufferData++;
...
U32 utf8_check( U8 *s ) {
  ...
  if( *s < 0x80 ) /* 0xxxxxxx */
    ...
    if( ( s[1] & 0xc0 ) != 0x80 || ( s[2] & 0xc0 ) != 0x80 || ( s[0] == 0xe0 && ( s[1] & 0xe0 ) == 0x80 )
        ||                                                            /* overlong? */
...
inline void XWRT_Encoder::encodeSpaces() {
  if( spaces == 1 ) {
    ...
    while( spaces > 0 ) {
      ...
      while( sp > 0 && spacesCodeword[sp] == 0 )
        sp--;
...
void XWRT_Encoder::encodeWord( U8 *s, int s_size, EWordType wordType, int &c ) {
  if( detect ) {
    ...
    if( ( s_size < 3 && s[0] >= '0' && s[0] <= '9' ) && wrtnum == 1 )
      return; //if text0 mode
  ...
  if( s_size >= WORD_MIN_SIZE ) {
    ...
    if( i < 0 ) {
      if( wordType
          == UTF8UPPER ) { //revert back to UPPERCASE UTF if not found. Well, this is not correct palce to to this
        wordType = LOWERWORD;
        if( ( U8 ) s[0] == 0xD0 && ( U8 ) s[1] >= 0xb0 && ( U8 ) s[1] <= 0xbf && s_size == 2 )
          s[1] = ( U8 ) s[1] - 0x20;
        else if( ( U8 ) s[0] == 0xD1 && ( U8 ) s[1] >= 0x80 && ( U8 ) s[1] <= 0x8f && s_size == 2 )
          s[1] = ( U8 ) s[1] - 0xe0, s[0] = 0xD0;
        else if( ( U8 ) s[0] == 0xce && ( U8 ) s[1] >= 0xb1 && ( U8 ) s[1] <= 0xbf && s_size == 2 )
          s[1] = ( U8 ) s[1] - 0x20;
        else if( ( U8 ) s[0] == 0xcf && ( U8 ) s[1] >= 0x80 && ( U8 ) s[1] <= 0x89 && s_size == 2 )
          s[1] = ( U8 ) s[1] - 0xe0, s[0] = 0xce;
        else if( ( U8 ) s[0] == 0xc3
                 && ( ( ( U8 ) s[1] >= 0xa0 && ( U8 ) s[1] <= 0xb6 ) || ( ( U8 ) s[1] >= 0xb8 && ( U8 ) s[1] <= 0xbe ) )
                 && s_size == 2 )
          s[1] = ( U8 ) s[1] - 0x20;
        else if( ( ( ( U8 ) s[1] >= 0x80 && ( U8 ) s[1] < 0xbf && ( ( U8 ) s[1] ) & 1 == 1 ) ) && ( U8 ) s[0] == 0xc4
                 && s_size == 2 )
          s[1] = ( U8 ) s[1] - 1;
        else if( ( ( ( U8 ) s[1] >= 0x81 && ( U8 ) s[1] <= 0xbe && ( ( U8 ) s[1] ) & 1 == 1 ) ) && ( U8 ) s[0] == 0xc5
                 && s_size == 2 )
          s[1] = ( U8 ) s[1] - 1;
      }
...
void XWRT_Encoder::WRT_encode( int filelen ) {
    ...
    if( wordSet[c] ) {
      ...
      if( s_size > 1 ) {
        ...
        if( y > 0 ) {
          ...
          //latin 1 suplement
          else if( ( ( ( U8 ) s[1] >= 0x80 && ( U8 ) s[1] <= 0x96 ) || ( ( U8 ) s[1] >= 0x98 && ( U8 ) s[1] <= 0x9e ) )
                   && ( U8 ) s[0] == 0xc3 && y == 2 ) {
            s[1] = ( U8 ) s[1] + 0x20;
            wordType = UTF8UPPER;
          } else if( ( ( ( U8 ) s[1] >= 0x80 && ( U8 ) s[1] < 0xbf && ( ( U8 ) s[1] ) & 1 == 0 ) )
                     && ( U8 ) s[0] == 0xc4 && y == 2 ) {
            s[1] = ( U8 ) s[1] + 1;
            wordType = UTF8UPPER;
          } else if( ( ( ( U8 ) s[1] >= 0x81 && ( U8 ) s[1] <= 0xbe && ( ( U8 ) s[1] ) & 1 == 0 ) )
                     && ( U8 ) s[0] == 0xc5 && y == 2 ) {
            s[1] = ( U8 ) s[1] + 1;
            wordType = UTF8UPPER;
          }
  ...
  encodeWord( s, s_size, wordType, c );
  s_size = 0;
  if( ( filelen - utfcount ) < ( filelen >> 1 ) )
    lowerfq = 1; // set flag if more utf then ascii
}
...
int XWRT_Encoder::WRT_detectFileType( int filelen ) {
  ...
  PRINT_DICT( ( "maxDict=%d allocatedMemory=%d hashTable=%d\n", dictionary,
                dictionary * WORD_AVG_SIZE + sizeof( U8 * ) * ( dictionary + 1 ) + sizeof( U8 ) * ( dictionary + 1 )
                    + sizeof( int ) * ( dictionary + 1 ),
                HASH_TABLE_SIZE * sizeof( word_hash[0] ) ) );
...
void XWRT_Encoder::WRT_detectFinish() {
  ...
  for( i = 1; i < sizeDict - 1; i++ ) {
    num = dictfreq[i];
    if( num >= minWordFreq
        || ( num >= minWordFreq2 && ( dictlen[i] >= 7 ) ) /*|| (lowerfq && num>=6 && utf8_check(dict[i]))*/ )
      ;
    else
      dictfreq[i] = 0;
  }
