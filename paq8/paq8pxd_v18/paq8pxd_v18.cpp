...
const char *typenames[datatypecount] = { "default", "jpeg",   "hdr", "1b-image", "4b-image", "8b-image", "24b-image",
                                         "audio",   "exe",    "cd",  "text",     "text0",    "utf-8",    "nes",
                                         "base64",  "base85", "gif", "SZDD",     "mrb",      "zlib",     "mdf" };
...
Filetype detect( FILE *in, U64 n, Filetype type, int &info, int &info2, int it = 0, int s1 = 0 ) {
  ...
  for( U64 i = 0; i < n; ++i ) {
    ...
    //detect LZSS compressed data in compress.exe generated archives
    if( ( buf0 == 0x88F02733 && buf1 == 0x535A4444 && !cdi ) || ( buf1 == 0x535A2088 && buf0 == 0xF02733D1 ) )
      fSZDD = i;
    if( fSZDD
    ...
    if( !soi && i >= 3 && ( ( buf0 & 0xfffffff0 ) == 0xffd8ffe0 /*|| buf0==0xffd8ffdb*/ ) )
...
void transform_encode_block( Filetype type, FILE *in, int len, Encoder &en, int info, int info2, char *blstr, int it,
  ...
  if( type == EXE || type == CD || type == MDF || type == IMAGE24 || type == MRBR
    ...
    if( type == IMAGE24 )
      ...
      decode_audio( en, int( tmpsize ), in, info, int( len ), info2, FCOMPARE, diffFound );
    else if( type == EXE )
      ...
      decode_mdf( tmp, int( tmpsize ), in, FCOMPARE, diffFound );
