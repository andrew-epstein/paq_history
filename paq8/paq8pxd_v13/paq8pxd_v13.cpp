...
PredictorTXTWRT::PredictorTXTWRT() :
  ...
  if( level >= 4 && recordModelw == 0 )
    recordModelw = new recordModelx( x ); //glogal recordmodel
  if( level >= 4 && sparseModel1 == 0 )
    sparseModel1 = new sparseModelx( x ); //glogal recordmodel
  if( level >= 4 && wordModel == 0 )
    wordModel = new wordModel1( x ); //glogal recordmodel
  if( level >= 4 && indirectModel == 0 )
    indirectModel = new indirectModel1( x ); //glogal recordmodel
  if( level >= 4 && dmcModel == 0 )
    dmcModel = new dmcModel1( x ); //glogal recordmodel
  if( level >= 4 && nestModel == 0 )
    nestModel = new nestModel1( x ); //glogal recordmodel
...
PredictorAUDIO::PredictorAUDIO() :
    pr( 2048 ), m( 925, 3095 + 256 * 7 + 256 * 8 + 256 * 7 + 2048 + 256 * 3 - 264, x, 7 ) {
...
void direct_encode_blockstream( Filetype type, FILE *in, uint64_t len, Encoder &en, uint64_t s1, uint64_t s2,
  ...
  if( type == DEFAULT || type == HDR )
    ...
    out = filestreams[8]; //text stream with lots of 0-9
...
uint64_t decompressStreamRecursive( FILE *out, uint64_t size, Encoder &en, FMode mode, int it = 0, uint64_t s1 = 0,
  ...
  while( i < size ) {
    ...
    if( type == DEFAULT || type == HDR )
      en.setFile( filestreams[0] );
    else if( type == JPEG )
      en.setFile( filestreams[1] );
    else if( type == IMAGE1 )
      en.setFile( filestreams[2] );
    else if( type == IMAGE4 )
      en.setFile( filestreams[3] );
    else if( type == IMAGE8 )
      en.setFile( filestreams[4] );
    else if( type == IMAGE24 )
      en.setFile( filestreams[5] );
    else if( type == AUDIO )
      en.setFile( filestreams[6] );
    else if( type == EXE )
      en.setFile( filestreams[7] );
    else if( type == TEXT0 )
      en.setFile( filestreams[8] ); //text stream with lots of 0-9
    else if( type == TXTUTF8 || type == DICTTXT || type == TEXT )
      en.setFile( filestreams[9] );
          ...
          if( i == 0 ) {
            ...
            while( datasegmentsize > 0 ) {
              while( datasegmentlen == 0 ) {
                ...
                if( !( datasegmenttype == DEFAULT || datasegmenttype == HDR ) )
                  datasegmentlen = 0;
