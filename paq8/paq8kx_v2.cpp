/* paq8kx file compressor/archiver.  Release by Jan Ondrus, Jul. 15, 2009
...
int jpegModel( Mixer &m ) {
  ...
  int pr = m1.p();
  m.add( stretch( pr ) );
  pr = a1.p( pr, hc & 511 | ( adv_pred[1] == 0 ? 0 : ( abs( adv_pred[1] ) - 4 ) & 63 ) << 9 );
...
int chartModel( Mixer &m, int ismatch ) {
  ...
  if( !bpos ) {
    ...
    for( int i = 0, s = 0, e = a[0], k = chart[0]; i<20; s = ++i> > 3, e = a[s],
             k = chart[i] ) {                                    //   k   e
      cm.set( k << s );                                          //  111 000
...
int contextModel2() {
  static ContextMap cm( MEM * 8, 9 );
  static RunContextMap rcm7( MEM ), rcm9( MEM ), rcm10( MEM );
  static Mixer m( 4000, 8000, 12, 128 );
