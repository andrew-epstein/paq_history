/* PAQ8C file compressor/archiver.
(C) 2006, Matt Mahoney under GPL, http://www.gnu.org/licenses/gpl.txt

PAQ8C is a file compressor and archiver.  To use:
...
PAQ8C never clobbers files.  If the file to be extracted already
exists, then PAQ8 compares it with the archived copy and reports
whether they differ.  Example:

  paq8c xyz foo bar      Compresses foo and bar to archive xyz.
  paq8c xyz baz          Extracts baz as a copy of foo, compares bar.
...
PAQ8C cannot update an archive.  You must create a new one instead.
...
TO COMPILE
  ...
  MINGW g++:
    ...
    g++ paq8c.cpp -O2 -Os -s -march=pentiumpro -fomit-frame-pointer -o paq8c.exe paq7asm.obj

  DJGPP g++:
    nasm paq7asm.asm -f coff --prefix _
    gxx paq8c.cpp -O2 -Os -s -march=pentiumpro -fomit-frame-pointer -o paq8c.exe paq7asm.obj

  Borland:
    nasm paq7asm.asm -f obj --prefix _
    bcc32 -O -w-8027 paq8c.cpp paq7asm.obj

  Mars:
    nasm paq7asm.asm -f obj --prefix _
    dmc paq8c.cpp paq7asm.obj

  UNIX/Linux (PC):
    nasm -f elf paq7asm.asm
    g++ paq8c.cpp -O2 -Os -s -march=pentiumpro -fomit-frame-pointer -o paq8c paq7asm.o

  Non PC (UNIX):
    g++ paq8c.cpp -O2 -DNOASM -s -o paq8c
...
#define PROGNAME "paq8c" // Please change this if you change the program.
...
enum { DEFAULT = 0, EXE, BINTEXT, TEXT }; // file types
...
void exeModel( Mixer &m ) {
  ...
  if( bpos == 0 ) {
    for( int i = 0; i < N; ++i )
      cm.set( execxt( i, buf( 1 ) * static_cast<int>( i > 4 ) ), i );
  }
  cm.mix( m );
}
...
Filter *Filter::make( const char *filename, Encoder *e ) {
  if( filename != nullptr ) {
    ...
    if( (ext != nullptr)
        && ( (equals( ext, ".exe" ) != 0) || (equals( ext, ".dll" ) != 0) || (equals( ext, ".obj" ) != 0) || (equals( ext, ".ocx" ) != 0)
             || (equals( ext, ".drv" ) != 0) ) )
      filetype = EXE;
    else if( (ext == nullptr) || ( (equals( ext, ".dbf" ) == 0) && (equals( ext, ".mdb" ) == 0) ) ) {
...
int main( int argc, char **argv ) {
  ...
  if( f == nullptr ) {
    ...
    if( argc != 2 ) {
      filenames = new std::string[argc];
      filetypes = new int[argc];
    }
    ...
    while( true ) {
      if( argc == 2 ) {
        ...
        if( (filename == nullptr) || (filename[0] == 0) )
          break;
        store_in_header( f, filename, total_size );
      } else {
        if( i == argc )
          break;
        filename = argv[i++];
    ...
    if( argc != 2 ) {
      ...
      for( i = 0; i < fc; i++ ) {
        ...
        filetypes[i] = filetype * 131072 + wrt.preprocFlag;
        int j = i - 1;
        while( j >= 0 && ( filetypes[j + 1] > filetypes[j] ) ) // Insertion Sort
        {
          filetype = filetypes[j + 1];
          fname = filenames[j + 1];
          filetypes[j + 1] = filetypes[j];
          filenames[j + 1] = filenames[j];
          filetypes[j] = filetype;
          filenames[j] = fname;
          j--;
        }
      }
      ...
      for( i = 0; i < fc; i++ )
        store_in_header( f, ( char * ) filenames[i].c_str(), total_size );
