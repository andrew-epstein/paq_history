/*  paq8q file compressor/archiver.  Release by Simon Berger Jul. 10, 2009
...
im8bitModel:
	...
	- RGB (8-bit; 24-bit (3 channels); 32-bit (4 channels))
im24bitModel:
	- BMP (24-bit)
	- TIFF (24-bit; uncompressed)
	- PPM (24-bit)
	- TGA (normal 24-bit; uncompressed)
wavModel:
	- WAVE (1 or 2 channels; 8 bits per sample)
	- WAVE (1 or 2 channels; 16 bits per sample)
	- AIFF (1 or 2 channels; 44100 sample rate; 8 bits per sample)
	- AIFF (1 or 2 channels; 44100 sample rate; 16 bits per sample)
	- MOD
	- S3M (8 bits per sample)
cd:
...
public:
  ...
  void compress( int c ) {
    ...
    if( level == 0 )
      ...
      code( ( c >> 7 ) & 1 ), code( ( c >> 6 ) & 1 ), code( ( c >> 5 ) & 1 ), code( ( c >> 4 ) & 1 );
      code( ( c >> 3 ) & 1 ), code( ( c >> 2 ) & 1 ), code( ( c >> 1 ) & 1 ), code( ( c ) &1 );
    }
  }
...
Filetype detect( FILE *in, int n, Filetype type, int &info ) {
  ...
  for( int i = 0; i < n; ++i ) {
    ...
    if( txtStart == 0 && cmode != 6 && cmode != 1 && (soi == 0) && (pgm == 0) && (rgbi == 0) && (bmp == 0) && (wavi == 0) && (cdi == 0) && (tga == 0)
...
int decode_cd( FILE *in, int size, FILE *out, FMode mode, int &diffFound ) {
  ...
  while( i < size ) {
    if( size - i == q ) {
      ...
      if( blk[15] != 1 )
        fread( blk + 16, 4, 1, in );
      bsize = 2048 + static_cast<int>( blk[15] == 3 ) * 276;
      i += 4 * static_cast<int>( blk[15] != 1 ) + 4;
      a = -1;
    } else {
      a = ( blk[12] << 16 ) + ( blk[13] << 8 ) + blk[14];
    }
    fread( blk + 16 + static_cast<int>( blk[15] != 1 ) * 8, bsize, 1, in );
    i += bsize;
    if( bsize > 2048 )
      blk[15] = 3;
...
void compressRecursive( FILE *in, long n, Encoder &en, int it = 0, int s1 = 0, int s2 = 0 ) {
