...
int jpegModel( Mixer &m ) {
  ...
  static Array<int> adv_pred( 4 ), sumu( 8 ), sumv( 8 );
      ...
      if( rs >= 0 ) {
        if( huffsize + ( rs & 15 ) == huffbits ) { // done decoding
          ...
          // UPDATE_ADV_PRED !!!!
          {
            ...
            for( int i = 0; i < 3; ++i ) {
              int p = ( sumu[zzu[zz]] * i + sumv[zzv[zz]] * ( 2 - i ) ) / 2;
              p /= ( qtab[q + zz] + 1 ) * 181;
              if( zz == 0 )
            ...
            for( int i = 0; i < 2; ++i ) {
              ...
              if( a == 0 )
                ...
                const int zz2 = zpos[zzu[zz] + 8 * zzv[zz] - ( (i & 1) != 0 ? 8 : 1 )];
  ...
  // Context model
  const int N = 4;       // size of t, number of contexts
  ...
  if( hbcount == 0 ) {
    ...
    cxt[2] = hash( ++n, hc, static_cast<U32>(comp == 0), rs1, adv_pred[1] );
    cxt[3] = hash( ++n, hc, lcp[0], lcp[1], adv_pred[1] );
  }
