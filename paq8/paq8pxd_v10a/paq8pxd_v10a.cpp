...
void im8bitModel( Mixer &m, int w ) {
  const int SC = 0x20000;
  static SmallStationaryContextMap scm1( SC ), scm2( SC ), scm3( SC ), scm4( SC ), scm5( SC ),
      scm6( SC * 2 ) /*,scm7(SC*2)*/, scm8( SC );
...
int contextModel2() {
  ...
  ;
  if( filetype == IMAGE8 )
    return im8bitModel( m, finfo ), m.p();
  if( filetype == IMAGE24 )
    return im24bitModel( m, finfo ), m.p();
  if( filetype == AUDIO )
  ...
  try {
    ...
    if( argc < 2 ) {
      printf( PROGNAME " archiver (C) 2008,2014, Matt Mahoney et al.\n"
                       ...
                       "-4 -5 -6 -7 -8 -9 = smaller (uses 133, 233, 435, 837, 1643, 3200+ MB)\n"
...
#  define atoll( S ) _atoi64( S )
    ...
    if( mode == COMPRESS ) {
      ...
      const char *typenames[13] = { "default", "jpeg", "hdr", "1b-image", "4b-image", "8b-image", "24b-image",
                                    "audio",   "exe",  "cd",  "text",     "utf-8",    "base64" };
      U32 ttc;
      uint64_t tts;
