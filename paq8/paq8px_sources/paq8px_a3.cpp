...
Filetype detect( FILE *in, int n, Filetype type ) {
  ...
  for( int i = 0; i < n; ++i ) {
    ...
    if( ( buf0 & 0xFFFF ) == 16973 )
      imgbpp = bsize = 0, imgcomp = -1, bmp = i; //possible 'BM'
    if( bmp != 0 ) {
      const int p = i - bmp;
      if( p == 4 )
      ...
      if( p == 16 && buf0 != 0x28000000 )
        bmp = 0; //windows bmp?
      if( p == 20 )
        bmpx = bswap( buf0 ), bmp = ( bmpx == 0 ? 0 : bmp ); //width
      if( p == 24 )
        bmpy = bswap( buf0 ), bmp = ( bmpy == 0 ? 0 : bmp ); //height
      if( p == 27 )
        imgbpp = c, bmp = ( ( imgbpp != 1 && imgbpp != 4 && imgbpp != 8 && imgbpp != 24 ) ? 0 : bmp );
      if( p == 31 )
        imgcomp = buf0, bmp = ( imgcomp != 0 ? 0 : bmp );
      if( ( type == BMPFILE1 || type == BMPFILE4 || type == BMPFILE8 || type == BMPFILE24 ) && imgbpp != 0
          && imgcomp == 0 ) {
        if( imgbpp == 1 )
          bsize = ( ( ( ( ( bmpx - 1 ) >> 5 ) + 1 ) * 4 * bmpy ) + bmpimgoff );
        if( imgbpp == 4 )
          bsize = ( ( ( ( ( bmpx - 1 ) >> 3 ) + 1 ) * 4 * bmpy ) + bmpimgoff );
        if( imgbpp == 8 )
          bsize = ( ( bmpx + 3 & -4 ) * bmpy + bmpimgoff );
        if( imgbpp == 24 )
          bsize = ( ( bmpx * 3 + 3 & -4 ) * bmpy * 3 + bmpimgoff );
    ...
    if( rgbi != 0 ) {
      ...
      if( p == 2 && c != 1 && c != 2 )
        rgbi = 0;
      if( p == 4 && ( buf0 & 0xFFFF ) != 1 && ( buf0 & 0xFFFF ) != 2 && ( buf0 & 0xFFFF ) != 3 )
        rgbi = 0;
      if( p == 6 )
        if( ( buf0 & 0xFFFF ) > 0 )
          rgb_x = buf0 & 0xFFFF;
      ...
      if( p == 8 )
        if( ( buf0 & 0xFFFF ) > 0 )
          rgb_size = ( buf0 & 0xFFFF ) * rgb_x + 512;
        else
          rgbi = 0;
      if( p == 10 && ( buf0 & 0xFFFF ) != 1 )
        rgbi = 0;
