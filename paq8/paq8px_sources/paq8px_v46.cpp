...
Filetype detect( FILE *in, int n, Filetype type, int &info ) {
  ...
  for( int i = 0; i < n; ++i ) {
    ...
    if( ( cdi != 0 ) && i > cdi ) {
      ...
      if( p == 8 && ( buf1 != 0xffffff00 || ( i - cdi > 2352 && cdm != ( buf0 & 0xff ) ) ) )
        cdi = 0;
      else if( p == 8 )
        cdm = ( buf0 & 0xff ), cdi = ( cdm != 1 && cdm != 2 ) ? 0 : cdi;
      else if( p == 16 && cdm == 2 && ( buf0 != buf1 || ( i - cdi > 2352 && buf0 != cdf ) ) )
        cdi = 0;
      else if( p == 20 && cdm == 2 )
        cdf = buf1;
      else if( p == 2068 && cdm == 1 && ( buf1 != 0 || buf0 != 0 ) )
        cdi = 0;
      else if( p == 0 ) {
        U8 data[2352];
...
void encode_cd( FILE *in, FILE *out, int len ) {
  ...
  for( int offset = 0; offset < len; offset += BLOCK ) {
    if( offset + BLOCK > len ) {
      ...
      if( offset == 0 )
        fwrite( &blk[12], 1, 4 + 4 * static_cast<int>( blk[15] == 2 ), out );
      fwrite( &blk[16 + 8 * static_cast<int>( blk[15] == 2 )], 1, 2048, out );
    }
  }
}
...
int decode_cd( Encoder &en, int size, int it ) {
  ...
  if( state == 0 && size2 >= BLOCK - 1 ) {
    int a;
    if( size2 == size - 1 ) {
      for( int i = 12; i < 16 + 4 * static_cast<int>( blk[15] == 2 ); i++ )
        blk[i] = decode( en, it );
      a = -1;
    } else
      a = ( blk[12] << 16 ) + ( blk[13] << 8 ) + blk[14];
    for( int i = 0; i < 2048; i++ )
      blk[16 + i + static_cast<int>( blk[15] == 2 ) * 8] = decode( en, it );
...
int decode( Encoder &en, int it = MAX_ITER ) {
  ...
  switch( type[it] ) {
    case EXE:
      return decode_exe( en, size[it] );
    case CD:
      return decode_cd( en, size[it], it );
    default:
