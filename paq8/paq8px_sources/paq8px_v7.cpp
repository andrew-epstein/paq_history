...
int im24bitModel( Mixer &m ) {
  ...
  static int w = 0;
  static int h = 0;      // size of image in bytes (pixels)
  ...
  // Detect .ppm header
  if( ( bpos == 0 ) && pos > eoi ) {
    if( ( ppm == 0 ) && buf( 3 ) == 'P' && buf( 2 ) == '6' && ISWHITESPACE( 1 ) ) {
      ppm = pos;
      ppm_ptr = 0;
    ...
    if( ( ppm != 0 ) && ppm_ptr != 3 ) { // parse header records
      ...
      if( ppm_ptr == 3 ) {
        ...
        ppm = 0;
      } else if( pos - ppm > 1024 )
        ppm = 0; // maximal length of header
    }
  }
  ...
  if( ( bpos == 0 ) && pos > eoi ) {
    ...
    if( buf( 54 ) == 'B' && buf( 53 ) == 'M' && i4( 44 ) == 54 && i4( 40 ) == 40 && i4( 24 ) == 0 )
      bo = 54;
    if( buf( 66 ) == 'B' && buf( 65 ) == 'M' && i4( 56 ) == 66 && i4( 52 ) == 40 && i4( 36 ) == 0 )
      bo = 66;
    if( bo != 0 ) {
...
int pgmModel( Mixer &m ) {
  ...
  int v_ptr;
  if( ( bpos == 0 ) && pos > eoi ) {
    if( ( pgm == 0 ) && buf( 3 ) == 'P' && buf( 2 ) == '5' && ISWHITESPACE( 1 ) ) { // Detect PGM file
      pgm = pos;
      pgm_ptr = 0;
    ...
    if( ( pgm != 0 ) && pgm_ptr != 3 ) { // PGM detected, let's parse header records
      ...
      if( pgm_ptr == 3 ) {
        ...
        pgm = 0;
      } else if( pos - pgm > 1024 )
        pgm = 0; // maximal length of header
    }
  }
  if( pos > eoi )
    return w = 0;
...
void pbmModel( Mixer &m ) {
  ...
  int v_ptr;
  if( ( bpos == 0 ) && pos > eoi ) {
    if( ( pbm == 0 ) && buf( 3 ) == 'P' && buf( 2 ) == '4' && ISWHITESPACE( 1 ) ) { // Detect PBM file
      pbm = pos;
      pbm_ptr = 0;
    ...
    if( ( pbm != 0 ) && pbm_ptr != 2 ) { // parse header records
      ...
      if( pbm_ptr == 2 ) {
        ...
        pbm = 0;
      } else if( pos - pbm > 1024 )
        pbm = 0; // maximal length of header
    }
  }
  if( pos > eoi )
    return;
...
Filetype detect( FILE *in, int n, Filetype type ) {
  ...
  for( int i = 0; i < n; ++i ) {
    ...
    if( bmp != 0 ) {
      ...
      if( p == 24 )
        bmpy = bswap( buf0 ), ( bmpy < 0 ? bmpy *= ( -1 ) : bmpy ), bmp = ( bmpy == 0 ? 0 : bmp ); //height
      if( p == 27 )
