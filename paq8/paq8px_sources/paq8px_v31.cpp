...
void wavModel( Mixer &m, int info ) {
  ...
  if( (bpos == 0) && blpos >= w ) {
    ...
    if( bits == 16 && wmode < 6 )
      t = static_cast<int>( msb != 0 ), z1 = s2( w + t ), z2 = s2( w * 2 + t ), z3 = s2( w * 3 + t ), z4 = s2( w * 4 + t ),
      z5 = s2( w * 5 + t );
    else if( bits == 8 )
      t = 1, z1 = buf( w ), z2 = buf( w * 2 ), z3 = buf( w * 3 ), z4 = buf( w * 4 ), z5 = buf( w * 5 );
    else
      t = static_cast<int>( msb == 0 ), z1 = t2( w + ( 1 - t ) ), z2 = t2( w * 2 + ( 1 - t ) ), z3 = t2( w * 3 + ( 1 - t ) ),
      z4 = t2( w * 4 + ( 1 - t ) ), z5 = t2( w * 5 + ( 1 - t ) );
    i = ch << 4;
