...
void wavModel( Mixer &m, int info ) {
  ...
  if( ( bpos == 0 ) && blpos >= w ) {
    ...
    const int x1 = buf( 1 ) ^ ( wmode == 4 ? 128 : 0 ) - 128;
    const int x2 = buf( 2 ) ^ ( wmode == 4 ? 128 : 0 ) - 128;
    ...
    int z5;
    if( bits == 16 && wmode < 6 )
      t = static_cast<int>( msb != 0 ), z1 = s2( w + t ), z2 = s2( w * 2 + t ), z3 = s2( w * 3 + t ),
      z4 = s2( w * 4 + t ), z5 = s2( w * 5 + t );
    else if( bits == 8 )
      t = 1, z1 = buf( w ), z2 = buf( w * 2 ), z3 = buf( w * 3 ), z4 = buf( w * 4 ), z5 = buf( w * 5 );
    else
      t = static_cast<int>( msb == 0 ), z1 = t2( w + ( 1 - t ) ), z2 = t2( w * 2 + ( 1 - t ) ),
      z3 = t2( w * 3 + ( 1 - t ) ), z4 = t2( w * 4 + ( 1 - t ) ), z5 = t2( w * 5 + ( 1 - t ) );
    i = ch << 4;
    ...
    if( aiff != 0 ) {
      ...
      if( p == 12 && ( buf1 != 0x41494646 || buf0 != 0x434f4d4d ) )
        ...
        if( ( bits == 8 || bits == 16 ) && ( chn == 1 || chn == 2 ) )
          aiffm = ( bits == 8 ? chn - 1 : chn + 5 );
        else
          aiff = 0;
      } else if( p == 42 ) {
        if( buf1 == 0x53534e44 )
          AUD_DET( AUDIO, aiff - 3, 54, buf0 - 8, aiffm );
        aiff = 0;
      }
    }
...
void compress( const char *filename, long filesize, Encoder &en ) {
  static const char *typenames[8] = {"default",     "jpeg",         "hdr",   "1-bit-image",
                                     "8-bit-image", "24-bit-image", "audio", "exe"};
