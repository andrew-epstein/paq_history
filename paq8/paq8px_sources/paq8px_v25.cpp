...
void wavModel( Mixer &m, int info ) {
  ...
  if( bpos == 0 ) {
    ...
    const int x1 = buf( 1 ) ^ ( wmode == 8 ? 128 : 0 ) - 128;
    const int x2 = buf( 2 ) ^ ( wmode == 8 ? 128 : 0 ) - 128;
...
int contextModel2() {
  ...
  static const char *typenames[8] = {"", "JPEG ", "HDR ", "1BIT ", "8BIT ", "24BIT ", "WAVE ", "EXE "};
...
Filetype detect( FILE *in, int n, Filetype type, int &info ) {
  ...
  for( int i = 0; i < n; ++i ) {
    ...
    if( wavi != 0 ) {
      ...
      if( p == 4 )
        ...
        if( ( wavch == 1 || wavch == 2 ) && ( wavbps == 8 || wavbps == 16 ) && wavd > 0 && wavsize >= wavd + 36
          ...
          AUD_DET( AUDIO, wavi - 3, 44 + wavm, wavd, wavch + wavbps );
        wavi = 0;
      }
    }
    ...
    if( ( buf0 == 0x4d2e4b2e || buf0 == 0x3643484e || buf0 == 0x3843484e // M.K. 6CHN 8CHN
      ...
      if( numpat < 65 )
        AUD_DET( AUDIO, i - 1083, 1084 + numpat * 256 * chn, len, 8 );
    ...
    if( s3mi != 0 ) {
      ...
      if( p == 4 )
        ...
        if( ( ok != 0 ) && sam_start < ( 1 << 16 ) )
          AUD_DET( AUDIO, s3mi - 31, sam_start, sam_end - sam_start, 9 );
        s3mi = 0;
        fseek( in, savedpos, SEEK_SET );
      }
    }
