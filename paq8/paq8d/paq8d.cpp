/* PAQ8D file compressor/archiver.
(C) 2006, Matt Mahoney under GPL, http://www.gnu.org/licenses/gpl.txt

PAQ8D is a file compressor and archiver.  To use:
...
PAQ8D never clobbers files.  If the file to be extracted already
exists, then PAQ8 compares it with the archived copy and reports
whether they differ.  Example:

  paq8d xyz foo bar      Compresses foo and bar to archive xyz.
  paq8d xyz baz          Extracts baz as a copy of foo, compares bar.
...
PAQ8D cannot update an archive.  You must create a new one instead.
...
TO COMPILE
  ...
  MINGW g++:
    ...
    g++ paq8d.cpp -O2 -Os -s -march=pentiumpro -fomit-frame-pointer -o paq8d.exe paq7asm.obj

  DJGPP g++:
    nasm paq7asm.asm -f coff --prefix _
    gxx paq8d.cpp -O2 -Os -s -march=pentiumpro -fomit-frame-pointer -o paq8d.exe paq7asm.obj

  Borland:
    nasm paq7asm.asm -f obj --prefix _
    bcc32 -O -w-8027 paq8d.cpp paq7asm.obj

  Mars:
    nasm paq7asm.asm -f obj --prefix _
    dmc paq8d.cpp paq7asm.obj

  UNIX/Linux (PC):
    nasm -f elf paq7asm.asm
    g++ paq8d.cpp -O2 -Os -s -march=pentiumpro -fomit-frame-pointer -o paq8d paq7asm.o

  Non PC (UNIX):
    g++ paq8d.cpp -O2 -DNOASM -s -o paq8d
...
#define PROGNAME "paq8d" // Please change this if you change the program.
...
int main( int argc, char **argv ) {
  ...
  if( f == nullptr ) {
    ...
    for( i = 0; i < filenames.size(); i++ ) {
      ...
      std::pair<int, std::string> p( ( 1 << (31 - 1) ) - ( filetype * 131072 + wrt.preprocFlag ), filenames[i] );
      filetypes.insert( p );
    }
