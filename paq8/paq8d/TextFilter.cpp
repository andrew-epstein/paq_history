...
#define SAMPLE_WORDS_COUNT_MAX ( SAMPLE_WORDS_COUNT * ( CHARSET_COUNT - 1 ) )
  ...
  int WRT_detectFileType( FILE *file, int part_length, int parts, int &recordLen ) {
    ...
    PRINT_DICT( ( "EOL=%d+%d/%d(%d) bin=%d/%d(%d) punct=%d/%d(%d) lang=%d/%d(%d) cc=%d\n", EOLCount, EOLCountBad,
                  part_length * parts / 158, IF_OPTION( OPTION_EOL_CODING ), binCount, part_length * parts / 13,
                  !IF_OPTION( OPTION_NORMAL_TEXT_FILTER ), punctCount, punctCountBad,
                  IF_OPTION( OPTION_WORD_SURROROUNDING_MODELING ), langSum, part_length * parts / 128,
                  IF_OPTION( OPTION_USE_DICTIONARY ), IF_OPTION( OPTION_CAPITAL_CONVERSION ) ) );
