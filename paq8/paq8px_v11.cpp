...
int contextModel2() {
  ...
  static const char *typenames[8] = {"", "jpeg ", "imghdr ", "1-bit ", "8-bit ", "24-bit ", "exe ", "text "};
  ...
  if( filetype == IMAGE24 )
    return im24bitModel( m, imgw ), m.p();
  if( filetype == IMAGE8 )
    return im8bitModel( m, imgw ), m.p();
  if( filetype == IMAGE1 )
    im1bitModel( m, imgw );
...
#define IMG_DET( type, start_pos, header_len, width, height )                                                          \
  return imgt = ( type ), imgh = ( header_len ), imgd = ( width ) * ( height ), imgw = ( width ),                      \
...
Filetype detect( FILE *in, int n, Filetype type, int &imgw ) {
  ...
  // For image detection
  static int imgh = 0;
  static int imgd = 0;  // image header/data size in bytes
  ...
  for( int i = 0; i < n; ++i ) {
    ...
    if( rgbi != 0 ) {
      ...
      if( p == 1 && c != 0 )
        ...
        rgbz = buf0 & 0xffff, rgbi = ( ( rgbz != 1 && rgbz != 3 && rgbz != 4 ) ? 0 : rgbi );
      if( ( rgbx != 0 ) && ( rgby != 0 ) && ( rgbz != 0 ) )
        IMG_DET( IMAGE8, rgbi - 1, 512, rgbx, rgby * rgbz );
    }
          ...
          for( int i = 0; i < dirsize; i++ ) {
            ...
            if( tagfmt == 3 || tagfmt == 4 ) {
              if( tag == 256 )
                ...
                if( tagfmt == 3 )
                  tiff = 0; // stop if offsets stored in 16-bit
                else if( taglen > 1 )
                  tifofs = tagval; // read later
                else
                  tifofval = tagval; // stored by value
              } else if( tag == 277 )
                tifz = tagval; // components per pixel
            }
          }
        }
        if( ( tiff != 0 ) && ( tifx != 0 ) && ( tify != 0 ) && ( tifzb != 0 ) && ( tifz == 1 || tifz == 3 )
            && ( tifc == 1 ) && ( ( tifofval != 0 ) || ( ( tifofs != 0 ) && tifofs + tiff < n ) ) ) {
          if( tifofval == 0 ) {
          ...
          if( ( tifofval != 0 ) && tifofval < 65536 && tifofval + tiff < n ) {
            if( tifz == 1 ) {
              if( tifzb == 1 )
                IMG_DET( IMAGE1, tiff - 3, tifofval, ( ( tifx - 1 ) >> 3 ) + 1, tify );
              if( tifzb == 8 )
                IMG_DET( IMAGE8, tiff - 3, tifofval, tifx, tify );
            } else if( tifz == 3 && tifzb == 8 )
              IMG_DET( IMAGE24, tiff - 3, tifofval, tifx * 3, tify );
          }
        }
        tiff = 0;
    ...
    if( cbuf[3] == 0 && ( cbuf[2] == 0 || cbuf[2] == 1 ) && ( cbuf[1] >= 1 && cbuf[1] <= 3 ) && cbuf[0] == 0 )
      tga = i, tgapalette = buf0 >> 24, tgaimgtype = buf0 >> 16;
    if( tga != 0 ) {
      const int p = i - tga;
      if( p == 4 )
        tga = ( buf0 & 0xFF000000 ) == 0 ? tga : 0;
      else if( p == 5 )
        tgapallen = ( bswap( buf0 ) & 0xFFFF );
      else if( p == 8 )
        tga = ( buf0 & 0xFFFF0000 ) == 0 ? tga : 0;
      else if( p == 10 )
        tga = ( buf0 & 0xFFFF0000 ) == 0 ? tga : 0;
      else if( p == 12 )
        tgax = ( bswap( buf0 ) & 0xFFFF );
      else if( p == 14 )
        tgay = ( bswap( buf0 ) & 0xFFFF );
      else if( p == 16 )
        tgaz = ( bswap( buf0 ) & 0xFF );
      if( ( tgax != 0 ) && ( tgay != 0 ) && ( tgaz != 0 ) ) {
        if( tgaz == 8 )
          IMG_DET( IMAGE8, tga - 3, 18 + tgapallen, tgax, tgay );
        if( tgaz == 24 )
          IMG_DET( IMAGE24, tga - 3, 18 + tgapallen, tgax * 3, tgay );
        tga = 0;
      }
      if( p > 18 )
        tga = 0;
...
int decode( Encoder &en ) {
  ...
  while( len == 0 ) {
    ...
    if( len < 0 )
      len = 1;
    if( type == IMAGE1 || type == IMAGE8 || type == IMAGE24 )
      for( int i = 0; i < 4; i++ )
        en.decompress();
  }
  --len;
