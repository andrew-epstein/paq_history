/* paq8o7 file compressor/archiver.  Release by KZ, Oct. 15, 2007
...
- To install, put paq8o7.exe or a shortcut to it on your desktop.
- To compress a file or folder, drop it on the paq8o7 icon.
- To decompress, drop a .paq8o7 file on the icon.

A .paq8o7 extension is added for compression, removed for decompression.
The output will go in the same folder as the input.

While paq8o7 is working, a command window will appear and report
...
- To install, put paq8o7.exe somewhere in your PATH.
- To compress:      paq8o7 [-N] file1 [file2...]
- To decompress:    paq8o7 [-d] file1.paq8o7 [dir2]
- To view contents: more < file1.paq8o7

The compressed output file is named by adding ".paq8o7" extension to
the first named file (file1.paq8o7).  Each file that exists will be
...
If file1.paq8o7 exists then it is overwritten.

If the first named file ends in ".paq8o7" then it is assumed to be
...
The -d option forces extraction even if there is not a ".paq8o7"
...
During extraction, directories are created as needed.  For example:
  ...
  paq8o7 -4 c:\tmp\foo bar

compresses foo and bar (if they exist) to c:\tmp\foo.paq8o7 at level 4.

  paq8o7 -d c:\tmp\foo.paq8o7 .
...
There are 2 files: paq8o7.cpp (C++) and paq7asm.asm (NASM/YASM).
paq7asm.asm is the same as in paq7 and paq8x.  paq8o7.cpp recognizes the
...
Recommended compiler commands and optimizations:
  ...
  MINGW g++:
    ...
    g++ paq8o7.cpp -DWINDOWS -O2 -Os -s -march=pentiumpro -fomit-frame-pointer -o paq8o7.exe paq7asm.obj

  Borland:
    nasm paq7asm.asm -f obj --prefix _
    bcc32 -DWINDOWS -O -w-8027 paq8o7.cpp paq7asm.obj

  Mars:
    nasm paq7asm.asm -f obj --prefix _
    dmc -DWINDOWS -Ae -O paq8o7.cpp paq7asm.obj

  UNIX/Linux (PC):
    nasm -f elf paq7asm.asm
    g++ paq8o7.cpp -DUNIX -O2 -Os -s -march=pentiumpro -fomit-frame-pointer -o paq8o7 paq7asm.o

  Non PC (e.g. PowerPC under MacOS X)
    g++ paq8o7.cpp -O2 -DUNIX -DNOASM -s -o paq8o7
...
Then
  ...
  paq8o7 archive \dir1\file1.txt \dir2

will create archive.paq8o7 with the header:
...
Decompression will fail if the first 7 bytes are not "paq8o7 -".  Sizes
...
paq8o7 uses a neural network to combine a large number of models.  The
...
paq8o7 uses preprocessing transforms on certain data types to improve
...
#define PROGNAME "paq8o7" // Please change this if you change the program.
        ...
        if( huffsize + ( rs & 15 ) == huffbits ) { // done decoding
          ...
          // UPDATE_ADV_PRED !!!!
          {
            ...
            if( zz == 0 ) {
              ...
              for( int i = 0; i < 64; ++i ) {
                sumu[zzu[i]] += ( zzv[i] != 0u ? ( (zzv[i] & 1) != 0 ? -256 : 256 ) : 181 ) * ( qtab[q + i] + 1 )
...
// To compress to file1.paq8o7: paq8o7 [-n] file1 [file2...]
// To decompress: paq8o7 file1.paq8o7 [output_dir]
int main( int argc, char **argv ) {
  ...
  try {
    ...
    // Print help message
    if( argc < 2 ) {
      printf( PROGNAME " archiver (C) 2007, KZ et al.\n"
