        ...
        if( huffsize + ( rs & 15 ) == huffbits ) { // done decoding
          ...
          // UPDATE_ADV_PRED !!!!
          {
            ...
            if( zz == 0 ) {
              ...
              for( int i = 0; i < 64; ++i ) {
                sumu[zzu[i]] += ( zzv[i] ? ( zzv[i] & 1 ? -256 : 256 ) : 181 ) * ( qtab[q + i] + 1 )
                                * cbuf2[cpos_dc_mcusize_width + i];
                sumv[zzv[i]] +=
                    ( zzu[i] ? ( zzu[i] & 1 ? -256 : 256 ) : 181 ) * ( qtab[q + i] + 1 ) * cbuf2[cpos_dc_ls_acomp + i];
              }
            } else {
              sumu[zzu[zz - 1]] -= ( zzv[zz - 1] ? 256 : 181 ) * ( qtab[q + zz - 1] + 1 ) * cbuf2[cpos - 1];
              sumv[zzv[zz - 1]] -= ( zzu[zz - 1] ? 256 : 181 ) * ( qtab[q + zz - 1] + 1 ) * cbuf2[cpos - 1];
            }
            for( int i = 0; i < 3; ++i )
