...
Filetype detect( FILE *in, int n, Filetype type, int &info ) {
  ...
  int cdi = 0;
  int cda;
  int cdm; // For CD sectors detection
  U32 cdf;
...
int decode_cd( Encoder &en, int size, int it ) {
  ...
  static int statei[MAX_ITER];
  static int size2i[MAX_ITER];
  static int cdfoi[MAX_ITER];
  static int start = 0;
  if( start == 0 ) {
    start = 1;
  ...
  if( state == 0 && size2 >= BLOCK - 1 ) {
    int a;
    if( size2 == size - 1 ) {
      for( int i = 12; i < 16 + 4 * static_cast<int>( blk[15] != 1 ); i++ )
        blk[i] = decode( en, it );
      cdfo = 1 + static_cast<int>( blk[15] == 3 );
      a = -1;
    } else
      a = ( blk[12] << 16 ) + ( blk[13] << 8 ) + blk[14];
    for( int i = 0; i < 2048 + 276 * static_cast<int>( cdfo == 2 ); i++ )
      blk[16 + i + static_cast<int>( blk[15] != 1 ) * 8] = decode( en, it );
    if( cdfo == 2 )
      blk[15] = 3;
    if( size2 < 2 * BLOCK - 1 && blk[15] != 1 )
      for( int i = 16; i < 20; i++ )
        blk[i] = decode( en, it );
...
int decode( Encoder &en, int it = MAX_ITER ) {
  ...
  static int len[MAX_ITER];
  static int size[MAX_ITER];
  static int start = 0;
  if( start == 0 ) {
    start = 1;
    printf( "(decode init)\n" );
  ...
  while( len[it] == 0 ) {
    ...
    if( len[it] < 0 )
      len[it] = 1;
    if( type[it] == IMAGE1 || type[it] == IMAGE8 || type[it] == IMAGE24 || type[it] == AUDIO )
      for( int i = 0; i < 4; i++ )
        en.decompress();
    size[it] = len[it];
  }
  ...
  switch( type[it] ) {
    case EXE:
      printf( "DECODE_EXE_START: Size: %d\n", size[it] );
      t1 = decode_exe( en, size[it] );
      printf( "DECODE_EXE_END: Size: %d\n", size[it] );
      return t1;
    case CD:
      printf( "DECODE_CD_START: Size: %d It: %d\n", size[it], it );
      t2 = decode_cd( en, size[it], it );
      printf( "DECODE_CD_END: Size: %d It: %d\n", size[it], it );
      return t2;
...
void iter_transform( FILE *in, int filesize, Encoder &en, int it = MAX_ITER, int s1 = 0, int s2 = 0 ) {
  ...
  FILE *tmp;
  if( --it == 0 ) {
    printf( "DIRECT_ENCODE_BLOCK_START: Type: DEFAULT Len: %d S1: %d S2: %d\n", filesize, s1, s2 );
    direct_encode_block( DEFAULT, in, filesize, -1, en, s1, s2 );
    printf( "DIRECT_ENCODE_BLOCK_END: Type: DEFAULT Len: %d S1: %d S2: %d\n", filesize, s1, s2 );
    return;
  }
  s2 += n;
  // Transform and test in blocks
  while( n > 0 ) {
    printf( "DETECT_START: Type: %d Len: %d\n", type, n );
    Filetype nextType = detect( in, n, type, info );
    printf( "DETECT_END: Type: %d Len: %d\n", type, n );
    long end = ftell( in );
    ...
    if( len > 0 ) {
      s2 -= len;
      printf( "%3d-%d | %12s | %d bytes [%ld - %ld]", blnum++, MAX_ITER - it - 1, typenames[type], len, begin,
              end - 1 );
      if( type == AUDIO )
      ...
      printf( "\n" );
      if( type == EXE || type == CD ) {
        printf( "(create tempfile start)\n" );
        tmp = tmpfile(); // temporary encoded file
        printf( "(create tempfile end)\n" );

        if( tmp == nullptr )
          perror( "tmpfile" ), quit();
        fprintf( tmp, "%c%c%c%c%c", type, len >> 24, len >> 16, len >> 8, len );
        if( type == EXE ) {
          printf( "ENCODE_EXE_START: Len: %d Begin: %ld\n", len, begin );
          encode_exe( in, tmp, len, begin );
          printf( "ENCODE_EXE_END: Len: %d Begin: %ld\n", len, begin );
        } else if( type == CD ) {
          printf( "ENCODE_CD_START: Len: %d Info: %d\n", len, info );
          encode_cd( in, tmp, len, info );
          printf( "ENCODE_CD_END: Len: %d Info: %d\n", len, info );
        }
        ...
        int c1 = 0;
        int c2 = 0;
        printf( "(test/decode start)\n" );
        for( j = 0; j < len; ++j )
          if( ( c1 = decode( en, 1 ) ) != ( c2 = getc( in ) ) )
            break;
        printf( "(test/decode end)\n" );
        ...
        if( j != len || getc( tmp ) != EOF ) {
          ...
          if( type == CD ) {
            printf( "ITER_TRANSFORM_START: Size: %ld S1: %d S2: %d\n", tmpsize, s1, s2 );
            iter_transform( tmp, tmpsize, en, it, s1, s2 );
            printf( "ITER_TRANSFORM_END: Size: %ld S1: %d S2: %d\n", tmpsize, s1, s2 );
          } else {
            printf( "DIRECT_ENCODE_BLOCK_START: Type: %d Len: %ld S1: %d S2: %d\n", type, -tmpsize, s1, s2 );
            direct_encode_block( type, tmp, -tmpsize, -1, en, s1, s2 );
            printf( "DIRECT_ENCODE_BLOCK_END: Type: %d Len: %ld S1: %d S2: %d\n", type, -tmpsize, s1, s2 );
          }
        }

        printf( "(delete tempfile start)\n" );
        fclose( tmp ); // deletes
        printf( "(delete tempfile end)\n" );
      } else {
        const int i1 = ( type == IMAGE1 || type == IMAGE8 || type == IMAGE24 || type == AUDIO ) ? info : -1;
        printf( "DIRECT_ENCODE_BLOCK_START: Type: %d Len: %d S1: %d S2: %d\n", type, len, s1, s2 );
        direct_encode_block( type, in, len, i1, en, s1, s2 );
        printf( "DIRECT_ENCODE_BLOCK_END: Type: %d Len: %d S1: %d S2: %d\n", type, len, s1, s2 );
      }
      s1 += len;
    }
