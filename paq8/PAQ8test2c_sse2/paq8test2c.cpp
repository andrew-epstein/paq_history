...
int decode_cd( Encoder &en, int size, int it ) {
  ...
  if( state == 0 && size2 >= BLOCK - 1 ) {
    int a;
    if( size2 == size - 1 ) {
      for( int i = 12; i < 16 + 4 * ( blk[15] != 1 ); i++ )
        blk[i] = decode( en, it );
      cdfo = 1 + ( blk[15] == 3 );
      a = -1;
    } else
      a = ( blk[12] << 16 ) + ( blk[13] << 8 ) + blk[14];
    for( int i = 0; i < 2048 + 276 * ( cdfo == 2 ); i++ )
      blk[16 + i + ( blk[15] != 1 ) * 8] = decode( en, it );
    if( cdfo == 2 )
      blk[15] = 3;
  ...
  while( n > 0 ) {
    ...
    if( len > 0 ) {
      ...
      if( type == EXE || type == CD ) {
        ...
        if( type == EXE ) {
          printf( "ENCODE_EXE_START: Len: %d Begin: %d\n", len, begin );
          encode_exe( in, tmp, len, begin );
          printf( "ENCODE_EXE_END: Len: %d Begin: %d\n", len, begin );
        } else if( type == CD ) {
        ...
        if( j != len || getc( tmp ) != EOF ) {
          ...
          if( type == CD ) {
            printf( "ITER_TRANSFORM_START: Size: %d S1: %d S2: %d\n", tmpsize, s1, s2 );
            iter_transform( tmp, tmpsize, en, it, s1, s2 );
            printf( "ITER_TRANSFORM_END: Size: %d S1: %d S2: %d\n", tmpsize, s1, s2 );
          } else {
