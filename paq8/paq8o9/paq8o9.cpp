/* paq8o9 file compressor/archiver.  Release by KZ, Feb. 16, 2008
...
- To install, put paq8o9.exe or a shortcut to it on your desktop.
- To compress a file or folder, drop it on the paq8o9 icon.
- To decompress, drop a .paq8o9 file on the icon.

A .paq8o9 extension is added for compression, removed for decompression.
The output will go in the same folder as the input.

While paq8o9 is working, a command window will appear and report
...
- To install, put paq8o9.exe somewhere in your PATH.
- To compress:      paq8o9 [-N] file1 [file2...]
- To decompress:    paq8o9 [-d] file1.paq8o9 [dir2]
- To view contents: more < file1.paq8o9

The compressed output file is named by adding ".paq8o9" extension to
the first named file (file1.paq8o9).  Each file that exists will be
...
If file1.paq8o9 exists then it is overwritten.

If the first named file ends in ".paq8o9" then it is assumed to be
...
The -d option forces extraction even if there is not a ".paq8o9"
...
During extraction, directories are created as needed.  For example:
  ...
  paq8o9 -4 c:\tmp\foo bar

compresses foo and bar (if they exist) to c:\tmp\foo.paq8o9 at level 4.

  paq8o9 -d c:\tmp\foo.paq8o9 .
...
There are 2 files: paq8o9.cpp (C++) and paq7asm.asm (NASM/YASM).
paq7asm.asm is the same as in paq7 and paq8x.  paq8o9.cpp recognizes the
...
Recommended compiler commands and optimizations:
  ...
  MINGW g++:
    ...
    g++ paq8o9.cpp -DWINDOWS -O2 -Os -s -march=pentiumpro -fomit-frame-pointer -o paq8o9.exe paq7asm.obj

  Borland:
    nasm paq7asm.asm -f obj --prefix _
    bcc32 -DWINDOWS -O -w-8027 paq8o9.cpp paq7asm.obj

  Mars:
    nasm paq7asm.asm -f obj --prefix _
    dmc -DWINDOWS -Ae -O paq8o9.cpp paq7asm.obj

  UNIX/Linux (PC):
    nasm -f elf paq7asm.asm
    g++ paq8o9.cpp -DUNIX -O2 -Os -s -march=pentiumpro -fomit-frame-pointer -o paq8o9 paq7asm.o

  Non PC (e.g. PowerPC under MacOS X)
    g++ paq8o9.cpp -O2 -DUNIX -DNOASM -s -o paq8o9
...
Then
  ...
  paq8o9 archive \dir1\file1.txt \dir2

will create archive.paq8o9 with the header:
...
Decompression will fail if the first 7 bytes are not "paq8o9 -".  Sizes
...
paq8o9 uses preprocessing transforms on certain data types to improve
...
#define PROGNAME "paq8o9" // Please change this if you change the program.
...
typedef enum { DEFAULT, JPEG, BMPFILE4, BMPFILE8, BMPFILE24, TIFFFILE, PGMFILE, RGBFILE, EXE, TEXT } Filetype;
...
Filetype detect( FILE *in, int n, Filetype type ) {
  ...
  for( int i = 0; i < n; ++i ) {
    ...
    if( bmp != 0 ) {
      ...
      if( ( type == BMPFILE4 || type == BMPFILE8 || type == BMPFILE24 )
          && ( imgbpp == 4 || imgbpp == 8 || imgbpp == 24 ) && imgcomp == 0 ) {
        int tbsize = 0;
        if( imgbpp == 4 )
          if( bsize != ( tbsize = ( ( bmpx * bmpy >> 1 ) + bmpimgoff ) ) )
            bsize = tbsize;
        if( imgbpp == 8 )
...
// To compress to file1.paq8o9: paq8o9 [-n] file1 [file2...]
// To decompress: paq8o9 file1.paq8o9 [output_dir]
