...
void XWRT_Common::initializeLetterSet() {
  ...
  for( c = 0; c < 256; c++ ) //                                                - _ . , :
    if( c > 127 || letterSet[c] == LOWERCHAR || letterSet[c] == UPPERCHAR || c == ' ' /*|| c=='\''*/ ) // || c=='&')
      ...
      wordSet[c] = 0;
  for( c = 0; c < 256; c++ )
    if( c == '&' || c == '#' || c == ';' || c >= 128 || letterSet[c] == LOWERCHAR || letterSet[c] == UPPERCHAR
        || letterSet[c] == NUMBERCHAR || c == '.' || c == '-' || c == '_' || c == ':' )
      startTagSet[c] = 1;
    else
      startTagSet[c] = 0;
...
void XWRT_Common::initializeCodeWords( int word_count, bool initMem ) {
...
#endif
        || ( IF_OPTION( OPTION_SPACELESS_WORDS ) && c == CHAR_NOSPACE ) || c == CHAR_HOURMINSEC || c == CHAR_HOURMIN
        || c == CHAR_IP || c == CHAR_PAGES || c == CHAR_DATE_ENG || c == CHAR_TIME || c == CHAR_REMAIN ) {
      reservedSet[c] = 1;
      addSymbols[c] = 0;
    }
  }
  ...
  PRINT_DICT( ( "codewordType=%d %d %d %d %d(%d) charsUsed=%d sizeDict=%d\n", codewordType, dict1size, dict2size,
                dict3size, dict4size, dictionary, charsUsed, sizeDict ) );
}
...
void XWRT_Common::WRT_print_options() {
  if( IF_OPTION( OPTION_UNICODE_LE ) || IF_OPTION( OPTION_UNICODE_BE ) )
    PRINT_DICT( ( "UNICODE " ) );
  if( IF_OPTION( OPTION_BINARY_DATA ) )
    PRINT_DICT( ( "BINARY_DATA " ) );
  //if (IF_OPTION(OPTION_USE_DICTIONARY)) PRINT_DICT(("USE_DICTIONARY "));
  if( IF_OPTION( OPTION_SPACELESS_WORDS ) )
    PRINT_DICT( ( "SPACELESS " ) );
  if( IF_OPTION( OPTION_SPACES_MODELING ) )
    PRINT_DICT( ( "SPACES_MODELING " ) );
  if( IF_OPTION( OPTION_TRY_SHORTER_WORD ) )
    PRINT_DICT( ( "TRY_SHORTER " ) );
  //if (IF_OPTION(OPTION_NUMBER_CONTAINER)) PRINT_DICT(("NUMBER_CONTAINER "));
  if( IF_OPTION( OPTION_ADD_SYMBOLS_14_31 ) )
    PRINT_DICT( ( "ADD_SYMBOLS_14_31 " ) );
  if( IF_OPTION( OPTION_ADD_SYMBOLS_0_5 ) )
    PRINT_DICT( ( "ADD_SYMBOLS_0_5 " ) );
  if( IF_OPTION( OPTION_SPACELESS_WORDS ) )
    PRINT_DICT( ( "SPACELESS_WORDS " ) );
