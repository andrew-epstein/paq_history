    ...
    if( c >= 0 ) {                                                                                                     \
      ...
      if( IF_OPTION( OPTION_UNICODE_BE ) ) {                                                                           \
        ...
        if( d < 0 )                                                                                                    \
          d = 65536;                                                                                                   \
        c = 256 * c + d;                                                                                               \
      } else if( IF_OPTION( OPTION_UNICODE_LE ) ) {                                                                    \
        READ_CHAR( d );                                                                                                \
        if( d < 0 )                                                                                                    \
          d = 256;                                                                                                     \
        c += 256 * d;                                                                                                  \
  ...
  if( i >= 0 ) {
    if( IF_OPTION( OPTION_SPACELESS_WORDS ) ) {
      if( ( s[0] >= 'a' && s[0] <= 'z' ) || ( s[0] >= 'A' && s[0] <= 'Z' ) ) {
        if( ( beforeWord == '/' || beforeWord == '-' || beforeWord == '\"' || beforeWord == '_'
              || beforeWord == '>' ) ) {
          if( spaces > 0 )
...
void XWRT_Encoder::WRT_encode( size_t bufferSize ) {
  ...
  while( true ) {
    ...
    if( detect ) {
      ...
      if( fftell == BYTES_TO_DETECT ) {
        ...
        else if( unicode_be * 4 / 3 > fftell / 2 )
      ...
      if( wordType == NUMBER ) {
        if( c == ':' && s_size <= 2 ) {
          ...
          if( c == '.' && lsize < 4 && s_size - lsize >= 2 && s_size - lsize <= 4 ) {
            ...
            s[s_size++] = c;
            ENCODE_GETC( c );
            if( c == EOF || letterSet[c] != NUMBERCHAR || c == '0' || s[0] == '0' || c2 == '0' || x1 > 255
                || x2 > 255 ) {
              encodeMixed( s, s_size, c );
              s_size = 0;
              wordType = LOWERWORD;
          ...
          if( s_size - lsize == 3 ) {
            ...
            ENCODE_PUTC( remain );
          } else if( lsize == 1 && s_size == 3 ) {
            ENCODE_PUTC( CHAR_REMAIN );
            int remain = ( s[0] - '0' ) * 10 + ( s[2] - '0' );

            ENCODE_PUTC( remain );
          } else if( lsize == 2 && s_size == 4 && ( s[0] == '1' || ( s[0] == '2' && s[1] <= '4' ) ) ) {
            ENCODE_PUTC( CHAR_REMAIN );
  ...
  if( detect && !IF_OPTION( OPTION_UNICODE_LE ) && !IF_OPTION( OPTION_UNICODE_BE ) ) {
    ...
    PRINT_DICT( ( "unicode_le=%d unicode_be=%d uni=%d\n", unicode_le, unicode_be,
                  IF_OPTION( OPTION_UNICODE_LE ) || IF_OPTION( OPTION_UNICODE_BE ) ) );
  }
...
void XWRT_Encoder::write_dict( int comprLevel ) {
  ...
  for( i = 0; i < count; i++ ) {
    cmn = 0;
    if( i > 0 )
      cmn = common( sortedDict[i - 1].c_str(), sortedDict[i].c_str(),
                    min( sortedDict[i].size(), sortedDict[i - 1].size() ) );
...
void XWRT_Encoder::WRT_start_encoding( FILE *in, FILE *out, unsigned int fileLen, bool type_detected ) {
  ...
  if( !IF_OPTION( OPTION_PAQ ) ) {
    PUTC( 1 * detectedSymbols[0] + 2 * detectedSymbols[1] + 4 * detectedSymbols[2] + 8 * detectedSymbols[3]
          + 16 * detectedSymbols[4] + 32 * detectedSymbols[5] + 64 * detectedSymbols[6] + 128 * detectedSymbols[7] );
    PUTC( 1 * detectedSymbols[8] + 2 * detectedSymbols[9] + 4 * detectedSymbols[10] + 8 * detectedSymbols[11]
          + 16 * detectedSymbols[12] + 32 * detectedSymbols[13] + 64 * detectedSymbols[14]
          + 128 * detectedSymbols[15] );
    PUTC( 1 * detectedSymbols[16] + 2 * detectedSymbols[17] + 4 * detectedSymbols[18] + 8 * detectedSymbols[19]
          + 16 * detectedSymbols[20] + 32 * detectedSymbols[21] + 64 * detectedSymbols[22]
          + 128 * detectedSymbols[23] );
  } else {
  }
...
int XWRT_Encoder::WRT_detectFileType() {
  ...
  if( dictmem && dict && dictlen && dictfreq ) {
    ...
    PRINT_DICT(
        ( "firstPassBlock=%d fileLenMB=%d getcBufferDataParts=%d\n", firstPassBlock, fileLenMB, getcBufferDataParts ) );
#ifdef DYNAMIC_DICTIONARY
