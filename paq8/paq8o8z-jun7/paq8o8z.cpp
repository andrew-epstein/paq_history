...
int original_cpu_flags=NONE_YET, cpu_flags=NONE_YET,
    fxsave_it = 0, dual_speedups = 0;
...
Filetype detect(FILE* in, int n, Filetype type) {
  ...
  for (int i=0; i<n; ++i) {
    ...
    if (soi && app==i && ((buf0&0xfff00000)==0xffe00000 || (buf0&0xffff0000)==0xfffe0000))
      app=i+(buf0&0xffff)+2;    
    if (app<i && soi && i-soi<0x10000 && (buf1&0xff)==0xff
	...
	if (bmp && (i-bmp)==16 && buf0==0x28000000)	bmp0=1; //if windows bmp
	if (bmp && (i-bmp)==4){
	...
	if (pgm && !pgmcomment) pgm_buf[pgm_ptr++]=c;
	if (pgm_ptr>=32) pgm=pgm_ptr=pgmw=pgmh=pgmc=pgmcomment=0;
	if (pgm && pgmcomment && c==0x0a) pgmcomment=0;
...
// (i.e. for me to pretend my P4 cpu supports MMX but doesn't support SSE2)
     ...
     original_cpu_flags = cpu_flags = MMX;
       ...
       PAQ8=getenv("PAQ8"); FXSAVE=getenv("FXSAVE");
       ...
       if (PAQ8 != NULL) {
         ...
         if (equals(PAQ8,"NOASM")) cpu_flags = NO_ASM;
         else if (equals(PAQ8,"MMX")) cpu_flags = MMX;
         else if (equals(PAQ8,"SSE")) cpu_flags = SSE2;
         else if (equals(PAQ8,"SSE2")) cpu_flags = SSE2;
         else if (equals(PAQ8,"BOTH")) dual_speedups++;   // helps ???
       }
