...
Filetype detect( FILE *in, int n, Filetype type ) {
  ...
  for( int i = 0; i < n; ++i ) {
    ...
    if( !soi && i >= 3 && ( buf0 & 0xfffffff0 ) == 0xffd8ffe0 )
      soi = i, app = i + 2;
    if( soi && app == i && ( ( buf0 & 0xfff00000 ) == 0xffe00000 || ( buf0 & 0xffff0000 ) == 0xfffe0000 ) )
      app = i + ( buf0 & 0xffff ) + 2;
    if( app < i && soi && i - soi < 0x10000 && ( buf1 & 0xff ) == 0xff && ( buf0 & 0xff0000ff ) == 0xc0000008 )
      sof = i;
    ...
    if( ( buf0 & 0xFFFF ) == 16973 )
      bmp = i; //possible 'BM'
    if( bmp && ( i - bmp ) == 16 && buf0 == 0x28000000 )
    ...
    if( pgm && !pgmcomment && c == 0x0a && !pgmh && pgm_ptr ) {
      pgm_buf[pgm_ptr++] = 0;
      pgmh = atoi( pgm_buf );
      pgm_ptr = 0;
    }
    if( pgm && !pgmcomment && c == 0x0a && !pgmc && pgm_ptr ) {
      pgm_buf[pgm_ptr++] = 0;
      pgmc = atoi( pgm_buf );
      pgm_ptr = 0;
    }
    ...
    if( pgm && pgmcomment && c == 0x0a )
      pgmcomment = 0;
    if( type == PGMFILE && pgm && pgmw && pgmh && pgmc ) {
      pgmsize = pgmw * pgmh + pgm + i - 1;
      return fseek( in, start + pgmsize, SEEK_SET ), DEFAULT;
    }
    if( pgm && pgmw && pgmh && pgmc ) {
...
// (i.e. for me to pretend my P4 cpu supports MMX but doesn't support SSE2)
  ...
  if( PAQ8 != NULL ) {
    ...
    if( equals( PAQ8, "NOASM" ) )
      cpu_flags = NO_ASM;
    else if( equals( PAQ8, "MMX" ) )
      cpu_flags = MMX;
    else if( equals( PAQ8, "SSE" ) )
      cpu_flags = SSE2;
    else if( equals( PAQ8, "SSE2" ) )
      cpu_flags = SSE2;
    else if( equals( PAQ8, "BOTH" ) )
      dual_speedups++; // helps ???
  }

  if( ( PAQ8 != NULL ) && ( equals( PAQ8, "BOTH" ) ) )
