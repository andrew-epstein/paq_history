/* paq8o8z file compressor/archiver.  Updated by Rugxulo.

   http://www.cs.fit.edu/~mmahoney/compression/

    Copyright (C) 2008 Matt Mahoney, Serge Osnach, Alexander Ratushnyak,
    Bill Pettis, Przemyslaw Skibinski, Matthew Fite, wowtiger, Andrew Paterson,
    Jan Ondrus, Andreas Morphis, Pavel L. Holoborodko, KZ, Rugxulo, et al.
...
- To install, put paq8o8z.exe or a shortcut to it on your desktop.
- To compress a file or folder, drop it on the paq8o8z icon.
- To decompress, drop a .paq8o8z file on the icon.

A .paq8o8z extension is added for compression, removed for decompression.
The output will go in the same folder as the input.

While paq8o8z is working, a command window will appear and report
...
- To install, put paq8o8z.exe somewhere in your PATH.
- To compress:      paq8o8z [-N] file1 [file2...]
- To decompress:    paq8o8z [-d] file1.paq8o8z [dir2]
- To view contents: more < file1.paq8o8z   (doesn't work in Vista or DOSBox)

The compressed output file is named by adding ".paq8o8z" extension to
the first named file (file1.paq8o8z).  Each file that exists will be
...
If file1.paq8o8z exists then it is overwritten.

If the first named file ends in ".paq8o8z" then it is assumed to be
...
The -d option forces extraction even if there is not a ".paq8o8z"
...
During extraction, directories are created as needed.  For example:
  ...
  paq8o8z -4 c:\tmp\foo bar

compresses foo and bar (if they exist) to c:\tmp\foo.paq8o8z at level 4.

  paq8o8z -d c:\tmp\foo.paq8o8z .
...
There are 2 files: paq8o8z.cpp (C++) and paq7asm.asm (NASM/YASM).
paq7asm.asm is the same as in paq7 and paq8x.  paq8o8z.cpp recognizes the
following compiler options:
  ...
  -DDEFAULT_OPTION=N  (to change the default compression level from 1 to N).
...
-DDEFAULT_OPTION=4 for 128 MB, 3 for 64 MB, 2 for 37 MB, etc.
...
Recommended compiler commands and optimizations:
  ...
  MINGW g++:
    ...
    g++ paq8o8z.cpp -DWINDOWS -O2 -Os -s -march=pentiumpro -fomit-frame-pointer -o paq8o8z.exe paq7asm.obj

  Borland:
    nasm paq7asm.asm -f obj --prefix _
    bcc32 -DWINDOWS -O -w-8027 paq8o8z.cpp paq7asm.obj

  Mars:
    nasm paq7asm.asm -f obj --prefix _
    dmc -DWINDOWS -Ae -O paq8o8z.cpp paq7asm.obj

  UNIX/Linux (PC):
    nasm -f elf paq7asm.asm
    g++ paq8o8z.cpp -DUNIX -O2 -Os -s -march=pentiumpro -fomit-frame-pointer -o paq8o8z paq7asm.o

  Non PC (e.g. PowerPC under MacOS X)
    g++ paq8o8z.cpp -O2 -DUNIX -DNOASM -s -o paq8o8
...
Then
  ...
  paq8o8z archive \dir1\file1.txt \dir2

will create archive.paq8o8z with the header:
...
The command:
  ...
  paq8o8z archive.paq8o8z C:\dir3
...
Decompression will fail if the first 9 bytes are not "paq8o8z -".  Sizes
...
paq8o8z uses a neural network to combine a large number of models.  The
...
paq8o8z uses preprocessing transforms on certain data types to improve
...
and 1/3 faster overall.  (However I found that SSE 2 code on an AMD-64,
...
**********************************************************************
Dec 29, 2007 - June 7, 2008
  tweaked by Rugxulo (paq8o8z for DJGPP & OpenWatcom using CPUID)
**********************************************************************
...
void enable_sse2( void );
int dot_product_mmx( short *t, short *w, int n );
int dot_product_sse2( short *t, short *w, int n );
void train_mmx( short *t, short *w, int n, int err );
void train_sse2( short *t, short *w, int n, int err );
}
#endif
...
#  define DEFAULT_OPTION 1 // 37 MB of RAM needed (minimum)
#endif                     // (try LPAQ if you need to use less RAM)

#ifdef __DJGPP__
#  define COMPILER "DJGPP v2"
#endif

#ifdef __WATCOMC__
#  undef WINDOWS // not sure this is correct or not
#  define COMPILER "OpenWatcom"
#  ifdef __NT__
#    define OS_TARGET "Win32"
#  endif
#endif

#if defined( __DOS__ ) || defined( MSDOS )
#  define OS_TARGET "FreeDOS"
#endif

#ifndef OS_TARGET
#  define OS_TARGET "whatever"
#endif

#ifndef COMPILER
#  define COMPILER "something"
#endif
...
enum { NONE_YET = -1, NO_ASM, MMX, SSE2 };
...
#if defined( TINY ) && defined( __DJGPP__ ) && !defined( __STRICT_ANSI__ )
#  include <crt0.h>

#  define UNUSED __attribute__( ( unused ) )

char **__crt0_glob_function( char *arg UNUSED ) {
  return 0;
}
void __crt0_load_environment_file( char *progname UNUSED ) {}
#endif

extern "C" int fxsave_it;

int original_cpu_flags = NONE_YET, cpu_flags = NONE_YET, fxsave_it = 0, dual_speedups = 0;

// **********************************************************************
void mention( const char *a1 ) {
#if !defined( NOINFO ) && !defined( NO_INFO )
  printf( "%s", a1 );
#endif
}

void mention( const char *s1, const char *s2 ) {
#if !defined( NOINFO ) && !defined( NO_INFO )
  printf( s1, s2 );
#endif
}
...
int is_MMX_or_SSE2() {
  ...
  if( is_CPUID_supported() != 0 ) {
    mention( "yes\n" );
    mention( "Testing CPUID feature flags for MMX or SSE2 ... " );
    result = get_cpu_flags();
    switch( result ) {
      case NO_ASM:
        mention( "no no\n" );
        break;
...
int dot_product_noasm( short *t, short *w, int n ) {
  int sum = 0;
  n = ( n + 7 ) & -8;
  for( int i = 0; i < n; i += 2 )
...
void train_noasm( short *t, short *w, int n, int err ) {
  n = ( n + 7 ) & -8;
  for( int i = 0; i < n; ++i ) {
...
void mention_speedup( char ch ) {
#if !defined( NOASM ) && !defined( NOINFO ) && !defined( NO_INFO )
#  if defined( SHOWME ) || defined( SHOW_ME )
  switch( cpu_flags ) {
    case SSE2:
      printf( " SSE2  (%c%d) \b\b\b\b\b\b\b\b\b\b\b\b", ch, cpu_flags );
      break;
    case MMX:
      printf( " MMX   (%c%d) \b\b\b\b\b\b\b\b\b\b\b\b", ch, cpu_flags );
      break;
    default:
      printf( " NOASM (%c%d) \b\b\b\b\b\b\b\b\b\b\b\b", ch, cpu_flags );
      break;
  }
#  endif
#endif
...
int dot_product( short *t, short *w, int n ) {
  ...
  switch( cpu_flags ) {
    case MMX:
      return dot_product_mmx( t, w, n );
    case SSE2:
      return dot_product_sse2( t, w, n );
    default:
      return dot_product_noasm( t, w, n );
  }
...
void train( short *t, short *w, int n, int err ) {
  ...
  switch( cpu_flags ) {
    case MMX:
      ...
      break;
    case SSE2:
      train_sse2( t, w, n, err );
      break;
    default:
      train_noasm( t, w, n, err );
      break;
  }
...
typedef enum { DEFAULT, JPEG, BMPFILE4, BMPFILE8, BMPFILE24, TIFFFILE, PGMFILE, EXE, TEXT } Filetype;
...
Filetype detect( FILE *in, int n, Filetype type ) {
  ...
  // For BMP detection
  int bmp = 0, bmp0 = 0, bsize = 0, imgbpp = 0;
  // For PGM detection
  int pgm = 0, pgmcomment = 0, pgmw = 0, pgmh = 0, pgmsize = 0, pgm_ptr = 0, pgmc = 0;
  char pgm_buf[32];
  // For JPEG detection
  ...
  for( int i = 0; i < n; ++i ) {
    ...
    if( (soi == 0) && i >= 3 && ( buf0 & 0xfffffff0 ) == 0xffd8ffe0 )
      soi = i, app = i + 2;
    if( (soi != 0) && app == i && ( ( buf0 & 0xfff00000 ) == 0xffe00000 || ( buf0 & 0xffff0000 ) == 0xfffe0000 ) )
      app = i + ( buf0 & 0xffff ) + 2;
    if( app < i && (soi != 0) && i - soi < 0x10000 && ( buf1 & 0xff ) == 0xff && ( buf0 & 0xff0000ff ) == 0xc0000008 )
      sof = i;
    if( (soi != 0) && (sof != 0) && sof > soi && i - soi < 0x10000 && i - sof < 0x1000 && ( buf0 & 0xffff ) == 0xffda ) {
      sos = i;
      if( type != JPEG )
    ...
    if( ( buf0 & 0xFFFF ) == 16973 )
      bmp = i; //possible 'BM'
    if( (bmp != 0) && ( i - bmp ) == 16 && buf0 == 0x28000000 )
      bmp0 = 1; //if windows bmp
    if( (bmp != 0) && ( i - bmp ) == 4 ) {
      bsize = bswap( buf0 ); //image size
    }
    if( (bmp != 0) && ( i - bmp ) == ( 27 ) )
      imgbpp = c; //image bpp
    if( ( type == BMPFILE4 || type == BMPFILE8 || type == BMPFILE24 ) && (bmp0 != 0) && buf0 == 0
        && ( imgbpp == 4 || imgbpp == 8 || imgbpp == 24 ) ) {
      return fseek( in, start + bsize, SEEK_SET ), DEFAULT;
    }
    if( (bmp0 != 0) && buf0 == 0 && imgbpp == 4 ) {
      return fseek( in, start + bmp - 1, SEEK_SET ), BMPFILE4;
    }
    if( (bmp0 != 0) && buf0 == 0 && imgbpp == 8 ) {
      return fseek( in, start + bmp - 1, SEEK_SET ), BMPFILE8;
    }
    if( (bmp0 != 0) && buf0 == 0 && imgbpp == 24 ) {
      return fseek( in, start + bmp - 1, SEEK_SET ), BMPFILE24;
    }
    // Detect .pgm image
    if( ( buf0 & 0xFFFFFF ) == 0x50350A )
      pgm = i; //possible 'P5 '
    if( (pgm != 0) && ( i - pgm ) == 1 && c == 0x23 )
      pgmcomment = 1; //pgm comment
    //not tested without comment
    if( (pgm != 0) && (pgmcomment == 0) && c == 0x20 && (pgmw == 0) && (pgm_ptr != 0) ) {
      pgm_buf[pgm_ptr++] = 0;
      pgmw = atoi( pgm_buf );
      pgm_ptr = 0;
    }
    if( (pgm != 0) && (pgmcomment == 0) && c == 0x0a && (pgmh == 0) && (pgm_ptr != 0) ) {
      pgm_buf[pgm_ptr++] = 0;
      pgmh = atoi( pgm_buf );
      pgm_ptr = 0;
    }
    if( (pgm != 0) && (pgmcomment == 0) && c == 0x0a && (pgmc == 0) && (pgm_ptr != 0) ) {
      pgm_buf[pgm_ptr++] = 0;
      pgmc = atoi( pgm_buf );
      pgm_ptr = 0;
    }
    if( (pgm != 0) && (pgmcomment == 0) )
      pgm_buf[pgm_ptr++] = c;
    if( pgm_ptr >= 32 )
      pgm = pgm_ptr = pgmw = pgmh = pgmc = pgmcomment = 0;
    if( (pgm != 0) && (pgmcomment != 0) && c == 0x0a )
      pgmcomment = 0;
    if( type == PGMFILE && (pgm != 0) && (pgmw != 0) && (pgmh != 0) && (pgmc != 0) ) {
      pgmsize = pgmw * pgmh + pgm + i - 1;
      return fseek( in, start + pgmsize, SEEK_SET ), DEFAULT;
    }
    if( (pgm != 0) && (pgmw != 0) && (pgmh != 0) && (pgmc != 0) ) {
...
// To compress to file1.paq8o8z: paq8o8z [-n] file1 [file2...]
// To decompress: paq8o8z file1.paq8o8z [output_dir]
int main( int argc, char **argv ) {
  // ---------------------------------------------------------------------------

#if !defined( NOINFO ) && !defined( NO_INFO )
  printf( "\n%s compiled by %s for %s\n", PROGNAME, COMPILER, OS_TARGET );
#endif

#ifndef NOASM
  original_cpu_flags = cpu_flags = is_MMX_or_SSE2();
#else
  original_cpu_flags = cpu_flags = NO_ASM;
#endif

// the following line is only useful to test "dual_speedups" w/ MMX/NOASM
// (i.e. for me to pretend my P4 cpu supports MMX but doesn't support SSE2)
#ifdef MMX_ONLY
  original_cpu_flags = cpu_flags = MMX;
#endif
  ...
  PAQ8 = getenv( "PAQ8" );
  FXSAVE = getenv( "FXSAVE" );

  if( FXSAVE != NULL ) {
    fxsave_it = 1;
    mention( "\nFXSAVE/FXRSTOR is on.\n" );
  }

  if( PAQ8 != NULL ) {
    mention( "\n%%PAQ8%%=`%s'\n", PAQ8 );

    if( equals( PAQ8, "NOASM" ) != 0 )
      cpu_flags = NO_ASM;
    else if( equals( PAQ8, "MMX" ) != 0 )
      cpu_flags = MMX;
    else if( equals( PAQ8, "SSE" ) != 0 )
      cpu_flags = SSE2;
    else if( equals( PAQ8, "SSE2" ) != 0 )
      cpu_flags = SSE2;
    else if( equals( PAQ8, "BOTH" ) != 0 )
      dual_speedups++; // helps ???
  }

  if( ( PAQ8 != NULL ) && (( equals( PAQ8, "BOTH" ) ) != 0) )
    printf( "Using %s\n\n", ( ( original_cpu_flags > MMX ) ? "SSE2/MMX" : "MMX/NOASM" ) );
  else {
    mention( "Using " );
    ...
    switch( cpu_flags ) {
      case SSE2:
        printf( "SSE2\n\n" );
#  ifdef __DJGPP__
        if( _get_dos_version( 1 ) != 0x532 ) // avoid on Win2k, XP, etc.
          enable_sse2();
#  endif
        break;
      case MMX:
  ...
  try {
    ...
    // Print help message
    if( argc < 2 ) {
      printf( "Free under GPL, http://www.gnu.org/licenses/gpl.txt\n\n"
#ifdef WINDOWS
              ...
              "  " PROGNAME " file                      (default level -%d, pause when done)\n"
              "level: -0 = store, -1 -2 -3 = faster (uses 37, 48, 59 MB)\n"
              ...
              // *** THIS DOESN'T WORK IN VISTA or DOSBox ***
              //     (ignores Ctrl-Z ??)
              //
    ...
    if( mode == COMPRESS ) {
      ...
      printf( "Creating archive %s via level %d with %d file(s)...\n", archiveName.c_str(), level, files );
