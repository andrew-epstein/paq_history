...
class Mixer {
  ...
  Array<short, 16> wx2; // N*M weights
...
public:
  ...
  void update() {
    for( int i = 0; i < ncxt; ++i ) {
      ...
      if( err ) {
        train( &tx[0], &wx[cxt[i] * N], nx, err * 1.77 );
        train( &tx[0], &wx2[cxt[i] * N], nx, err * 7.99 );
      }
    }
    nx = base = ncxt = 0;
  }
  ...
  int p() {
    ...
    if( mp ) {      // combine outputs
      ...
      for( int i = 0; i < ncxt; ++i ) {
        pr[i] =
            squash( dot_product( &tx[0], &wx[cxt[i] * N], nx ) + dot_product( &tx[0], &wx2[cxt[i] * N], nx ) + 1 >> 6 );
        mp->add( stretch( pr[i] ) );
      }
      mp->set( 0, 1 );
      return mp->p();
