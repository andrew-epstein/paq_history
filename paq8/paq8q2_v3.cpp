/*  paq8q2 file compressor/archiver.  Release by Simon Berger May. 13, 2009
...
- To install, put paq8q2.exe or a shortcut to it on your desktop.
- To compress a file or folder, drop it on the paq8q2 icon.
- To decompress, drop a .paq8q2 file on the icon.

A .paq8q2 extension is added for compression, removed for decompression.
The output will go in the same folder as the input.

While paq8q2 is working, a command window will appear and report
...
- To install, put paq8q2.exe somewhere in your PATH.
- To compress:      paq8q2 [-N] [-mM] file1 [file2...]
- To decompress:    paq8q2 [-d] file1.paq8q2 [dir2]
- To view contents: more < file1.paq8q2

The compressed output file is named by adding ".paq8q2" extension to
the first named file (file1.paq8q2).  Each file that exists will be
...
If file1.paq8q2 exists then it is overwritten.

If the first named file ends in ".paq8q2" then it is assumed to be
...
The -d option forces extraction even if there is not a ".paq8q2"
...
During extraction, directories are created as needed.  For example:
  ...
  paq8q2 -4 c:\tmp\foo bar

compresses foo and bar (if they exist) to c:\tmp\foo.paq8q2 at level 4
and mode -m5.

  paq8q2 -d c:\tmp\foo.paq8q2 .
...
There are 2 files: paq8q2.cpp (C++) and paq7asm.asm (NASM/YASM).
paq7asm.asm is the same as in paq7 and paq8x.  paq8q2.cpp recognizes the
...
Recommended compiler commands and optimizations:
  ...
  MINGW g++:
    ...
    g++ paq8q2.cpp -DWINDOWS -O2 -Os -s -march=pentiumpro -fomit-frame-pointer -o paq8q2.exe paq7asm.obj

  Borland:
    nasm paq7asm.asm -f obj --prefix _
    bcc32 -DWINDOWS -O -w-8027 paq8q2.cpp paq7asm.obj

  Mars:
    nasm paq7asm.asm -f obj --prefix _
    dmc -DWINDOWS -Ae -O paq8q2.cpp paq7asm.obj

  UNIX/Linux (PC):
    nasm -f elf paq7asm.asm
    g++ paq8q2.cpp -DUNIX -O2 -Os -s -march=pentiumpro -fomit-frame-pointer -o paq8q2 paq7asm.o

  Non PC (e.g. PowerPC under MacOS X)
    g++ paq8q2.cpp -O2 -DUNIX -DNOASM -s -o paq8q2
...
Then
  ...
  paq8q2 archive \dir1\file1.txt \dir2

will create archive.paq8q2 with the header:
...
Decompression will fail if the first 6 bytes are not "paq8q2 ".  Sizes
...
paq8q2 uses a neural network to combine a large number of models.  The
...
paq8q2 uses preprocessing transforms on certain data types to improve
...
Added nestModel from paq8q2
Modified wordModel from paq8q2
Modified .pbm, .pgm, .ppm, .bmp, .rgb detection (from paq8q2)
Modified WAV model (from paq8p2)
Modified JPEG model (from paq8p2)
Added im1bitModel (1-bit) (from paq8q2)
Added compression of PPM, PBM images (from paq8q2)
Removed pic model
...
Added text detection (from paq8q2)
Splited sparseMotel to sparseTextModel and sparseModel (from paq8q2)
Model long matches directly after normal model (from paq8q2)
Added -mM (mode/speed) parameter
Added turning on and off of models by suffix
*/

#define PROGNAME "paq8q2" // Please change this if you change the program.
...
void wavModel( Mixer &m, int info ) {
  ...
  if( bpos == 0 ) {
    ...
    const int x1 = ( buf( 1 ) + x3 ) & 255;
    const int x2 = ( buf( 2 ) + x3 ) & 255;
...
typedef enum { DEFAULT, JPEG, HDR, IMAGE1, IMAGE8, IMAGE24, AUDIO, EXE, TEXT } Filetype;
...
int contextModel2() {
  ...
  static const char *typenames[9] = {"", "JPEG ", "HDR ", "1BIT ", "8BIT ", "24BIT ", "WAVE ", "EXE ", "TEXT "};
  ...
  if( bpos == 0 ) {
    ...
    if( blpos == 0 )
      filetype = ft2, printf( "%s(%d bytes) ", typenames[filetype], size );
  }
  ...
  rcm9.mix( m );
  rcm10.mix( m );
  if( ismatch > 160 && cmode != 6 ) { // Model long matches directly, len 1024
    m.set( 0, 8 );
    return m.p();
  }
...
Filetype detect( FILE *in, int n, Filetype type, int &info ) {
  ...
  for( int i = 0; i < n; ++i ) {
    ...
    if( txtStart != 0 ) {
      ...
      else if( ( ( c & 0xe0 ) == 0xc0 ) && txtUTF8b2 == 0 && txtUTF8b3 == 0 )
        txtLen++, txtUTF8b2 = 1;
      else if( ( ( buf0 & 0xE0C0 ) == 0xC080 ) && txtUTF8b2 == 1 )
        txtLen++, txtUTF8b2 = 0, txtIsUTF8 = 1;
      //if possible UTF8 3 byte
      else if( ( ( c & 0xf0 ) == 0xe0 ) && txtUTF8b3 == 0 && txtUTF8b2 == 0 )
        txtLen++, txtUTF8b3 = 1, txtIsUTF8 = 1;
      else if( ( ( buf0 & 0xF0C0 ) == 0xE080 ) && txtUTF8b3 == 1 )
        txtLen++, txtUTF8b3 = 2;
      else if( ( ( buf0 & 0xF0C0C0 ) == 0xE08080 ) && txtUTF8b3 == 2 )
        txtLen++, txtUTF8b3 = 0;
      //if possible UTF8 4 byte
      else if( ( ( c & 0xf8 ) == 0xf0 ) && txtUTF8b4 == 0 && txtUTF8b3 == 0 && txtUTF8b2 == 0 )
        txtLen++, txtUTF8b4 = 1, txtIsUTF8 = 1;
      else if( ( ( buf0 & 0xF8C0 ) == 0xf080 ) && txtUTF8b4 == 1 )
        txtLen++, txtUTF8b4 = 2;
      else if( ( ( buf0 & 0xF8C0C0 ) == 0xf08080 ) && txtUTF8b4 == 2 )
        txtLen++, txtUTF8b4 = 3;
      else if( ( ( buf0 & 0xF8C0C0C0 ) == 0xf0808080 ) && txtUTF8b4 == 3 )
        txtLen++, txtUTF8b4 = 0;
      //if possible UTF8 5 byte
      else if( ( ( c & 0xFC ) == 0xf8 ) && txtUTF8b5 == 0 && txtUTF8b4 == 0 && txtUTF8b3 == 0 && txtUTF8b2 == 0 )
        txtLen++, txtUTF8b5 = 1, txtIsUTF8 = 1;
      else if( ( ( buf0 & 0xfcc0 ) == 0xf880 ) && txtUTF8b5 == 1 )
        txtLen++, txtUTF8b5 = 2;
      else if( ( ( buf0 & 0xfcc0c0 ) == 0xf88080 ) && txtUTF8b5 == 2 )
        txtLen++, txtUTF8b5 = 3;
      else if( ( ( buf0 & 0xfcc0c0c0 ) == 0xf8808080 ) && txtUTF8b5 == 3 )
        txtLen++, txtUTF8b5 = 4;
      else if( ( ( buf0 & 0xc0c0c0c0 ) == 0x80808080 ) && txtUTF8b5 == 4 )
        txtLen++, txtUTF8b5 = 0;
      //if possible UTF8 6 byte
      else if( ( ( c & 0xFE ) == 0xfc ) && txtUTF8b6 == 0 && txtUTF8b5 == 0 && txtUTF8b4 == 0 && txtUTF8b3 == 0
               && txtUTF8b2 == 0 )
        txtLen++, txtUTF8b6 = 1, txtIsUTF8 = 1;
      else if( ( ( buf0 & 0xfec0 ) == 0xfc80 ) && txtUTF8b6 == 1 )
        txtLen++, txtUTF8b6 = 2;
      else if( ( ( buf0 & 0xfec0c0 ) == 0xfc8080 ) && txtUTF8b6 == 2 )
        txtLen++, txtUTF8b6 = 3;
      else if( ( ( buf0 & 0xfec0c0c0 ) == 0xfc808080 ) && txtUTF8b6 == 3 )
        txtLen++, txtUTF8b6 = 4;
      else if( ( ( buf0 & 0xc0c0c0c0 ) == 0x80808080 ) && txtUTF8b6 == 4 )
        txtLen++, txtUTF8b6 = 5;
      else if( ( ( buf0 & 0xc0c0c0c0 ) == 0x80808080 ) && txtUTF8b6 == 5 )
        txtLen++, txtUTF8b6 = 0;
      else if( txtLen < txtMinLen )
        txtStart = txtLen = txtOff = txtUTF8b2 = txtUTF8b3 = txtUTF8b4 = 0, txtIsUTF8 = -1;
      else if( txtLen >= txtMinLen ) {
        if( txtUTF8b2 == 1 )
          --txtLen;
        if( txtUTF8b3 == 1 )
        ...
        if( txtUTF8b6 == 1 )
          ...
          --txtLen, --txtLen, --txtLen, --txtLen, --txtLen;
        if( type == TEXT )
...
// To compress to file1.paq8q2: paq8q2 [-n] file1 [file2...]
// To decompress: paq8q2 file1.paq8q2 [output_dir]
  ...
  try {
    ...
    if( argc < 2 ) {
      printf( PROGNAME " archiver (C) 2008, Matt Mahoney et al.\n"
                       ...
                       "mode: -m1 to -m6 (-m6 = maximum compression; -m1 = fastest; -m5 = Default)\n"
