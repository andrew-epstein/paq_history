/* paq8jb file compressor/archiver.
(C) 2006, Matt Mahoney under GPL, http://www.gnu.org/licenses/gpl.txt
Release by Serge Osnach.

To install and use in Windows:

- To install, put paq8jb.exe or a shortcut to it on your desktop.
- To compress a file or folder, drop it on the paq8jb icon.
- To decompress, drop a .paq8jb file on the icon.

A .paq8jb extension is added for compression, removed for decompression.
The output will go in the same folder as the input.

While paq8jb is working, a command window will appear and report
...
- To install, put paq8jb.exe somewhere in your PATH.
- To compress:      paq8jb [-N] file1 [file2...]
- To decompress:    paq8jb [-d] file1.paq8jb [dir2]
- To view contents: more < file1.paq8jb

The compressed output file is named by adding ".paq8jb" extension to
the first named file (file1.paq8jb).  Each file that exists will be
...
If file1.paq8jb exists then it is overwritten.

If the first named file ends in ".paq8jb" then it is assumed to be
...
The -d option forces extraction even if there is not a ".paq8jb"
...
During extraction, directories are created as needed.  For example:
  ...
  paq8jb -4 c:\tmp\foo bar

compresses foo and bar (if they exist) to c:\tmp\foo.paq8jb at level 4.

  paq8jb -d c:\tmp\foo.paq8jb .
...
There are 2 files: paq8jb.cpp (C++) and paq7asm.asm (NASM/YASM).
paq7asm.asm is the same as in paq7 and paq8x.  paq8jb.cpp recognizes the
...
Recommended compiler commands and optimizations:
  ...
  MINGW g++:
    ...
    g++ paq8jb.cpp -DWINDOWS -O2 -Os -s -march=pentiumpro -fomit-frame-pointer -o paq8jb.exe paq7asm.obj

  Borland:
    nasm paq7asm.asm -f obj --prefix _
    bcc32 -DWINDOWS -O -w-8027 paq8jb.cpp paq7asm.obj

  Mars:
    nasm paq7asm.asm -f obj --prefix _
    dmc -DWINDOWS -Ae -O paq8jb.cpp paq7asm.obj

  UNIX/Linux (PC):
    nasm -f elf paq7asm.asm
    g++ paq8jb.cpp -DUNIX -O2 -Os -s -march=pentiumpro -fomit-frame-pointer -o paq8jb paq7asm.o

  Non PC (e.g. PowerPC under MacOS X)
    g++ paq8jb.cpp -O2 -DUNIX -DNOASM -s -o paq8jb
...
Then
  ...
  paq8jb archive \dir1\file1.txt \dir2

will create archive.paq8jb with the header:
...
Decompression will fail if the first 6 bytes are not "paq8jb -".  Sizes
...
paq8jb uses a neural network to combine a large number of models.  The
...
paq8jb uses preprocessing transforms on certain data types to improve
...
#define PROGNAME "paq8jb" // Please change this if you change the program.
...
// To compress to file1.paq8jb: paq8jb [-n] file1 [file2...]
// To decompress: paq8jb file1.paq8jb [output_dir]
