...
int contextModel2() {
  ...
  static Mixer m(448, 128*(7+14+14+12+14+16), 6, 512);
...
/////  }
  ...
  if (bpos==0) {
    ...
    if(f2=='.'||f2=='O'||f2=='!'||f2==')'||f2==('}'-'{'+'P')) { if (b1!=f2 && buf(3)!=f2 ) i=13, x4=x4*256+f2; }
...
/////    if (fsize==513216) picModel(m);
  ...
  m.tx[430]=0;
		...
		c=(w4&15);
		m.set(order*128 + 16*(min(7,(spacecount+1)/4)) + c, 128*7);
...
void Predictor::update() {
  ...
  if (c0>=256) {
	...
	if(c0=='.' || c0=='O' || c0=='!' || c0==')' || c0==('}'-'{'+'P')) {	w5=(w5<<8)|0x3ff, x5=(x5<<8)+c0, f4=(f4&0xfffffff0)+2;
					if(c0!='!' && c0!='O') w4|=12;
					if(c0!='!') b2='.';
                                }
	f4=f4*16+(c0>>4);
...
int main(int argc, char** argv) {
  ...
  header=ftell(f);
        ...
        FILE *dictfile=fopen("./to_train_models.dic","rb"),
                *tmpfi=fopen("./tmp_paq8hp10tmp.dic","wb");
        if (dictfile==0) perror("to_train_models.dic"), exit(1);
        if (tmpfi   ==0) perror("tmp_paq8hp10tmp.dic"), exit(1);
        filetype=0;
		printf(".");
        Encoder en(COMPRESS, tmpfi);
        en.compress(0);
		printf(".");
        for (int i=0; i<463780; ++i)  en.compress(getc(dictfile));
		printf(".");
        en.flush();
	fclose(tmpfi);
		printf("\n");
	remove("tmp_paq8hp10tmp.dic");
        }
#endif
