...
#ifndef NO_TRAINING
  {
    FILE *dictfile = fopen( "./to_train_models.dic", "rbe" ), *tmpfi = fopen( "./tmp_paq8hp10tmp.dic", "wbe" );
    if( dictfile == 0 )
      perror( "to_train_models.dic" ), exit( 1 );
    if( tmpfi == 0 )
      perror( "tmp_paq8hp10tmp.dic" ), exit( 1 );
    filetype = 0;
    printf( "." );
    Encoder en( COMPRESS, tmpfi );
    en.compress( 0 );
    printf( "." );
    for( int i = 0; i < 463780; ++i )
