...
public:
  ...
  void update() {
    for( int i = 0; i < ncxt; ++i ) {
      ...
      if( err != 0 )
        train( &tx[0], &wx[cxt[i] * N], nx, err ), perror1++;
    }
    nx = base = ncxt = 0;
  }
...
void printStatus( int n, int size ) {
  printf( "%6.2f%% %6.2f%%\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b", float( 100 ) * n / ( size + 1 ),
...
int decode_txt0( FILE *in, int size, FILE *out, FMode mode, int &diffFound ) {
  ...
  dtmp = tmpfile();
  if( dtmp == nullptr )
    perror( "ERR txt0 tmpfile" ), exit( 1 );
  for( int i = 0; i < size; i++ ) {
    c = fgetc( in );
    putc( c, dtmp );
    putc( b, dtmp );
