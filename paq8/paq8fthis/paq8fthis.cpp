...
void assert_base()
{
  if (base_state != 0) return; else base_state=1;

  double ci, ck = 1.0 / sqrt ( 8.0 );
  int n, k, i = 0;
  for ( n = 0; n < 8; n++) base[i++] = (int)(ck * 256); // 1st row
  
  ck = sqrt( 1.0 / 4 );
  for ( k = 1; k < 8; k++ ) {  // 2nd to 8th row
    ci = (3.14159 * k) / 16;
    for ( n = 0; n < 8; n++ ) base[i++] = (int)(ck * cos( (n + n + 1.0) * ci ) * 256);
  }
}

inline void dct_8x8(bool inv)
{
  if (base_state == 0) assert_base();
  int i, j, i2, acc;
  for ( i = 0; i < 16; i++)
  {
    for ( i2 = 0; i2 < 8; i2++ ) {
      acc = 0;
      for ( j = 0; j < 8; j++) acc += (base[(inv?i2+8*j:8*i2+j)] * (i < 8?input[i + j*8]:temp[(i - 8)*8 + j])) >> 8;
      if (i < 8) temp[i + i2*8] = acc; else output[(i-8)*8 + i2] = acc;
    }
  }
}
  ...
  if (!bpos) {
    ...
    if (buf(4)==FF && buf(3)==DQT) {
		...
		if (pos >= dqt_end) {
			...
			if (dqt_state % 65 == 0) qnum = buf(1); else
    ...
    if (mcusize && buf(1+(!bpos))!=FF) {  // skip stuffed byte
      ...
      if (rs>=0) {
        if (huffsize+(rs&15)==huffbits) { // done decoding
		  ...
		  if (xx == 0) {
            //top
            for ( q = 0; q < 64; q++) input[zzu[q]+zzv[q]*8] = (qtab[64*qmap[compx]+q] * cbuf2[cpos_dc+q-mcusize*width]) << 8;
            dct_8x8(true); // idct
            for ( q = 0; q < 64; q++) LL1[q] = output[q%8 + 8*7]+(output[q%8 + 8*7]-output[q%8 + 8*6])/4;

            //left
            for ( q = 0; q < 64; q++) input[zzu[q]+zzv[q]*8] = (qtab[64*qmap[compx]+q] * cbuf2[cpos_dc+q-les]) << 8;
            dct_8x8(true); // idct
            for ( q = 0; q < 64; q++) LL2[q] = output[(q/8)*8 + 7]+(output[(q/8)*8 + 7]-output[(q/8)*8 + 6])/4;
          }

          //normal
	      for ( q = 0; q < 64; q++) input[zzu[q]+zzv[q]*8] = (q >= xx?0:(qtab[64*qmap[compx]+q] * cbuf2[cpos_dc+q]) << 8);
	      dct_8x8(true); // idct
	      for ( q = 0; q < 64; q++) {
          ...
          //adv_pred1
          for ( q = 0; q < 64; q++) input[q] = LL3[q];
          dct_8x8(false); // dct
          adv_pred[0] = (output[zzu[xx]] / qtab[64*qmap[compx] + xx]) >> 8;

          //adv_pred2
	      for ( q = 0; q < 64; q++) input[q] = LL4[q];
	      dct_8x8(false); //dct
