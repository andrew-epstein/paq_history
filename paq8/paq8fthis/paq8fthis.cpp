...
int input[64], output[64], temp[64];
int LL1[64], LL2[64], LL3[64], LL4[64];
int input2[8], output2[8];
int base[64];
int base_state = 0;
...
void assert_base() {
  ...
  double ci, ck = 1.0 / sqrt( 8.0 );
  int n, k, i = 0;
  for( n = 0; n < 8; n++ )
    base[i++] = ( int ) ( ck * 256 ); // 1st row

  ck = sqrt( 1.0 / 4 );
  for( k = 1; k < 8; k++ ) { // 2nd to 8th row
    ci = ( 3.14159 * k ) / 16;
    for( n = 0; n < 8; n++ )
      base[i++] = ( int ) ( ck * cos( ( n + n + 1.0 ) * ci ) * 256 );
  }
}

inline void dct_8x8( bool inv ) {
  if( base_state == 0 )
    assert_base();
  int i, j, i2, acc;
  for( i = 0; i < 16; i++ ) {
    for( i2 = 0; i2 < 8; i2++ ) {
      ...
      for( j = 0; j < 8; j++ )
        acc +=
            ( base[( inv ? i2 + 8 * j : 8 * i2 + j )] * ( i < 8 ? input[i + j * 8] : temp[( i - 8 ) * 8 + j] ) ) >> 8;
      if( i < 8 )
        temp[i + i2 * 8] = acc;
      else
        output[( i - 8 ) * 8 + i2] = acc;
        ...
        for( int j = 0; j < nf; ++j ) {
          if( buf[sos + 2 * i + 5] == buf[sof + 3 * j + 10] ) { // Cs == C ?
            ...
            while( hv != 0 ) {
              ...
              color[mcusize] = i;
              qmap[mcusize] = buf[sof + 3 * j + 12]; // Quantizazion table mapping
              --hv;
              ++mcusize;
            }
          }
        ...
        if( huffsize + ( rs & 15 ) == huffbits ) { // done decoding
          ...
          int les = 0;
          {
            ...
            for( int a = 1; a < poc_comp; a++ )
              if( color[( poc_comp + compx + a ) % poc_comp] == color[compx] )
                les = ( poc_comp - a ) << 6;
          }
          if( xx == 0 ) {
            //top
          ...
          for( q = 0; q < 64; q++ )
            input[q] = LL3[q];
          dct_8x8( false ); // dct
          adv_pred[0] = ( output[zzu[xx]] / qtab[64 * qmap[compx] + xx] ) >> 8;

          //adv_pred2
          for( q = 0; q < 64; q++ )
            input[q] = LL4[q];
          dct_8x8( false ); //dct
          adv_pred[1] = ( output[zzv[xx] * 8] / qtab[64 * qmap[compx] + xx] ) >> 8;
