// TextFilter 1.2 for PAQ (based on WRT 4.5) by P.Skibinski, 08.02.2006, inikep@o2.pl

#define CHAR_FIRSTUPPER 127 // for encode lower word with first capital letter
#define CHAR_UPPERWORD 12   // for encode upper word
#define CHAR_LOWERWORD 11   // for encode lower word with a few capital letter
...
#define OPTION_TO_LOWER_AFTER_PUNCTUATION 1
...
#define OPTION_RECORD_INTERLEAVING 16384
#define OPTION_DNA_QUARTER_BYTE 32768
#define OPTION_SPACE_AFTER_EOL 65536
...
public:
  ...
  int preprocFlag, tryShorterBound, preprocessing, s_size, WRTd_c, WRTd_qstart, WRTd_qend, WRTd_type;
  int fftell, fftelld, originalFileLen, autoSwitch;
...
#define OUTPUT_BUFFER_MIN_SIZE 10240
  ...
  public:
    ...
    uint SrcPtr, SrcLen;
    uc *TargetBuf;
    uint TgtPtr, TgtMarker, TgtLen;

    MemoryBuffer() {
      SourceBuf = NULL, TargetBuf = NULL, SrcPtr = 0;
      TgtPtr = 0;
      TgtMarker = 0;
    };
    ~MemoryBuffer() {
...
#define ORIGINAL_CHARSET( c )                                                                                          \
  ...
  inline int checkHashExactly( const unsigned char *s, int s_size, int h ) {
    ...
    i = word_hash[h];
    if( i > 0 ) {
      //	printf("szukam=%s (%d) mam=%s (%d)\n",s,s_size,dict[i],dictlen[i]);
...
#define DECODE_WORD( dictNo, i )                                                                                       \
  ...
  inline int decodeCodeWord( FILE *file, unsigned char *s, int &c ) {
    ...
    if( codeword2sym[c] < dict1size ) {
      ...
      return s_size;
    } else if( codeword2sym[c] < dict1plus2 )
      i = dict1size * ( codeword2sym[c] - dict1size );
    else if( codeword2sym[c] < dict1plus2plus3 ) {
  ...
  void initializeCodeWords() {
    ...
    if( IF_OPTION( OPTION_ADD_SYMBOLS_0_4 ) )
      for( c = 0; c <= 4; c++ )
        addSymbols[c] = 1;
    ...
    for( c = 0; c < 256; c++ ) {
      ...
      if( ( IF_OPTION( OPTION_USE_DICTIONARY ) && addSymbols[c] ) || c == CHAR_ESCAPE || c == CHAR_LOWERWORD
          || c == CHAR_FIRSTUPPER || c == CHAR_UPPERWORD || c == CHAR_CR_LF
    ...
    if( IF_OPTION( OPTION_USE_DICTIONARY ) ) {
      ...
      dictionary = ( dict1size * dict2size * dict3size * dict4size + dict1size * dict2size * dict3size
                     + dict1size * dict2size + dict1size );
...
#define SAMPLE_WORDS_COUNT_MAX ( SAMPLE_WORDS_COUNT * ( CHARSET_COUNT - 1 ) )
  ...
  int longDictLen, shortDictLen, longDict, shortDict;
  ...
  int WRT_detectFileType( FILE *file, int part_length, int parts, int &recordLen ) {
    ...
    if( ( parts == 1 && langSum + shortLangSum >= 18 ) || langSum + shortLangSum > part_length * parts / 250 ) {
      max = 0.0;
      for( i = 0; i < langCount; i++ ) {
        if( memcmp( SHORT_DICTNAME, langName[i], sizeof( SHORT_DICTNAME ) - 1 ) != 0 ) {
    ...
    if( longDictLen <= 0 && shortDictLen <= 0 )
      TURN_OFF( OPTION_USE_DICTIONARY );

    if( langSum + shortLangSum < part_length * parts / 150 )
      TURN_OFF( OPTION_USE_DICTIONARY );
    ...
    if( binCount > part_length * parts / 15 ) // 3333
    {
    ...
    if( IF_OPTION( OPTION_EOL_CODING ) && EOLCount > 256 && EOLCount < 5120 && EOLCountBad / EOLCount < 6
        && spaceAfterLF > 1000 )
      TURN_ON( OPTION_SPACE_AFTER_EOL )
    else if( EOLCount < 256 || EOLCount > 5120 || EOLCountBad / EOLCount > 3 ) {
      if( !forceEOLcoding )
    ...
    PRINT_DICT( ( "EOLCount=%d+%d(%d) binCount=%d/%d(%d) punctCount=%d/%d(%d) langSum=%d/%d(%d) ngram=%d\n", EOLCount,
                  EOLCountBad, IF_OPTION( OPTION_EOL_CODING ), binCount, part_length * parts / 50,
                  !IF_OPTION( OPTION_NORMAL_TEXT_FILTER ), punctCount, punctCountBad,
                  IF_OPTION( OPTION_WORD_SURROROUNDING_MODELING ), shortLangSum + langSum, part_length * parts / 100,
                  IF_OPTION( OPTION_USE_DICTIONARY ), IF_OPTION( OPTION_USE_NGRAMS ) ) );
  ...
  void WRT_print_options() {
    PRINT_DICT( ( "OPTION_TO_LOWER_AFTER_PUNCTUATION=%d ", IF_OPTION( OPTION_TO_LOWER_AFTER_PUNCTUATION ) ) );
    PRINT_DICT( ( "OPTION_SPACELESS_WORDS=%d ", IF_OPTION( OPTION_SPACELESS_WORDS ) ) );
    PRINT_DICT( ( "OPTION_CAPITAL_CONVERSION=%d ", IF_OPTION( OPTION_CAPITAL_CONVERSION ) ) );
    PRINT_DICT( ( "OPTION_SPACE_AFTER_CC_FLAG=%d ", IF_OPTION( OPTION_SPACE_AFTER_CC_FLAG ) ) );
    PRINT_DICT( ( "OPTION_USE_DICTIONARY=%d ", IF_OPTION( OPTION_USE_DICTIONARY ) ) );
    PRINT_DICT( ( "OPTION_TRY_SHORTER_WORD=%d ", IF_OPTION( OPTION_TRY_SHORTER_WORD ) ) );
    PRINT_DICT( ( "OPTION_WORD_SURROROUNDING_MODELING=%d ", IF_OPTION( OPTION_WORD_SURROROUNDING_MODELING ) ) );
    PRINT_DICT( ( "OPTION_NORMAL_TEXT_FILTER=%d ", IF_OPTION( OPTION_NORMAL_TEXT_FILTER ) ) );
    PRINT_DICT( ( "OPTION_EOL_CODING=%d ", IF_OPTION( OPTION_EOL_CODING ) ) );
    PRINT_DICT( ( "OPTION_ADD_SYMBOLS_0_4=%d ", IF_OPTION( OPTION_ADD_SYMBOLS_0_4 ) ) );
    PRINT_DICT( ( "OPTION_ADD_SYMBOLS_14_31=%d ", IF_OPTION( OPTION_ADD_SYMBOLS_14_31 ) ) );
    PRINT_DICT( ( "OPTION_ADD_SYMBOLS_A_Z=%d ", IF_OPTION( OPTION_ADD_SYMBOLS_A_Z ) ) );
    PRINT_DICT( ( "OPTION_ADD_SYMBOLS_MISC=%d ", IF_OPTION( OPTION_ADD_SYMBOLS_MISC ) ) );
    PRINT_DICT( ( "OPTION_USE_NGRAMS=%d ", IF_OPTION( OPTION_USE_NGRAMS ) ) );
    PRINT_DICT( ( "OPTION_RECORD_INTERLEAVING=%d ", IF_OPTION( OPTION_RECORD_INTERLEAVING ) ) );
    PRINT_DICT( ( "OPTION_DNA_QUARTER_BYTE=%d ", IF_OPTION( OPTION_DNA_QUARTER_BYTE ) ) );
      ...
      switch( argv[1][1] ) {
        ...
        case '3':
          if( firstTime )
            switch( argv[1][1] ) {
              case '0':
                printf( "* LZ77 optimized preprocessing\n" );
                break;
              case '1':
                printf( "* BWT optimized preprocessing\n" );
                break;
              case '2':
                printf( "* PPM optimized preprocessing\n" );
                break;
              case '3':
                printf( "* PAQ optimized preprocessing\n" );
                break;
            }
  ...
  inline void addWord( unsigned char *mem, int &i, int &sizeFullDict ) {
    dictlen[sizeFullDict] = i;
    dict[sizeFullDict] = mem;
    int j = stringHash( mem, i );

    if( word_hash[j] != 0 ) {
      if( dictlen[word_hash[j]] != dictlen[sizeFullDict]
          || sizeFullDict / SAMPLE_WORDS_COUNT_MAX != word_hash[j] / SAMPLE_WORDS_COUNT_MAX
          || memcmp( ( const char * ) dict[word_hash[j]], ( const char * ) dict[sizeFullDict], i ) != 0 )
        word_hash[j] = -1;
    } else
      word_hash[j] = sizeFullDict;
      ...
      for( int j = 0; j < 4; j++ ) {
        ...
        switch( c ) {
          ...
          case 't':
            ...
            break;
          default:
            printf( "error: use WRT with -q option\n" );
            return;
        }
      }
      fputc( d, fileout );
...
#define DECODE_PUTC( c )                                                                                               \
  ...
  void WRT_encode( FILE *file, FILE *fileout, int fileLen ) //,int fileType)
  {
    ...
    while( !feof( file ) ) {
      ...
      /*		if ( IF_OPTION(OPTION_WORD_SURROROUNDING_MODELING) && PUNCTUATION_CHARS(c) &&
			(last_c==10 || last_c==32 || (last_c>='a' && last_c<='z') || (last_c>='A' && last_c<='Z')) )
		{ 
			ENCODE_PUTC(32,fileout);
		} 
*/
  ...
  void WRT_start_encoding( FILE *file, FILE *fileout, unsigned int fileLen ) {
    ...
    readDicts( DICTNAME "*" DICTNAME_EXT, ( char * ) dictPath, dictPathLen );
    WRT_detectFileType( file, 10240, 5, recordLen );
    //	printf("recordLen=%d record=%d dna=%d\n",recordLen,IF_OPTION(OPTION_RECORD_INTERLEAVING),IF_OPTION(OPTION_DNA_QUARTER_BYTE));
    ...
    if( IF_OPTION( OPTION_DNA_QUARTER_BYTE ) )
      ...
      WRT_encode( file, fileout, fileLen ); //,WRT_detectFileType(argv[2],file,10240,5));
      writeEOLstream( fileout );            //,fileLen);
  ...
  void WRT_start_decoding( FILE *file, FILE *fileout, int header ) {
    ...
    if( IF_OPTION( OPTION_DNA_QUARTER_BYTE ) )
      ...
      for( i = 0, fileLen = 0; i < 4; i++ )
        fileLen = fileLen * 256 + fgetc( file );
#else
      WRTd_filter->reads += EOLlen + 4;
#endif

      fileLen -= EOLlen + 4; // EOLlen + sizeof(EOLlen)

      originalFileLen = fileLen;
      bufferedChar = -1;
      EOLType = UNDEFINED;
      lastChar = 0;
      fftell = 0;
