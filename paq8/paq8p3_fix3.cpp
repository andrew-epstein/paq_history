...
class RunContextMap {
  ...
  int mulc;
public:
  RunContextMap(int m, int c=8): t(m/4), mulc(c) {cp=t[0]+1;}
...
void model24bit(Mixer& m,int w=0) {
  ...
  static SmallStationaryContextMap scm1(SC), scm2(SC),
    scm3(SC), scm4(SC), scm5(SC), scm6(SC), scm7(SC), scm8(SC),scm9(SC), scm10(SC*2),scm11(SC);
...
int ppmModel(Mixer& m) {
	...
	if (!bpos && (pos>eoi))
	{
		if(buf(3)=='P' && buf(2)=='6' && ISWHITESPACE(1)) // Detect PPM file
		{
			...
			if(ppm && ppm_ptr!=3) 		// PPM detected, let's parse header records
			{ 
...
void model8bit(Mixer& m, int w) {
	const int SC=0x20000;
	static SmallStationaryContextMap scm1(SC), scm2(SC),
		scm3(SC), scm4(SC), scm5(SC), scm6(SC*2),scm7(SC),scm8(SC);
...
typedef enum {DEFAULT, JPEG, BMPFILE1, BMPFILE4, BMPFILE8, BMPFILE24, TIFFFILE,
              PGMFILE, RGBFILE, EXE, TEXT, WAVFILE, MRBFILE, TXTUTF8, PPMFILE,
              PBMFILE,CWAVFILE} Filetype;
...
static const char* typenames[maxTypes]={"unkn", "jpeg ", "bmp1 ", "bmp4 ",
                                 "bmp8 ", "bmp24 ", "tiff ", "pgm ", "rgb ",
                                 "exe ", "text ", "wav ", "mrb ","utf8 ","ppm ",
                                 "pbm ","cwav"};
...
int contextModel2() {
  ...
  if (bpos==0) {
    ...
    if (size==-5) {
      ...
      if (filetype<maxTypes) /*printf("\n(%s%d)", typenames[filetype], size),*/ typesize[filetype]+=size,++typecount[filetype];
...
Filetype detect(FILE* in, int n, Filetype type) {
  ...
  for (int i=0; i<n; ++i) {
    ...
    if (bmp){
		...
		if ((type==BMPFILE1 || type==BMPFILE4 || type==BMPFILE8 || type==BMPFILE24 ) && (imgbpp==1 || imgbpp==4 || imgbpp==8 || imgbpp==24) && imgcomp==0){
            ...
            if (imgbpp==8)
                if (bsize !=(tbsize=(((bmpx)+3&-4)*bmpy+bmpimgoff))) bsize=tbsize;
            if (imgbpp==24)
				if (bsize !=(tbsize=(((bmpx*3)+3&-4)*bmpy+bmpimgoff))) bsize=tbsize;
  ...
  /*   // Detect .bmp image without 'BM' 
    ...
    if (wavi){
        if ((i-wavi)==4){ 
           wavsize=bswap(buf0);
           if (wavsize==0) wavi=wavdatas=wavsize=iswavcompressible=0;
        }
	    if ((i-wavi)==8  && (buf0!='WAVE')) wavi=wavdatas=wavsize=iswavcompressible=0; //'WAVE'
        if ((i-wavi)==12 && (buf0!='fmt ')) wavi=wavdatas=wavsize=iswavcompressible=0; //'fmt '
		if ((i-wavi)==16) {
    	...
    	if ((i-wavi)==18){  
            ...
            if (wavAudioFormat==1 || wavAudioFormat==0xFFFE) { // PCM, Extensible wave format
                                  ...
                                  else{iswavcompressible=-1;}
        }
    	if ((i-wavi)==20){
            wavChannels=((buf0&0xFFFF)>>8)+((buf0&0xFF)<<8);
            if (wavChannels==0) wavi=wavdatas=wavsize=iswavcompressible=0;
        }
    	if ((i-wavi)==36 && buf0!='data') wavi=wavdatas=wavsize=iswavcompressible=0;    //'data' 
		if ((i-wavi)==40) {
	    	wavdatas=bswap(buf0);
			if (wavdatas==0 || (wavsize<(wavdatas+0x24))) wavi=wavdatas=wavsize=0;}
        if (!wavsize == 0 && !wavdatas==0){
			if (type==WAVFILE || type==CWAVFILE) return fseek(in, start+wavsize+8, SEEK_SET),DEFAULT;
			if (iswavcompressible==0) return 	fseek(in, start+wavi-3, SEEK_SET),WAVFILE;
			if (iswavcompressible==-1) return 	fseek(in, start+wavi-3, SEEK_SET),CWAVFILE;
...
void encode(FILE* in, FILE* out, int n,const char* filename) {
  ...
  while (n>0) {
    ...
    if (len>0) {
            ...
            if (level==0)  typesize[type]+=len,++typecount[type];
      ...
      switch(type) {
		...
		case WAVFILE: encode_wav(in, out, len); break;
...
int decode(Encoder& en) {
  ...
  while (len==0) {
    ...
    len|=en.decompress();
    if (len<0) len=1;
     if (level==0 && (en.getMode()!=COMPRESS))  typesize[type]+=len,++typecount[type];
  }
  --len;
  switch (type) {
	...
	case WAVFILE: return decode_wav(en);
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    if (!logfile) perror("Cant open logfile\n"), quit();
    if (mode==COMPRESS) {
