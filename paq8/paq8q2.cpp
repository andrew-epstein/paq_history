/*  paq8q3 file compressor/archiver.  Release by Simon Berger May. 13, 2009
...
- To install, put paq8q3.exe or a shortcut to it on your desktop.
- To compress a file or folder, drop it on the paq8q3 icon.
- To decompress, drop a .paq8q3 file on the icon.

A .paq8q3 extension is added for compression, removed for decompression.
The output will go in the same folder as the input.

While paq8q3 is working, a command window will appear and report
...
- To install, put paq8q3.exe somewhere in your PATH.
- To compress:      paq8q3 [-N] [-mM] file1 [file2...]
- To decompress:    paq8q3 [-d] file1.paq8q3 [dir2]
- To view contents: more < file1.paq8q3

The compressed output file is named by adding ".paq8q3" extension to
the first named file (file1.paq8q3).  Each file that exists will be
...
If file1.paq8q3 exists then it is overwritten.

If the first named file ends in ".paq8q3" then it is assumed to be
...
The -d option forces extraction even if there is not a ".paq8q3"
...
During extraction, directories are created as needed.  For example:
  ...
  paq8q3 -4 c:\tmp\foo bar

compresses foo and bar (if they exist) to c:\tmp\foo.paq8q3 at level 4
and mode -m5.

  paq8q3 -d c:\tmp\foo.paq8q3 .
...
There are 2 files: paq8q3.cpp (C++) and paq7asm.asm (NASM/YASM).
paq7asm.asm is the same as in paq7 and paq8x.  paq8q3.cpp recognizes the
...
Recommended compiler commands and optimizations:
  ...
  MINGW g++:
    ...
    g++ paq8q3.cpp -DWINDOWS -O2 -Os -s -march=pentiumpro -fomit-frame-pointer -o paq8q3.exe paq7asm.obj

  Borland:
    nasm paq7asm.asm -f obj --prefix _
    bcc32 -DWINDOWS -O -w-8027 paq8q3.cpp paq7asm.obj

  Mars:
    nasm paq7asm.asm -f obj --prefix _
    dmc -DWINDOWS -Ae -O paq8q3.cpp paq7asm.obj

  UNIX/Linux (PC):
    nasm -f elf paq7asm.asm
    g++ paq8q3.cpp -DUNIX -O2 -Os -s -march=pentiumpro -fomit-frame-pointer -o paq8q3 paq7asm.o

  Non PC (e.g. PowerPC under MacOS X)
    g++ paq8q3.cpp -O2 -DUNIX -DNOASM -s -o paq8q3
...
Then
  ...
  paq8q3 archive \dir1\file1.txt \dir2

will create archive.paq8q3 with the header:
...
Decompression will fail if the first 6 bytes are not "paq8q3 ".  Sizes
...
paq8q3 uses a neural network to combine a large number of models.  The
...
paq8q3 uses preprocessing transforms on certain data types to improve
...
// To compress to file1.paq8q3: paq8q3 [-n] file1 [file2...]
// To decompress: paq8q3 file1.paq8q3 [output_dir]
