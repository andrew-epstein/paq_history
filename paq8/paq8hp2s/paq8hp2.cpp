...
void wordModel( Mixer &m ) {
  ...
  static ContextMap cm( MEM * 32, 19 );
...
void sparseModel( Mixer &m ) {
  static ContextMap cm( MEM * 4, 12 ), scm( MEM, 8 );
  if( bpos == 0 ) {
...
int contextModel2() {
  ...
  if( ismatch > 640 ) { // Model long matches directly
    m.set( 0, 8 );
    return m.p();
  }
  /////  else if (isjpeg) {
  ...
  //m.set(buf(1)+8, 264);
  //m.set(c0, 256);
  m.set( order + 8 * ( c4 >> 5 & 7 ) + 64 * ( buf( 1 ) == buf( 2 ) ), 128 ); /////+128*(filetype==EXE), 256);
                                                                             //m.set(buf(2), 256);
