/* paq8jd file compressor/archiver.
// Release by Bill Pettis (ilovemyking@gmail.com)
...
- To install, put paq8jd.exe or a shortcut to it on your desktop.
- To compress a file or folder, drop it on the paq8jd icon.
- To decompress, drop a .paq8jd file on the icon.

A .paq8jd extension is added for compression, removed for decompression.
The output will go in the same folder as the input.

While paq8jd is working, a command window will appear and report
...
- To install, put paq8jd.exe somewhere in your PATH.
- To compress:      paq8jd [-N] file1 [file2...]
- To decompress:    paq8jd [-d] file1.paq8jd [dir2]
- To view contents: more < file1.paq8jd

The compressed output file is named by adding ".paq8jd" extension to
the first named file (file1.paq8jd).  Each file that exists will be
...
If file1.paq8jd exists then it is overwritten.

If the first named file ends in ".paq8jd" then it is assumed to be
...
The -d option forces extraction even if there is not a ".paq8jd"
...
During extraction, directories are created as needed.  For example:
  ...
  paq8jd -4 c:\tmp\foo bar

compresses foo and bar (if they exist) to c:\tmp\foo.paq8jd at level 4.

  paq8jd -d c:\tmp\foo.paq8jd .
...
There are 2 files: paq8jd.cpp (C++) and paq7asm.asm (NASM/YASM).
paq7asm.asm is the same as in paq7 and paq8x.  paq8jd.cpp recognizes the
...
Recommended compiler commands and optimizations:
  ...
  MINGW g++:
    ...
    g++ paq8jd.cpp -DWINDOWS -O2 -Os -s -march=pentiumpro -fomit-frame-pointer -o paq8jd.exe paq7asm.obj

  Borland:
    nasm paq7asm.asm -f obj --prefix _
    bcc32 -DWINDOWS -O -w-8027 paq8jd.cpp paq7asm.obj

  Mars:
    nasm paq7asm.asm -f obj --prefix _
    dmc -DWINDOWS -Ae -O paq8jd.cpp paq7asm.obj

  UNIX/Linux (PC):
    nasm -f elf paq7asm.asm
    g++ paq8jd.cpp -DUNIX -O2 -Os -s -march=pentiumpro -fomit-frame-pointer -o paq8jd paq7asm.o

  Non PC (e.g. PowerPC under MacOS X)
    g++ paq8jd.cpp -O2 -DUNIX -DNOASM -s -o paq8jd
...
Then
  ...
  paq8jd archive \dir1\file1.txt \dir2

will create archive.paq8jd with the header:
...
Decompression will fail if the first 6 bytes are not "paq8jd -".  Sizes
...
paq8jd uses a neural network to combine a large number of models.  The
...
paq8jd uses preprocessing transforms on certain data types to improve
...
#define PROGNAME "paq8jd" // Please change this if you change the program.
...
// To compress to file1.paq8jd: paq8jd [-n] file1 [file2...]
// To decompress: paq8jd file1.paq8jd [output_dir]
