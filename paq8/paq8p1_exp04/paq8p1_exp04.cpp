        ...
        if( huffsize + ( rs & 15 ) == huffbits ) { // done decoding
          ...
          // UPDATE_ADV_PRED !!!!
          {
            ...
            if( zz == 0 ) {
              ...
              for( int i = 0; i < 64; i++ ) {
                sumu2[zzu[i]] +=
                    we[zzv[i]] * ( zzv[i] & 1 ? -1 : +1 ) * ( qtab[q + i] + 1 ) * cbuf2[cpos_dc_mcusize_width + i];
                sumv2[zzv[i]] +=
                    we[zzu[i]] * ( zzu[i] & 1 ? -1 : +1 ) * ( qtab[q + i] + 1 ) * cbuf2[cpos_dc_ls_acomp + i];
                sumu3[zzu[i]] += ( zzv[i] ? ( zzv[i] & 1 ? -256 : 256 ) : 181 ) * ( qtab[q + i] + 1 )
                                 * cbuf2[cpos_dc_mcusize_width + i];
                sumv3[zzv[i]] +=
                    ( zzu[i] ? ( zzu[i] & 1 ? -256 : 256 ) : 181 ) * ( qtab[q + i] + 1 ) * cbuf2[cpos_dc_ls_acomp + i];
              }
            } else {
...
int wavModel( Mixer &m ) {
  ...
  if( !bpos ) {
    ...
    const int t = ( msb != 0 ), z1 = s2( w + t ), z2 = s2( w * 2 + t ), z3 = s2( w * 3 + t ), z4 = s2( w * 4 + t ),
              z5 = s2( w * 5 + t );
    i = ch << 4;
    if( !msb ) {
