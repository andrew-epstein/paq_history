...
U32 b1=0, b2=0, b3=0, b4=0, b5=0, b6=0, b7=0, b8=0, c4=0, x4=0, x5=0, w4=0, w5=0, f4=0; // Last 4 whole bytes, packed.  Last byte is bits 0-7.
int order, bpos=0, cxtfl=3; // bits in c0 (0 to 7)
...
public:
  ...
  void update2() {
    train(&tx[0], &wx[0], nx, ((y<<12)-base)*2);
    nx=0;
  }
...
void picModel(Mixer& m) {
  ...
  static U32 col, spafdo=0, spaces=0, spacecount=0, words=0, wordcount=0, fails=0, failz=0, failcount=0;
...
void wordModel(Mixer& m) {
  ...
  if (bpos==0) {
	...
	if (word0) {
	  ...
	  word1=word0*11;
	  word0=0;
	  if( c=='.'||c=='?' ) f=1, spafdo=0; else { ++spafdo; spafdo=min(63,spafdo); }
	}
    }
...
///  static int rcount1=0, rcount2=0;  // candidate counts
  static ContextMap cm(32768/4, 2), cn(32768/2, 4), co(32768*2, 3), cp(32768, 4), cq(32768*4, 3);
...
void sparseModel(Mixer& m) {
  ...
  static SmallStationaryContextMap scm1(0x20000), scm2(0x20000), scm3(0x20000), scm4(0x20000), scm5(0x20000), scm6(0x20000), scm7(0x2000), scm8(0x8000), scm9(0x1000);
...
static char arrdict[420139]="arrdict";
...
int contextModel2() {
  ...
  static Mixer m(443, 128*7 + 2048+ 256*(8+6+8+8), 6, 192);
...
/////  }
  ...
  if (bpos==0) {
    ...
    if(f2=='.'||f2=='?') { if (b1!=f2 && buf(3)!=f2 ) i=13, x4=x4*256+f2; }
...
/////    if (fsize==513216) picModel(m);
		...
		c=(w4&15);
		m.set(order*128 + 16*(min(7,spacecount/4)) + c, 128*7);

		c=(words>>1)&3;  if(bpos>1)  c=c0>>(bpos-2);
		m.set(c1*8+c, 2048);
		...
		else c=((words>>2)&3)+4*(c2/64)+(c1&240);
		m.set(c, 1536);

		c=bpos*256; c2=(c0<<(8-bpos)) | (c1>>bpos);
		m.set(order + c + (c2&248), 2048);
...
void Predictor::update() {
  ...
  if (c0>=256) {
	...
	w5=w5*4+WRT_mpw[c0>>4];
	b8=b7, b7=b6, b6=b5, b5=b4,
	b4=b3; b3=b2; b2=b1; b1=c0;
	if(c0=='.' || c0=='?')	b2='.', w4|=12, w5=(w5<<8)|0x3ff, x5=(x5<<8)+c0, f4=(f4&0xfffffff0)+2;
	f4=f4*16+(c0>>4);
	x5=(x5<<8)+c0;
    c4=(c4<<8)+c0;
    c0=1;
  ...
  bpos=(bpos+1)&7;
        ...
        if (fails&0x00001000) --failcount;
        fails=fails*2;
        failz=failz*2;
        if (y) pr^=4095;
        if (pr>=1792) ++fails, ++failcount;
        if (pr>=1104) ++failz;

  // Filter the context model with APMs
  pr=contextModel2();
  int rate=6 + (pos>18*256*1024) + (pos>29*512*1024);
  ...
  pt=a3.p(pr ,	hash(c0, x5&0xffff)&0xffff, rate);
...
int main(int argc, char** argv) {
  ...
  if (option<'0'||option>'9')
	...
	for (int i=420138; i>=0; --i) putc(arrdict[i], dictfile);
	fclose(dictfile);
