...
U32 b1 = 0, b2 = 0, b3 = 0, b4 = 0, b5 = 0, b6 = 0, b7 = 0, b8 = 0, c4 = 0, x4 = 0, x5 = 0, w4 = 0, w5 = 0,
    f4 = 0;                     // Last 4 whole bytes, packed.  Last byte is bits 0-7.
...
void Predictor::update() {
  ...
  if( c0 >= 256 ) {
    ...
    b1 = c0;
    if( c0 == '.' || c0 == '?' )
      b2 = '.', w4 |= 12, w5 = ( w5 << 8 ) | 0x3ff, x5 = ( x5 << 8 ) + c0, f4 = ( f4 & 0xfffffff0 ) + 2;
    f4 = f4 * 16 + ( c0 >> 4 );
    x5 = ( x5 << 8 ) + c0;
    c4 = ( c4 << 8 ) + c0;
    c0 = 1;
...
int main( int argc, char **argv ) {
  ...
  for( int i = 420138; i >= 0; --i )
    putc( arrdict[i], dictfile );
  fclose( dictfile );
  ...
  if( f ) {
    ...
    if( compressed_size > 0 && total_size > 0 && start_time > 0 ) {
      printf( " (%1.4f bpc) in %1.2f sec (%1.3f KB/sec), %d Kb", 8.0 * compressed_size / total_size,
