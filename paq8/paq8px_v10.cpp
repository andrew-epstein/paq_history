...
Filetype detect( FILE *in, int n, Filetype type, int &imgw ) {
  ...
  for( int i = 0; i < n; ++i ) {
    ...
    if( tiff != 0 ) {
      if( i - tiff == 4 && bswap( buf0 ) != 8 && bswap( buf0 ) + tiff < n ) {
          ...
          for( int i = 0; i < dirsize; i++ ) {
            ...
            if( tagfmt == 3 || tagfmt == 4 ) {
              if( tag == 256 )
                ...
                tifzb = tagval; // bits per component
              else if( tag == 259 )
        ...
        if( ( tiff != 0 ) && ( tifx != 0 ) && ( tify != 0 ) && ( tifzb != 0 ) && ( tifz == 1 || tifz == 3 )
            && ( tifc == 1 ) && ( tifofs != 0 ) && tifofs + tiff < n ) {
          fseek( in, start + tiff + tifofs - 3, SEEK_SET );
          for( int j = 0; j < 4; j++ )
            b[j] = getc( in );
          tifofs = b[0] + ( b[1] << 8 ) + ( b[2] << 16 ) + ( b[3] << 24 );
          if( ( tifofs != 0 ) && tifofs < 65536 && tifofs + tiff < n ) {
            if( tifz == 1 ) {
              if( tifzb == 1 )
                IMG_DET( IMAGE1, tiff - 3, tifofs, ( ( tifx - 1 ) >> 3 ) + 1, tify );
              if( tifzb == 8 )
                IMG_DET( IMAGE8, tiff - 3, tifofs, tifx, tify );
            } else if( tifz == 3 && tifzb == 8 )
              IMG_DET( IMAGE24, tiff - 3, tifofs, tifx * 3, tify );
          }
        }
        tiff = 0;
    ...
    if( tga != 0 ) {
      const int p = i - tga;
      if( p == 4 )
        tga = ( bswap( buf0 ) & 0xFF ) == 0 ? tga : 0;
      else if( p == 5 )
        tga = ( bswap( buf0 ) & 0xFFFF ) == 0 ? tga : 0;
      else if( p == 8 )
        tga = ( bswap( buf0 ) & 0xFFFF ) == 0 ? tga : 0;
      else if( p == 10 )
        tga = ( bswap( buf0 ) & 0xFFFF ) == 0 ? tga : 0;
      ...
      if( ( tgax != 0 ) && ( tgay != 0 ) && ( tgaz != 0 ) ) {
        if( tgaz == 24 )
          IMG_DET( IMAGE24, tga - 3, 18, tgax * 3, tgay );
      }
      if( p > 20 )
        tga = 0;
    }
