...
void recordModel( Mixer &m ) {
  ...
  if( bpos == 0 ) {
    ...
    */
    ...
    if( ( ( ( U16 )( c4 >> 8 ) == ( ( SPACE << 8 ) + SPACE ) ) && ( c0 != SPACE ) )
        || ( (( c4 >> 8 ) == 0u) && (c0 != 0) && ( ( padding != SPACE ) || ( pos - prevTransition > rlen[0] ) ) ) ) {
      prevTransition = pos;
      padding = ( U8 ) d;
    }
...
int jpegModel( Mixer &m ) {
  ...
  if( bpos == 0 ) {
    ...
    if( (jpeg == 0) && buf( 4 ) == FF && buf( 3 ) == SOI && buf( 2 ) == FF
        && ( buf( 1 ) == 0xC0 || buf( 1 ) == 0xC4 || ( buf( 1 ) >= 0xDB && buf( 1 ) <= 0xFE ) ) ) {
      jpeg = 1;
      sos = sof = htsize = data = mcusize = linesize = 0, app = static_cast<int>( buf( 1 ) >> 4 == 0xE ) * 2;
