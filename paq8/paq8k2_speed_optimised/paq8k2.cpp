    ...
    if( mp ) {      // combine outputs
      ...
      for( int i = 0; i < ncxt; ++i ) {
        for( int j = bits - 1; j >= 0; j-- )
          pr[i] =
              a[j * 12 + i].p( pr[i], min( pr[j * 12] + 128 >> 8, 15 ) << 4 | min( pr[( j + 1 ) * 12] + 128 >> 8, 15 ),
                               8 )
                      * ( weight[j] )
                  + b[j * 12 + i].p(
                        pr[i], min( pr[j * 12 + 1] + 128 >> 8, 15 ) << 4 | min( pr[( j + 1 ) * 12 + 1] + 128 >> 8, 15 ),
                        8 )
                        * ( weight[j] )
                  + c[j * 12 + i].p(
                        pr[i], min( pr[j * 12 + 3] + 128 >> 8, 15 ) << 4 | min( pr[( j + 1 ) * 12 + 3] + 128 >> 8, 15 ),
                        8 )
                        * ( weight[j] )
                  + pr[i] * ( 128 - weight[j] * 3 ) + 64
              >> 7;
      }
      for( int i = 0; i < ncxt; ++i ) {
        mp->add( stretch( pr[i] ) );
