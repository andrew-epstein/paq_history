/* paq8px file compressor/archiver.  Release by Jan Ondrus, Mar. 20, 2016
...
void recordModel( Mixer &m ) {
  ...
  static int rlen1 = 3;
  static int rlen2 = 4;
  static int rlen3 = 5; // run length and 2 candidates
  ...
  if( bpos == 0 ) {
    ...
    if( rcount1 > 12 && rlen != rlen1 && rlen1 * 2 != rlen1 )
      rlen1 = rlen = rlen1, rcount1 = rcount2 = rcount3 = 0;
    if( rcount2 > 18 && rlen != rlen2 && rlen1 * 2 != rlen2 )
      rlen1 = rlen, rlen = rlen2, rcount1 = rcount2 = rcount3 = 0;
    if( rcount3 > 24 && rlen != rlen3 && rlen1 * 2 != rlen3 )
      rlen1 = rlen, rlen = rlen1, rcount1 = rcount2 = rcount3 = 0;
...
Filetype detect( FILE *in, int n, Filetype type, int &info ) {
  ...
  for( int i = 0; i < n; ++i ) {
    ...
    if( gif != 0 ) {
      ...
      if( gif == 3 && i == gifi + 7 )
        gif = 4, gifc = 255, gifa = gifi = i + 2 + ( ( c & 128 ) != 0 ? ( 3 * ( 2 << ( c & 7 ) ) ) : 0 );
      if( gif == 4 && i == gifi ) {
        if( c > 0 && gifc != 255 )
          gifw = 0;
        if( c > 0 )
        ...
        while( shift >= bits ) {
          ...
          if( code == ( 1 << codesize ) ) {
            if( maxcode > ( 1 << codesize ) + 1 ) {
              if( (clearpos != 0) && clearpos != 4095 - maxcode )
                return 0;
            ...
            if( last != -1 ) {
              ...
              if( phase == 0 && last > ( 1 << codesize ) ) {
                bool diff = false;
                for( int m = ( 1 << codesize ) + 2; m < maxcode; m++ )
...
int decode_gif( FILE *in, int size, FILE *out, FMode mode, int &diffFound ) {
  ...
  if( diffcount > 4096 || clearpos > 256 )
    return 1;
  int codesize = fgetc( in );
  int bits = codesize + 1;
  int shift = 0;
  ...
  while( size-- >= 0 && ( input = fgetc( in ) ) >= 0 ) {
    ...
    for( int i = ( 1 << codesize ) + 2; i <= maxcode; i++ )
      if( dict[i] == key )
        code = i;
...
void compressRecursive( FILE *in, long n, Encoder &en, char *blstr, int it, float p1, float p2 ) {
  static const char *typenames[12] = {"default", "jpeg", "hdr", "1b-image", "8b-image", "24b-image",
