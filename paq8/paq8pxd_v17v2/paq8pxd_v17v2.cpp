...
inline U64 CMlimit(U64 size){
    if (size>0x7FFFFFFF) return 0x7FFFFFFF; //limit to 2GB power of 2
    return (size);
}
...
ContextMap::ContextMap(U32 m, int c): C(c), t(m>>6), cp(c), cp0(c),cpm(C),
    cxt(c), runp(c), cn(0),result(0) {
...
int ContextMap::mix1(Mixer& m, int cc, int bp, int c1, int y1) {
  ...
  for (int i=0; i<cn; ++i) {
    ...
    for (int i=0; i<cn; ++i) if (cpm[i]==255)  cp[i]=cp0[i]+1+(cc&1);  
		 
		 break;
		 }
      case 4: case 7: {  
	     for (int i=0; i<cn; ++i) if (cpm[i]==255)  cp[i]=cp0[i]+3+(cc&3);
	     
	  	  break;
			}
      case 2: case 5:  { 
	    for (int i=0; i<cn; ++i) {
    	 if (cpm[i]==255) cp0[i]=cp[i]=t[(cxt[i]+cc)&(t.size()-1)].get(cxt[i]>>16); }
		 break;
		 }
      default:  {
	    for (int i=0; i<cn; ++i) { 
...
class MatchContext   {
	...
	int match;
public:
    MatchContext( BlockData& bd,int m,int val): x(bd),buf(bd.buf),  MAXLEN(val), 
    t(m),h(0), ptr(0),len(0),result(0),ilen(0),mlen(0),match(0) { }
    int get(){return result;}
    void mix(Mixer& m)  {
  ...
  for (uint64_t i=0; i<n; ++i) {
    ...
    if (cdi && i>cdi) {
      ...
      if (p==8 && (buf1!=0xffffff00 || ((buf0&0xff)!=1 && (buf0&0xff)!=2))) cdi=0;
        ...
        if (fread(data, 1, 2352, in)!=2352) printf("");
    ...
    if (wavi) {
      ...
      if (p==4) wavsize=bswap(buf0);
        ...
        if (wavsr==44100) wsr=3;
        if (wavsr==22050) wsr=2;
        if (wavsr==11025) wsr=1;
        if (wavsr==8000) wsr=0;
        //if (wsr==-1) printf("Detect wav sr %d",wavsr),quit();
        if ((wavch==1 || wavch==2) && (wavbps==8 || wavbps==16) && wavt==1 && wavd>0 && wavsize>=wavd+36
           && wavd%((wavbps/8)*wavch)==0 && wsr>=0) AUD_DET(AUDIO,wavi-3,44+wavm,wavd,wavch+wavbps/4-3+(wsr<<5));
           //32bit IEEE
...
if (fread(&blk[0], 1, len-offset, in)!=len-offset) printf("");
      fwrite(&blk[0], 1, len-offset, out);
    } else {
    
if (  fread(&blk[0], 1, BLOCK, in)!=BLOCK) printf("");
      if (info==3) blk[15]=3;
...
void DetectRecursive(FILE *in, uint64_t n, Encoder &en, char *blstr, int it=0, uint64_t s1=0, uint64_t s2=0) {
  static const char* audiotypes[6]={"8b mono", "8b stereo", "16b mono",
    "16b stereo","32b mono",
    "32b stereo"};
  ...
  while (n>0) {
    ...
    typenamess[type][it]+=len,  typenamesc[type][it]++; 
      ...
      if (type==EXE || type==CD || type==IMAGE24  || ((type==TEXT || type==TXTUTF8) && len>0xA00000)  || type==BASE64 ||type==SZDD|| type==AUDIO) {
        ...
        else if (type==AUDIO) decode_audio(en, int(tmpsize), in,info,int(len),  FCOMPARE, diffFound);
...
uint64_t decompressStreamRecursive(FILE *out, uint64_t size, Encoder& en, FMode mode, int it=0, uint64_t s1=0, uint64_t s2=0) {
  ...
  if (srid>=0) en.setFile(filestreams[srid]);
    ...
    else if (type==AUDIO) len=decode_audio(en, int(len), out,info,info2,mode, diffFound);
  ...
  for (int i=0; i<jobs.size(); ++i) {
      if (jobs[i].state==OK) {
        ...
        fclose(filesmt[jobs[i].streamid]);
             ...
             #endif
            for (int i=0; i<streamc; ++i) fclose(filestreams[i]);
            ...
            for (int i=0; i<streamc; ++i) {
                ...
                if (datasegmentsize>0){ //if segment contains data
            		...
            		pp = (char *)calloc(len+1,1); //alloc mem for decompressed buf
   					...
   					for (int k=0;k<len;++k) pp[k]=enm->decompress();
   				 	pp[len] = 0;
