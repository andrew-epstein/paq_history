/* paq8pxd17v2 file compressor/archiver.  Release by Kaido Orav, Apr. 19, 2016
...
- To install, put paq8pxd_v17.exe or a shortcut to it on your desktop.
- To compress a file or folder, drop it on the paq8pxd17 icon.
- To decompress, drop a .paq8pxd17 file on the icon.

A .paq8pxd17 extension is added for compression, removed for decompression.
The output will go in the same folder as the input.

While paq8pxd17 is working, a command window will appear and report
...
- To install, put paq8pxd_v17.exe somewhere in your PATH.
- To compress:      paq8pxd_v17 [-N] file1 [file2...]
- To decompress:    paq8pxd_v17 [-d] file1.paq8pxd17 [dir2]
- To view contents: paq8pxd_v17 -l file1.paq8pxd17

The compressed output file is named by adding ".paq8pxd17" extension to
the first named file (file1.paq8pxd17).  Each file that exists will be
...
If file1.paq8pxd17 exists then it is overwritten. Level -0 only
transforms or decompresses data.

If the first named file ends in ".paq8pxd17" then it is assumed to be
...
The -d option forces extraction even if there is not a ".paq8pxd17"
...
During extraction, directories are created as needed.  For example:
  ...
  paq8pxd_v17 -4 c:\tmp\foo bar

compresses foo and bar (if they exist) to c:\tmp\foo.paq8pxd17 at level 4.

  paq8pxd_v17 -d c:\tmp\foo.paq8pxd17 .
...
There are 2 files: paq8pxd_v17.cpp (C++) and wrtpre.cpp (C++).
paq8pxd_v17.cpp recognizes the following compiler options:
...
Recommended compiler commands and optimizations:
  ...
  MINGW g++ (x86,x64):
   with multithreading:
    g++ paq8pxd_v17.cpp -DWINDOWS -DMT -msse2 -O3 -s -static -o paq8pxd_v17.exe 
   without multithreading:
    g++ paq8pxd_v17.cpp -DWINDOWS -msse2 -O3 -s -static -o paq8pxd_v17.exe 

  UNIX/Linux (PC x86,x64):
   with multithreading:
    g++ paq8pxd_v17.cpp -DUNIX -DMT -msse2 -O3 -s -static -lpthread -o paq8pxd_v17
   without multithreading:
    g++ paq8pxd_v17.cpp -DUNIX -msse2 -O3 -s -static -lpthread -o paq8pxd_v17

  Non PC (e.g. PowerPC under MacOS X)
    g++ paq8pxd_v17.cpp -O2 -DUNIX -s -o paq8pxd_v17
...
Then
  ...
  paq8pxd_v17 archive \dir1\file1.txt \dir2

will create archive.paq8pxd17 

The command:

  paq8pxd_v17 archive.paq8pxd17 C:\dir3
...
Decompression will fail if the first 10 bytes are not "paq8pxd17 -".  Sizes
...
paq8pxd_v17 uses a neural network to combine a large number of models.  The
...
paq8pxd_v17 uses preprocessing transforms on certain data types to improve
...
DIFFERENCES FROM PAQ8PXD_V17
changed matchcontex etc.
removed models /match,jpg,dmc/
added i8 cfg file
chnged szdd, pgm detection
compress cfg files to reduce output size
cfg file chages
vm changes
quick and slow compression modes / -s or -q /
config files:
 IMAGE1 test3img.cfg 
 IMAGE8 test3i8.cfg  
 IMAGE24 test3i24.cfg
 All other types:
 Quick test3s.cfg 
 Slow test3d.cfg 
...
#define PROGNAME "paq8pxd17v2" // Please change this if you change the program.
...
public:
  ...
  BlockData() : y( 0 ), c0( 1 ), c4( 0 ), bpos( 0 ), blpos( 0 ), filetype( DEFAULT ), finfo( 0 ) {
...
///////////////////////////// Squash //////////////////////////////
#define maxx( a, b ) ( a - ( ( a - b ) & ( a - b ) >> 31 ) )
#define minn( a, b ) ( b + ( ( a - b ) & ( a - b ) >> 31 ) )
...
public:
  ...
  int operator()( int p ) const {
    ...
    // if (p<-2047) return 0;
    p = p > 2047 ? 2048 : p;   //minn(p,2048);
                               // if (p<-2047) return 0;
    p = p < -2048 ? -2048 : p; //maxx(p,-2048);
    return t[p + 2048];
  }
} squash;

Squash::Squash() : t( 4097 ) {
...
StateMapContext::StateMapContext( int n, BlockData &bd ) : N( n ), cxt( 0 ), t( n ), x( bd ) {
  for( int i = 0; i < N; ++i )
    t[i] = 1 << 31;
}
...
inline U32 hash0( U32 a, U32 b, U32 c = 0xffffffff, U32 d = 0xffffffff, U32 e = 0xffffffff ) {
  U32 h = a * 200002981u + b * 30005491u + c * 50004239u + d * 70004807u + e * 110002499u;
  return h ^ ( h ) >> 9 ^ a >> 2 ^ b >> 3 ^ c >> 4 ^ d >> 5 ^ e >> 6;
}
///////////////////////////// BH ////////////////////////////////
...
public:
  MContextMap( size_t m, int c ) : t( m ), cp( c ), cxt( c ), cn( 0 ), result( 0 ) {
    sm = new StateMap[c];
    for( int i = 0; i < c; ++i ) {
      cp[i] = 0;
    }
...
inline U8 *ContextMap::E::get( U16 ch ) {
  ...
  //get mask, if set get first index and return value
...
#  endif
  ...
  // test last in range 0...6, return count in pcount and mask as indexes in t0
  ...
  for( int i = pcount; i < 7; ++i ) {
    ...
    // clear bit set and test again
    //#if ((__GNUC__ >= 4) || ((__GNUC__ == 3) && (__GNUC_MINOR__ >= 4)))
    //asm("btr %1,%0" : "+r"(t0) : "r"(bitt)); //https://gcc.gnu.org/bugzilla/show_bug.cgi?id=47769
    //#else
    t0 &= ~( 1 << bitt );
    //#endif
    int pri = bh[bitt][0];
...
ContextMap::ContextMap( U32 m, int c ) :
    C( c ), t( m >> 6 ), cp( c ), cp0( c ), cpm( C ), cxt( c ), runp( c ), cn( 0 ), result( 0 ) {
      ...
      for( int i = 0; i < cn; ++i ) {
        if( cpm[i] == 255 ) {
          ...
          if( cp0[i][3] == 2 ) {
            ...
            p[0] = 1 + ( ( c >> 5 ) & 1 );                      //0
            p[1 + ( ( c >> 5 ) & 1 )] = 1 + ( ( c >> 4 ) & 1 ); //1-2
            p[3 + ( ( c >> 4 ) & 3 )] = 1 + ( ( c >> 3 ) & 1 ); //3-6
...
public:
  MatchContext( BlockData &bd, int m, int val ) :
      x( bd ),
      buf( bd.buf ),
      MAXLEN( val ),
      t( m ),
...
public:
  Predictor( char *m ) : pr0( 2048 ), vm( m, x ) {}
  ~Predictor() {
    vm.killvm();
  }
  int p() const {
  ...
  void update() {
    update0();
    pr0 = vm.doupdate( x.y, x.c0, x.bpos, x.c4, x.buf.pos );
  }
};
...
void encode_cd( FILE *in, FILE *out, int len, int info ) {
  ...
  for( int offset = 0; offset < len; offset += BLOCK ) {
    if( offset + BLOCK > len ) {
      if( fread( &blk[0], 1, len - offset, in ) != len - offset )
        printf( "" );
      fwrite( &blk[0], 1, len - offset, out );
    } else {
      if( fread( &blk[0], 1, BLOCK, in ) != BLOCK )
        printf( "" );
      if( info == 3 )
        blk[15] = 3;
      if( offset == 0 )
...
void compressStream( int streamid, uint64_t size, FILE *in, FILE *out ) {
  ...
  datasegmentlen = 0;
  // datastreams
  // DEFAULT HDR JPEG IMAGE1 IMAGE4 IMAGE8 IMAGE24 AUDIO EXE TEXT0 TXTUTF8 DICTTXT TEXT  bigtext
  ...
  if( moin ) {
    ...
    char *pp = "int c,c1,*t; \n void update(int y,int c0,int b,int c4,int p){ \n"
               ...
               "int main() { \n int i; \n if (!(t=malloc(0x1ffffff*sizeof(int)))) exit(-1); \n"
               "c1=0,c=1; \n for (i=0; i<0x1ffffff; i++) t[i]=32768;}";
    ...
    enm->compress( fsz >> 16 );
    enm->compress( fsz >> 8 );
    enm->compress( fsz ); // config file length
    for( int k = 0; k < fsz; ++k )
      enm->compress( getc( moin ) );
    enm->flush();
    delete enm;
    delete prm;
...
// To compress to file1.paq8pxd17: paq8pxd_v17 [-n] file1 [file2...]
// To decompress: paq8pxd_v17 file1.paq8pxd17 [output_dir]
  ...
  try {
      ...
      if( argv[1][1] == 'd' && !argv[1][2] )
        ...
        if( argv[1][1] == 'q' )
          modeQuick = true;
               ...
               && strlen( argv[1] ) == 5 && ( argv[1][1] == 's' || ( argv[1][1] == 'q' ) ) ) {
        if( argv[1][1] == 'q' )
          modeQuick = true;
        topt = argv[1][4] - '0';
        level = argv[1][2] - '0';
    ...
    if( argc < 2 ) {
      printf( PROGNAME " archiver (C) 2008,2014, Matt Mahoney et al.\n"
...
#ifdef VMJIT
                       "Compiled with VM x86 JIT.\n"
#else
                       "Compiled with VM emulation.\n"
#endif
#ifdef WINDOWS
                       ...
                       "M: select mode q or s                     (q - quick, s - slow)\n"
                       "level: -[q|s]0 = store,\n"
                       "  -[q|s]1...-[q|s]3 = faster (uses 35, 48, 59 MB)\n"
                       "  -s4...-s8  = smaller       (uses 133, 233, 435, 837, 1643 MB)\n"
                       "  -s9...-s15 = experimental  (uses 3.1, 6.0, 7.8, 9.3, 12.0, 17.8, 25.3 GB)\n"
                       "  -q4...-q8  = faster        (uses 81, 126, 216, 396, 756 MB)\n"
                       "  -q9...-q15 = exp. faster   (uses 1476, ...... MB)\n"
#ifdef MT
    ...
    pp = "int x, *t; \n int update(int y,int c0,int bpos,int c4,int pos){ \n"
         "if (y) t[x]=t[x]+((65536-t[x])>>5); \n else t[x]=t[x]-(t[x]>>5); \n"
         "if ((x=x*2+y)>=512) x=1; \n return (t[x]>>4);} \n"
         "void block(int a,int b){} \n int main(){ \n int i; \n"
         "if (!(t=malloc(512*sizeof(int)))) exit(-1); \n "
         "for (i=0; i<512; i++) t[i]=32768; \n x=1;}";
          ...
          for( int i = 0; i < jobs.size(); ++i ) {
            if( jobs[i].state == FINISHED || jobs[i].state == FINISHED_ERR ) {
              ...
              if( jobs[i].state == FINISHED_ERR )
                quit( "thread error" ); //exit program on thread error
              ++job_count;
              --thread_count;
            }
          }
    ...
    // If there is no dir1, then extract to .
      ...
      for( int i = 0; i < streamc; ++i ) {
        ...
        if( datasegmentsize > 0 ) {              //if segment contains data
          ...
          len += enm->decompress();
          pp = ( char * ) calloc( len + 1, 1 ); //alloc mem for decompressed buf
