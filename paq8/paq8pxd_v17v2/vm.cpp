...
void components( VM *v, int a, int b, int c, int d, int e, int f, int g, int h, int i ) {
  if( v->initdone == 1 )
    printf( "VM vms error: vms allowed only in main\n " ), quit();
  if( v->totalc > 0 )
    printf( "VM vms error: vms allready called\n " ), quit();
  v->smc = a, v->apm1 = b, v->apm2 = c, v->rcm = d, v->scm = e, v->mcm = f, v->cm = g, v->mx = h, v->mc = i;
  v->totalc = a + d + e + f + g + i;
  if( v->totalc > 0 && h == 0 )
    quit( "No mixer defined VM\n" );
  if( v->totalc == 0 && h > 0 )
    quit( "No inputs for mixer defined VM\n" );
  if( a > 0 )
    v->smC = new StateMapContext *[a]; //uses mixer if set
  if( b > 0 )
    v->apm1C = new APM1 *[b];
  if( c > 0 )
    v->apm2C = new APM2 *[c];
  ...
  if( v->totalc > 0 )
    v->mcomp = ( int * ) calloc( v->totalc * sizeof( int ), 1 ); //alloc memory for component list array
  //uses mixer
  if( d > 0 )
    v->rcmC = new RunContextMap *[d];
  if( e > 0 )
    v->scmC = new SmallStationaryContextMap *[e];
  if( f > 0 )
    v->mcmC = new MContextMap *[f];
  if( g > 0 )
    v->cmC = new ContextMap *[g];
  if( h > 0 )
    v->mxC = new Mixer *[h];
  if( i > 0 )
...
void initcomponent( VM *v, int c, int i, int f, int d, int e ) {
  assert( f != 0 );
  if( v->initdone == 1 )
    printf( "VM vmi error: vmi allowed only in main\n " ), quit();
  if( v->currentc > v->totalc )
    printf( "VM vmi error: component %d not set %d - %d\n ", c, v->currentc, v->totalc ), quit();
  //  v->smc=a, v->apm1=b, v->apm2=c, v->rcm=d, v->scm=e, v->mcm=f, v->cm=g, v->mx=h;
  if( c == vmAPM2 )
    printf( "VM vmi error: APM2 not usable.\n " ), quit();
  const int ii = i + 1;
  switch( c ) {
    case vmSMC:
      if( ii > v->smc )
        printf( "VM vmi error: smc(%d) defined %d, max %d\n", c, ii, v->smc ), quit();
      break;
    case vmAPM1:
      if( ii > v->apm1 )
        printf( "VM vmi error: apm1(%d) defined %d, max %d\n", c, ii, v->apm1 ), quit();
      break;
    case vmAPM2:
      if( ii > v->apm2 )
        printf( "VM vmi error: apm2(%d) defined %d, max %d\n", c, ii, v->apm2 ), quit();
      break;
    case vmRCM:
      if( ii > v->rcm )
        printf( "VM vmi error: rcm(%d) defined %d, max %d\n", c, ii, v->rcm ), quit();
      break;
    case vmSCM:
      if( ii > v->scm )
        printf( "VM vmi error: scm(%d) defined %d, max %d\n", c, ii, v->scm ), quit();
      break;
    case vmMCM:
      if( ii > v->mcm )
        printf( "VM vmi error: mcm(%d) defined %d, max %d\n", c, ii, v->mcm ), quit();
      break;
    case vmCM:
      if( ii > v->cm )
        printf( "VM vmi error: cm(%d) defined %d, max %d\n", c, ii, v->cm ), quit();
      break;
    case vmMX:
      if( ii > v->mx )
        printf( "VM vmi error: mx(%d) defined %d, max %d\n", c, ii, v->mx ), quit();
      break;
    case vmMC:
      if( ii > v->mc )
        printf( "VM vmi error: mc(%d) defined %d, max %d\n", c, ii, v->mc ), quit();
      break;
    default:
      quit( "VM vmi error\n" );
  }
  // if e is -1 then no mixer is needed
  if( ( c > vmAPM2 && c < vmMX ) || ( c == vmSMC && e != -1 ) || ( c == vmMC ) )
    v->mcomp[v->currentc++] = e + ( i << 16 ) + ( c << 8 ); // 0x00iiccmm index,component, mixer
  switch( c ) {
    case vmSMC:
      v->smC[i] = new StateMapContext( f, v->x );
      break;
    case vmAPM1:
      v->apm1C[i] = new APM1( f, v->x );
      break;
    case vmAPM2:
      v->apm2C[i] = new APM2( f, v->x );
      break;
    case vmRCM:
      v->rcmC[i] = new RunContextMap( CMlimit( f < 0 ? MEM() / ( !f + 1 ) : MEM() * f ), v->x );
      break;
    case vmSCM:
      v->scmC[i] = new SmallStationaryContextMap( f );
      break;
    case vmMCM:
      v->mcmC[i] = new MContextMap( CMlimit( f < 0 ? MEM() / ( !f + 1 ) : MEM() * f ), d );
      break;
    case vmCM:
      v->cmC[i] = new ContextMap( CMlimit( f < 0 ? MEM() / ( !f + 1 ) : MEM() * f ), d );
      break;
    case vmMX:
      v->mxC[i] = new Mixer( f, d, v->x, e );
      break;
    case vmMC:
    ...
    if( a == mi ) {              //if user called mixer found
      ...
      switch( ( v->mcomp[i] >> 8 ) & 0xff ) { // select component and mix
        case vmSMC:
          ...
          break;
        case vmRCM:
          v->rcmC[j]->mix( *v->mxC[mi] );
          break;
        case vmSCM:
          v->scmC[j]->mix( *v->mxC[mi] );
          break;
        case vmMCM:
          v->mcmC[j]->mix( *v->mxC[mi] );
          break;
        case vmCM:
          v->cmC[j]->mix( *v->mxC[mi] );
          break;
        case vmMC:
...
void VM::expr( int lev ) {
  ...
  if( !tk ) {
    kprintf( "%d: unexpected eof in expression\n", line );
    exit( -1 );
  } else if( tk == Num ) {
    *++e = IMM;
    *++e = ival;
    ...
    *++e = ival;
    ...
    if( tk == '(' )
      ...
      kprintf( "%d: open paren expected in sizeof\n", line );
      exit( -1 );
    }
    ty = iINT;
    if( tk == Int )
    ...
    if( tk == ')' )
      ...
      kprintf( "%d: close paren expected in sizeof\n", line );
      exit( -1 );
    }
    *++e = IMM;
    *++e = ( ty == rCHAR ) ? sizeof( char ) : ( ty == sSHORT ) ? sizeof( short ) : sizeof( int );
    ty = iINT;
  } else if( tk == Id ) {
    d = id;
    next();
    if( tk == '(' ) {
      ...
      if( d[Class] == Sys ) {
        *++e = d[Val];
        ...
        if( t != fc && fc != 0 ) {
          kprintf( "%d: wrong number of arguments\n", line );
          exit( -1 );
        }
      } else if( d[Class] == Fun ) {
        *++e = JSR;
    ...
    if( tk == Int || tk == Char || tk == Short ) {
      t = ( tk == Int ) ? iINT : ( tk == Short ) ? sSHORT : rCHAR;
      next();
      while( tk == Mul ) {
        next();
        t = t + PTR;
      ...
      if( tk == ')' )
        ...
        kprintf( "%d: close paren expected\n", line );
        exit( -1 );
      }
    }
  } else if( tk == Mul ) {
    ...
    *++e = IMM;
    ...
    if( *e == LC ) {
      ...
      *++e = LS;
      ...
      kprintf( "%d: bad lvalue in pre-increment\n", line );
      exit( -1 );
    }
    *++e = PSH;
    *++e = IMM;
    *++e = ( ty > PTR ) ? sizeof( int ) : ( ty > iINT ) ? sizeof( short ) : sizeof( char );
    ;
    *++e = ( t == Inc ) ? ADD : SUB;
    ...
    if( tk == Comma ) {
      ...
      if( *e == LC || *e == LI || *e == LS )
        *e = PSH;
        ...
        kprintf( "%d: bad lvalue in assignment\n", line );
        exit( -1 );
      }
      expr( Assign );
      *++e = ( ( ty = t ) == rCHAR ) ? SC : ( ( ty = t ) == sSHORT ) ? SS : SI;
    } else if( tk == Cond ) {
      next();
      *++e = BZ;
      d = ++e;
      ...
      if( tk == ':' )
        ...
        kprintf( "%d: conditional missing colon\n", line );
        exit( -1 );
      }
      *d = ( intptr_t )( e + 3 );
      *++e = JMP;
      ...
      *++e = MOD;
      ...
      if( *e == LC ) {
        ...
        *++e = LS;
        ...
        kprintf( "%d: bad lvalue in post-increment\n", line );
        exit( -1 );
      }
      *++e = PSH;
      *++e = IMM;
      ...
      *++e = PSH;
      ...
      if( tk == ']' )
        ...
        kprintf( "%d: close bracket expected\n", line );
        exit( -1 );
      }
      if( t > PTR ) {
        *++e = PSH;
        ...
        *++e = MUL;
        ...
        kprintf( "%d: pointer type expected\n", line );
        exit( -1 );
      }
      *++e = ADD;
...
U32 h4( U32 a, U32 b, U32 c, U32 d ) {
  return hash0( a, b, c, d );
}
U32 h5( U32 a, U32 b, U32 c, U32 d, U32 e ) {
  return hash0( a, b, c, d, e );
}

#ifndef VMJIT
int VM::dovm( int *ttt ) {
  if( !( pc = ttt ) ) {
    kprintf( "main() not defined\n" );
    return -1;
  }
  pc0 = pc;
  ...
  while( 1 ) {
    ...
    // else if (i == MSET) a = (int)memset((char *)sp[2], sp[1], *sp);
      ...
      a = ( int ) memcpy( ( char * ) sp[2], ( char * ) sp[1], *sp );
    else if( i == SMP )
      a = ( int ) smp( this, sp[2], sp[1], *sp );
    else if( i == SMN )
      a = ( int ) smn( this, *sp );
    else if( i == APM )
      a = ( int ) ap( this, sp[3], sp[2], sp[1], *sp );
    else if( i == VMS )
      a = 0, components( this, sp[8], sp[7], sp[6], sp[5], sp[4], sp[3], sp[2], sp[1], *sp );
    else if( i == VMI )
      a = 0, initcomponent( this, sp[4], sp[3], sp[2], sp[1], *sp );
    else if( i == VMX )
      a = 0, setcomponent( this, sp[2], sp[1], *sp );
    else if( i == MXP )
      a = ( int ) mxp( this, *sp );
    else if( i == MXC )
      a = ( int ) mxc( this, *sp );
    else if( i == GCR )
      a = ( int ) gcr( this, sp[2], sp[1], *sp );
    else if( i == STRE )
      a = ( int ) stretch( *sp );
    else if( i == SQUA )
      a = ( int ) squash( *sp );
      ...
      a = 0, mxa( this, sp[1], *sp );
    } else if( i == MXS ) {
      a = 0, mxs( this, sp[2], sp[1], *sp );
    } else if( i == H2 )
      a = h2( ( U32 ) sp[1], ( U32 ) *sp );
    else if( i == H3 )
      a = h3( ( U32 ) sp[2], ( U32 ) sp[1], ( U32 ) *sp );
    else if( i == H4 )
      a = h4( ( U32 ) sp[3], ( U32 ) sp[2], ( U32 ) sp[1], ( U32 ) *sp );
    else if( i == H5 )
      a = h5( ( U32 ) sp[4], ( U32 ) sp[3], ( U32 ) sp[2], ( U32 ) sp[1], ( U32 ) *sp );
    else if( i == ABS )
      a = ( U32 ) absolute( ( int ) *sp );
    else if( i == VTHIS )
      *--sp;               //ignore
    else if( i == EXIT ) { /*printf("exit(%d) cycle = %d\n", *sp, cycle);*/
      return *sp;
    } else {
      kprintf( "unknown instruction = %d! cycle = %d\n", i, cycle );
      return -1;
    }
  }
}
...
int VM::dojit() {
  ...
  if( !jitmem ) {
    kprintf( "could not mmap(%d) jit executable memory\n", poolsz );
    return -1;
  }
  ...
  while( pc <= e ) {
    ...
    *pc++ = ( ( intptr_t ) je << 8 ) | i; // for later relocation of JMP/JSR/BZ/BNZ
    ...
    if( i == LEA ) {
      ...
      *( int * ) je = 0x00b60f;
      je = je + 3;
      dprintf( "\tmovzx eax,BYTE PTR [eax]\n" );
    } else if( i == LS ) {
      *( int * ) je = 0x00B70F;
      je = je + 3;
      dprintf( "\tmovzx eax,WORD PTR [eax]\n" );
    } else if( i == SI ) {
      *( int * ) je = 0x018959;
      je = je + 3;
      ...
      *( int * ) je = 0x840fc085;
      je = je + 8;
      dprintf( "\ttest eax, eax\n\tjz  %x\n", *( pc - 1 ) );
    } // test %eax, %eax; jz <off32>
    else if( i == BNZ ) {
      ++pc;
      *( int * ) je = 0x850fc085;
      je = je + 8;
      dprintf( "\ttest eax, eax\n\tjnz  %x\n", *( pc - 1 ) );
    } // test %eax, %eax; jnz <off32>
    else if( i == VTHIS ) {
      *je++ = 0xb8;
...
*/
      if( i == PRTF ) {
        ...
        tmp = ( intptr_t ) components;
      } else if( i == VMI ) {
        tmp = ( intptr_t ) initcomponent;
      } else if( i == VMX ) {
        tmp = ( intptr_t ) setcomponent;
      } else if( i == MXP ) {
        tmp = ( intptr_t ) mxp;
      } else if( i == STRE ) {
        tmp = ( intptr_t ) stre;
      ...
      if( *pc++ == ADJ ) {
        ...
        kprintf( "no ADJ after native proc!\n" );
        exit( 2 );
      }
      *je++ = 0xb9;
      *( int * ) je = i << 2;
      ...
      *( int * ) je = 0x895af0e6;
      je += 4; // $0xfffffff0, %esi; pop %edx; mov..
      *( int * ) je = 0xe2fc8e54;
      je += 4; // ..%edx, -4(%esi,%ecx,4); loop..  -- reversing args order

      *( int * ) je = 0xe8f487f9;
      je += 4; // ..<'pop' offset>; xchg %esi, %esp; call    -- saving old stack in %esi
      ...
      *( int * ) je = tmp - ( intptr_t )( je + 4 );
      je = je + 4; // <*tmp offset>;
      *( int * ) je = 0xf487;
      je += 2; // xchg %esi, %esp  -- ADJ, back to old stack without arguments
      dprintf( "\txchg esp,esi\n" );
    } else {
      kprintf( "code generation failed for %d!\n", i );
      return -1;
    }
  }
  ...
  while( pc <= e ) {
    i = *pc & 0xff;
    je = ( char * ) ( ( ( intptr_t ) *pc++ >> 8 )
                      | ( ( intptr_t ) jitmem & 0xff000000 ) ); // MSB is restored from jitmem
    if( i == JSR || i == JMP || i == BZ || i == BNZ ) {
      tmp = ( *( intptr_t * ) ( *pc++ ) >> 8 ) | ( ( intptr_t ) jitmem & 0xff000000 ); // extract address
      if( i == JSR || i == JMP ) {
        je += 1;
        *( intptr_t * ) je = tmp - ( intptr_t )( je + 4 );
      } else if( i == BZ || i == BNZ ) {
        je += 4;
        *( intptr_t * ) je = tmp - ( intptr_t )( je + 4 );
      }
...
int VM::block( int info1, int info2 ) {
  ...
  jitmain = reinterpret_cast<int ( * )( int, int )>( *( intptr_t * ) ( idp[Val] ) >> 8
                                                     | ( ( intptr_t ) jitmem & 0xff000000 ) );
...
int VM::doupdate( int y, int c0, int bpos, U32 c4, int pos ) {
  if( mx > 0 && totalc > 0 )
    for( int i = 0; i < mx; i++ )
      mxC[i]->update(); //update all mixers
#ifdef VMJIT
  int ( *jitmain )( int, U32, int, int, int ); // c4 vm pushes first argument first, unlike cdecl
  jitmain = reinterpret_cast<int ( * )( int, U32, int, int, int )>( *( unsigned * ) ( idupdate[Val] ) >> 8
                                                                    | ( ( intptr_t ) jitmem & 0xff000000 ) );
...
int VM::initvm() {
  ...
  if( !( sym = ( int * ) malloc( poolsz ) ) ) {
    kprintf( "could not malloc(%d) symbol area\n", poolsz );
    return -1;
  }
  if( !( text = le = e = ( int * ) malloc( poolsz ) ) ) {
    kprintf( "could not malloc(%d) text area\n", poolsz );
    return -1;
  }
  if( !( data = data0 = ( char * ) malloc( poolsz ) ) ) {
    kprintf( "could not malloc(%d) data area\n", poolsz );
    return -1;
  }
  if( !( sp = sp0 = ( int * ) malloc( poolsz ) ) ) {
    kprintf( "could not malloc(%d) stack area\n", poolsz );
    return -1;
  }
  ...
  p = "char else enum if int short return for sizeof while printf abs smp smn apm vms vmi vmx mxp mxc mxa mxs gcr buf "
      "bufr malloc memcmp memcpy stretch squash ilog h2 h3 h4 h5 exit void block update main";
  i = Char;
  while( i <= While ) {
    next();
    id[Tk] = i++;
  ...
  next();
  idp = id; // keep track of block
  next();
  idupdate = id; // keep track of updater
  next();
  ...
  while( tk ) {
    ...
    if( tk == Int )
      ...
      if( tk == '{' ) {
        ...
        while( tk != '}' ) {
          if( tk != Id ) {
            kprintf( "%d: bad enum identifier %d\n", line, tk );
            return -1;
          }
          next();
          if( tk == Assign ) {
            next();
            if( tk != Num ) {
              kprintf( "%d: bad enum initializer\n", line );
              return -1;
            }
            i = ival;
            next();
    ...
    while( tk != ';' && tk != '}' ) {
      ...
      if( id[Class] ) {
        kprintf( "%d: duplicate global definition\n", line );
        return -1;
      }
      next();
      id[Type] = ty;
      if( tk == '(' ) { // function
        ...
        while( tk != ')' ) {
          ...
          if( tk != Id ) {
            kprintf( "%d: bad parameter declaration\n", line );
            return -1;
          }
          if( id[Class] == Loc ) {
            kprintf( "%d: duplicate parameter definition\n", line );
            return -1;
          }
          id[HClass] = id[Class];
          id[Class] = Loc;
        ...
        next();
        if( tk != '{' ) {
          kprintf( "%d: bad function definition\n", line );
          return -1;
        }
        loc = ++i;
        next();
        while( tk == Int || tk == Char || tk == Short ) {
          ...
          while( tk != ';' ) {
            ...
            if( tk != Id ) {
              kprintf( "%d: bad local declaration\n", line );
              return -1;
            }
            if( id[Class] == Loc ) {
              kprintf( "%d: duplicate local definition\n", line );
              return -1;
            }
            id[HClass] = id[Class];
            id[Class] = Loc;
  ...
  jitmain =
      reinterpret_cast<int ( * )()>( *( unsigned * ) ( idmain[Val] ) >> 8 | ( ( intptr_t ) jitmem & 0xff000000 ) );
  return jitmain();
