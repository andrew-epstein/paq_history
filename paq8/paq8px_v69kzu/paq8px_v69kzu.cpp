...
void im24bitModel(Mixer& m, int w) {
  ...
  if (!bpos) {
    ...
    const int logvar=ilog(var);
        int pxW=buf(1),pxWW=buf(2+color-1),pxNE=buf(w-2+color-1),pxN=buf(w-1+color-1),pxNW=buf(w+color-1),pxNWW=buf(w+1+color-1),pxNNE=buf(w*color-2+color-1),pxNN=buf(w*color-1+color-1);
    int gV=abs(pxW- pxWW)+ abs(pxN- pxNW)+ abs(pxN -pxNE);
    int gH=abs(pxW- pxNW)+ abs(pxN- pxNN) +abs(pxNE- pxNNE);
    int pxP;
if ((gV- gH)>80) pxP=pxW;
else if ((gV- gH)<-80)  pxP=pxN;
...
void im8bitModel(Mixer& m, int w) {
  ...
  if (!bpos) {
    ...
    mean>>=2;
    const int logvar=ilog(var);
    int pxW=buf(1),pxWW=buf(2),pxNE=buf(w-2),pxN=buf(w-1),pxNW=buf(w),pxNWW=buf(w+1),pxNNE=buf(w*2-2),pxNN=buf(w*2-1);
    int gV=abs(pxW- pxWW)+ abs(pxN- pxNW)+ abs(pxN -pxNE);
    int gH=abs(pxW- pxNW)+ abs(pxN- pxNN) +abs(pxNE- pxNNE);
    int pxP;
if ((gV- gH)>80) pxP=pxW;
...
Filetype detect(FILE* in, int n, Filetype type, int &info) {
  ...
  for (int i=0; i<n; ++i) {
    ...
    //text and utf-8 text
      if (txtStart==0 && ((c<128 && c>=32) || c==10 || c==13 || c==0x12 || c==9 )) txtStart=1,txtOff=i;
    if (txtStart) {
  ...
  b=0;
    while (b!=-1) {
      ...
      if (mode==FDECOMPRESS) {
        ...
        if (b!=getc(out) && !diffFound) diffFound=1;
      }
    }
    fclose(dtmp);
  return ftell(out);
...
void compressRecursive(FILE *in, long n, Encoder &en, char *blstr, int it=0, int s1=0, int s2=0) {
  static const char* typenames[11]={"default", "jpeg", "hdr",
    "1b-image", "8b-image", "24b-image", "audio", "exe", "cd", "text","utf-8"};
  ...
  while (n>0) {
    ...
    if (len>0) {
      ...
      if (type==EXE || type==CD || type==IMAGE24  || type==TEXT || type==TXTUTF8) {
        ...
        else if (type==TEXT) encode_txt(in, tmp, len);
...
int decompressRecursive(FILE *out, long size, Encoder& en, FMode mode, int it=0, int s1=0, int s2=0) {
  ...
  while (i<size) {
    ...
    else if (type==TEXT || type==TXTUTF8) len=decode_txt(en, len, out, mode, diffFound);
    else if (type==CD) {
      tmp=tmpfile();
