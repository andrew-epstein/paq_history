...
DIFFERENCES FROM PAQ8Px_v69 - Kaido Orav
Dynamic record model len up to 32 bytes (prints output of learned lens)
indirectmodel2 again
better utf detection - text min len 1024*2 bytes
split compression by type
Modified BMP 8,24 bit - do gradient estimation (better results on non-dihtered 8bit images)
split compression by type
static dict preproccess (detected text/can be commented out)
default compress option 7

*/

#define PROGNAME "paq8pxkzu" // Please change this if you change the program.
...
void wordModel( Mixer &m ) {
  ...
  static ContextMap cm( MEM * 16, 42 );
...
void recordModel( Mixer &m ) {
  ...
  if( bpos == 0 ) {
    ...
    int r = pos - cpos1[c];
    if( r >= 2 && ( r == cpos1[c] - cpos2[c] || r == cpos2[c] - cpos3[c] || r == cpos3[c] - cpos4[c] )
        && r < ( rlencount + 2 ) ) {
...
void im24bitModel( Mixer &m, int w ) {
  ...
  static ContextMap cm( MEM * 4, 13 + 7 - 2 );
  ...
  if( bpos == 0 ) {
    ...
    int pxW = buf( 1 ), pxWW = buf( 2 + color - 1 ), pxNE = buf( w - 2 + color - 1 ), pxN = buf( w - 1 + color - 1 ),
        pxNW = buf( w + color - 1 ), pxNWW = buf( w + 1 + color - 1 ), pxNNE = buf( w * color - 2 + color - 1 ),
        pxNN = buf( w * color - 1 + color - 1 );
    int gV = abs( pxW - pxWW ) + abs( pxN - pxNW ) + abs( pxN - pxNE );
    int gH = abs( pxW - pxNW ) + abs( pxN - pxNN ) + abs( pxNE - pxNNE );
    int pxP;
    if( ( gV - gH ) > 80 )
      pxP = pxW;
    else if( ( gV - gH ) < -80 )
      pxP = pxN;
    //else pxP=(pxW+pxN)/2+ (pxNE- pxNW)/4;
    //if  ((gV- gH)>32) pxP=(pxP+pxW)/2;
    //else if ((gV- gH)>8) pxP=(3*pxP+pxW)/4;
    //else if ((gV- gH)<-32) pxP=(pxP+pxN)/2;
    //else if ((gV- gH)<-8) pxP=(3*pxP+pxN)/4;
    int i = color << 4;

    //do gradient estimation
    cm.set( hash( ++i, static_cast<U32>( ( gV - gH ) > 80 ), pxW ) );
    cm.set( hash( ++i, static_cast<U32>( ( gV - gH ) < -80 ), pxN ) );
    cm.set( hash( ++i, ( pxW + pxN ) / 2, ( pxNE - pxNW ) / 4 ) );
    // cm.set(hash(++i, ((gV- gH)>32), (pxP+pxW)/2));
    cm.set( hash( ++i, static_cast<U32>( ( gV - gH ) > 8 ), ( 3 * pxP + pxW ) / 4 ) );
    //  cm.set(hash(++i, ((gV- gH)<-32), (pxP+pxN)/2));
    cm.set( hash( ++i, static_cast<U32>( ( gV - gH ) < -8 ), ( 3 * pxP + pxN ) / 4 ) );
...
void im8bitModel( Mixer &m, int w ) {
  ...
  static SmallStationaryContextMap scm1( SC ), scm2( SC ), scm3( SC ), scm4( SC ), scm5( SC ), scm6( SC * 2 ),
      scm7( SC );
  static ContextMap cm( MEM * 4, 32 + 7 - 2 );
  ...
  if( bpos == 0 ) {
    ...
    mean >>= 2;
    const int logvar = ilog( var );
    int pxW = buf( 1 ), pxWW = buf( 2 ), pxNE = buf( w - 2 ), pxN = buf( w - 1 ), pxNW = buf( w ), pxNWW = buf( w + 1 ),
        pxNNE = buf( w * 2 - 2 ), pxNN = buf( w * 2 - 1 );
    int gV = abs( pxW - pxWW ) + abs( pxN - pxNW ) + abs( pxN - pxNE );
    int gH = abs( pxW - pxNW ) + abs( pxN - pxNN ) + abs( pxNE - pxNNE );
    int pxP;
    if( ( gV - gH ) > 80 )
...
typedef enum { DEFAULT, JPEG, HDR, IMAGE1, IMAGE8, IMAGE24, AUDIO, EXE, CD, TEXT, TXTUTF8 } Filetype;
...
int contextModel2() {
  ...
  if( bpos == 0 ) {
    ...
    ++blpos;
    if( size == -1 )
      ft2 = ( Filetype ) buf( 1 ) /*,recordmodelreset()*/;
  ...
  if( level >= 4 && filetype != IMAGE1 ) {
    switch( filetype ) {
      ...
      default: {
        /*    if (ismatch>192) {   // Model long matches directly, len 1024
    m.set(0, 8);
    return m.p();
  }	*/
        wordModel( m );
...
//OPTION_TRY_SHORTER_WORD				1024
//OPTION_USE_DICTIONARY				128
//OPTION_NORMAL_TEXT_FILTER			64
//OPTION_CAPITAL_CONVERSION			4
//
//long size;
...
Filetype detect( FILE *in, int n, Filetype type, int &info ) {
  ...
  for( int i = 0; i < n; ++i ) {
    ...
    //text and utf-8 text
    if( txtStart == 0 && ( ( c < 128 && c >= 32 ) || c == 10 || c == 13 || c == 0x12 || c == 9 ) )
      txtStart = 1, txtOff = i;
    if( txtStart != 0 ) {
...
int decode_txt( Encoder &en, int size, FILE *out, FMode mode, int &diffFound ) {
  ...
  b = 0;
  while( b != -1 ) {
    b = wrt.WRT_decode_char( dtmp, NULL, 0 );
    //printf("%c",b);
    if( mode == FDECOMPRESS ) {
      fputc( b, out );
      printf( "%c", b );
...
void compressRecursive( FILE *in, long n, Encoder &en, char *blstr, int it = 0, int s1 = 0, int s2 = 0 ) {
  static const char *typenames[11] = {"default", "jpeg", "hdr", "1b-image", "8b-image", "24b-image",
                                      "audio",   "exe",  "cd",  "text",     "utf-8"};
  ...
  while( n > 0 ) {
    ...
    if( len > 0 ) {
      ...
      printf( "\n" );
      if( type == EXE || type == CD || type == IMAGE24 || type == TEXT || type == TXTUTF8 ) {
