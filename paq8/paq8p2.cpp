/* paq8p2 file compressor/archiver.  Release by Jan Ondrus, Mar. 2, 2009
...
- To install, put paq8p2.exe or a shortcut to it on your desktop.
- To compress a file or folder, drop it on the paq8p2 icon.
- To decompress, drop a .paq8p2 file on the icon.

A .paq8p2 extension is added for compression, removed for decompression.
The output will go in the same folder as the input.

While paq8p2 is working, a command window will appear and report
...
- To install, put paq8p2.exe somewhere in your PATH.
- To compress:      paq8p2 [-N] file1 [file2...]
- To decompress:    paq8p2 [-d] file1.paq8p2 [dir2]
- To view contents: more < file1.paq8p2

The compressed output file is named by adding ".paq8p2" extension to
the first named file (file1.paq8p2).  Each file that exists will be
...
If file1.paq8p2 exists then it is overwritten.

If the first named file ends in ".paq8p2" then it is assumed to be
...
The -d option forces extraction even if there is not a ".paq8p2"
...
During extraction, directories are created as needed.  For example:
  ...
  paq8p2 -4 c:\tmp\foo bar

compresses foo and bar (if they exist) to c:\tmp\foo.paq8p2 at level 4.

  paq8p2 -d c:\tmp\foo.paq8p2 .
...
There are 2 files: paq8p2.cpp (C++) and paq7asm.asm (NASM/YASM).
paq7asm.asm is the same as in paq7 and paq8x.  paq8p2.cpp recognizes the
...
Recommended compiler commands and optimizations:
  ...
  MINGW g++:
    ...
    g++ paq8p2.cpp -DWINDOWS -O2 -Os -s -march=pentiumpro -fomit-frame-pointer -o paq8p2.exe paq7asm.obj

  Borland:
    nasm paq7asm.asm -f obj --prefix _
    bcc32 -DWINDOWS -O -w-8027 paq8p2.cpp paq7asm.obj

  Mars:
    nasm paq7asm.asm -f obj --prefix _
    dmc -DWINDOWS -Ae -O paq8p2.cpp paq7asm.obj

  UNIX/Linux (PC):
    nasm -f elf paq7asm.asm
    g++ paq8p2.cpp -DUNIX -O2 -Os -s -march=pentiumpro -fomit-frame-pointer -o paq8p2 paq7asm.o

  Non PC (e.g. PowerPC under MacOS X)
    g++ paq8p2.cpp -O2 -DUNIX -DNOASM -s -o paq8p2
...
Then
  ...
  paq8p2 archive \dir1\file1.txt \dir2

will create archive.paq8p2 with the header:
...
Decompression will fail if the first 7 bytes are not "paq8p2 -".  Sizes
...
paq8p2 uses a neural network to combine a large number of models.  The
...
paq8p2 uses preprocessing transforms on certain data types to improve
...
#define PROGNAME "paq8p2" // Please change this if you change the program.
...
// To compress to file1.paq8p2: paq8p2 [-n] file1 [file2...]
// To decompress: paq8p2 file1.paq8p2 [output_dir]
