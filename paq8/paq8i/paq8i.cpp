...
#define PROGNAME "paq8i" // Please change this if you change the program.
...
int contextModel2() {
  ...
  static Mixer m( 512, ( 16 + 256 * 3 + 512 * 13 ), 7, 128 );
  ...
  //m.set(buf(1)+8, 264);
  //m.set(c0, 256);
  m.set( order + 8 * ( c4 >> 5 & 7 ) + 64 * static_cast<int>( buf( 1 ) == buf( 2 ) ) + 128 * static_cast<int>( filetype == EXE ), 256 );
  //m.set(buf(2), 256);
  ...
  c = bpos * 256 + ( ( c0 << ( 8 - bpos ) ) + ( c1 >> bpos ) & 255 );
  m.set( c, 2048 );
...
public:
  ...
  void compress( int c ) {
    ...
    if( level == 0 ) // level==0  -> store
      putc( c, archive );
    else
      for( int i = 7; i >= 0; --i )
        code( ( c >> i ) & 1 ); // Encode every bit by bitwise arithmetic encoding
  }
...
// change the behavior of decode().  It is const to prevent some errors but
...
protected:
  ...
  virtual void encode( FILE *f, int n ) = 0; // user supplied
...
int main( int argc, char **argv ) {
  ...
  if( f == nullptr ) {
    ...
    // Detect file types and make map of it
    ...
    // Write filesize, filename for each file in archive header.
  ...
  while( ( filename = getline( f ) ) != 0 ) {
    ...
    if( mode == COMPRESS ) {
      ...
      filetype = 0;                                 // BMP image = 0, no preprocessing
      Filter *fp = Filter::make( filename, &en );   // sets filetype
      fp->compress( fi, size );                     // Actual compression
      printf( " -> %4ld   \n", ftell( f ) - body ); // Report on compressed size
      delete fp;
