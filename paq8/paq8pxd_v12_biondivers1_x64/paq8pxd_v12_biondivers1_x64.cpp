/* paq8pxd12 file compressor/archiver.  Release by Kaido Orav, Jul. 08, 2014
...
- To install, put paq8pxd_v12.exe or a shortcut to it on your desktop.
- To compress a file or folder, drop it on the paq8pxd12 icon.
- To decompress, drop a .paq8pxd12 file on the icon.

A .paq8pxd12 extension is added for compression, removed for decompression.
The output will go in the same folder as the input.

While paq8pxd12 is working, a command window will appear and report
...
- To install, put paq8pxd_v12.exe somewhere in your PATH.
- To compress:      paq8pxd_v12 [-N] file1 [file2...]
- To decompress:    paq8pxd_v12 [-d] file1.paq8pxd12 [dir2]
- To view contents: paq8pxd_v12 -l file1.paq8pxd12

The compressed output file is named by adding ".paq8pxd12" extension to
the first named file (file1.paq8pxd12).  Each file that exists will be
...
If file1.paq8pxd12 exists then it is overwritten. Level -0 only
transforms or decompresses data.

If the first named file ends in ".paq8pxd12" then it is assumed to be
...
The -d option forces extraction even if there is not a ".paq8pxd12"
...
During extraction, directories are created as needed.  For example:
  ...
  paq8pxd_v12 -4 c:\tmp\foo bar

compresses foo and bar (if they exist) to c:\tmp\foo.paq8pxd12 at level 4.

  paq8pxd_v12 -d c:\tmp\foo.paq8pxd12 .
...
There are 2 files: paq8pxd_v12.cpp (C++) and wrtpre.cpp (C++).
paq8pxd_v12.cpp recognizes the following compiler options:

  -DWINDOWS           (to compile in Windows)
  -DUNIX              (to compile in Unix, Linux, Solairs, MacOS/Darwin, etc)
  -DNOASM             (to replace train, dotproduct SIMD with equivalent C++)
...
Recommended compiler commands and optimizations:
  ...
  MINGW g++:
    g++ paq8pxd_v12.cpp -DWINDOWS -Ofast -msse2 -s -march=pentium4 -o paq8pxd_v12.exe 

  UNIX/Linux (PC):
    g++ paq8pxd_v12.cpp -DUNIX -Ofast -msse2 -s -march=pentium4 -o paq8pxd_v12

  Non PC (e.g. PowerPC under MacOS X)
    g++ paq8pxd_v12.cpp -O2 -DUNIX -DNOASM -s -o paq8pxd_v12
...
Then
  ...
  paq8pxd_v12 archive \dir1\file1.txt \dir2

will create archive.paq8pxd12 

The command:

  paq8pxd_v12 archive.paq8pxd12 C:\dir3
...
Decompression will fail if the first 10 bytes are not "paq8pxd12 -".  Sizes
...
paq8pxd_v12 uses a neural network to combine a large number of models.  The
...
paq8pxd_v12 uses preprocessing transforms on certain data types to improve
...
- RLE (MBR): Decodes RLE encoded images in .hlp files.
...
-wordmodel memory increase, dmcmodel memory decrease
-small fixes
-SSE2/SSSE3 version of ContextMap::Get
*/

#define PROGNAME "paq8pxd12" // Please change this if you change the program.
...
public:
  void alloc( U64 n ) { // report memory allocated, may be negative
    memused += n;
    if( memused > maxmem )
      maxmem = memused;
  }
...
U64 level = DEFAULT_OPTION; // Compression level 0 to 10
...
int blpos = 0, rm1 = 1; // Relative position in block
typedef enum {
  DEFAULT,
  JPEG,
  HDR,
...
#elif defined( __SSE2__ ) || defined( __SSSE3__ ) // faster
#  include <emmintrin.h>
...
public:
  ...
  void update2() {
    if( filetype == DICTTXT )
      train( &tx[0], &wx[0], nx, ( ( y << 12 ) - base ) * 3 / 2 ), nx = base = ncxt = 0;
    else
      update();
  }
...
inline int mix2( Mixer &m, int s, StateMap &sm ) {
  int p1 = sm.p( s ); //(filetype==DICTTXT)
  int n0 = -!nex( s, 2 );
  int n1 = -!nex( s, 3 );
  int st = stretch( p1 ) >> 2;
  m.add( st );
...
inline U64 CMlimit( U64 size ) {
  if( size > 0x7FFFFFFF + 1 )
    return 0x7FFFFFFF + 1; //limit to 2GB power of 2 (era 7)
  return ( size );
}
...
int matchModel( Mixer &m ) {
  ...
  static Array<U64> t( CMlimit( MEM_() ) ); // hash table of pointers to contexts
...
void wordModel( Mixer &m ) {
  ...
  static ContextMap cm( CMlimit( MEM_() * 16 ), 44 );
  ...
  if( bpos == 0 ) {
    ...
    if( ( c >= 'a' && c <= 'z' ) || c == 1 || c == 2 || ( c >= 128 && ( b2 != 3 ) ) ) {
      ...
      if( word0 ) {
        ...
        ccword = 0;
        word0 = wordlen = 0;
        if( ( c == '.' || c == '!' || c == '?' || c == '}' || c == ')' ) && buf( 2 ) != 10 && filetype != EXE )
          f = 1;
      }
      if( ( c4 & 0xFFFF ) == 0x3D3D )
    ...
    if( ( c4 & 0xff ) != 0 ) {
      if( isalpha( c4 & 0xff ) )
        fl = 1;
      else if( ispunct( c4 & 0xff ) )
        fl = 2;
      else if( isspace( c4 & 0xff ) )
        fl = 3;
      else if( ( c4 & 0xff ) == 0xff )
        fl = 4;
      else if( ( c4 & 0xff ) < 16 )
        fl = 5;
      else if( ( c4 & 0xff ) < 64 )
...
void im1bitModel( Mixer &m, int w ) {
  ...
  const int N = 8;               // number of contexts
...
public:
  recordModel1( U64 msize = CMlimit( MEM_() ) ) :
      cpos1( 256 ),
      cpos2( 256 ),
      cpos3( 256 ),
      cpos4( 256 ),
...
void sparseModel( Mixer &m, int seenbefore, int howmany ) {
  static ContextMap cm( CMlimit( MEM_() * 2 ), 40 + 2 );
  if( bpos == 0 ) {
    cm.set( seenbefore );
    cm.set( howmany );
...
void distanceModel( Mixer &m ) {
  static ContextMap cr( CMlimit( MEM_() ), 3 );
  if( bpos == 0 ) {
...
public:
  ...
  void p( Mixer &m ) {
    if( !bpos ) {
      ...
      // if(abs(errr-pNWNW)>255)id8++; //if total (id9/id8)<4 should use model commented out below
      // else id9++;
...
public:
  ...
  int p( Mixer &m ) {
        ...
        if( rs >= 0 ) {
          if( huffsize + ( rs & 15 ) == huffbits ) { // done decoding
            ...
            // UPDATE_ADV_PRED !!!!
            {
              ...
              for( int i = 0; i < 8; ++i )
                x -= ( zzu[zz] < i ) * sumu[i] + ( zzv[zz] < i ) * sumv[i];
              x /= ( qtab[q + zz] + 1 ) * 181;
              if( zz == 0 )
                x -= cbuf2[cpos_dc - ls[acomp]];
    ...
    switch( hbcount ) {
      case 0:
        for( int i = 0; i < N; ++i )
          cp[i] = t[cxt[i]] + 1, m1.add( stretch( sm[i].p( *cp[i] ) ) );
        break;
      case 1: {
        int hc = 1 + ( huffcode & 1 ) * 3;
        for( int i = 0; i < N; ++i )
          cp[i] += hc, m1.add( stretch( sm[i].p( *cp[i] ) ) );
      } break;
      default: {
        int hc = 1 + ( huffcode & 1 );
        for( int i = 0; i < N; ++i )
...
public:
  wavModel1( int wi ) :
    ...
    recModel = 0;
    if( rlen > 1 )
      recModel = new recordModel1( CMlimit( MEM_() ) );
  }
  void p( Mixer &m ) {
...
void exeModel( Mixer &m ) {
  const int N = 14;
  static ContextMap cm( CMlimit( MEM_() ), N );
  if( !bpos ) {
    for( int i = 0; i < N; ++i )
      cm.set( execxt( i + 1, buf( 1 ) * ( i > 6 ) ) );
  }
  cm.mix( m );
}
...
void indirectModel( Mixer &m ) {
  static ContextMap cm( CMlimit( MEM_() ), 9 + 3 );
  static U32 t1[256];
...
void dmcModel( Mixer &m ) {
  ...
  static Array<DMCNode> t( CMlimit( MEM_() ) ); // state graph
  static StateMap sm;
...
void nestModel( Mixer &m ) {
  ...
  static ContextMap cm( CMlimit( MEM_() / 2 ), 14 - 4 );
  // static U32 mask = 0;
  if( bpos == 0 ) {
...
public:
  sparseModelx() :
      ...
      scm6( 0x2000 ),
      scma( 0x10000 ) {}
  void p( Mixer &m, int seenbefore, int howmany ) {
    if( bpos == 0 ) {
      scm5.set( seenbefore );
      scm6.set( howmany );
    ...
    cm.set((x4&0x80f0f0ff));*/
      ...
      scm1.set( words & 127 );
      scm2.set( ( words & 12 ) * 16 + ( w4 & 12 ) * 4 + ( f4 & 0xf ) );
      scm3.set( w4 & 15 );
...
int normalModel( Mixer &m ) {
  static ContextMap cm( CMlimit( MEM_() * 16 ), 9 );
  static RunContextMap rcm7( CMlimit( MEM_() / 4 ) ), rcm9( CMlimit( MEM_() / 4 ) ), rcm10( CMlimit( MEM_() / 2 ) );
  ...
  if( bpos == 0 ) {
    ...
    if( ( buf( 2 ) == '.' || buf( 2 ) == '!' || buf( 2 ) == '?' || buf( 2 ) == '}' ) && buf( 3 ) != 10
        && ( filetype == DICTTXT || filetype == TXTUTF8 || filetype == TEXT ) )
...
int contextModel2() {
  static Mixer m( 925, 3095 + 256 * 7 + 256 * 8 + 256 * 7 + 2048 + 256 * 3 - 264, 7 );
  ...
  if( filetype == IMAGE1 )
    return im1bitModel( m, finfo ), m.p();
  if( filetype == IMAGE8 )
    return im8bitModel->p( m ), m.p();
  if( filetype == MRBR )
    return im8bitModel->p( m ), m.p();
  if( filetype == IMAGE24 )
    return im24bitModel->p( m ), m.p();
  if( filetype == AUDIO )
    return wavModel->p( m ), m.p();
  if( filetype == JPEG )
    if( jpegModel->p( m ) )
      return m.p();
  ...
  U8 d = c0 << ( 8 - bpos );
  if( filetype == EXE ) {
    m.set( order + 8 * ( c4 >> 6 & 3 ) + 32 * ( bpos == 0 ) + 64 * ( c1 == c2 ) + ( c2 == 0x8b || c2 == 0x89 ) * 128,
           256 ); //8b 89 for mov
    m.set( c3, 256 );
  } else {
  ...
  if( bpos ) {
    ...
    c = c3 / 128 + ( c4 >> 31 ) * 2 + 4 * ( c2 / 64 ) + ( c1 & 240 );
  m.set( c, 1536 );
  return m.p();
}
...
Encoder::Encoder( Mode m, FILE *f ) : mode( m ), archive( f ), x1( 0 ), x2( 0xffffffff ), x( 0 ), alt( 0 ) {
  ...
  if( level >= 4 && recordModel == 0 )
    recordModel = new recordModel1(); //glogal recordmodel
}
...
#define MRBRLE_DET( start_pos, header_len, data_len, width, height )                                                   \
  return dett = ( MRBR ), deth = ( header_len ), detd = ( data_len ), info = ( width ), info2 = ( height ),            \
         fseeko( in, start + ( start_pos ), SEEK_SET ), HDR
...
Filetype detect( FILE *in, uint64_t n, Filetype type, int &info, int &info2, int it = 0 ) {
  ...
  uint64_t mrb = 0, mrb0 = 0, mrb1 = 0, mrbTell = 0;
  int mrbsize = 0, mrbcsize = 0, mrbPictureType = 0, mrbPackingMethod = 0, mrbTell1 = 0, mrbw = 0,
  ...
  for( uint64_t i = 0; i < n; ++i ) {
    ...
    //detect mrb images in .hlp files
    //only RLE encoded, one image only
    if( ( buf0 & 0xFFFF ) == 0x6c70 || ( buf0 & 0xFFFF ) == 0x6C50 )
      mrb = i;
    /*if (mrb) {
        const int p=i-mrb;
        if (p==1 && c!=1) mrb=0;
        else if (p==7) mrb=c==5?mrb:0,mrbPictureType=c;
        else if (p==8) mrb=c==1?mrb:0,mrbPackingMethod=c;
        else if (p==10) 
    }*/
    if( mrb && ( i - mrb ) == 1 && c > 1 && c != 0 ) { //cant handle more then one image
      //if (c>=2) mrb1=mrb;
      mrb = 0; //if not 1 image per/file
    }
    //if (!mrb && mrb1 && (i-mrb1)==6 ) mrbTell1=bswap(buf0);
    if( mrb && ( i - mrb ) == 7 && ( c == 5 || c == 6 || c == 8 ) ) //       5=DDB   6=DIB   8=metafile
      mrbPictureType = c;
    if( mrb && ( i - mrb ) == 8 && ( /*c==0 ||*/ c == 1 /*|| c==2 || c==3*/ ) ) //      0=uncomp 1=RunLen 2=LZ77 3=both
      mrbPackingMethod = c;
    if( mrb && ( i - mrb ) == 10 ) {
      if( ( mrbPictureType == 6 || mrbPictureType == 5 )
          && ( mrbPackingMethod == 1 /*|| mrbPackingMethod==2 || mrbPackingMethod==3*/ ) ) {
        //save ftell
        mrbTell = ftello( in ) - 2;
        ...
        if( BitCount != 8 || mrbsize < 100 || mrbw > 3000 || mrbw > 3000 ) {
        ...
        CompressedOffset = bswap( CompressedOffset );
        HotspotOffset = bswap( HotspotOffset );
        mrbsize = mrbcsize + ftello( in ) - mrbTell + 10; // ignore HotspotSize
        if( mrbsize < 100 || mrbw > 3000 || mrbw > 3000 ) {
          mrbPictureType = mrb = mrbsize = 0;
          mrbTell = mrbTell + 2;
          fseeko( in, mrbTell, SEEK_SET );
        }
    ...
    /* //multiple image
    if (mrb1 && mrbTell1 &&  (i-mrb1)==(mrbTell1+1-2) && (c==5 || c==6 || c==8)) 
        mrbPictureType=c;
    if (mrb1 && mrbTell1 && (i-mrb1)==(mrbTell1+2-2) && (c==0 || c==1 || c==2 || c==3)) //                       0=uncomp 1=RunLen 2=LZ77 3=both
        mrbPackingMethod=c;
    if (mrb1 && mrbTell1 && (i-mrb1)==(mrbTell1+4-2)){
        if ((mrbPictureType==6 || mrbPictureType==5) && (mrbPackingMethod==1 || mrbPackingMethod==2 || mrbPackingMethod==3))
    {
        //save ftell
        mrbTell=ftell(in)-2;
    ...
    mrbw=GetCDWord(in);
        ...
        if (mrbsize<100 || mrbw>3000 || mrbw>3000) {//BitCount!=8 || 
            mrbPictureType=mrb=mrbsize=0;
            mrbTell=mrbTell+2;
            fseek(in,mrbTell,SEEK_SET);
        }
        ...
        else if (mrbPictureType==8 && (mrbPackingMethod==1 || mrbPackingMethod==2))
        {
            ...
            CompressedOffset=bswap(CompressedOffset);
            HotspotOffset=bswap(HotspotOffset);
            mrbsize=mrbcsize+ftell(in)-mrbTell+10; // ignore HotspotSize
            if ( mrbsize<100 || mrbw>3000 || mrbw>3000) {
            mrbPictureType=mrb=mrbsize=0;
            mrbTell=mrbTell+2;
            fseek(in,mrbTell,SEEK_SET);
    ...
    if( type == MRBR && ( mrb || mrb1 ) && ( mrbPictureType == 6 || mrbPictureType == 8 ) && mrbsize ) {
      //pgmsize=pgmw *pgmh +mrb+i-1;
      return fseeko( in, start + mrbsize, SEEK_SET ), DEFAULT;
    }
    if( ( mrb || mrb1 ) && ( mrbPictureType == 6 ) && mrbsize ) {
      if( mrb1 )
        mrb = mrb1;
    ...
    /*if (buf0==0x88F027D1 && buf1==0x4B57414A) fKWAJ=i;
    if (fKWAJ && (i-fKWAJ) ==1){     // header has no file size
        switch (c){
            case 2:{
                printf("K SZDD\n");
                break;
                }
            case 3:{printf("K LZ + Huff\n");
                break;
                           }
            case 4:{printf("K MS-ZIP\n");
                break;
                           }
        default:{
...
void printStatus( uint64_t n, uint64_t size ) {
  if( level > 0 )
    printf( "%6.3f%%\b\b\b\b\b\b\b", float( 100 ) * n / ( size + 1 ) ), fflush( stdout );
}
...
void encode_mrb( FILE *in, FILE *out, int len, int width, int height ) {
  ...
  int totalSize = ( width + pad ) * height;
  printf( " - RLE" );
  U32 count = 0;
  U8 value = 0;
  // RLE decode
...
int encodeRLE( U8 *dst, const U8 *src_start, const U8 *src_end ) {
  const U8 *ptr;
  int i = 0;
  for( ptr = src_start; ptr < src_end; ptr++ ) {
    if( ptr[0] != ptr[1] || ptr[1] != ptr[2] ) {
      ...
      const U8 *ptr1;
      for( ptr1 = ptr + 1; ptr1 < src_end; ptr1++ )
        if( ( ptr1[0] == ptr1[1] && ptr1[2] == ptr1[0] ) || ptr1 - ptr >= 127 )
          break;
      int pixels = ptr1 - ptr;
      *dst++ = 0x80 | pixels;
      i++;
...
int decode_mrb( Encoder &en, int size, int width, FILE *out1, FMode mode, uint64_t &diffFound ) {
  //U32 outlen=0;
  U8 *ptr, *fptr;
  ptr = ( U8 * ) calloc( size, 1 );
  if( !ptr )
    quit( "Out of memory (e_MBR)" );
  fptr = ( U8 * ) calloc( size, 1 );
  if( !fptr )
    quit( "Out of memory (e_MBR)" );

  //decompress file
  for( int i = 0; i < size; i++ ) {
    fptr[i] = en.decompress();
  }
  int msize = encodeRLE( ptr, fptr, fptr + size );
  programChecker.alloc( size * 2 );

  //Write out or compare
  if( mode == FDECOMPRESS ) {
    fwrite( &ptr[0], 1, msize, out1 );
  } else if( mode == FCOMPARE ) {
...
void direct_encode_block( Filetype type, FILE *in, uint64_t len, Encoder &en, uint64_t s1, uint64_t s2,
                          int info = -1 ) {
  printf( "\n" );
  ...
  if( info != -1 ) {
    ...
    finfo = info;
    if( im8bitModel != 0 )
      delete im8bitModel, im8bitModel = 0; //if model exists then delete it
    if( type == MRBR || type == IMAGE8 )
      im8bitModel = new im8bitModel1( info ); // create new model
    if( im24bitModel != 0 )
      delete im24bitModel, im24bitModel = 0; //if model exists then delete it
    if( type == IMAGE24 )
      im24bitModel = new im24bitModel1( info ); // create new model
    if( wavModel != 0 )
      delete wavModel, wavModel = 0; //if model exists then delete it
    if( type == AUDIO )
      wavModel = new wavModel1( info ); // create new model
                                        //if (type==MRBR) int pad=((info+3)/4)*4-info,finfo=info+pad;
  }
  if( !( type == MRBR || type == IMAGE8 ) && ( im8bitModel != 0 ) )
    delete im8bitModel, im8bitModel = 0; // if not 8 bit image type delete model if exists
  if( ( type == DICTTXT || type == TXTUTF8 ) && ( recordModelw == 0 ) )
    recordModelw = new recordModelx(); // create new model
  if( !( type == DICTTXT || type == TXTUTF8 ) && ( recordModelw != 0 ) )
    delete recordModelw, recordModelw = 0;
  if( ( type == DICTTXT || type == TXTUTF8 ) && ( sparseModel1 == 0 ) )
    sparseModel1 = new sparseModelx(); // create new model
  if( !( type == DICTTXT || type == TXTUTF8 ) && ( sparseModel1 != 0 ) )
    delete sparseModel1, sparseModel1 = 0;
  if( type != IMAGE24 && ( im24bitModel != 0 ) )
    delete im24bitModel, im24bitModel = 0; // if not 24 bit image type delete model if exists
  if( type != AUDIO && ( wavModel != 0 ) )
    delete wavModel, wavModel = 0;
  if( type == JPEG && jpegModel == 0 )
    pos = 0, jpegModel = new jpegModelx();
  if( type != JPEG && ( jpegModel != 0 ) )
    delete jpegModel, jpegModel = 0;
...
uint64_t typenamess[15][5] = {0}; //total type size for levels 0-5
U32 typenamesc[15][5] = {0};      //total type count for levels 0-5
...
void compressRecursive( FILE *in, uint64_t n, Encoder &en, char *blstr, int it = 0, uint64_t s1 = 0, uint64_t s2 = 0 ) {
  static const char *typenames[15] = {"default",  "jpeg",      "hdr",    "1b-image", "4b-image",
                                      "8b-image", "24b-image", "audio",  "exe",      "cd",
                                      "text",     "utf-8",     "base64", "SZDD",     "MRBR"};
  ...
  while( n > 0 ) {
    ...
    if( len > 0 ) {
      ...
      if( type == EXE || type == CD || type == IMAGE24 || type == TEXT || type == TXTUTF8 || type == BASE64
        ...
        if( !tmp )
          perror( "compressRecursive tmpfile" ), quit();
        if( type == IMAGE24 )
        ...
        if( ( type == TEXT || type == TXTUTF8 ) && ( tmpsize < ( len - 256 ) ) )
        ...
        if( type == IMAGE24 )
          ...
          decode_mrb( en, int( tmpsize ), info, in, FCOMPARE, diffFound );
        else if( type == EXE )
...
uint64_t decompressRecursive( FILE *out, uint64_t size, Encoder &en, FMode mode, int it = 0, uint64_t s1 = 0,
                              uint64_t s2 = 0 ) {
  Filetype type;
  uint64_t len = 0L, i = 0;
  uint64_t diffFound = 0;
  ...
  while( i < size ) {
    ...
    if( type != JPEG && ( jpegModel != 0 ) )
      delete jpegModel, jpegModel = 0;
    //if (recordModel==0) recordModel=new recordModel1(); //glogal recordmodel

    if( type == IMAGE24 )
      len = decode_bmp( en, int( len ), info, out, mode, diffFound );
    else if( type == MRBR )
      len = decode_mrb( en, int( len ), info, out, mode, diffFound );
    else if( type == EXE )
      len = decode_exe( en, int( len ), out, mode, diffFound, int( s1 ), int( s2 ) );
    else if( type == DICTTXT )
...
// To compress to file1.paq8pxd12: paq8pxd_v12 [-n] file1 [file2...]
// To decompress: paq8pxd_v12 file1.paq8pxd12 [output_dir]
  ...
  try {
    ...
    if (argc>1 && argv[1][0]=='-' && argv[1][1] && !argv[1][2]) {
      if (argv[1][1]>='0' && argv[1][1]<='10')
        ...
        quit("Valid options are -0 through -10, -d, -l\n");
      --argc;
      ++argv;
      pause=false;
    }
    */
    ...
    if( argc < 2 ) {
      printf( "\n"
              "              paq8pxd12 archiver (C) 2008,2014, Matt Mahoney et al.\n"
              "               Free under GPL, http://www.gnu.org/licenses/gpl.txt\n"
              "                                Luca Biondi Version\n"
              "\n"
              "To compress:\n"
              "   paq8pxd12 -level file          (compresses to file.paq8pxd12)\n"
              "   paq8pxd12 -level arc files...  (creates arc.paq8pxd12)\n"
              "   paq8pxd12 file                 (level -10, pause when done)\n"
              "   paq8pxd12 -level arc dir       (creates arc.paq8pxd12 with dirname included)\n"
              "   paq8pxd12 -level arc dir/      (creates arc.paq8pxd12 with dirname excluded)\n"
              "\n"
              "To extract or compare:\n"
              "   paq8pxd12 -d dir1/archive.paq8pxd12        (extract to dir1)\n"
              "   paq8pxd12 -d dir1/archive.paq8pxd12 dir2   (extract to dir2)\n"
              "   paq8pxd12 archive.paq8pxd12                (extract, pause when done)\n"
              "\n"
              "To view contents: paq8pxd12 -l archive.paq8pxd12\n"
              "\n"
              "Levels: -0 = store  (39 MB)  | -4 = smaller  (184 MB) |  \n"
              "        -1 = faster (89 MB)  | -5 = smaller  (284 MB) |  \n"
              "        -2 = faster (95 MB)  | -6 = smaller  (491 MB) |  \n"
              "        -3 = faster (106 MB) | -7 = smaller  (926 MB) |  \n"
              "        -8  = smaller (1795 MB) \n"
              "        -9  = smaller (3535 MB) \n"
              "        -10 = smaller (7015 MB) \n"
              "        -11 = smallest \n"
              "\n"
              "      don't assign any -level to use -11\n"
              "\n"
              "Also you can drop a file/folder on the paq8pxd12 icon to compress or extract.\n"
    ...
    if( mode == DECOMPRESS ) {
      ...
      if( segment.hpos == 0 || segment.pos == 0 )
        printf( "Segment data not found." ), quit();
      ...
      if( fread( &segment[0], 1, segment.pos, archive ) < segment.pos )
        printf( "Segment data corrupted." ), quit();
    ...
    if( level >= 11 )
      buf.setsize( 0x20000000 ); //limit 256mb
    else
      buf.setsize( CMlimit( MEM_() * 8 ) );
    //buf.setsize(MEM*8);
    poswr = buf.size() - 1;
#ifdef DEBUG
    printf( "\n Buf size %d bytes)\n", poswr );
...
#  define atoll( S ) _atoi64( S )
    ...
    if( mode == COMPRESS ) {
      ...
      const char *typenames[15] = {"default", "jpeg", "hdr",  "1b-image", "4b-image", "8b-image", "24b-image", "audio",
                                   "exe",     "cd",   "text", "utf-8",    "base64",   "SZDD",     "MRBR"};
      U32 ttc;
      uint64_t tts;
