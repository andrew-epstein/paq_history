  ...
  int p( Mixer &m, int val1 = 0, int val2 = 0 ) {
    ...
    if( x.bpos == 0 ) {
      ...
      if( ( buf( 2 ) == '.' || buf( 2 ) == '!' || buf( 2 ) == '?' || buf( 2 ) == '}' ) && buf( 3 ) != 10
          && ( x.filetype == DICTTXT || x.filetype == TXTUTF8 || x.filetype == TEXT ) )
...
public:
  ...
  virtual ~Predictors() {
    ...
    if( exeModel != 0 )
      delete exeModel;
    if( recordModel != 0 )
      delete recordModel; //glogal recordmodel
    if( distanceModel != 0 )
      delete distanceModel; //glogal recordmodel
    if( sparseModel != 0 )
      delete sparseModel; //glogal recordmodel
    if( wordModel != 0 )
      delete wordModel; //glogal recordmodel
    if( indirectModel != 0 )
      delete indirectModel; //glogal recordmodel
    if( dmcModel != 0 )
      delete dmcModel; //glogal recordmodel
    if( nestModel != 0 )
      delete nestModel; //glogal recordmodel
    if( matchModel != 0 )
      delete matchModel; //glogal recordmodel
    if( normalModel != 0 )
      delete normalModel;
    if( im1bitModel != 0 )
      delete im1bitModel;
...
PredictorJPEG::PredictorJPEG() :
    pr( 2048 ), ismatch( 0 ), m( 925, 3095 + 256 * 7 + 256 * 8 + 256 * 7 + 2048 + 256 * 3 - 264, x, 5 ) {
  matchModel = new matchModel1( x );
...
PredictorIMG4::PredictorIMG4() :
    ...
    a5( 0x10000, x ),
    a6( 0x10000, x ) {
  //in models   mixer.input contextsize  mixer.num of context
  matchModel = new matchModel1( x ); // 3      0               0
  recordModel = new recordModel1( x );
  normalModel = new normalModel1( x );
}

void PredictorIMG4::setmixer() {
  U32 c1 = x.buf( 1 ), c2 = x.buf( 2 ), c3 = x.buf( 3 ), c;
  m.set( c1 + 8, 264 );
  m.set( x.c0, 256 );
  c = c2;
...
PredictorIMG8::PredictorIMG8() :
    ...
    order( 0 ),
    ismatch( 0 ),
    m( 3 + 6 + 45 * 2 + 1, 8 + 8 + 32 + 256 + 512 + 1024 + 1024, x, 6 ),
    a( 256, x ),
    a1( 0x10000, x ),
    a2( 0x10000, x ),
    a3( 0x10000, x ),
  ...
  //in models   mixer.input contextsize  mixer.num of context
  matchModel = new matchModel1( x );   // 3      0               0
  im8bitModel = new im8bitModel1( x ); // 6+45*2 8+8+32+256+512+1024 6
}

void PredictorIMG8::setmixer() {
  U32 c1 = x.buf( 1 ), c2 = x.buf( 2 ), c3 = x.buf( 3 ), c;
  m.set( c1 + 8, 264 );
  m.set( x.c0, 256 );
  c = c2;
...
PredictorTXTWRT::PredictorTXTWRT() :
    ...
    a5( 0x10000, x ),
    a6( 0x10000, x ) {
  if( level >= 4 && recordModelw == 0 )
    recordModelw = new recordModelx( x );
  if( level >= 4 && sparseModel1 == 0 )
...
PredictorIMG1::PredictorIMG1() : pr( 2048 ), m( 8 + 3, 256 * 4, x, 4 ) {
  matchModel = new matchModel1( x );   // 3 0
  im1bitModel = new im1bitModel1( x ); // 8 256*4
}
...
Encoder::Encoder( Mode m, FILE *f, Predictors &predict ) :
  ...
  // predictor=0;
  // if (level>0) blockdata=new BlockData();
  // if (level>0) predictor=new Predictor(blockdata);
...
void DetectRecursive( FILE *in, uint64_t n, Encoder &en, char *blstr, int it = 0, uint64_t s1 = 0, uint64_t s2 = 0 ) {
  ...
  while( n > 0 ) {
    ...
    if( len > 0 ) {
      ...
      if( type == EXE || type == CD || type == IMAGE24 || type == TEXT || type == TEXT0 || type == TXTUTF8
        ...
        if( ( diffFound || fgetc( tmp ) != EOF ) ) {
          ...
          } else if( ( type == TEXT || type == TEXT0 || type == TXTUTF8 ) ) {
            //if (tmpsize<(len-256)) //if WRT is smaller then original block
            //   direct_encode_blockstream(DICTTXT, tmp, tmpsize, en, s1, s2);
            //else {// encode as text
            fseeko( in, begin, SEEK_SET );
  ...
  try {
    ...
    if( argc > 1 && argv[1][0] == '-' && argv[1][1] && strlen( argv[1] ) <= 3 ) {
      ...
      if( argv[1][1] == 'd' && !argv[1][2] )
        ...
        level = ( ( ~atol( argv[1] ) ) + 1 );
#ifdef MT
      else if( argv[1][1] >= '0' && argv[1][1] <= '9' && ( argv[1][3] <= '9' && argv[1][3] > '0' )
               && strlen( argv[1] ) == 4 ) {
        topt = argv[1][3] - '0';
        level = argv[1][1] - '0';
    ...
    if( mode == DECOMPRESS ) {
      ...
      if( fread( &filestreamsize[0], 1, 8 * 10, archive ) < 8 * 10 )
...
#  endif
      ...
      fwrite( &filestreamsize[0], 1, 8 * 10, archive );
