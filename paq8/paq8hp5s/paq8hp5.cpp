...
public:
  ...
  int mix(Mixer& m) {return mix1(m, c0, buf(1), y);}
};
...
void sparseModel(Mixer& m) {
  ...
  static SmallStationaryContextMap scm1(0x20000), scm2(0x20000), scm3(0x20000),
     scm4(0x20000), scm5(0x20000), scm6(0x20000), scm7(0x20000), scm8(0x20000), scm9(0x20000);
  static U32 t1[256];
...
static U32 WRT_map[16]= { 0, 16, 32, 48, 64, 80, 96, 112, 128, 128, 128, 128, 128, 192, 192, 224 };
		...
		if (c1==10 || c1==32) c1=16; c1a=WRT_map[c1>>4];
		if (c2==10 || c2==32) c2=16; c2a=WRT_map[c2>>4];

		c=c1a>>4; if(c>8) c=3+c/2;
		m.set(order + 8*(min(7,spacecount/4)) + 64*c, 1024-320); /////+128*(filetype==EXE), 256);

		c=c2a/64;	if(bpos>1)  c=c0>>(bpos-2);
		c=c1*8+c;
		m.set(c, 2048);

		c=c0*4+(c1a/64); if(bpos<4) c=(c&1022) + (c2/128);
		m.set(c, 1024);
