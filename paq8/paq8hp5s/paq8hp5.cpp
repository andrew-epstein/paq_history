...
int matchModel( Mixer &m ) {
  ...
  static Array<int> t( MEM * 4 ); // hash table of pointers to contexts
...
int contextModel2() {
  ...
  if( c > 8 )
    c = 3 + c / 2;
  m.set( order + 8 * ( min( 7, spacecount / 4 ) ) + 64 * c, 1024 - 320 ); /////+128*(filetype==EXE), 256);
  ...
  c = c1a / 32 + 8 * ( c2a / 32 ) + ( c3 & 128 ) + ( c4 >> 31 ) * 64;
  m.set( c, 256 );
...
void Predictor::update() {
  ...
  pr = contextModel2();
  int rate = 6 + ( pos > 2048 * 1024 ) + ( pos > 4096 * 1024 );
  ...
  pv = a5.p( pr, ( c0 * 16 ) ^ hash( pv, ps & 0xfc, pw ) & 0xffff, rate );
  pu = a4.p( pr1, c0 ^ hash( ps, pt, pu ) & 0xffff, rate );
  pt = a3.p( pr1, ( c0 * 4 ) ^ hash( ps, pt ) & 0xffff, rate );
  ps = a2.p( pr, c0 + 256 * ps, rate );
  pw = a6.p( pu, c0 + 256 * ( fails & 255 ), rate );
  pr = ps * 2 + pt * 8 + pu * 8 + pv * 6 + pw * 8 + 16 >> 5;
}
