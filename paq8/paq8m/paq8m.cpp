/* paq8m file compressor/archiver.  Release by Matt Mahoney, Aug. 4, 2007.
...
- To install, put paq8m.exe or a shortcut to it on your desktop.
- To compress a file or folder, drop it on the paq8m icon.
- To decompress, drop a .paq8m file on the icon.

A .paq8m extension is added for compression, removed for decompression.
The output will go in the same folder as the input.

While paq8m is working, a command window will appear and report
...
- To install, put paq8m.exe somewhere in your PATH.
- To compress:      paq8m [-N] file1 [file2...]
- To decompress:    paq8m [-d] file1.paq8m [dir2]
- To view contents: more < file1.paq8m

The compressed output file is named by adding ".paq8m" extension to
the first named file (file1.paq8m).  Each file that exists will be
...
If file1.paq8m exists then it is overwritten.

If the first named file ends in ".paq8m" then it is assumed to be
...
The -d option forces extraction even if there is not a ".paq8m"
...
During extraction, directories are created as needed.  For example:
  ...
  paq8m -4 c:\tmp\foo bar

compresses foo and bar (if they exist) to c:\tmp\foo.paq8m at level 4.

  paq8m -d c:\tmp\foo.paq8m .
...
There are 2 files: paq8m.cpp (C++) and paq7asm.asm (NASM/YASM).
paq7asm.asm is the same as in paq7 and paq8x.  paq8m.cpp recognizes the
...
Recommended compiler commands and optimizations:
  ...
  MINGW g++:
    ...
    g++ paq8m.cpp -DWINDOWS -O2 -Os -s -march=pentiumpro -fomit-frame-pointer -o paq8m.exe paq7asm.obj

  Borland:
    nasm paq7asm.asm -f obj --prefix _
    bcc32 -DWINDOWS -O -w-8027 paq8m.cpp paq7asm.obj

  Mars:
    nasm paq7asm.asm -f obj --prefix _
    dmc -DWINDOWS -Ae -O paq8m.cpp paq7asm.obj

  UNIX/Linux (PC):
    nasm -f elf paq7asm.asm
    g++ paq8m.cpp -DUNIX -O2 -Os -s -march=pentiumpro -fomit-frame-pointer -o paq8m paq7asm.o

  Non PC (e.g. PowerPC under MacOS X)
    g++ paq8m.cpp -O2 -DUNIX -DNOASM -s -o paq8m
...
Then
  ...
  paq8m archive \dir1\file1.txt \dir2

will create archive.paq8m with the header:
...
Decompression will fail if the first 7 bytes are not "paq8m -".  Sizes
...
paq8m uses a neural network to combine a large number of models.  The
...
paq8m uses preprocessing transforms on certain data types to improve
...
DIFFERENCES FROM PAQ8L

Added the improved JPEG model by Jan Ondrus from paq8fthis.  It fixes
a bug which caused paq8fthis to crash (division by 0) on some malformed
JPEG data.

*/

#define PROGNAME "paq8m" // Please change this if you change the program.
...
// Buf2 is like Buf but elements are type int rather than U8
class Buf2 {
  Array<int> b;
...
int ContextMap::mix1( Mixer &m, int cc, int bp, int c1, int y1 ) {
  ...
  for( int i = 0; i < cn; ++i ) {
    ...
    result += mix2( m, cp[i] != nullptr ? *cp[i] : 0, sm[i] );
  }
  if( bp == 7 )
    cn = 0;
  return result;
...
// Discrete cosine transform, inverse if inv is true
inline void dct_8x8( bool inv, int *input, int *output ) {
  int i, j, i2, acc;
  static int temp[64];
  static const int base[64] = {90,   90,   90,   90,  90,  90,   90,   90,   125, 106,  71,   24,  -24,
                               -71,  -106, -125, 118, 48,  -48,  -118, -118, -48, 48,   118,  106, -24,
                               -125, -71,  71,   125, 24,  -106, 90,   -90,  -90, 90,   90,   -90, -90,
                               90,   71,   -125, 24,  106, -106, -24,  125,  -71, 48,   -118, 118, -48,
                               -48,  118,  -118, 48,  24,  -71,  106,  -125, 125, -106, 71,   -24};
  if( inv ) {
    for( i = 0; i < 8; i++ ) {
      for( i2 = 0; i2 < 8; i2++ ) {
        acc = 0;
        for( j = 0; j < 8; j++ )
          acc += base[i2 + 8 * j] * input[i + j * 8];
        temp[i + i2 * 8] = acc >> 8;
      }
    }
    for( i = 0; i < 8; i++ ) {
...
int jpegModel( Mixer &m ) {
  ...
  static int cpos = 0, cpos_dc = 0;                      // position in cbuf
  ...
  static int adv_pred[3];
  static int LL1[64], LL2[64], LL3[64], LL4[64];
  static int input[64], output[64];
    ...
    if( buf( 4 ) == FF && buf( 3 ) == DQT ) {
      ...
      if( pos >= dqt_end ) {
        ...
        if( dqt_state % 65 == 0 )
          ...
          qtab[qnum * 64 + ( ( dqt_state % 65 ) - 1 )] = buf( 1 );
        }
        dqt_state++;
      }
    }
        ...
        for( int j = 0; j < nf; ++j ) {
          if( buf[sos + 2 * i + 5] == buf[sof + 3 * j + 10] ) { // Cs == C ?
            ...
            while( hv != 0 ) {
              ...
              jassert( tq >= 0 && tq < 4 );
              qmap[mcusize] = buf[tq]; // Quantizazion table mapping
              --hv;
              ++mcusize;
            }
          }
    ...
    if( (mcusize != 0) && buf( 1 + static_cast<int>( bpos == 0 ) ) != FF ) { // skip stuffed byte
      ...
      if( rs >= 0 ) {
        if( huffsize + ( rs & 15 ) == huffbits ) { // done decoding
          ...
          // UPDATE ADV_PRED
          //!!!!!!!!
          int xx = 0, compx = mcupos >> 6, q;
          jassert( compx >= 0 && compx < 10 );
          while( cpos_dc + xx < cpos )
            xx++;
          if( xx > 63 ) {
            jassert( xx == 64 );
            cpos_dc = cpos;
          ...
          int les = 0;
          {
            int poc_comp = mcusize >> 6;
            jassert( poc_comp > 0 && poc_comp <= 10 );
            les = mcusize;
            for( int a = 1; a < poc_comp; a++ ) {
              int k = ( poc_comp + compx + a ) % poc_comp;
              jassert( k >= 0 && k < 10 );
              jassert( compx >= 0 && compx < 10 );
              if( color[k] == color[compx] )
                les = ( poc_comp - a ) << 6;
            }
          }
          if( xx == 0 ) {
            //top
            for( q = 0; q < 64; q++ )
              input[zzu[q] + zzv[q] * 8] = ( qtab[64 * qmap[compx] + q] * cbuf2[cpos_dc + q - mcusize * width] ) << 8;
            dct_8x8( true, input, output ); // idct
            for( q = 0; q < 64; q++ )
              LL1[q] = output[q % 8 + 8 * 7] + ( output[q % 8 + 8 * 7] - output[q % 8 + 8 * 6] ) / 4;

            //left
            for( q = 0; q < 64; q++ )
              input[zzu[q] + zzv[q] * 8] = ( qtab[64 * qmap[compx] + q] * cbuf2[cpos_dc + q - les] ) << 8;
            dct_8x8( true, input, output ); // idct
            for( q = 0; q < 64; q++ )
              LL2[q] = output[( q / 8 ) * 8 + 7] + ( output[( q / 8 ) * 8 + 7] - output[( q / 8 ) * 8 + 6] ) / 4;
          }

          //normal
          for( q = 0; q < 64; q++ )
            input[zzu[q] + zzv[q] * 8] = ( q >= xx ? 0 : ( qtab[64 * qmap[compx] + q] * cbuf2[cpos_dc + q] ) << 8 );
          dct_8x8( true, input, output ); // idct
          for( q = 0; q < 64; q++ ) {
            LL3[q] = LL1[q] - output[q % 8];         //top
            LL4[q] = LL2[q] - output[( q / 8 ) * 8]; //left
          }

          //adv_pred1
          dct_8x8( false, LL3, output ); // dct
          int qq = 64 * qmap[compx] + xx;
          jassert( qq < 256 );
          qq = qtab[qq];
          jassert( qq > 0 );
          adv_pred[0] = ( output[zzu[xx]] / qq ) >> 8;

          //adv_pred2
          dct_8x8( false, LL4, output ); //dct
          adv_pred[1] = ( output[zzv[xx] * 8] / qq ) >> 8;

          if( xx == 0 ) { // DC
            adv_pred[0] = adv_pred[0] - cbuf2[cpos_dc - les];
            adv_pred[1] = adv_pred[1] - cbuf2[cpos_dc - les];
          };
          adv_pred[2] = ( adv_pred[0] + adv_pred[1] ) / 2;

          for( q = 0; q < 3; q++ ) {
            if( abs( adv_pred[q] ) < 4 )
              adv_pred[q] = adv_pred[q];
            else if( abs( adv_pred[q] ) < 16 )
              adv_pred[q] = ( adv_pred[q] >> 1 ) << 1;
            else if( abs( adv_pred[q] ) < 64 )
              adv_pred[q] = ( adv_pred[q] >> 2 ) << 2;
            else if( abs( adv_pred[q] ) < 256 )
              adv_pred[q] = ( adv_pred[q] >> 4 ) << 4;
            else
              adv_pred[q] = ( adv_pred[q] >> 8 ) << 8;
          }
          //!!!!!!!!!!!
        }
      }
  ...
  if( hbcount == 0 ) {
    ...
    /*
      ...
      */
    cxt[1] = hash( ++n, hc, rs1, adv_pred[0], comp );
    cxt[2] = hash( ++n, hc, rs1, adv_pred[1], comp );
    cxt[3] = hash( ++n, hc, rs1, adv_pred[2], comp );
...
void Predictor::update() {
  static APM a( 256 ), a1( 0x10000 ), a2( 0x10000 ), a3( 0x10000 ), a4( 0x10000 ), a5( 0x10000 ), a6( 0x10000 );
...
Filetype detect( FILE *in, int n, Filetype type ) {
  ...
  for( int i = 0; i < n; ++i ) {
    ...
    if( i >= 3 && ( buf0 & 0xfffffff0 ) == 0xffd8ffe0 )
      soi = i;
    if( (soi != 0) && i - soi < 0x10000 && ( buf1 & 0xff ) == 0xff && ( buf0 & 0xff0000ff ) == 0xc0000008 )
      sof = i;
...
// To compress to file1.paq8m: paq8m [-n] file1 [file2...]
// To decompress: paq8m file1.paq8m [output_dir]
