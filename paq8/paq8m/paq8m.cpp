  ...
  // Decode Huffman
  {
    if (mcusize && buf(1+(!bpos))!=FF) {  // skip stuffed byte
	  ...
	  if (xx == 0) {
            ...
            for ( q = 0; q < 64; q++) 
              input[zzu[q]+zzv[q]*8] 
                = (qtab[64*qmap[compx]+q] * cbuf2[cpos_dc+q-mcusize*width]) << 8;
            ...
            for ( q = 0; q < 64; q++)
              input[zzu[q]+zzv[q]*8] 
                = (qtab[64*qmap[compx]+q] * cbuf2[cpos_dc+q-les]) << 8;
            dct_8x8(true, input, output); // idct
            for ( q = 0; q < 64; q++)
          ...
          for ( q = 0; q < 64; q++)
            input[zzu[q]+zzv[q]*8] 
              = (q >= xx?0:(qtab[64*qmap[compx]+q] * cbuf2[cpos_dc+q]) << 8);
          dct_8x8(true, input, output); // idct
          for ( q = 0; q < 64; q++) {
          ...
          for ( q = 0; q < 3; q++) {
            if (abs(adv_pred[q]) < 4) adv_pred[q] = adv_pred[q];
            else if (abs(adv_pred[q]) < 16) adv_pred[q] = (adv_pred[q] >> 1) << 1;
            else if (abs(adv_pred[q]) < 64) adv_pred[q] = (adv_pred[q] >> 2) << 2;
            else if (abs(adv_pred[q]) < 256) adv_pred[q] = (adv_pred[q] >> 4) << 4;
            else adv_pred[q] = (adv_pred[q] >> 8) << 8;
          }
          //!!!!!!!!!!!
