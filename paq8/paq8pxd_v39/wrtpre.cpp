...
bool is_ascii( unsigned char *s, int len ) {
  for( int i = 0; i < len; i++ ) {
    if( s[i] < 127 )
      return false;
  }
...
int XWRT_Encoder::WRT_detectFileType( int filelen ) {
  ...
  PRINT_DICT( ( "maxDict=%d allocatedMemory=%d hashTable=%d\n", dictionary,
                dictionary * WORD_AVG_SIZE + sizeof( unsigned char * ) * ( dictionary + 1 )
                    + sizeof( unsigned char ) * ( dictionary + 1 ) + sizeof( int ) * ( dictionary + 1 ),
