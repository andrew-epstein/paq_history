...
inline int mix2(Mixer& m, int s, StateMap& sm) {
 ...
 if (cxtfl) {
  m.add(st/4);  //firm
  int p0=4095-p1;
  m.add((p1-p0)*3/64);		//	1/32 - net, 1/16 - net
  m.add(st*(n1-n0)*3/16);	//f
  m.add(((p1&n0)-(p0&n1)) /16);	//	3/64-net, 5/64-net
  m.add(((p0&n0)-(p1&n1))*7/64);//	6/64~=7/64
  return s>0;
 }
  m.add(st*9/32);		//	11/32 - net
  m.add(st*(n1-n0)*3/16);	//f
  int p0=4095-p1;
  m.add(((p1&n0)-(p0&n1)) /16);	//	3/64-net, 5/64-net
  m.add(((p0&n0)-(p1&n1))*7/64);//	6/64~=7/64
...
void sparseModel(Mixer& m) {
  ...
  static SmallStationaryContextMap scm1(0x20000,16), scm2(0x20000,12), scm3(0x20000,12),
				   scm4(0x20000,13), scm5(0x10000,12), scm6(0x20000,12),
				   scm7(0x2000 ,12), scm8(0x8000 ,13), scm9(0x1000 ,12);
...
int contextModel2() {
  ...
  static Mixer m(448, 128*(16+14+14+12+14+16), 6, 512);
...
//m.tx[430]=0;
		...
		m.set(256*bpos + (order>>1)*64 + (words&63), 512*4);
