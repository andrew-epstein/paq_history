...
#define OPTION_SPACE_AFTER_CC_FLAG 4
#define OPTION_TRY_SHORTER_WORD 8
#define OPTION_ADD_SYMBOLS_0_5 16
...
#define SAMPLE_WORDS_COUNT_MAX ( SAMPLE_WORDS_COUNT * CHARSET_COUNT )
  ...
  int WRT_detectFileType( FILE *file, int part_length, int parts, int &recordLen ) {
    ...
    langSum = 0;
    fc_max = 0;
    skip_first = 2000; // skip header (e.g. for Project Gutenberg)
    ...
    if( shortDict >= 0 )
      PRINT_DICT( ( "choosen short %s %d/%d\n", langName[shortDict], lang[shortDict],
                    part_length * parts / ( 112 * XML_MULT ) ) );

    if( longDict >= 0 )
      PRINT_DICT( ( "choosen long %s %d\n", langName[longDict], lang[longDict] ) );

    if( shortDict >= 0
        && ( lang[shortDict] < part_length * parts / ( 96 * XML_MULT ) || lang[shortDict] < 10 ) ) // =534, 538=trans
      shortDict = -1;
    ...
    if( ( longDictLen <= 0 && shortDictLen <= 0 )
        || langSum < part_length * parts / ( 171 * XML_MULT ) ) // ==300, 240=14329-8.txt (bad)
    ...
    if( EOLCount > 0 ) {
      if( IF_OPTION( OPTION_EOL_CODING ) && EOLCount > part_length * parts / 158 && EOLCount < part_length * parts / 10
          && EOLCountBad / EOLCount < 6 && spaceAfterLF > part_length * parts / 53 )
        TURN_ON( OPTION_SPACE_AFTER_EOL )
      else if( EOLCount < part_length * parts / 158 || EOLCount > part_length * parts / 10
               || EOLCountBad / EOLCount > 3 ) {
        if( !forceEOLcoding )
          TURN_OFF( OPTION_EOL_CODING );
      }
    } else
    ...
    PRINT_DICT( ( "EOL=%d+%d/%d(%d) bin=%d/%d(%d) punct=%d/%d(%d) lang=%d/%d(%d) cc=%d utf8=%d xml=%d\n", EOLCount,
                  EOLCountBad, part_length * parts / 158, IF_OPTION( OPTION_EOL_CODING ), binCount,
                  part_length * parts / 13, !IF_OPTION( OPTION_NORMAL_TEXT_FILTER ), punctCount, punctCountBad,
                  IF_OPTION( OPTION_WORD_SURROROUNDING_MODELING ), langSum, part_length * parts / ( 171 * XML_MULT ),
                  IF_OPTION( OPTION_USE_DICTIONARY ), IF_OPTION( OPTION_CAPITAL_CONVERSION ), IF_OPTION( OPTION_UTF8 ),
                  XML_MULT ) );
  ...
  void WRT_print_options() {
    ...
    PRINT_DICT( ( "OPTION_ADD_SYMBOLS_0_5=%d ", IF_OPTION( OPTION_ADD_SYMBOLS_0_5 ) ) );
