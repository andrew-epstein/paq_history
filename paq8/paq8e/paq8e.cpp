/* PAQ8E file compressor/archiver.
(C) 2006, Matt Mahoney under GPL, http://www.gnu.org/licenses/gpl.txt

PAQ8E is a file compressor and archiver.  To use:
...
PAQ8E never clobbers files.  If the file to be extracted already
exists, then PAQ8 compares it with the archived copy and reports
whether they differ.  Example:

  paq8e xyz foo bar      Compresses foo and bar to archive xyz.
  paq8e xyz baz          Extracts baz as a copy of foo, compares bar.
...
PAQ8E cannot update an archive.  You must create a new one instead.
...
TO COMPILE
  ...
  MINGW g++:
    ...
    g++ paq8e.cpp -O2 -Os -s -march=pentiumpro -fomit-frame-pointer -o paq8e.exe paq7asm.obj

  DJGPP g++:
    nasm paq7asm.asm -f coff --prefix _
    gxx paq8e.cpp -O2 -Os -s -march=pentiumpro -fomit-frame-pointer -o paq8e.exe paq7asm.obj

  Borland:
    nasm paq7asm.asm -f obj --prefix _
    bcc32 -O -w-8027 paq8e.cpp paq7asm.obj

  Mars:
    nasm paq7asm.asm -f obj --prefix _
    dmc paq8e.cpp paq7asm.obj

  UNIX/Linux (PC):
    nasm -f elf paq7asm.asm
    g++ paq8e.cpp -O2 -Os -s -march=pentiumpro -fomit-frame-pointer -o paq8e paq7asm.o

  Non PC (UNIX):
    g++ paq8e.cpp -O2 -DNOASM -s -o paq8e
...
#define PROGNAME "paq8e" // Please change this if you change the program.
  ...
  if( filename != nullptr ) {
    ...
    if( file != nullptr ) {
      if( fgetc( file ) == 'M' && fgetc( file ) == 'Z' )
        filetype = EXE;
      else if( (ext == nullptr) || ( (equals( ext, ".dbf" ) == 0) && (equals( ext, ".mdb" ) == 0) && (equals( ext, ".tar" ) == 0) ) ) {
...
int main( int argc, char **argv ) {
  ...
  if( argc < 2 ) {
    printf( PROGNAME " compressor/archiver (C) 2006, Matt Mahoney.\n"
                     ...
                     "Options are -0 (fast, 18MB) to -9 (slow, 4GB memory, best compr.), default: -4\n"
  ...
  if( f == nullptr ) {
    ...
    for( i = 0; i < filenames.size(); i++ ) {
      ...
      std::pair<int, std::string> p( ( 1 << (31 - 1) ) - ( filetype * 262144 + wrt.preprocFlag ), filenames[i] );
      filetypes.insert( p );
    }
