...
#define SAMPLE_WORDS_COUNT_MAX ( SAMPLE_WORDS_COUNT * CHARSET_COUNT )
  ...
  int WRT_detectFileType( FILE *file, int part_length, int parts, int &recordLen ) {
    ...
    if( ( longDictLen <= 0 && shortDictLen <= 0 ) || langSum < pp / ( 104 * XML_MULT )
        || langSum < 25 ) // ==492 (*2 bin), 494=ok(11317-8.txt)  1200=ok(CC)
    ...
    if( EOLCount > 0 ) {
      if( IF_OPTION( OPTION_EOL_CODING ) && EOLCount > pp / 158 && EOLCount < pp / 10 && EOLCountBad / EOLCount < 6
          && spaceAfterLF > pp / 53 )
        TURN_ON( OPTION_SPACE_AFTER_EOL )
      else if( EOLCount < pp / 158 || EOLCount > pp / 10 || EOLCountBad / EOLCount > 3 ) {
        if( !forceEOLcoding )
          TURN_OFF( OPTION_EOL_CODING );
      }
    } else
    ...
    PRINT_DICT( ( "EOL=%d+%d/%d(%d) bin=%d/%d(%d) punct=%d/%d(%d) lang=%d/%d(%d) cc=%d utf8=%d xml=%d\n", EOLCount,
                  EOLCountBad, pp / 158, IF_OPTION( OPTION_EOL_CODING ), binCount, pp / 13,
                  !IF_OPTION( OPTION_NORMAL_TEXT_FILTER ), punctCount, punctCountBad,
                  IF_OPTION( OPTION_WORD_SURROROUNDING_MODELING ), langSum, pp / ( 171 * XML_MULT ),
