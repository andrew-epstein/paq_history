    ...
    if( aiff != 0 ) {
      ...
      if( p == 12 && ( buf1 != 0x41494646 || buf0 != 0x434f4d4d ) )
        ...
        do {
          ...
          csize = getc( in ), csize <<= 8, csize |= getc( in ), csize <<= 8, csize |= getc( in ), csize <<= 8,
          csize |= getc( in );
          if( chunk[0] == 'S' && chunk[1] == 'S' && chunk[2] == 'N' && chunk[3] == 'D' )
            break;
          if( fseek( in, csize, SEEK_CUR ) != 0 )
            aiff = 0;
          j += ( csize + 8 );
        } while( ( aiff != 0 ) && j < 512 );
        if( ( aiff != 0 ) && j < 512 )
          AUD_DET( AUDIO, aiff - 3, 38 + j + ( 4 * 4 ), csize - 8, aiffm );
