...
static const U8 LB[256] = {
    0, 2, 1, 2, 0, 1, 2, 2, 1, 1, 2, 2, 2, 1, 2, 1, 0, 0, 1, 1, 2, 2, 0, 0, 1, 1, 2, 2, 0, 0, 0, 1, 1, 2, 2, 0, 0,
    0, 0, 1, 1, 2, 2, 0, 1, 0, 2, 0, 1, 1, 2, 2, 1, 1, 1, 2, 2, 2, 1, 1, 2, 2, 1, 1, 2, 2, 1, 1, 2, 2, 1, 1, 2, 2,
    1, 1, 2, 0, 1, 1, 2, 2, 0, 1, 2, 0, 1, 1, 2, 2, 0, 1, 2, 0, 1, 2, 0, 1, 2, 0, 1, 2, 0, 1, 2, 0, 1, 2, 0, 1, 2,
    2, 1, 2, 1, 1, 2, 0, 0, 1, 2, 0, 1, 2, 0, 1, 2, 0, 1, 2, 0, 1, 2, 0, 1, 2, 0, 1, 2, 2, 1, 2, 1, 1, 2, 2, 1, 2,
    1, 1, 2, 2, 1, 1, 2, 2, 1, 1, 2, 2, 1, 1, 2, 2, 1, 1, 2, 2, 1, 1, 2, 2, 1, 1, 2, 2, 1, 1, 2, 2, 1, 1, 2, 2, 1,
    1, 2, 2, 1, 1, 2, 2, 1, 1, 2, 2, 1, 1, 2, 2, 1, 1, 2, 2, 1, 1, 2, 2, 1, 1, 2, 2, 1, 1, 2, 2, 1, 1, 2, 1, 2, 1,
...
int stadd0( int x ) {
  if( x < 214 || rnd() <= statable[x].p0 )
    x = statable[x].s1;
  return x;
}
int stadd1( int x ) {
  if( x < 214 || rnd() <= statable[x].p1 )
    x = statable[x].s0;
  return x;
}

#  define nex( x, y ) ( y == 2 ? statable[x].n0 : ( y == 3 ? statable[x].n1 : ( y == 0 ? stadd0( x ) : stadd1( x ) ) ) )
...
public:
  ...
  U8 *operator[]( U32 i ) {
    ...
    for( int j = 0; j < M; j += 3 ) {
      ...
      if( p[1] < b )
        b = p[1], q = p; // best replacement so far
    }
    //  ++replaced;
    memset( q, 0, B );
    q[0] = chk;
...
public:
  int bh() { // return bit history
    *cp = nex01( *cp, y );
    cp = (bpos & 3) != 0 ? cp + ( (y + 1) << ( ( bpos & 3 ) - 1 ) ) : t[cxt ^ c0 * 9] + 1;
    return *cp;
  }
  NonstationaryContextMap( int m ) : cxt( 0 ), t( m / 16 ) {
    cp = t[0] + 1;
  }
  void set( U32 cx ) {
    cxt = cx;
  ...
  int mix( Mixer &m ) {
    ...
    m.add( ( (p0 & (static_cast<int>(-static_cast<int>(n1)) == 0)) ) - ( (p1 & (static_cast<int>(-static_cast<int>(n0)) == 0)) ) );
...
public:
  ...
  SmallNonstationaryContextMap( int m ) : cxt( 0 ), t( m / 2 ) {
    cp = &t[0];
  }
  void set( U32 cx ) {
...
int bmpModel( Mixer &m ) {
  ...
  if( (bpos == 0) && pos >= eoi && buf( 54 ) == 'B' && buf( 53 ) == 'M' && i4( 44 ) == 54 && i4( 40 ) == 40
    ...
    if( eoi - pos >= iwidth * height * 3 && eoi - pos <= ( iwidth + 3 ) * height * 3 && width >= 1 && height >= 1 ) {
...
int contextModel2() {
  ...
  m.set( buf( 1 ) + 8, 264 );
  m.set( c0, 256 );
  m.set( order + 8 * ( c4 >> 5 & 7 ) + 64 * static_cast<int>( buf( 1 ) == buf( 2 ) ), 128 );
  m.set( buf( 2 ), 256 );
  int pr = m.p();
  return pr;
}
...
int main( int argc, char **argv ) {
  // Check arguments
  if( argc < 2 ) {
    printf( LONG_PROGNAME " file compressor/archiver (based on PAQ 7)\n"
  ...
  // Report statistics
  const double elapsed_time = double( clock() - programChecker2.start_time() ) / CLOCKS_PER_SEC;
