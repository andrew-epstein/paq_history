/* fp8 file compressor/archiver.  Release by Jan Ondrus, May. 2, 2010
...
Recommended compiler commands and optimizations:
  ...
  MINGW g++:
    ...
    g++ fp8.cpp -DWINDOWS -O2 -Os -s -march=pentiumpro -fomit-frame-pointer -o fp8.exe paq7asm.obj

  Borland:
    nasm paq7asm.asm -f obj --prefix _
    bcc32 -DWINDOWS -O -w-8027 fp8.cpp paq7asm.obj

  Mars:
    nasm paq7asm.asm -f obj --prefix _
    dmc -DWINDOWS -Ae -O fp8.cpp paq7asm.obj

  UNIX/Linux (PC):
    nasm -f elf paq7asm.asm
    g++ fp8.cpp -DUNIX -O2 -Os -s -march=pentiumpro -fomit-frame-pointer -o fp8 paq7asm.o

  Non PC (e.g. PowerPC under MacOS X)
    g++ fp8.cpp -O2 -DUNIX -DNOASM -s -o fp8
...
public:
  ...
  explicit String( const char *s = "" ) : Array<char>( 1 ) {
    ( *this ) += s;
  }
};
...
public:
  explicit RunContextMap( int m ) : t( m / 4 ) {
    cp = t[0] + 1;
  }
...
public:
  explicit SmallStationaryContextMap( int m ) : t( m / 2 ), cxt( 0 ) {
...
public:
  explicit ContextMap( int m, int c = 1 ); // m = memory in bytes, a power of 2, C = c
  ~ContextMap();
...
inline U8 *ContextMap::E::get( U16 ch ) {
  ...
  return last = 0xf0 | bi, chk[bi] = ch, static_cast<U8 *>( memset( &bh[bi][0], 0, 7 ) );
}
...
int contextModel2() {
  ...
  if( bpos == 0 ) {
    ...
    ++blpos;
    if( size == -1 ) {
      ft2 = static_cast<Filetype>( buf( 1 ) );
    }
  ...
  static U32 word0 = 0, word1 = 0; // hashes
  ...
  if( bpos == 0 ) {
    ...
    cm.set( ismatch | buf( 2 ) << 8 );
    cm.set( ismatch | buf( 3 ) << 8 );
    cm.set( ( buf( 3 ) << 8 ) | buf( 2 ) );
    cm.set( ( buf( 4 ) << 8 ) | buf( 2 ) );
    cm.set( ( buf( 4 ) << 8 ) | buf( 3 ) );

    // indirect model
    U32 id = c4 & 0xffff, ic = id & 255;
    U32 &r1 = t1[id >> 8];
    r1 = r1 << 8 | ic;
    U16 &r2 = t2[c4 >> 8 & 0xffff];
    r2 = r2 << 8 | ic;
    const U32 t = ic | t1[ic] << 8;
    const U32 t0 = id | t2[id] << 16;
    cm.set( t & 0xff00 );
    cm.set( t0 & 0xff0000 );
    cm.set( t & 0xffff );
    cm.set( t0 & 0xffffff );
...
void ecc_compute( const U8 *src, U32 major_count, U32 minor_count, U32 major_mult, U32 minor_inc, U8 *dest ) {
  ...
  for( int i = 0; i < n; ++i ) {
    ...
    if( bmp != 0 ) {
      ...
      if( p == 12 ) {
        ...
        bmpy = abs( static_cast<int>( bswap( buf0 ) ) ), bmp = ( ( bmpy == 0 || bmpy > 0x10000 ) ? 0 : bmp ); //height
      } else if( p == 27 ) {
    ...
    if( tga != 0 ) {
      if( i - tga == 8 ) {
        ...
        if( tgaz == static_cast<int>( ( buf0 & 0xffff ) >> 8 ) && ( tgax != 0 ) && ( tgay != 0 ) ) {
          if( tgat == 1 ) {
...
int decompressRecursive( FILE *out, long size, Encoder &en, FMode mode, int it = 0, int s1 = 0, int s2 = 0 ) {
  ...
  s2 += size;
  while( i < size ) {
    type = static_cast<Filetype>( en.decompress() );
...
int main( int argc, char **argv ) {
  ...
  try {
    ...
    FILE *archive = nullptr;        // compressed file
