/* PAQ6-Emilcont-Febas - Experimental version by EmilCont - File archiver and compressor.
...
inline void SparseModel::model() {
  ...
  if( ch.bpos() == 0 ) {
    ...
    const int g = min( 255, int( ch.pos() - ch.pos( ch( 1 ), 1 ) ) ); // gap to prior ch1
    t7.update( hash( ch( 1 ), ch( 2 ), g ) );
    const int g2 = min( 255, int( ch.pos() - ch.pos( ch( 2 ), 1 ) ) ); // gap to prior ch1
    t8.update( hash( ch( 1 ), ch( 2 ), g2 ) );
  }
...
//////////////////////////// sparseModel2 EMILCONT////////////////////////////

// A SparseModel models several order-4
...
public:
  SparseModel2() : SIZE( ( MEM >= 0 ) * ( MEM + 15 - ( MEM >= 6 ) ) ), t0( SIZE ), t1( SIZE ), t2( SIZE ), t3( SIZE ) {}
  void model(); // Update and predict
};
