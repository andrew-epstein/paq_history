...
public:
  SparseModel2() : SIZE( ( MEM >= 0 ) * ( MEM + 14 ) ), t0( SIZE ), t1( SIZE ), t2( SIZE ), t3( SIZE ), t4( SIZE ) {}
  void model(); // Update and predict
};
...
inline void Predictor::update( int y ) {
  ...
  if( MEM >= 1 ) {
    context =
        ch( 0 ) * 8 + ( ch( 1 ) / 64 ) * 2 + ( ch.pos( 0, 3 ) < ch.pos( 32, 3 ) ) * ( _wlen == _clen ); // for SSE 64
    ssep = ssemap( nextp );
...
int main( int argc, char **argv ) {
  // Check arguments
  if( argc < 2 ) {
    printf( PROGNAME " v2 file compressor/archiver, (C) 2004, Matt Mahoney, mmahoney@cs.fit.edu\n"
