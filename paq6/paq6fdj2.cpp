...
/* PAQ6fdj - File archiver and compressor.
...
#define PROGNAME "PAQ6fdj"  // Please change this if you change the program
...
#define Top_value U64(0XFFFFFFFFFFFFFFFFull)	/* Largest code value */
/* HALF AND QUARTER POINTS IN THE CODE VALUE RANGE. */
#define First_qtr U64(Top_value/4+1)  /* Point after first quarter    */
#define Half	  U64(2*First_qtr)    /* Point after first half  */
#define Third_qtr U64(3*First_qtr)    /* Point after third quarter */
bit_byts in;
bit_byts out;
...
//  assert(sum>0);
  return (int)((float)(PSCALE-1)*n1/sum);
}
...
*/
  ...
  if (s0>0 && s1>0) {
    ...
    const U32 sy1=0xffffffff/sy+(rn&255) >> 8;
    const U32 s1 =0xffffffff/s +((rn>>8)&255) >> 8;
...
public:
  ...
  void write() {
    ...
    if (d>>1==ch()) {
      if (d & 1)
        mixer.write(0, t[cxt].n);
      else
        mixer.write(t[cxt].n, 0);
    }
  }
};
...
public:
  WordModel(): SIZE((MEM>=4)*(MEM+17-(MEM>=6))),
               t0(SIZE), t1(SIZE), t2(SIZE), t3(SIZE), t4(SIZE), t5(SIZE), t6(SIZE), t7(SIZE) {
    for (int i=0; i<N; ++i)
      cxt[i]=word[i]=0;
  }
  void model() {
    ...
    if (ch(4)==0xe8) {
      ...
      if (t[i].n>0) {
        ...
        if (ch(0)==ck && ch(1)==((r>>16)&0xff)) {
          if (y)
            n1=0xffff; //t[i].n*(ch.bpos()+23);
          else
            n0=0xffff; //t[i].n*(ch.bpos()+23);
        }
      }
	}
    ...
    if (ch(3)==0xe8) {
      ...
      if (t[i].n>0) {
        ...
        if (ch(0)==ck && ch(1)==((r>>8)&0xff)) {
          if (y)
            n1=0xffff; //t[i].n*(ch.bpos()+17);
          else
            n0=0xffff; //t[i].n*(ch.bpos()+17);
        }
      }
    }
...
private:
  ...
  U64 x1, x2;            // Range, initially [0, 1), scaled by 2^32
  U64 x;                 // Last 4 input bytes of archive.
  int  LPS;
public:
  ...
  void unflush();          // Call when done uncompressing
};
...
inline int input_bit(void)
{
   ...
   if (t == -2) {		/* Read the next byte if no */
      ...
      if (garbage_bits > 68 ) { /* after eof, but check */
	 fprintf(stderr, "**Bad  should not occurr** \n");
	 fprintf(stderr, "** archive file to short ** \n");
	 exit(-1);
      }
   }
   if (t < 0) {
...
// Constructor
Encoder::Encoder(Mode m, FILE* f): predictor(), mode(m), archive(f), x1(0ull),
                                   x2(0xffffffffffffffffull), x(0) {

  // In DECOMPRESS mode, initialize x to the first 4 bytes of the archive
  if (mode==DECOMPRESS) {
    in.ir(archive); /* connect to archive */
    x = 1;
...
void Encoder::unflush() {
  if (mode==DECOMPRESS) {
  if ((( x1 <= First_qtr ) && x == First_qtr ) ||
   ( x == Third_qtr )) printf(" archive seems ok ");
  else printf( " *** archive corrupted *** ");
   }
}
