...
/* PAQ602 - File archiver and compressor.
...
#define PROGNAME "PAQ602" // Please change this if you change the program
...
void Mixer::update( int y ) {
  ...
  if( s0 > 0 && s1 > 0 ) {
    ...
    for( int i = 0; i <= n; ++i ) {
      ...
      wt[c][i] = min( 0xfffff, max( 1, int( wt[c][i] + dw ) ) );
    }
  }
  n = -1;
}
  ...
  void model() override {
    if( ch.bpos() == 0 ) {
      ...
      if( ( isalpha( c ) != 0 ) || c >= 192 )
        word[0] ^= hash( word[0], tolower( c ) ) + 0x12345678;
      else {
        for( int i = N - 1; i > 0; i-- )
          word[i] = word[i - 1];
        word[0] = 0;
...
inline void Predictor::update( int y ) {
  ...
  if( MEM >= 1 ) {
    context = ch( 0 ) * 8 + ch( 1 ) / 32 + static_cast<unsigned int>( ch.pos( 0, 3 ) < ch.pos( 32, 3 ) ); // for SSE
    context &= 2047;
    ssep = ssemap( nextp );
