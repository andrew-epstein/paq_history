...
class RecordModel: public Model {
  ...
  CounterMap t0, t1, t2, t3, t4, t5, t6, t7, t8, t9, t10, t11, t12;  // Model
  int repeat1, repeat2;  // 2 last cycle lengths
public:
  RecordModel(): SIZE((MEM>=4)*(16+MEM-(MEM>=6))),
                 t0(SIZE), t1(SIZE), t2(SIZE), t3(SIZE), t4(SIZE), t5(SIZE), t6(SIZE), t7(SIZE), t8(SIZE), t9(SIZE), t10(SIZE), t11(SIZE), t12(SIZE),
                 repeat1(2), repeat2(3) {}
  void model(); 
};
...
class WordModel: public Model {
  const int SIZE;
  enum {N=3};
  CounterMap t0, t1, t2, t3, t4, t5, t6 ,t7, t8, t9, t10, t11;
...
public:
  WordModel(): SIZE((MEM>=4)*(MEM+17-(MEM>=6))),
               t0(SIZE), t1(SIZE), t2(SIZE), t3(SIZE), t4(SIZE), t5(SIZE), t6(SIZE), t7(SIZE), t8(SIZE), t9(SIZE), t10(SIZE), t11(SIZE) {
    for (int i=0; i<N; ++i)
      cxt[i]=word[i]=0;
	_c=0;
  }
  void model() {
    if (ch.bpos()==0) {
      ...
      t6.update(-word[0]);
      t7.update(word[1]-word[0]);
      t8.update(word[2]-word[0]);
      t9.update(word[3]-word[0]);
      t10.update(word[4]-word[0]);
      t11.update(word[5]-word[0]);
    }
    t0.write();
    t1.write();
    t2.write();
