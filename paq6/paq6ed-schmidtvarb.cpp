/* PAQ6ED-SchmidtVarB - Experimental version by Emilcont AKA Berto Destasio - File archiver and compressor.
...
#define PROGNAME "PAQ6ED-SchmidtVarB"  // Please change this if you change the program
...
//  ---- ---- --- --- --- --- ---------  ----------   ----- -- --
    ...
    {  0,  8,  1,  4,  1,  3,4294967295u,4294967295u}, // 1 (0,1)
    {  8,  0,  2,  5,  2,  4,4294967295u,4294967295u}, // 2 (1,0)
    {  0,  8,  1,  4,  3,  6,4294967295u,4294967295u}, // 3 (0,2)
    {  3,  3,  4,  8,  4,  7,4294967295u,4294967295u}, // 4 (1,1)
    {  8,  0,  5,  9,  2,  4,4294967295u,4294967295u}, // 5 (2,0)
    {  0, 14,  1,  4,  6, 10,4294967295u,4294967295u}, // 6 (0,3)
    ...
    {  3,  3,  8, 13,  7, 11,4294967295u,4294967295u}, // 12 (2,2)
    ...
    {  3,  4,  8, 13, 11, 16,4294967295u,4294967295u}, // 17 (2,3)
    {  4,  3, 13, 19,  7, 11,4294967295u,4294967295u}, // 18 (3,2)
    ...
    {  1,  4, 12, 18, 16, 22,4294967295u,4294967295u}, // 23 (2,4)
...
class CharModel: public Model {
  enum {N=11};        // Number of models
  Counter *t0, *t1;   // Model orders 0, 1 [256], [65536]
  CounterMap  _t0,_t1,t2, t3;
  CounterMap2 t4, t5, t6, t7, t8, t9, t10;  // Model orders 2-9
...
class RecordModel2: public Model {
  ...
  int _c1,_c2;
public:
  RecordModel2(): SIZE((MEM>=4)*(16+MEM-(MEM>=6))),
                 t0(SIZE), t1(SIZE), t2(SIZE), t3(SIZE), t4(SIZE), t5(SIZE),
                 repeat1(2), repeat2(3), _c1(0), _c2(0) {}
  void model(); 
};
...
class RecordModel3: public Model {
  ...
  int _c1,_c2;
public:
  RecordModel3(): SIZE((MEM>=4)*(16+MEM-(MEM>=6))),
...
//             t13(SIZE) 
  ...
  void model() {
    if (ch.bpos()==0) {
      ...
      t3.update(cxt[3]-cxt[0]);
      t4.update(cxt[4]-cxt[0]);
      t5.update(cxt[5]-cxt[0]);
//    t6.update(cxt[6]-cxt[0]);

      t7.update(-word[0]);
      t8.update(word[1]-word[0]);
      t9.update(word[2]-word[0]);
      t10.update(word[3]-word[0]);
      t11.update(word[4]-word[0]);
      t12.update(word[5]-word[0]);
//    t13.update(word[6]-word[0]);
    }
    t0.write();
    t1.write();
    t2.write();
...
int main(int argc, char** argv) {
  ...
  if (argc<2) {
    printf(
      "\n"
      PROGNAME " File Compressor/Archiver\n"
      ...
      "\n"
      "To compress:\n"
      "" PROGNAME " -3 archive filenames...  (archive will be created)\n"
      "  or (MSDOS):\n"
      "dir/b | " PROGNAME " -3 archive  (reads file names from input)\n"
      "\n"
      "To extract/compare:\n"
      "" PROGNAME " archive  (does not clobber existing files)\n"
      "\n"
      "To view contents:\n"
      "more < archive\n"
      "\n"
