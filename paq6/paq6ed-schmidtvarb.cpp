...
public:
  CharModel() :
      ...
      t10( static_cast<int>( MEM >= 5 ) * ( MEM + 18 - static_cast<int>( MEM >= 6 ) ) ),
      cxt( new U32[N] ) {
    cp0 = &t0[0];
    cp1 = &t1[0];
...
public:
  SparseModel2() :
      SIZE( static_cast<int>( MEM >= 0 ) * ( MEM + 14 ) ), t0( SIZE ), t1( SIZE ), t2( SIZE ), t3( SIZE ) {}
  void model() override; // Update and predict
};
...
inline void Predictor::update( int y ) {
  ...
  if( MEM >= 1 ) {
    context = ch( 0 ) * 8 + ( ch( 1 ) / 64 ) * 2
              + static_cast<int>( ch.pos( 0, 3 ) < ch.pos( 32, 3 ) ) * static_cast<int>( _wlen == _clen ); // for SSE
    ssep = ssemap( nextp );
  ...
  if( archive != nullptr ) {
    ...
    for( int i = 0; i < int( filename.size() ); ++i ) {
      ...
      // Extract to new file
        ...
        for( long j = 0; j < size; ++j ) {
          if( j % step == 0 )
            printf( "\b\b\b\b\b\b%5.1f%%", ( ( double ) j / size ) * 100 );
          int c = e.decode();
          if( f != nullptr )
            putc( c, f );
        }
