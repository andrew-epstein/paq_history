...
public:
  CharModel(): t0(new Counter[256]), t1(new Counter[65536]),	
               t2(MEM+16), t3(MEM+18), t4(MEM+20),
               t5((MEM>=1)*(MEM+20)),
               t6((MEM>=3)*(MEM+20)), t7((MEM>=3)*(MEM+20)),
...
public:
  SparseModel(): SIZE((MEM>=5)*(MEM+17)),
                 t0(SIZE), t1(SIZE), t2(SIZE), t3(SIZE), t4(SIZE),
                 t5(SIZE), t6(SIZE), t7(SIZE), t8(SIZE), t9(SIZE) {}
  void model();  // Update and predict
};
...
class AnalogModel: public Model {
  ...
  int pos3;  // pos % 3
public:
  AnalogModel(): SIZE((MEM>=5)*(MEM+17)), t0(SIZE), t1(SIZE),
...
public:
  WordModel(): SIZE((MEM>=4)*(MEM+18)),
               t0(24), t1(SIZE), t2(SIZE), t3(SIZE), t4(SIZE), t5(SIZE) {
    for (int i=0; i<N; ++i)
      cxt[i]=word[i]=0;
  }
  void model() {
