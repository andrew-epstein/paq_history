...
Encoder::Encoder( Mode m, FILE *f ) :
  ...
  if( mode == DECOMPRESS ) {
    for( int i = 0; i < 8; ++i ) {
      int c = getc( archive );
      if( c == EOF )
        c = 0; // PAQ6v2 bug fix (thanks to Alexander Ratushnyak)
      x = ( x << 8 ) + ( c & 0xff );
    }
  }
}
...
inline void Encoder::encode( int y ) {
  ...
  while( ( ( x1 ^ x2 ) & 0xff00000000000000ULL ) == 0 ) {
    putc( x2 >> 56, archive );
    x1 <<= 8;
    x2 = ( x2 << 8 ) + 255;
  }
...
inline int Encoder::decode() {
  ...
  // Shift equal MSB's out
  while( ( ( x1 ^ x2 ) & 0xff00000000000000LL ) == 0 ) {
    x1 <<= 8;
    x2 = ( x2 << 8 ) + 255;
    int c = getc( archive );
    if( c == EOF )
...
void Encoder::flush() {
  ...
  if( mode == COMPRESS ) {
    ...
    putc( x2 >> 56, archive ); // First unequal byte
  }
}
