/* PAQ605fbj - Experimental - File archiver and compressor.
...
#define PROGNAME "PAQ605fb" // Please change this if you change the program
...
public:
  ...
  void update( int y ) {
    ...
    if( s0 > 0 && s1 > 0 ) {
      ...
      const U32 sy1 = ( 0xffffffff / sy + ( rn & 127 ) ) >> 7;
      const U32 s1 = ( 0xffffffff / s + ( ( rn >> 7 ) & 127 ) ) >> 7;
      const U32 syd = sy1 - s1;
...
public:
  ...
  int predict() {
    U32 p1 = m1.predict( ( ch( 1 ) >> 5 ) + 8 * static_cast<int>( ch.pos( 0, 3 ) < ch.pos( 32, 3 ) ) );
    if( MEM >= MINMEM ) {
...
inline void RecordModel::model() {
  if( ch.bpos() == 0 ) {
    ...
    //    t5.update(hash(ch(r1), ch(r1*3), r1));
    t6.update( hash( ch( r2 ), ch( r2 * 3 ), r2 ) );

    //    t7.update(hash(ch(r1), ch(r1*4), r1));
    t8.update( hash( ch( r2 ), ch( r2 * 4 ), r2 ) );

    //    t9.update(hash(ch(r1), ch(r1*5), r1));
    t10.update( hash( ch( r2 ), ch( r2 * 5 ), r2 ) );

    //    t11.update(hash(ch(r1), ch(r1*6), r1));
    t12.update( hash( ch( r2 ), ch( r2 * 6 ), r2 ) );

    //    t13.update(hash(ch(r1), ch(r1*7), r1));
    t14.update( hash( ch( r2 ), ch( r2 * 7 ), r2 ) );
  }
...
class SparseModel : public Model {
  ...
  CounterMap t0, t1, t2, t3, t4, t5, t6, t7, t8, t9, t10, t11, t12,
      //t13,
      t14, t15,
      //t16,
      //t17,
...
inline void SparseModel::model() {
  ...
  if( ch.bpos() == 0 ) {
    ...
    t6.update( hash( ch( 2 ), ch( 5 ) ) );
    t7.update( hash( ch( 2 ), ch( 6 ) ) );

    t8.update( hash( ch( 3 ), ch( 3 ) ) );
    t9.update( hash( ch( 3 ), ch( 4 ) ) );
    t10.update( hash( ch( 3 ), ch( 5 ) ) );
    t11.update( hash( ch( 3 ), ch( 6 ) ) );

    t12.update( hash( ch( 4 ), ch( 3 ) ) );
    //    t13.update(hash(ch(4), ch(4)));
    t14.update( hash( ch( 4 ), ch( 5 ) ) );
    t15.update( hash( ch( 4 ), ch( 6 ) ) );

    //    t16.update(hash(ch(5), ch(3)));
    //    t17.update(hash(ch(5), ch(4)));
    //    t18.update(hash(ch(5), ch(5)));
    //    t19.update(hash(ch(5), ch(6)));

    //    t20.update(hash(ch(6), ch(3)));
    //    t21.update(hash(ch(6), ch(4)));
    //    t22.update(hash(ch(6), ch(5)));
    //    t23.update(hash(ch(6), ch(6)));

    const int g = min( 255, int( ch.pos() - ch.pos( ch( 1 ), 2 ) ) ); // gap to prior ch1

    t24.update( hash( ch( 1 ), g ) );
    t25.update( hash( ch( 1 ), ch( 2 ), g ) );
  }
