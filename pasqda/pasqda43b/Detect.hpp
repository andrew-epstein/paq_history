...
int WRT_detectFileType(FILE* file, int part_length, int parts)
{
	...
	PRINT_DICT(("EOLCount=%d+%d(%d) binCount=%d/%d(%d) punctCount=%d/%d(%d) langSum=%d/%d(%d) SP_EOL=%d\n",EOLCount,EOLCountBad,IF_OPTION(OPTION_EOL_CODING),binCount,part_length*parts/50,!IF_OPTION(OPTION_NORMAL_TEXT_FILTER),punctCount,punctCountBad,IF_OPTION(OPTION_WORD_SURROROUNDING_MODELING),langSum,part_length*parts/100,IF_OPTION(OPTION_USE_DICTIONARY),IF_OPTION(OPTION_SPACE_AFTER_EOL)));
