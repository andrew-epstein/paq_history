/* PAsQDa 4.0, based on PAQAR 4.0 - File archiver and compressor.
...
#define LONG_PROGNAME "PAsQDa (PAQ+Dictionary) v4.0"
#define CREATION_DATE "31.03.2005"
...
protected:
	...
	int en;
public:
	Mixer(int n):wt(new U32[n][65]),sse(new SSEContext[2048][33]){ en=n; init(); }
	void init()
	{
		for(int i=0;i<en;i++)
			for(int j=0;j<65;j++)
...
class MultiMixer{
	...
	int b0,b1;
public:
	MultiMixer():m0(256),m1(256),m2(256),m3(256),m4(2048),m5(2048),m6(2048),m7(2048),m8(2048),m9(2048),ma(2048),mb(2048){ init(); }
	void init() {
		if (restrt)
		{
			m0.init();
...
cm.add();         -- Predict and add to previously written counts
 ...
 class CounterMap2{
	 ...
	 Counter *snap_cp[8];
 public:
	 CounterMap2(int n):ht2(n+MEM-10) { init(); };
	 void init()
	 {
		 if (restrt)
			 ht2.init();
 ...
 class CharModel{
	 ...
	 U32 *snap_cxt;
 public:
	 CharModel():t2(21),t3(23),t4(23),t5(24),t6(24),t7(24),t8(24),t9(23),ta(23) { snap_cxt=NULL; snap_t0=NULL; init(); }
 	 void init() 
	 {
		 if (restrt)
		 {
 ...
 class MatchModel{
	 ...
	 U32 *ptr;
 public:
	 MatchModel():ptr(new U32[1<<21]){ init(); }
	 void init()
	 {
		 memset(ptr,0,1<<23);
		 hash[0]=hash[1]=0;
 ...
 public:
	 RecordModel():t0(17-MEM),tb(17-MEM),t1(19-MEM),t2(20-MEM),t3(SIZE),t4(SIZE),t5(SIZE),t6(SIZE),t7(SIZE),t8(SIZE),t9(SIZE),ta(SIZE) { init2(); }
	 void init2() 
	 {
		 r1=r3=2;
		 r2=r4=3;
...
class WordModel{
 ...
 public:
	 WordModel():t0(SIZE-2),t1(SIZE),t2(SIZE),t3(SIZE),t4(SIZE-1),t5(SIZE-1),t6(SIZE-1),t7(SIZE-2),t8(SIZE-2),t9(SIZE-2),ta(SIZE-1),tb(SIZE),tc(SIZE-1){ init(); }
	 void init() {
		 wlen=0;
		 for(int i=5;i>=0;i--)
			 wxt[i]=cxt[i]=0;
...
/*		if (usingDictionary() && !bp && ch(1)>127)
		{
			if (ch(1)<128+98)
				nextp=1;
			else
 			 ...
 			 // Extract to new file
				 ...
				 if (f && size>0)
				 {
					...
					WRT_decode(archive,f,size+exe); // PAsQDa addon
	 				...
	 				restrt=1;
#ifdef POWERED_BY_PAQ
 					if (filename.size()>1 && filesize[i]==610856)
						e->snapshot();
#endif
				 }
	  ...
	  if (argc>2)
		  ...
		  for (i=0; i<int(filename.size()); ++i) {
			  ...
			  if (!f) {
				  ...
				  int j=lastFile-2;
				  while (j>=0 && (filetype[lastFile]>filetype[j] || filesize[lastFile]==610856)) // Insertion Sort
				  {
...
#ifdef POWERED_BY_PAQ
 					  if (lastFile>1 && filesize[i]==610856)
						  e->snapshot();
#endif
				  }
