/* PAsQDa 4.4, based on PAQAR 4.1 - File archiver and compressor.
...
#endif
#define HEADER_PROGNAME PROGNAME "44"
#define LONG_PROGNAME PROGNAME " (PAQ+Dictionary) v4.4"
#define CREATION_DATE "03.01.2006"
...
int main(int argc, char** argv) {
	 ...
	 if (argc<2) {
		 printf(
			 LONG_PROGNAME " file compressor/archiver (based on PAQAR 4.1)\n"
	...
	for (int i=1; i<argc && (argv[i][0]=='-' || argv[i][0]=='/'); argc--,argv++)
	{
	 ...
	 if (archive) {
		 ...
		 if (s.substr(0, string(HEADER_PROGNAME).size()) != HEADER_PROGNAME) {
			 printf("Archive file %s not in " HEADER_PROGNAME " format\n", argv[1]);
			 return 1;
		 }
   ...
   // Compress files
	  ...
	  if (argc>2)
		  ...
		  fprintf(archive, HEADER_PROGNAME " -%d", MEM);
