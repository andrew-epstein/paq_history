/* PAsQDa 2.1, based on PAQAR 4.0 - File archiver and compressor.
...
The -5 is optional, and is used to trade off compression vs. speed
and memory.  Valid options are -0 to -9.  Higher numbers compress better
but run slower and use more memory.  -5 is the default, and gives a
reasonable tradeoff.  Recommended options are:
  -0 to -2 for fast but not very good compression, uses ~30-45 Mb memory
  -3 for reasonably fast and good compression, uses <60 Mb,
  -4 better compression but slower, uses <100 Mb,
  -5 the default, slightly better compression, uses ~180 Mb,
  -6 about like -5, uses <330 Mb,
  -7 to -9 use <630 Mb, ~930 Mb, ~1850 Mb, about the same speed as -6
...
#define LONG_PROGNAME "PAsQDa (PAQ+Dictionary) v2.1"
#define CREATION_DATE "31.01.2005"
...
int main(int argc, char** argv) {
	 ...
	 if (argc<2) {
		 printf(
			 ...
			 " -7                <630 Mb\n"
			 " -8                ~930 Mb\n"
			 " -9               ~1850 Mb (best compression, slowest)\n"
		     "use \"-Ne\" (e.g., -5e) if you think x86 code can be present in your data.");
		 return 1;
	 }
	  ...
	  if (argc>2)
		  ...
		  for (i=0; i<int(filename.size()); ++i) {
			  ...
			  if (!f) {
				  ...
				  int j=lastFile-2;
				  while (j>=0 && filetype[lastFile]>filetype[j]) // Insertion Sort
				  {
