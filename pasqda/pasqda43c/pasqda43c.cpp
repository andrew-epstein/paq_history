/* PAsQDa 4.3c, based on PAQAR 4.0 - File archiver and compressor.
...
PAsQDa is PAQAR with added a preprocessor (WRT by P.Skibinski), which
...
To compress:      PAsQDa -5 archive file file...  (1 or more file names), or
  or (MSDOS):     dir/b | PAsQDa -5 archive       (read file names from input)
  or (UNIX):      ls    | PAsQDa -5 archive
To decompress:    PAsQDa archive                  (no option)
...
line, then PAsQDa prompts for them, reading until the first blank line or
end of file.
...
which must exist.  PAsQDa does not create directories.  If the file to be
...
gxx -O PAsQDa.cpp	DJGPP 2.95.2
bcc32 -O2 PAsQDa.cpp	Borland 5.5.1
sc -o PAsQDa.cpp		Digital Mars 8.35n
...
A PAsQDa archive has a header, listing the names and lengths of the files
it contains in human-readable format, followed by the compressed data.
The first line of the header is "PAsQDa -m" where -m is the memory option.
...
#define CALGARY_CORPUS_OPTIM // turn on optmizations for Calgary Corpus (PAsQdaCC.exe)
#define POWERED_BY_PAQ       // turn on PAQ compression/decompression

#ifdef CALGARY_CORPUS_OPTIM
	#define PROGNAME "PAsQDaCC"
#else
	#define PROGNAME "PAsQDa"
#endif
#define LONG_PROGNAME PROGNAME " (PAQ+Dictionary) 4.3c"
#define CREATION_DATE "19.12.2005"
...
public:
	...
	void upd(int y){
		if(mp>0){
			...
			int mi=8;
#ifdef CALGARY_CORPUS_OPTIM
			if (tf==0 && fsize!=FILE_PIC)
#else
			if (zz!=0 && fsize!=FILE_PIC)
#endif
				mi=2;
			for(i=mp;i>=0;i--){
...
class CharModel{
	 ...
	 Counter *snap_t0;
 public:
	 CharModel():t2(SIZE-2),t3(SIZE),t4(SIZE),t5(SIZE+(MEM<6)),t6(SIZE+(MEM<6)),t7(SIZE+(MEM<6)),t8(SIZE+(MEM<6)),t9(SIZE),ta(SIZE) 
	 { 
#else
 public:
	 CharModel():t2(SIZE-2),t3(SIZE),t4(SIZE),t5(SIZE+1),t6(SIZE+1),t7(SIZE+1),t8(SIZE+1),t9(SIZE),ta(SIZE) 
	 { 
#endif
#ifdef CALGARY_CORPUS_OPTIM
		 snap_cxt=NULL; 
 ...
 public:
	 RecordModel():t0(17-MEM),tb(17-MEM),t1(19-MEM),t2(20-MEM),t3(SIZE),t4(SIZE),t5(SIZE),t6(SIZE),t7(SIZE),t8(SIZE),t9(SIZE),ta(SIZE) 
	 { 
#ifdef CALGARY_CORPUS_OPTIM
		 init2(); 
	 }
...
#else
	 AnalogModel():t7(18-MEM+(MEM<6)),t0(23-MEM+(MEM<6)),t1(SIZE-1+(MEM<6)),t2(SIZE-1+(MEM<6)),t3(SIZE+(MEM<6)),t4(23-MEM+(MEM<6)),t5(23-MEM+(MEM<6)),t6(SIZE+(MEM<6)),t8(SIZE+(MEM<6)) { };
	 void init() 
	 {
		 t0.init();
		 t1.init();
...
class WordModel{
	 ...
	 void model(){
		 if(!bp){
			...
			if(isalpha(c)){
				...
				if (c>127) // && usingDictionary())
				{
					wxt[0]^=hash(wxt[0],c,wlen++);
				}
				else
				{
					wlen=0;
 ...
 public:
...
#ifdef CALGARY_CORPUS_OPTIM
	 void snapshot() { charModel.snapshot(); }

	 void recover(int dealloc) { charModel.recover(dealloc); }
#endif
	 ...
	 void upd(int y){
		 ...
		 if(tf)
			 nextp=((6*p1+6*p2+3*p3+2*p4+2*p5)*10/32+9+nextp*19)/(19*11);
		 else
			 nextp=((3*p1+4*p2+3*p3+2*p4+2*p5)*13/32+7+nextp*14)/(14*14);
	 }
 };
 ...
 */
...
#ifdef CALGARY_CORPUS_OPTIM
	 void restart();
	 void snapshot() { predictor.snapshot(); };
	 void recover(int dealloc) { predictor.recover(dealloc); };
#endif
 };
 ...
 void Encoder::restart()
 {
	 if (restrt!=0)
	 {
		...
		if (fsize==FILE_GEO || fsize==FILE_PIC) 
			predictor.recordModel.init2();

		for (int i=0; i<64; i++)
			bc0[i]=bc1[i]=mxrs[i]=0;
		bc0[64]=bc1[64]=0;
 ...
 */
 ...
 public:
	 Transformer() {};
	 void start(Mode mode, FILE* f) { e.start(mode, f); };
	 void encode(int c) {
		 if ((zz==0 || tf!=0) && c/32==1 && ((c&15)==11 || (c&15)==15)) c^=16;
		 for (int i=7; i>=0; --i)
			 e.encode((c>>i)&1);
	 }
	 U32 decode() {
	 ...
	 void snapshot() { e.snapshot(); };
	 void recover(int dealloc) { e.recover(dealloc); };
#endif 
 };
...
void setWeight()
{
		switch (fsize)
		{
			case FILE_BIB: ow=28; break;
			case FILE_BOOK1: ow=6; break;
			case FILE_BOOK2: ow=16; break;
			case FILE_PIC: ow=4; break;
			case FILE_GEO: ow=32; break;
			case FILE_OBJ2: ow=28; break;
			case FILE_NEWS: ow=16; break;
			case FILE_PAPER2: ow=8; break;
			case FILE_TRANS: ow=24; break;
...
Transformer* e;
#include "pasqda.hpp"
inline bool usingDictionary() { return IF_OPTION(OPTION_USE_DICTIONARY);}
...
int main(int argc, char** argv) {
	 ...
	 if (argc<2) {
		 printf(
			 LONG_PROGNAME " file compressor/archiver (based on PAQAR 4.0)\n"
	...
	o_w=-1,o_i=-1,o_j=-1;
	for (int i=1; i<argc && (argv[i][0]=='-' || argv[i][0]=='/'); argc--,argv++)
	{
		switch ( toupper(argv[i][1]) ) 
		{
            case 'W': o_w=CLAMP(atoi(argv[i]+2),0,32); break; // default 16 and 28 for exe (SSE to predicion weight balance)
            case 'I': o_i=CLAMP(atoi(argv[i]+2),0,10); break; // default 0 and 8 for exe (even mixers updating weight)
            case 'J': o_j=8*CLAMP(atoi(argv[i]+2),0,20); break; // default 2 and 20 for exe (odd mixers updating weight)
            default : 
				if (isdigit(argv[1][1])) 
				{
	 ...
	 if (archive) {
		 ...
		 WRT_read_dict(false); // PAsQDa addon
		 readEOLstream(archive); // PAsQDa addon
		...
		do
		{
			...
			switch (s[pos+1]) 
			{
				case 'w': o_w=atoi(&s[pos+2]); break; // SSE to predicion weight balance (depends on file type)
				case 'i': o_i=atoi(&s[pos+2]); break;
				case 'j': o_j=8*atoi(&s[pos+2]); break;
 				default : 
				if (isdigit(s[pos+1]))
				{
					 // Read and remove -MEM option
		 ...
		 */
		 ...
		 // Transformer e(DECOMPRESS, archive); // PAsQDa removes
			 ...
			 if (f) {      // PAsQDa addon
 			 ...
 			 // Extract to new file
				 ...
				 if (f && size>0)
				 {
					...
					WRT_decode(archive,f,size); // PAsQDa addon
   ...
   // Compress files
	  ...
	  if (argc>2)
		  ...
		  WRT_read_dict(true); // PAsQDa addon
		  coder.StartEncode(); // PAsQDa addon
		  ...
		  for (i=0; i<int(filename.size()); ++i) {
			  ...
			  if (!f) {
				  ...
				  filetype[lastFile]=WRT_detectFileType(f,10240,5); // PAsQDa addon
				  filenames[lastFile]=filename[i];
				  fclose(f);

				  int j=lastFile-2;
				  while (j>=0 && (filetype[lastFile]>filetype[j] || (filesize[lastFile]==FILE_BOOK2))) // Insertion Sort
				  {
		  ...
		  // Write data
		  // Transformer e(COMPRESS, archive); // PAsQDa removes
#ifdef POWERED_BY_PAQ
		  e=new Transformer();
		  e->start(COMPRESS, archive); // PAsQDa addon
#endif
				...
				//  printf("ft=%d ",filetype[i]);
				  if (f && size>0)
				  {
					...
					//  e->snapshot();
					  ...
					  WRT_encode(f,archive,size,filetype[i]); // PAsQDa addon
				  ...
				  if (f)
					  fclose(f);
				  if (f && exe && size>0) remove(PAQ_TEMP);
	...
	//	  delete(filenames);
		  ...
		  writeEOLstream(archive); // PAsQDa addon
		  
		  compressed_bytes=ftell(archive);
		  fclose(archive);
  }
  
  WRT_deinitialize(); // PAsQDa addon
