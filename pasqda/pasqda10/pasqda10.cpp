/* PAsQDa 1.0, based on PAQ6v2 - File archiver and compressor.
(C) 2005, Matt Mahoney, mmahoney@cs.fit.edu & Przemyslaw Skibinski, inikep@o2.pl
...
#define PROGNAME "PAsQDa" // Please change this if you change the program
...
// Constructor
Encoder::Encoder() : predictor(), x1( 0 ), x2( 0xffffffff ), x( 0 ) {}
...
int main( int argc, char **argv ) {
  // Check arguments
  if( argc < 2 ) {
    printf( PROGNAME " file compressor/archiver (based on PAQ6 v2)\n"
                     "(c)2005, Matt Mahoney, mmahoney@cs.fit.edu & Przemyslaw Skibinski, inikep@o2.pl\n"
                     ...
                     " -7                 414 MB\n"
                     " -8                 818 MB\n"
                     " -9                1626 MB (best compression, slowest)\n" );
    return 1;
  }
  ...
  // Compress files
    ...
    for( int i = 0; i < int( filename.size() ); ++i ) {
      ...
      if( size >= 0 ) {
        ...
        if( f )
          WRT_encode( f, archive, size ); // PAsQDa addon
