...
int bound4, bound3, dict123size, dict12size, maxWordSize;
...
inline int checkHash( const unsigned char *s, int s_size, int h ) {
  ...
  i = word_hash[h];
  if( i > 0 ) {
    if( dictlen[i] > s_size || memcmp( dict[i], s, dictlen[i] ) != 0 ) {
      i = word_hash[( h + s_size * HASH_DOUBLE_MULT ) & ( HASH_TABLE_SIZE - 1 )];
      if( i > 0 ) {
        if( dictlen[i] > s_size || memcmp( dict[i], s, dictlen[i] ) != 0 ) {
          i = word_hash[( h + s_size * HASH_DOUBLE_MULT * HASH_DOUBLE_MULT ) & ( HASH_TABLE_SIZE - 1 )];
...
inline void encodeCodeWord( int i, FILE *fileout ) {
  ...
  if( first >= bound4 ) {
    ...
    PRINT_CODEWORDS( ( "1st=%d ", sym2codeword[dict1size + fourth] ) );

    ENCODE_PUTC( sym2codeword[dict1plus2 + third], fileout );
    PRINT_CODEWORDS( ( "2nd=%d ", sym2codeword[dict1size + third] ) );
    ...
    ENCODE_PUTC( sym2codeword[dict1plus2 + third], fileout );
    PRINT_CODEWORDS( ( "1st=%d ", sym2codeword[dict1size + third] ) );
...
inline void encodeWord( FILE *fileout, unsigned char *s, int s_size, EWordType wordType ) {
  ...
  if( i >= 0 ) {
    //		printf("s=%s s_size=%d i=%d dict[i]=%s\n",s,s_size,i,dict[i]);
...
void WRT_initializeCodeWords() {
  ...
  for( c = 0; c < 256; c++ ) {
    /*		if (IF_OPTION(OPTION_USE_DICTIONARY) && addSymbols[c])
		...
		if (IF_OPTION(OPTION_USE_DICTIONARY) && addSymbols[c]
			&& !IF_OPTION(OPTION_CAPTIAL_CONVERSION))
			addSymbols[c]=0;
*/
  ...
  if( IF_OPTION( OPTION_ADD_SYMBOLS_A_Z ) && IF_OPTION( OPTION_CAPTIAL_CONVERSION ) )
  ...
  if( IF_OPTION( OPTION_USE_DICTIONARY ) ) {
    ...
    //		if (sizeDict>=dictionary)
    //			sizeDict=dictionary-1;
    PRINT_DICT(
        ( "%d %d %d %d charsUsed=%d sizeDict=%d\n", dict1size, dict2size, dict3size, dict4size, charsUsed, sizeDict ) );
  }
  ...
  /*	for (c=0; c<256; c++)
	...
	printf("-addSymbols usedSet=%d IF_OPTION(OPTION_CAPTIAL_CONVERSION)=%d\n",usedSet,IF_OPTION(OPTION_CAPTIAL_CONVERSION));
*/
  //	printf("dict1size=%d %d bound4=%d dictionary=%d\n",dict1size,dict2size,bound4,dictionary);
}
...
bool initialize( unsigned char *dictName, unsigned char *shortDictName, bool encoding ) {
  ...
  if( IF_OPTION( OPTION_USE_DICTIONARY ) ) {
    ...
    fileLen = flen( file );
    //		fscanf(file,"%d",&proposedDictSize);
