...
public:
  ...
  void upd( int y ) {
    ...
    if( fsize != FILE_PIC ) {
      if( fsize == FILE_OBJ2 )
        mixer.wri( 28, 12 );
      else if( fsize == FILE_BOOK2 )
        mixer.wri( 24, 12 );
      else if( fsize == FILE_NEWS )
        mixer.wri( 20, 12 );
      else
        mixer.wri( 16, 12 );
...
public:
  ...
  void encode( int c ) {
    if( ( zz == 0 || tf != 0 ) && c / 32 == 1 && ( ( c & 15 ) == 11 || ( c & 15 ) == 15 ) )
      c ^= 16;
    for( int i = 7; i >= 0; --i )
      e.encode( ( c >> i ) & 1 );
  }
...
Transformer *e;
#include "pasqda.hpp"
inline bool usingDictionary() {
  return IF_OPTION( OPTION_USE_DICTIONARY );
}
...
int main( int argc, char **argv ) {
  // Check arguments
  if( argc < 2 ) {
    printf( LONG_PROGNAME " file compressor/archiver (based on PAQAR 4.0)\n"
  ...
  for( int i = 1; i < argc && ( argv[i][0] == '-' || argv[i][0] == '/' ); argc--, argv++ ) {
    switch( toupper( argv[i][1] ) ) {
      ...
      case 'P':
        o_p = CLAMP( atoi( argv[i] + 2 ), 0, 4096 );
        break;
      default:
        if( isdigit( argv[1][1] ) ) {
