...
#define PROGNAME "PAQ5-EMILCONT-DEUTERIUM" // Please change this if you change the program
...
public:
  ProgramChecker() {
    ...
    static_assert( sizeof( char * ) == sizeof( unsigned long ), "" );
  }
...
public:
  explicit CounterMap1( int n ) : N( n - 1 ), t( new S[1 << N] ), cxt( 0 ) {}
  void update( U32 h ) {
    if( ch.bpos() == 0 ) {
      if( t[cxt].n == 0 ) {
        t[cxt].n = 1;
...
public:
  explicit CounterMap2( int n ); // Use 2^n bytes memory
  void add();
  void update( U32 h );
  void write() {
    mixer.write( 0, 0 );
...
public:
  explicit CounterMap3( int n ) : cm1( n - 2 ), cm2( n ) {}
  void update( U32 h ) {
    cm1.update( h );
    cm2.update( h );
  }
...
inline void CharModel::model() {
  ...
  if( ch.bpos() == 0 ) { // Start new byte
    ...
    t6.update( cxt[6] );
    t7.update( cxt[7] );
    t8.update( cxt[8] ); //							emilcont
    t9.update( cxt[9] ); //							emilcont
  }
  cp0 = &t0[ch()];
  cp1 = &t1[ch() + 256 * ch( 1 )];
...
public:
  MatchModel() : ptr( new U32[1 << ( 22 )] ) {
    memset( ptr, 0, ( 1 << ( 22 ) ) * sizeof( U32 ) );
    hash[0] = hash[1] = 0;
    for( int i = 0; i < M; ++i )
      begin[i] = end[i] = 0;
  }
...
inline void SparseModel::model() {
  // Update context
  if( ch.bpos() == 0 ) { // Start new byte
    t0.update( hash( ch( 1 ), ch( 2 ) ) );
    t1.update( hash( ch( 1 ), ch( 3 ) ) );
    t2.update( hash( ch( 1 ), ch( 4 ) ) );
    t3.update( hash( ch( 1 ), ch( 5 ) ) );
    t4.update( hash( ch( 1 ), ch( 6 ) ) );
    t5.update( hash( ch( 1 ), ch( 7 ) ) );
    t6.update( hash( ch( 1 ), ch( 8 ) ) );
    t7.update( hash( ch( 2 ), ch( 3 ) ) );
    t8.update( hash( ch( 2 ), ch( 4 ) ) );
    t9.update( hash( ch( 2 ), ch( 5 ) ) );
  }
...
public:
  AnalogModel() :
      ...
      t1( 18 ),
      t2( 18 ),
      t3( 18 ), //F22,F23,F24,F25 - Emilcont (18,18,18,18)
      t4( 18 ),
      t5( 18 ) {} //F26,F27 - Emilcont (18,18)
  void model() override {
    if( ch.bpos() == 0 ) {
      if( ++pos3 == 3 )
        pos3 = 0;
...
Predictor::Predictor() : sse( ( SSEContext( * )[SSE2 + 1] ) new SSEContext[SSE1][SSE2 + 1] ), nextp( PSCALE / 2 ) {
