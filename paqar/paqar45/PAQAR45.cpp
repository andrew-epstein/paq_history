...
public:
	...
	int predict(){
		...
		m7.predict(c);
		c=(ch(2)/8 )*8 +(max(bp,4));
		if(tf) c=ch(2)/16+16*(ch(1)/32+8*(wlen==clen));
		m0.predict(c);
		m2.predict(c);
...
//	 CounterMap3 t0,t1,t2,t3,t4,t5,t6,t7,t8,t9,ta,tb;
	 ...
	 CounterMap2 tb;
 public:
	 SparseModel():t0(SIZE-1),t1(SIZE),t2(SIZE-1),t3(SIZE-1),t4(SIZE),t5(SIZE-1),t6(SIZE-1),t7(SIZE),t8(SIZE),t9(SIZE),ta(SIZE),tb(SIZE) {} 
#ifdef CALGARY_CORPUS_OPTIM
	 void init() 
	 {
		 t0.init();
 ...
 class AnalogModel{
	 ...
	 CounterMap3 t7;
 public:
	 AnalogModel():t0(17-MEM),t1(SIZE  ),t2(SIZE-1),t3(SIZE+1),t4(24-MEM),t5(23-MEM),t6(SIZE+1),t7(SIZE+2) { };
#ifdef CALGARY_CORPUS_OPTIM
	 void init() 
	 {
		 t0.init();
 ...
 class PicModel{
	 ...
	 CounterMap2 tx;
 public:
	 PicModel():t0(SIZE),t1(SIZE),t2(SIZE),t3(SIZE),t4(SIZE),t5(SIZE),t6(SIZE),t7(SIZE),t8(SIZE),t9(SIZE),ta(SIZE),tb(SIZE),tc(SIZE),td(SIZE),te(SIZE),tf(SIZE),tg(SIZE),th(SIZE),ti(SIZE),tj(SIZE),tk(SIZE),tl(SIZE),tm(SIZE),tn(SIZE),to(SIZE),tp(SIZE),tq(SIZE),tr(SIZE),ts(SIZE),tt(SIZE),tu(SIZE),tv(SIZE),tw(SIZE),tx(SIZE),ty(SIZE),tz(SIZE) { };
 ...
 public:
	 ...
	 void upd(int y){
		...
		else
		{
		 ...
		 c4=ch(1)*8+ch(2)/32;
		 c5=ch(2)*8+ch(1)/32;
		 if(tf||fsize==513216)c5=l8*8+ch(1)/32;
