/*
PAQAR 2.0 - Experimental version by Alexander Ratushnyak - File archiver and compressor.
...
class RecordModel {
  enum { SIZE = 20 };
  CounterMap3 t0, t1, t2, tb;
  CounterMap3 t3, t4, t5, t6, t7, t8, t9, ta;
...
public:
  ...
  void upd( int y ) {
    ...
    if( tf != 0 )
      nextp = ( ( 3 * p3 + 6 * p1 + 5 * p2 + 1 * p4 + 1 * p5 ) * 10 / 32 + 8 + nextp * 16 ) / ( 16 * 11 );
    else
      nextp = ( ( 3 * p3 + 3 * p1 + 4 * p2 + 2 * p4 + 2 * p5 ) * 14 / 32 + 7 + nextp * 14 ) / ( 15 * 14 );
  }
};
...
int main( int argc, char **argv ) {
  // Check arguments
  if( argc < 2 ) {
    printf( PROGNAME " v2.0 file compressor/archiver, (C)2004, Matt Mahoney, mmahoney@cs.fit.edu\n"
...
*/
        ...
        if( f != nullptr ) {
          ...
          if( ( exe != 0 ) && size > 0 ) {
            ...
            if( ( st0 = ( char * ) malloc( CONSTA + 2 * flen + 256 ) ) == NULL )
              handler();
            st = st0 + 256 - ( ( uintptr_t ) st0 & 255 ); // 256-byte-alignment
  ...
  if( uncompressed_bytes > 0 && elapsed_time > 0 ) {
    printf( " (%1.4f bpb, %1.2f%% at %1.0f KB/s)", compressed_bytes * 8.0 / uncompressed_bytes,
