...
*/
  ...
  PRINT_DICT( ( "%s EOLCount=%d+%d(%d) binCount=%d/%d(%d) punctCount=%d/%d(%d) langSum=%d/%d(%d) SP_EOL=%d\n", filename,
                EOLCount, EOLCountBad, IF_OPTION( OPTION_EOL_CODING ), binCount, part_length * parts / 150,
                !IF_OPTION( OPTION_NORMAL_TEXT_FILTER ), punctCount, punctCountBad,
                IF_OPTION( OPTION_WORD_SURROROUNDING_MODELING ), langSum, part_length * parts / 150,
                IF_OPTION( OPTION_USE_DICTIONARY ), IF_OPTION( OPTION_SPACE_AFTER_EOL ) ) );

  return preprocFlag;
}

int getSourcePath( char *buf, char *argv0 ) {
  int i, pos = strlen( strcpy( buf, argv0 ) );
