...
int bufferedChar,autoSwitch,lastEOL,lastChar,llast,llbckp=0;
EEOLType EOLType;
...
{ \
        if (fftell+((bufferedChar>=0)?1:0)<originalFileLen) \
	{ \
		c=e->decode(); \
 \
...
void WRT_encode(FILE* file,FILE* fileout,int fileLen,int fileType)
{
...
#ifdef POWERED_BY_PAQ
	if (fnu>10) //!IF_OPTION(OPTION_USE_DICTIONARY))
	{
		e->recover(1);
		e->restart();
	}
    ...
    printf("      ");
    long step=(long)ceil((double)fileLen/100);

	while (!feof(file))
	{
		if (fftell%step==0) printf("\b\b\b\b\b\b%5.0f%%",((double)fftell/fileLen)*100);
...
//	printf("- decoding file (%d bytes->%d bytes), EOL stream=%d bytes\n",fileLen,fileEnd);
    ...
    printf("      ");
    long step=(long)ceil((double)decomprFileLen/1000);

	while (c!=EOF)
	{
		if (fftell%step==0) printf("\b\b\b\b\b\b%5.0f%%",((double)fftell/decomprFileLen)*100);
...
void WRT_read_dict(bool encoding, char *argv0)
{
	int dictPathLen;
	...
	dictPathLen=getSourcePath((char*)dictPath,argv0);
...
void main2(int argc, char* argv[])
{
	...
	encoding=true;
	if (getc(file)==WRT_HEADER[0] && getc(file)==WRT_HEADER[1] && 
		getc(file)==WRT_HEADER[2] && getc(file)==WRT_HEADER[3])
	{
		encoding=false;
