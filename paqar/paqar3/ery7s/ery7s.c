...
void main( int argc, char *argv[] ) {
  ...
  if( SourceFile == 0 || TargetFile == 0 || ( c != 'c' && c != 'd' ) )
    printf( Usage ), exit( 0 );

  fseek( SourceFile, 0L, SEEK_END ), flen = ftell( SourceFile ), fseek( SourceFile, 0L, 0 );
  if( c == 'c' )
    m = 3;
  else
    m = 4;

  if( ( st0 = malloc( CONSTA + 2 * flen + 256 ) ) == NULL )
    printf( "Can't allocate memory\n" ), exit( 1 );
  st = st0 + 256 - ( ( int ) st0 & 255 ); //te8e9() needs 256-byte-alignment
  flen = fread( st + CONSTA - 32768, 1, flen, SourceFile );

  if( flen )
    te8e9( st, m, flen, &data2write[0] );
  if( data2write[0] )
    fwrite( ( char * ) data2write[1], 1, data2write[0], TargetFile );
  if( data2write[2] && m < 4 )
    fwrite( ( char * ) data2write[3], 1, data2write[2], TargetFile );
}
